...
When changing `frontend/`, run `make test-frontend-unit`, `make test-frontend-unit-tsx` when a `.tsx` module changed, `make test-go PACKAGE=./server`, and the focused Playwright viewer suite. Run `make test-frontend-visual` for visual or accessibility changes and review snapshot changes rather than replacing them blindly.
