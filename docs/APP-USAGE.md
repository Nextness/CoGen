...
The URL is a clean per-view path such as `/overview` or `/article` and carries no query parameters. `search_id`, `search_revision_id`, `plan_id`, and `run_id` plus filters, sorting, pagination, sections, expanded rows, graph state, artifact inspection, focused `note_id`, focused `anchor_id`, and `pdf_page` live in `history.state` mirrored to `sessionStorage`, so navigation, reload, browser back and forward, and internal links preserve that state. Legacy `?view=...` URLs are no longer supported.
...
The application opens on Home. Every route shows a breadcrumb beginning at Home. Deepdive has five tabular destinations in this order: Overview, Corpus, Relationships, Provenance, and Advanced. Moving between those destinations or a detail page performs a native full-page load, while filters, pagination, sections, and other changes within the current view update browser history without reloading the shell. On narrow screens, the Menu button opens the same Deepdive navigation and reports its expanded state to assistive technology.
...
Home summarizes the available search terms, search revisions, execution plans, and run attempts. Search-history cards provide one Continue action for the latest complete context, while the run-attempt table reports each attempt's start time, duration, outcome, visibility, direct Explore action, and reversible lifecycle action.
...
Corpus uses one labeled collection selector for Articles, Authors, References, Author identity / ORCID evidence, and Source records instead of a nested tab row. Tables support bounded pagination, allowlisted sorting, search, page-size controls, and row disclosures. Multiple rows may remain expanded, and explicit controls remain available when row click also toggles expansion. The article list shows DOI, Title, Year, Journal, Source, PDF availability, Review status, and Review source; the author and identity tables omit internal IDs, and identity-candidate payloads are excluded from the collection table.

Articles in the analysis-ready collection are valid normalized revisions for the selected run. Expanding an article row shows a Matched search terms block: how many of the run's recorded search terms matched the article and which terms matched in Title, Abstract, Keywords, and Keywords plus. Runs without stored term data show No search terms recorded. The matches are derived deterministically by the pipeline from the recorded source queries and the revision's stored fields, using stemming so inflected forms match their base term; they are not captured pipeline evidence. Author and reference collections can include occurrence or mention records connected to multiple revision snapshots; repeated conceptual names are possible and should be interpreted with revision and producer-stage context.

Detail pages remain within Corpus navigation. Return to Corpus restores the article filters, sorting, page size, page, and expanded rows; Previous unreviewed and Next unreviewed stay within that filtered article collection. Article detail places full-width PDF status above an equal-width Document reader and Article review workspace, followed by full-width Provenance summary, Bibliographic metadata, and a Search term coverage panel, then related authors and references, stage history, an article-scoped audit timeline, and raw evidence. The coverage panel lists matched terms per field, an All search terms disclosure with matched and unmatched terms and their source databases, and a Not recorded or No matched terms state per field. Author detail contains the uncertain ORCID candidate rows and links to preserved provider evidence; author and reference details retain observed or mentioned values rather than presenting uncertain data as confirmed identity.
...
Name-search outcomes include `no_orcid_candidate`, `orcid_is_unclear`, and `provider_failed`. No candidate uses a neutral label, while unclear uses a warning label; treat both and provider-error outcomes as recorded evidence about an attempted resolution, not proof of identity or absence.

Each identity-evidence row keeps the title and DOI of one article together, links to its current normalized revision when available, and labels the pipeline stage where the search was captured. The same search can appear beside multiple articles when its captured occurrence is shared; summary counts count searches once. Author details also show evidence from earlier revisions of the same work and run when author position, recorded names, and observed ORCID match. Matching names in unrelated articles do not transfer evidence.
...
Provenance has Audit, Artifacts, Cache, Stages, and Run detail sections. Audit presents a compact timeline grouped by UTC day, with aligned times, inline affected records, and visible outcomes, validation reasons, and recorded source, stage, provider, or duration. Full timestamps remain available on the time label; Recorded data expands to load the privacy-scrubbed payload separately. Text search plus category, action, actor, entity, stage, outcome, and run-context filters use bounded cursor pagination. Loading older events appends them without closing already-open disclosures and reports the loaded count or beginning of history beside the action. Pipeline, enrichment, validation, PDF, and review actions retain their own semantic labels. Review-decision events visibly compare the complete previous and new status, optional reason, and every sub-status without duplicating note bodies, selected text, reviewer email, or browser drafts.
...
## 11. Article review and PDFs

Corpus → Articles is the single place to browse normalized work, inspect PDF inventory, and track the selected run's review status. Review progress, Start review, the run Notes index, and PDF, review, source, qualifier, and progress filters are part of this collection. Inventory dates and detailed metadata appear in row expansion. `not_available` means no validated selected PDF bytes are present; `available` means validated bytes and an inventory timestamp exist. Review status is a contextual interpretation. Existing `/evaluation` routes open `/corpus` with their saved review filters.
...
Notes support headings through level four, paragraphs, block quotes, bullet or `1.` lists, fenced code, and simple tables. The editor and safe preview appear before the independently searchable saved-note list. Open Insert evidence link or Note syntax and link examples only when those tools are needed. Link forms are `[[note:123|display]]`, `[[article:10.1000/example|display]]`, `[[pdf:page=5|display]]`, `[[anchor:methods-1|display]]`, and `[[ext:https://example.test|display]]`; a DOI therefore uses the `article` target type, not `doi`. Preview content is escaped. A visible Unresolved link label means syntax was accepted but the selected context does not currently contain the target; it may resolve later without rewriting the note version.
...
The custom PDF reader renders one current page with selectable text, page navigation, zoom, rotation, Fit width, a Dark/Light theme toggle that inverts the rendered page, and keyboard page or zoom shortcuts. One Previous or Next activation loads exactly one adjacent page, and the controls become unavailable at the first and last page. The document remains inside its own scrollable viewport instead of covering later article content. Selecting text on one page opens the PDF anchors section and a work-scoped label form; the repository generates the stable opaque anchor ID used by note links. The keyboard-operable anchor list identifies current or inherited state, page, availability, selected text, and history. An anchor tied to different PDF content remains recorded but is labeled unavailable, is not drawn over the current PDF, and does not offer an unqualified page action.

The toolbar Fullscreen button expands the reading workspace to the browser fullscreen surface (or a fixed-viewport fallback when the Fullscreen API is unavailable), with the PDF filling the main area and the review panel becoming a collapsible drawer on the right, expanded by default. The drawer edge control collapses and expands the review panel, and handing a text selection to the review flow auto-expands a collapsed drawer so the anchor candidate form is visible. Exiting fullscreen restores the embedded two-column article layout; fullscreen state is transient and never changes the URL.
...
If the viewer cannot start, confirm that the metadata path exists, is a file, is writable by the local reviewer, has V00027 and required review triggers, and has been migrated explicitly. If context selectors are empty, confirm that the database contains searches, revisions, plans, and runs. If PDF status is unavailable, confirm the relative companion binding and database move together with metadata. On a database migrated only to V00024, the corpus and article detail pages still work but show No search terms recorded because the term-coverage tables do not exist; V00026 is required for generated anchor IDs and work-scoped labels, while V00027 records completed empty term-match reconciliation.
