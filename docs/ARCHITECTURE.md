...
The same binary serves a local read-only web viewer over an existing metadata database and its bundle-relative PDF store. The viewer is a vanilla JavaScript URL-state SPA embedded in the Go binary by default; `--assets-dir` changes only the frontend asset source for development.
...
| `src/database/` | Writable metadata SQLite connection, migrations, repositories, immutable relationships, attempts, artifacts, cache, metrics, and audit. |
...
| `src/server/` | Existing-only read-only SQLite viewer API, bounded queries, details, audit, artifact access, PDF delivery, and embedded assets. |
...
| `migrations/corpus.metadata/` | Metadata migrations V00001-V00021. |
| `migrations/corpus.pdf/` | Companion PDF migrations V00001-V00002. |
| `src/server/frontend/` | Embedded SPA source, stylesheets, and generated pinned D3-force browser bundle. |
| `frontend/` | Node lock data, Playwright runner and configuration, browser tests, unit tests, and snapshots. |
| `src/server/testdata/` | Committed viewer fixture metadata database plus its generated ignored companion PDF fixture. |
...
`src/main.go` is the only production entry point. Pipeline behavior belongs in `src/workspace/`; provider gatherers do not call database repositories, repository and store functions do not perform provider HTTP requests, and `src/server/` does not import workspace orchestration or call writable `database.Open`.
...
`build/something-json <path>` evaluates a SOMETHING file and writes JSON. `build/doccheck` implements the documentation workflow. `build/coveragecheck` is built by `make coveragecheck` or `make tools` and is normally invoked through `make coverage` with the configured policy.
...
`config/workspace.something` includes `config/baseline.something`, declares format-version-2 workspaces, source queries and filter counts, rename and keep mappings, enrichment settings, reuse policy, and cache policy. `workspace.Load` reads the selected file once, evaluates the exact bytes, ignores iteration scopes without a `workspace` value, rejects duplicate selectors, and produces typed `manifest.ResolvedManifest` and `enrich.Config` values.
...
The metadata chain is V00001-V00021 and the PDF chain is V00001-V00002. Migration execution follows SOMETHING iteration order, skips applied filenames, records checksums without later revalidation, does not use `previous` or `upgrade` to order execution, and has no downgrade runner even though files retain `-- ==DOWN==` sections.

The metadata schema includes `schema_migrations`, `pipeline_runs`, `searches`, `search_revisions`, `execution_plans`, `run_sources`, `source_records`, `source_filter_counts`, `artifacts`, `artifact_blobs`, `run_artifacts`, `run_steps`, `pipeline_run_metrics`, `audit_events`, `works`, `work_identifiers`, `work_revisions`, `run_work_stages`, `people`, `author_occurrences`, `authorships`, `author_identity_resolutions`, `author_identity_candidates`, `reference_mentions`, `cache_entries`, `run_cache_uses`, `pdf_store_binding`, and `pdf_audit_links`.
...
Append-only triggers reject updates and deletes from audit events, work revisions, author occurrences, authorships, reference mentions, and PDF download attempts. PDF blobs reject updates, and the runtime exposes no PDF-blob deletion path. Run visibility and purge eligibility are separate lifecycle repository operations, but the viewer exposes no mutation endpoint.
...
`server.Open` rejects empty, missing, and directory paths; opens metadata SQLite using `mode=ro` and `query_only`; discovers non-system tables and columns; and optionally opens the bound PDF store after path and schema validation. It never invokes the writable database package.

The server accepts GET and HEAD. JSON errors use an `error` message, collection inputs and response sizes are bounded, table and sort identifiers are validated against discovered schema, artifact and PDF downloads use conservative headers, and API request contexts time out after five seconds. HTTP server timeouts are five seconds for headers, ten seconds for reads, fifteen seconds for writes, and thirty seconds idle.

| Route | Responsibility and bounds |
|---|---|
| `/api/health` | Reports read-only server and database health. |
...
browser GET/HEAD
      |
      v
method + path + query validation
      |
      v
five-second request context
      |
      v
read-only SQLite query -> bounded JSON or attachment response
```
...
The URL is the source of truth for `search_id`, `search_revision_id`, `plan_id`, `run_id`, view, section, filters, sort, page, expanded row, selected graph node, and artifact inspector state where applicable. Every internal link uses `state.link`; hard-coded `?view=...` links lose research context.
...
| `api.js` | Abort-aware JSON fetching, endpoint construction, shared errors, and table discovery cache. |
...
app.js
  ...
  |     +-- detail.js ------------+-- state.js, api.js, pagination.js
  +-- api.js
  +-- context-selector.js
  +-- shell.js -> api.js
...
Frontend unit tests use `node:test`, `node:assert`, and jsdom without a browser or server. Playwright uses an isolated viewer on an operating-system-assigned loopback port and unique output under `build/playwright/`; `viewer.spec.cjs` covers committed-fixture behavior, `ui-quality.spec.cjs` covers axe, responsive behavior, and reviewed visual snapshots, and the environment-guarded `e2e.spec.cjs` compares Corpus, Provenance, and Evaluation against a database generated by the real pipeline binary.
...
The viewer opens databases read-only, defaults to loopback, supports no mutation endpoint, validates identifiers and bounded inputs, escapes frontend content, and makes no production CDN request. Non-loopback binding only emits a warning and must be treated as an explicit operator decision because the viewer has no authentication or authorization.
...
- The viewer is local and read-only, with bounded endpoints and no authentication layer; loopback is the normal privacy boundary.
...
- The viewer reads an existing database and is not a migration or repair path.
...
- The viewer has no authentication or authorization; a non-loopback warning does not prevent deliberate exposure.
...
- Add a viewer endpoint by keeping GET or HEAD semantics, read-only SQLite, allowlisted identifiers, bounded inputs and output, context timeouts, and integration coverage.
