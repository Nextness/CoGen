...
| Skip link | `.skip-link` | Hidden-until-focused navigation to main content. |
| Header | `.site-header`, `.brand`, `.eyebrow`, `.server-status` | Product identity, research framing, health, and local-review state. |
| Status | `.rw-status-dot--unavailable`, `.rw-label` | Health indicator and persistent textual status label. |
...
| Labels | `.ui.label`, `.ui.green.label`, `.ui.orange.label`, `.ui.red.label`, `.ui.blue.label`, `.ui.grey.label`, `.ui.violet.label` | Neutral, status, review, and note classification with selected, link, and disabled states. |
| Messages | `.ui.message`, `.ui.negative.message`, `.ui.error.message`, `.ui.warning.message`, `.ui.info.message`, `.ui.success.message` | Global and local feedback with a symbol, text, semantic boundary, and soft color. |
| Headers | `.ui.header`, `.ui.top.attached.header`, `.rw-page-header`, `.rw-eyebrow` | Product, page, panel, section, eyebrow, and supporting-heading hierarchy. |
...
| Family | Principal selectors | Responsibility |
|---|---|---|
| Grid | `.ui.grid`, `.rw-context-grid`, `.rw-dashboard-grid`, `.rw-grid-span-all` | Flexible and fixed-column layouts with full-width spans. |
| Menus | `.ui.menu`, `.ui.tabular.menu`, `.ui.pagination.menu`, `.rw-section-tabs` | Deepdive, URL-backed section, local tab, and pagination navigation. |
...
| Forms and controls | `.ui.form`, `.rw-filter-bar`, `.rw-filter-bar__search`, `.rw-filter-panel`, `.rw-filter-panel__fields`, `.rw-filter-panel__actions`, `.rw-filter-disclosure`, `.rw-filter-field-grid--advanced`, `.rw-filter-summary`, `.rw-filter-chips`, `.rw-filter-chip`, `.rw-filter-clear` | Shared control states, simple and complex filter surfaces, compact advanced filter grids, one removable chip per value, sorting, search, and page-size inputs. |
| Panels and stacks | `.rw-page-stack`, `.rw-content-stack`, `.rw-inline-group` | Parent surfaces and layout-owned separation. |
...
| Lifecycle modal | `.rw-review-dialog`, `.rw-review-dialog__header`, `.rw-review-dialog__body`, `.rw-review-dialog__actions` | Dimmed confirmation hierarchy and reversible Home run-visibility action. |
...
| Audit | `.rw-audit-layout`, `.rw-audit-summary`, `.rw-audit-filter`, `.rw-audit-stream`, `.rw-audit-day`, `.rw-audit-events`, `.rw-audit-event*`, `.rw-review-audit-change`, `.rw-review-audit-state`, `.rw-review-audit-substatuses`, `.rw-audit-load`, `.rw-record-audit*` | Full-width filters, inline result totals, compact UTC timeline with aligned time and content columns, inline record identity and wrapping facts, category markers, visible review-decision comparisons, compact lazy recorded-data disclosure, append status, and record investigation. |
...
| Family | Principal selectors | Responsibility |
|---|---|---|
| Filters | `.rw-graph__controls`, `.rw-graph__filter-actions`, `.rw-graph__common-filters` | Mode-specific and shared bounded query controls. |
