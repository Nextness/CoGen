...
- Relationships and expectations: The pipeline computes matches from the normalized revision fields and the run's parsed terms and replaces the run's rows transactionally, so the table holds derived data rather than immutable evidence and has no append-only trigger. Matching is case-insensitive with whole-word semantics and `*` prefix wildcard support; keywords are matched per element. The viewer reads this table for the article detail and corpus article payloads; `matched_total` is the distinct matched term count per revision.
