...
Every application-generated URL is a clean per-view path such as `/overview`, `/corpus`, or `/article`; the URL carries no query parameters. Research context and route state live in a `history.state` object mirrored to `sessionStorage` under `rw-viewer-state`, so reload and browser back and forward restore the same state and cross-view navigation persists the destination state before the full page load. Same-view links and programmatic state changes without modifier keys push or replace browser history and render without reloading the document. Anchors that carry state beyond the view path render a `data-state` JSON attribute consumed by the click handler; without it, navigation keeps the current state. External links, downloads, modified clicks, and non-application paths retain normal browser behavior. Legacy `?view=...` query parameters are rejected entirely: the app ignores `location.search` at boot, so old bookmarks and shared URLs open with the default state.
...
- Canonical application links are clean per-view paths with no query parameters; the state lives in `history.state` mirrored to `sessionStorage`, and legacy `?view=...` URLs are no longer supported.
- A path without a matching view document (for example an unknown `/nonexistent`) is not routed by the server; every supported view has one owning path and Home uses `/`.
