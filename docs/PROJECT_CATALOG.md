...
| [`NormalizeDOI`](../src/article/article.go#L136) | function | 136-144 | `func NormalizeDOI(value string) string` | NormalizeDOI normalizes a complete structured identifier without citation-boundary or HTML heuristics. |
| [`ExtractDOI`](../src/article/article.go#L147) | function | 147-154 | `func ExtractDOI(text string) string` | ExtractDOI returns the first DOI found in text, or empty string. |
| [`ParseAuthors`](../src/article/article.go#L160) | function | 160-196 | `func ParseAuthors(authorsStr, affiliationsStr string) []Author` | ParseAuthors parses semicolon/newline-separated author and affiliation strings. Affiliations are matched by position (same index as authors). BibTeX uses " and " as separator - this is tried as a fallback when semicolons don't produce a useful split. |
| [`ParseReferences`](../src/article/article.go#L201) | function | 201-223 | `func ParseReferences(refsStr string) []Reference` | ParseReferences parses a newline or semicolon-separated reference list with DOI extraction. CSV exports generally use semicolons while BibTeX exports use one reference per line. |
| [`RenameFields`](../src/article/article.go#L226) | function | 226-250 | `func RenameFields(m map[string]string, patches map[string]string)` | RenameFields renames keys in m according to patches (old -> new), in-place. |
| [`KeepFields`](../src/article/article.go#L253) | function | 253-263 | `func KeepFields(m map[string]string, keep []string)` | KeepFields deletes keys not in the keep set, in-place. |
| [`RequiredFieldError`](../src/article/article.go#L266) | struct | 266-268 | `type RequiredFieldError struct { Missing []string }` | RequiredFieldError is returned when a required field is missing. |
| [`(*RequiredFieldError).Error`](../src/article/article.go#L271) | method | 271-273 | `func (*RequiredFieldError).Error() string` | Error returns the receiver's diagnostic message. |
| [`CheckRequired`](../src/article/article.go#L276) | function | 276-291 | `func CheckRequired(a *Article) []string` | CheckRequired returns the list of missing required fields. |
| [`NewFromMap`](../src/article/article.go#L296) | function | 296-331 | `func NewFromMap(entry map[string]string, source string) (*Article, error)` | NewFromMap builds an Article from a canonical-field map[string]string. Text values are sanitized; structured DOI punctuation is preserved. Returns nil + *RequiredFieldError if required fields are missing. |
| [`ArticleToMap`](../src/article/article.go#L334) | function | 334-371 | `func ArticleToMap(a *Article) map[string]any` | ArticleToMap serialises an Article to a plain map for JSON output. |
| [`MergeBySource`](../src/article/article.go#L375) | function | 375-400 | `func MergeBySource(articlesBySource map[string][]*Article) (unique, duplicates []*Article)` | MergeBySource merges articles from multiple sources, deduplicating by DOI. Returns (unique, duplicates). Articles without DOI are always kept. |
| [`lowerAll`](../src/article/article.go#L403) | function | 403-408 | `func lowerAll(ss []string) []string` | lowerAll returns lowercase copies of all input strings. |
| [`sortedKeys`](../src/article/article.go#L411) | function | 411-418 | `func sortedKeys(m map[string][]*Article) []string` | sortedKeys returns keys in deterministic order. |
...
| [`(*Parser).LoadFile`](../src/bibtex/bibtex.go#L38) | method | 38-45 | `func (*Parser).LoadFile(filepath string) (string, error)` | LoadFile reads and logs a BibTeX input file without parsing it. Deprecated: workspace ingestion reads source bytes once and calls Parse directly. This method remains for external callers that use the public parser API. |
| [`tokenType`](../src/bibtex/bibtex.go#L48) | type | 48 | `type tokenType int` | tokenType identifies lexical tokens in the constrained BibTeX grammar. |
| [`token`](../src/bibtex/bibtex.go#L63) | struct | 63-66 | `type token struct { typ tokenType value string }` | token carries one BibTeX token category and its optional text value. |
| [`lexicalError`](../src/bibtex/bibtex.go#L69) | struct | 69-72 | `type lexicalError struct { position int message string }` | lexicalError identifies malformed constrained BibTeX input by byte position. |
| [`(*lexicalError).Error`](../src/bibtex/bibtex.go#L75) | method | 75-77 | `func (*lexicalError).Error() string` | Error returns the lexical diagnostic with its input byte position. |
| [`readBracedContent`](../src/bibtex/bibtex.go#L81) | function | 81-110 | `func readBracedContent(data string, i, n int, stripBraces bool) (string, int, error)` | readBracedContent reads content inside balanced braces. When stripBraces is true, the outermost braces are not included in the result. |
| [`isIdentChar`](../src/bibtex/bibtex.go#L113) | function | 113-116 | `func isIdentChar(c byte) bool` | isIdentChar reports whether a byte is accepted inside a BibTeX identifier. |
| [`tokenize`](../src/bibtex/bibtex.go#L119) | function | 119-202 | `func tokenize(data string, stripBraces bool) ([]token, error)` | tokenize converts BibTeX source text into the constrained parser token stream. |
| [`(*Parser).Parse`](../src/bibtex/bibtex.go#L208) | method | 208-315 | `func (*Parser).Parse(data, source string, stripBraces bool) (Library, error)` | Parse parses raw BibTeX data into a Library. Only "article" entries are retained. Duplicate citation keys get a numeric suffix. The source parameter is stored in the "article_source" field of each entry. A leading UTF-8 byte order mark is stripped because exporters commonly prefix it. |
...
| [`(*RunSourceRepository).Create`](../src/database/attempts.go#L75) | method | 75-93 | `func (*RunSourceRepository).Create(pipelineRunID int64, sourceName, sourceType, expectedFile, query, requestedFields string, expectedResultCount int, exportDate string) (int64, error)` | Create inserts a new run source link. Returns the source ID. |
...
| [`(*ArtifactRepository).Create`](../src/database/attempts.go#L267) | method | 267-308 | `func (*ArtifactRepository).Create(contentHash, contentType string, byteSize int64) (int64, error)` | Create inserts a new artifact. Returns the artifact ID. If the content_hash already exists, returns the existing artifact ID. |
| [`(*ArtifactRepository).CreateWithBlob`](../src/database/attempts.go#L311) | method | 311-370 | `func (*ArtifactRepository).CreateWithBlob(contentHash, contentType string, byteSize, pipelineRunID int64, data []byte) (int64, error)` | CreateWithBlob atomically records an artifact and its bytes for a pipeline run. |
| [`(*ArtifactRepository).GetByHash`](../src/database/attempts.go#L373) | method | 373-389 | `func (*ArtifactRepository).GetByHash(contentHash string) (*Artifact, error)` | GetByHash returns an artifact by its content hash, or nil if not found. |
| [`(*ArtifactRepository).GetByID`](../src/database/attempts.go#L392) | method | 392-408 | `func (*ArtifactRepository).GetByID(id int64) (*Artifact, error)` | GetByID returns an artifact by its primary key, or nil if not found. |
| [`ArtifactBlob`](../src/database/attempts.go#L411) | struct | 411-417 | ``type ArtifactBlob struct { ID int64 `json:"id"` ArtifactID int64 `json:"artifact_id"` PipelineRunID int64 `json:"pipeline_run_id"` Data []byte `json:"-"` CreatedAt string `json:"created_at"` }`` | ArtifactBlob stores the raw bytes for an artifact inline in the database. |
| [`ArtifactBlobRepository`](../src/database/attempts.go#L420) | struct | 420-422 | `type ArtifactBlobRepository struct { db *Database }` | ArtifactBlobRepository provides CRUD for the artifact_blobs table. |
| [`(*ArtifactBlobRepository).Create`](../src/database/attempts.go#L426) | method | 426-468 | `func (*ArtifactBlobRepository).Create(artifactID, pipelineRunID int64, data []byte) (int64, error)` | Create inserts a new artifact blob. Returns the blob ID. If the artifact_id already exists, returns the existing blob ID (deduplicated). |
| [`(*ArtifactBlobRepository).GetByArtifactID`](../src/database/attempts.go#L471) | method | 471-487 | `func (*ArtifactBlobRepository).GetByArtifactID(artifactID int64) (*ArtifactBlob, error)` | GetByArtifactID returns the blob for a given artifact, or nil if not found. |
| [`RunStepRepository`](../src/database/attempts.go#L490) | struct | 490-492 | `type RunStepRepository struct { db *Database }` | RunStepRepository provides CRUD for the run_steps table. |
| [`runStepTimestamp`](../src/database/attempts.go#L495) | function | 495-497 | `func runStepTimestamp() string` | runStepTimestamp returns a microsecond-precision UTC timestamp for persisted stage timing. |
| [`(*RunStepRepository).Create`](../src/database/attempts.go#L500) | method | 500-518 | `func (*RunStepRepository).Create(pipelineRunID int64, stepName string) (int64, error)` | Create inserts a new run step record. Returns the step ID. |
| [`(*RunStepRepository).UpdateStatus`](../src/database/attempts.go#L523) | method | 523-546 | `func (*RunStepRepository).UpdateStatus(stepID int64, status string) error` | UpdateStatus updates the status and optional finish time of a run step. The status must be a valid manifest.StageOutcome value. finished_at is only set for terminal statuses (completed, skipped, reused, failed). |
| [`(*RunStepRepository).LinkReuse`](../src/database/attempts.go#L551) | method | 551-562 | `func (*RunStepRepository).LinkReuse(stepID int64, reusedFromRunID int64) error` | LinkReuse records that a step reused output from a prior run. It sets both the status to "reused" and the finished_at timestamp since reuse is a terminal stage outcome. |
| [`(*RunStepRepository).LinkInputArtifact`](../src/database/attempts.go#L565) | method | 565-576 | `func (*RunStepRepository).LinkInputArtifact(stepID, artifactID int64) error` | LinkInputArtifact records that a step consumed a specific artifact as input. |
| [`(*RunStepRepository).LinkOutputArtifact`](../src/database/attempts.go#L579) | method | 579-590 | `func (*RunStepRepository).LinkOutputArtifact(stepID, artifactID int64) error` | LinkOutputArtifact records that a step produced a specific artifact as output. |
| [`(*RunStepRepository).SetFingerprints`](../src/database/attempts.go#L594) | method | 594-603 | `func (*RunStepRepository).SetFingerprints(stepID int64, inputFingerprint, outputFingerprint string) error` | SetFingerprints records the immutable stage input and output fingerprints used to decide whether this stage may be reused by a later attempt. |
| [`(*RunStepRepository).ListByRun`](../src/database/attempts.go#L606) | method | 606-654 | `func (*RunStepRepository).ListByRun(pipelineRunID int64) ([]*RunStep, error)` | ListByRun returns all steps for a given pipeline run, ordered by ID. |
| [`SourceFilterCount`](../src/database/attempts.go#L657) | struct | 657-662 | ``type SourceFilterCount struct { ID int64 `json:"id"` PipelineRunID int64 `json:"pipeline_run_id"` SourceName string `json:"source_name"` FilterData string `json:"filter_data"` // JSON array of {filters: string[], count: int} }`` | SourceFilterCount holds per-source filter stage article counts for a run. |
| [`SourceFilterCountRepository`](../src/database/attempts.go#L665) | struct | 665-667 | `type SourceFilterCountRepository struct { db *Database }` | SourceFilterCountRepository provides CRUD for the source_filter_counts table. |
| [`(*SourceFilterCountRepository).SetFilterData`](../src/database/attempts.go#L670) | method | 670-683 | `func (*SourceFilterCountRepository).SetFilterData(pipelineRunID int64, sourceName, filterData string) error` | SetFilterData upserts the filter data for a source in a run. |
| [`(*SourceFilterCountRepository).ListByRun`](../src/database/attempts.go#L686) | method | 686-713 | `func (*SourceFilterCountRepository).ListByRun(pipelineRunID int64) ([]*SourceFilterCount, error)` | ListByRun returns filter data for all sources in a run, ordered by source name. |
| [`(*SourceFilterCountRepository).GetByRunAndSource`](../src/database/attempts.go#L716) | method | 716-731 | `func (*SourceFilterCountRepository).GetByRunAndSource(pipelineRunID int64, sourceName string) (*SourceFilterCount, error)` | GetByRunAndSource returns filter data for a specific source in a run. |
...
| [`(*CacheEntryRepository).Upsert`](../src/database/cache.go#L67) | method | 67-84 | `func (*CacheEntryRepository).Upsert(entry *CacheEntry) (int64, error)` | Upsert appends an immutable response version; existing run uses retain the exact response they observed. |
| [`(*CacheEntryRepository).Get`](../src/database/cache.go#L88) | method | 88-94 | `func (*CacheEntryRepository).Get(provider, namespace, fingerprint, extractorVersion string) (*CacheEntry, error)` | Get returns the latest response for an exact request and extractor, regardless of expiry. Policy execution decides whether an expired entry is stale or may be reused. |
| [`(*CacheEntryRepository).GetGlobal`](../src/database/cache.go#L98) | method | 98-106 | `func (*CacheEntryRepository).GetGlobal(provider, namespace, fingerprint, extractorVersion string) (*CacheEntry, error)` | GetGlobal returns an entry only after a run explicitly published it to the global layer. Entries written only to active_run remain private to that run. |
| [`(*CacheEntryRepository).get`](../src/database/cache.go#L109) | method | 109-130 | `func (*CacheEntryRepository).get(query string, args ...any) (*CacheEntry, error)` | get executes a cache-entry query and returns its nullable payload and expiry fields. |
| [`(*RunCacheUseRepository).Create`](../src/database/cache.go#L133) | method | 133-146 | `func (*RunCacheUseRepository).Create(use *RunCacheUse) (int64, error)` | Create validates and inserts one run-scoped cache-use evidence record. |
| [`(*RunCacheUseRepository).ListByRun`](../src/database/cache.go#L149) | method | 149-168 | `func (*RunCacheUseRepository).ListByRun(runID int64) ([]*RunCacheUse, error)` | ListByRun returns cache-use evidence for a run in insertion order. |
| [`(*RunCacheUseRepository).FindEntry`](../src/database/cache.go#L172) | method | 172-174 | `func (*RunCacheUseRepository).FindEntry(runID int64, layer, provider, namespace, fingerprint, extractorVersion string) (*CacheEntry, error)` | FindEntry returns the latest entry with this exact versioned key recorded for a run and cache layer. It supports active-run and named-prior-run reads. |
| [`(*RunCacheUseRepository).FindAnyEntry`](../src/database/cache.go#L179) | method | 179-206 | `func (*RunCacheUseRepository).FindAnyEntry(runID int64, provider, namespace, fingerprint, extractorVersion string) (*CacheEntry, error)` | FindAnyEntry returns the latest exact cache key used by a prior run. Named prior-run policy reads use this because their source layer is provenance, not a restriction on how the source run originally obtained the response. |
| [`(*RunCacheUseRepository).findEntry`](../src/database/cache.go#L209) | method | 209-237 | `func (*RunCacheUseRepository).findEntry(layerClause string, args []any) (*CacheEntry, error)` | findEntry returns the latest cache entry matching a run-layer predicate. |

### [`src/database/cache_integration_test.go`](../src/database/cache_integration_test.go)

| Symbol | Kind | Lines | Declaration, inputs, and outputs | Source description |
|---|---|---:|---|---|
| [`TestCacheEntryUpsertAndKeySeparation`](../src/database/cache_integration_test.go#L18) | test | 18-70 | `func TestCacheEntryUpsertAndKeySeparation(t *testing.T)` | TestCacheEntryUpsertAndKeySeparation verifies immutable response history and key separation across provider/request/version. |
| [`TestCacheEntryConcurrentUpsertAndRunUse`](../src/database/cache_integration_test.go#L73) | test | 73-123 | `func TestCacheEntryConcurrentUpsertAndRunUse(t *testing.T)` | TestCacheEntryConcurrentUpsertAndRunUse verifies cache entry concurrent upsert and run use. |
...
| [`(*MetricsRepository).Set`](../src/database/metrics_audit.go#L43) | method | 43-56 | `func (*MetricsRepository).Set(runID int64, metric, source string, value int) error` | Set inserts or replaces a metric value for a given run, metric name, and source. If source is empty, it records a whole-run metric. |
| [`(*MetricsRepository).Get`](../src/database/metrics_audit.go#L60) | method | 60-77 | `func (*MetricsRepository).Get(runID int64, metric, source string) (*PipelineRunMetric, error)` | Get returns a single metric for a given run, metric name, and source. Returns nil, nil if the metric is not recorded for this run. |
| [`(*MetricsRepository).ListByRun`](../src/database/metrics_audit.go#L80) | method | 80-107 | `func (*MetricsRepository).ListByRun(runID int64) ([]*PipelineRunMetric, error)` | ListByRun returns all metrics for a given pipeline run, ordered by metric name then source. |
| [`AuditEventRepository`](../src/database/metrics_audit.go#L110) | struct | 110-112 | `type AuditEventRepository struct { db *Database }` | AuditEventRepository provides CRUD for the audit_events table. |
| [`(*AuditEventRepository).Insert`](../src/database/metrics_audit.go#L116) | method | 116-151 | `func (*AuditEventRepository).Insert(event *manifest.AuditEvent) (int64, error)` | Insert stores a new audit event. The event's action is validated against the manifest lifecycle vocabulary before insertion. |
| [`(*AuditEventRepository).ListByRun`](../src/database/metrics_audit.go#L154) | method | 154-167 | `func (*AuditEventRepository).ListByRun(runID int64) ([]*AuditEventRecord, error)` | ListByRun returns all audit events for a given pipeline run, ordered by ID. |
| [`(*AuditEventRepository).ListByEntity`](../src/database/metrics_audit.go#L170) | method | 170-183 | `func (*AuditEventRepository).ListByEntity(entityType, entityID string) ([]*AuditEventRecord, error)` | ListByEntity returns all audit events for a given entity type and ID, ordered by ID. |
| [`scanAuditEvents`](../src/database/metrics_audit.go#L186) | function | 186-223 | `func scanAuditEvents(rows *sql.Rows) ([]*AuditEventRecord, error)` | scanAuditEvents decodes audit events from a database row. |
| [`PurgeEligibility`](../src/database/metrics_audit.go#L226) | struct | 226-240 | ``type PurgeEligibility struct { // Eligible is true when no other run references this run's artifacts or // reusable outputs. Eligible bool `json:"eligible"` // SharedArtifactCount is the number of artifacts from this run that are // referenced by other runs. SharedArtifactCount int `json:"shared_artifact_count"` // ReusedByCount is the number of other runs that reuse one or more stages // from this run. ReusedByCount int `json:"reused_by_count"` // OwnedReviewContextCount is the number of review contexts initialized for this run. OwnedReviewContextCount int `json:"owned_review_context_count"` // DependentReviewContextCount is the number of child contexts inheriting directly or indirectly from this run's context. DependentReviewContextCount int `json:"dependent_review_context_count"` }`` | PurgeEligibility describes whether a pipeline run can be safely purged. |
| [`(*PipelineRunRepository).CheckPurgeEligibility`](../src/database/metrics_audit.go#L245) | method | 245-316 | `func (*PipelineRunRepository).CheckPurgeEligibility(runID int64) (*PurgeEligibility, error)` | CheckPurgeEligibility verifies that no other run shares artifacts or reusable stage outputs from the given run. It is the safety check before purge. Returns an error if no pipeline run with the given ID exists. |
...
| [`(*PipelineRunRepository).startAttempt`](../src/database/pipeline_runs.go#L85) | method | 85-163 | `func (*PipelineRunRepository).startAttempt(executionPlanID int64, step, searchQuery string, rejectRunning bool) (int64, int, error)` | startAttempt atomically starts the next plan attempt, optionally rejecting an already-running attempt. |
| [`(*PipelineRunRepository).FinishRun`](../src/database/pipeline_runs.go#L169) | method | 169-192 | `func (*PipelineRunRepository).FinishRun(runID int64, status string, summary string) error` | FinishRun marks a pipeline run as completed (or failed). Supports both legacy runs and new attempt-based runs. It validates the status against the manifest lifecycle vocabulary and only sets finished_at for terminal statuses (completed, failed). |
| [`(*PipelineRunRepository).Trash`](../src/database/pipeline_runs.go#L195) | method | 195-206 | `func (*PipelineRunRepository).Trash(runID int64, reason string) error` | Trash marks a pipeline run as trashed (soft-deleted). |
| [`(*PipelineRunRepository).Restore`](../src/database/pipeline_runs.go#L209) | method | 209-220 | `func (*PipelineRunRepository).Restore(runID int64) error` | Restore sets a trashed pipeline run back to active visibility. |
| [`(*PipelineRunRepository).GetByID`](../src/database/pipeline_runs.go#L223) | method | 223-267 | `func (*PipelineRunRepository).GetByID(runID int64) (*PipelineRun, error)` | GetByID returns a pipeline run by its primary key, or nil if not found. |
| [`(*PipelineRunRepository).ListByPlan`](../src/database/pipeline_runs.go#L270) | method | 270-284 | `func (*PipelineRunRepository).ListByPlan(executionPlanID int64) ([]*PipelineRun, error)` | ListByPlan returns all runs for a given execution plan, ordered by attempt number. |
| [`(*PipelineRunRepository).ListByVisibility`](../src/database/pipeline_runs.go#L287) | method | 287-301 | `func (*PipelineRunRepository).ListByVisibility(visibilityState string) ([]*PipelineRun, error)` | ListByVisibility returns all runs with a given visibility state, ordered by ID. |
| [`scanPipelineRuns`](../src/database/pipeline_runs.go#L304) | function | 304-348 | `func scanPipelineRuns(rows *sql.Rows) ([]*PipelineRun, error)` | scanPipelineRuns decodes pipeline runs from a database row. |
| [`scanAll`](../src/database/pipeline_runs.go#L351) | function | 351-380 | `func scanAll(rows *sql.Rows) ([]map[string]any, error)` | scanAll converts arbitrary SQL rows to maps keyed by column name. |
| [`isRetryableError`](../src/database/pipeline_runs.go#L384) | function | 384-394 | `func isRetryableError(err error) bool` | isRetryableError returns true if the error is a transient SQLite error that can be retried: UNIQUE constraint violation or SQLITE_BUSY (database locked). |
| [`(*Database).withTx`](../src/database/pipeline_runs.go#L397) | method | 397-414 | `func (*Database).withTx(ctx context.Context, fn func(*sql.Tx) error) error` | withTx runs fn inside a transaction, rolling back on error and committing on success. |
| [`nullStrPtr`](../src/database/pipeline_runs.go#L417) | function | 417-422 | `func nullStrPtr(s *string) any` | nullStrPtr returns nil if s is nil, otherwise returns a sql.NullString with the value. |
| [`(*PipelineRunRepository).RecoverAbandoned`](../src/database/pipeline_runs.go#L425) | method | 425-448 | `func (*PipelineRunRepository).RecoverAbandoned(ctx context.Context, runID int64) error` | RecoverAbandoned atomically fails a running attempt and records recovery audit; the caller must hold exclusive pipeline ownership. |
...
| [`(*PipelineRunReviewerRepository).Insert`](../src/database/reviewers.go#L22) | method | 22-39 | `func (*PipelineRunReviewerRepository).Insert(runID int64, username, email string) error` | Insert records one immutable reviewer identity for a newly created run. |
| [`(*PipelineRunReviewerRepository).Get`](../src/database/reviewers.go#L42) | method | 42-54 | `func (*PipelineRunReviewerRepository).Get(runID int64) (*PipelineRunReviewer, error)` | Get returns the reviewer captured for a run, or nil when a legacy writer omitted it. |

### [`src/database/reviews.go`](../src/database/reviews.go)

| Symbol | Kind | Lines | Declaration, inputs, and outputs | Source description |
|---|---|---:|---|---|
| [`boundedText`](../src/database/reviews.go#L39) | struct | 39-42 | `type boundedText struct { text sql.NullString bytes int }` | boundedText decodes a bounded SQL text projection with its original byte length. |
| [`(boundedText).optional`](../src/database/reviews.go#L45) | method | 45-51 | `func (boundedText).optional() (*string, bool)` | optional returns the bounded value and whether SQL truncated its original byte sequence. |
| [`ReviewConflictError`](../src/database/reviews.go#L54) | struct | 54-57 | `type ReviewConflictError struct { Expected *int64 Current *int64 }` | ReviewConflictError reports that a context head changed after the caller read it. |
| [`(*ReviewConflictError).Error`](../src/database/reviews.go#L60) | method | 60 | `func (*ReviewConflictError).Error() string` | Error returns a safe optimistic-concurrency diagnostic. |
| [`ReviewError`](../src/database/reviews.go#L63) | struct | 63-66 | `type ReviewError struct { Kind string Message string }` | ReviewError is a safe repository error with a stable client-visible category. |
| [`(*ReviewError).Error`](../src/database/reviews.go#L69) | method | 69 | `func (*ReviewError).Error() string` | Error returns the safe repository diagnostic. |
| [`ReviewContextParentConflictError`](../src/database/reviews.go#L72) | struct | 72-75 | `type ReviewContextParentConflictError struct { Requested *int64 Existing *int64 }` | ReviewContextParentConflictError reports conflicting idempotent initialization choices. |
| [`ReviewAnchorLabelConflictError`](../src/database/reviews.go#L78) | struct | 78-80 | `type ReviewAnchorLabelConflictError struct { Label string }` | ReviewAnchorLabelConflictError reports a duplicate human anchor label within one work. |
| [`(*ReviewAnchorLabelConflictError).Error`](../src/database/reviews.go#L83) | method | 83-85 | `func (*ReviewAnchorLabelConflictError).Error() string` | Error returns a safe work-scoped label diagnostic. |
| [`(*ReviewContextParentConflictError).Error`](../src/database/reviews.go#L88) | method | 88-90 | `func (*ReviewContextParentConflictError).Error() string` | Error returns a safe immutable-lineage diagnostic. |
| [`reviewValidation`](../src/database/reviews.go#L93) | function | 93-95 | `func reviewValidation(message string) error` | reviewValidation reports invalid caller-controlled review input. |
| [`reviewNotFound`](../src/database/reviews.go#L98) | function | 98 | `func reviewNotFound(message string) error` | reviewNotFound reports a missing review-scoped record. |
| [`reviewLifecycle`](../src/database/reviews.go#L101) | function | 101 | `func reviewLifecycle(message string) error` | reviewLifecycle reports a valid request rejected by immutable run lifecycle. |
| [`NoteSyntaxError`](../src/database/reviews.go#L104) | struct | 104 | `type NoteSyntaxError struct{ Errors []notes.SyntaxError }` | NoteSyntaxError reports parser diagnostics that make a note version unsaveable. |
| [`(*NoteSyntaxError).Error`](../src/database/reviews.go#L107) | method | 107 | `func (*NoteSyntaxError).Error() string` | Error returns a safe note-language diagnostic. |
| [`ReviewContext`](../src/database/reviews.go#L110) | struct | 110-115 | ``type ReviewContext struct { ID int64 `json:"id"` PipelineRunID int64 `json:"pipeline_run_id"` ParentContextID *int64 `json:"parent_context_id,omitempty"` CreatedAt string `json:"created_at"` }`` | ReviewContext is one explicitly initialized interpretation context for a completed run. |
| [`ReviewContextCandidate`](../src/database/reviews.go#L118) | struct | 118-127 | ``type ReviewContextCandidate struct { ContextID int64 `json:"context_id"` PipelineRunID int64 `json:"pipeline_run_id"` SearchID string `json:"search_id"` SearchRevision string `json:"search_revision"` ExecutionPlanID int64 `json:"execution_plan_id"` AttemptNumber int `json:"attempt_number"` StartedAt string `json:"started_at"` InheritedWorkCount int `json:"inherited_work_count"` }`` | ReviewContextCandidate describes one eligible parent context without materializing inherited state. |
| [`WorkReviewVersion`](../src/database/reviews.go#L130) | struct | 130-142 | ``type WorkReviewVersion struct { ID int64 `json:"id"` WorkID int64 `json:"work_id"` WorkRevisionID int64 `json:"work_revision_id"` CreatedInContextID int64 `json:"created_in_context_id"` ParentVersionID *int64 `json:"parent_version_id,omitempty"` Status string `json:"status"` Substatuses []string `json:"sub_statuses"` Reason *string `json:"reason"` ReasonTruncated bool `json:"reason_truncated"` CreatedAt string `json:"created_at"` ReviewerDisplay string `json:"reviewer_display"` }`` | WorkReviewVersion is one immutable complete article-review snapshot. |
| [`WorkReviewState`](../src/database/reviews.go#L145) | struct | 145-151 | ``type WorkReviewState struct { ContextID int64 `json:"context_id"` WorkID int64 `json:"work_id"` WorkRevisionID int64 `json:"work_revision_id"` Version *WorkReviewVersion `json:"version,omitempty"` InheritedFromContextID *int64 `json:"inherited_from_context_id,omitempty"` }`` | WorkReviewState is the complete current state for one context work head. |
| [`workReviewAuditState`](../src/database/reviews.go#L154) | struct | 154-158 | ``type workReviewAuditState struct { Status string `json:"status"` Reason *string `json:"reason"` Substatuses []string `json:"sub_statuses"` }`` | workReviewAuditState is the bounded decision payload stored in audit before/after state. |
| [`ReviewRepository`](../src/database/reviews.go#L161) | struct | 161 | `type ReviewRepository struct{ db *Database }` | ReviewRepository owns context initialization and immutable review, note, and anchor versions. |
| [`(*ReviewRepository).CorpusID`](../src/database/reviews.go#L164) | method | 164-170 | `func (*ReviewRepository).CorpusID(ctx context.Context) (string, error)` | CorpusID returns the opaque corpus identity used to namespace browser-local drafts. |
| [`(*ReviewRepository).GetContextByRun`](../src/database/reviews.go#L173) | method | 173-175 | `func (*ReviewRepository).GetContextByRun(ctx context.Context, runID int64) (*ReviewContext, error)` | GetContextByRun returns the one initialized review context for a run, if present. |
| [`queryRower`](../src/database/reviews.go#L178) | interface | 178-180 | `type queryRower interface { QueryRowContext(context.Context, string, ...any) *sql.Row }` | queryRower is the shared single-row query boundary for database and transaction callers. |
| [`reviewQuerier`](../src/database/reviews.go#L183) | interface | 183-186 | `type reviewQuerier interface { queryRower QueryContext(context.Context, string, ...any) (*sql.Rows, error) }` | reviewQuerier is the shared query boundary for review reads that need single and multiple rows. |
| [`getReviewContext`](../src/database/reviews.go#L189) | function | 189-204 | `func getReviewContext(ctx context.Context, q queryRower, runID int64) (*ReviewContext, error)` | getReviewContext reads the optional immutable context associated with one run. |
| [`(*ReviewRepository).ProposeParent`](../src/database/reviews.go#L207) | method | 207-225 | `func (*ReviewRepository).ProposeParent(ctx context.Context, runID int64) (*ReviewContextCandidate, error)` | ProposeParent selects the latest initialized context from the same plan, then the same search. |
| [`reviewTargetRecord`](../src/database/reviews.go#L228) | struct | 228-231 | `type reviewTargetRecord struct { RunID, PlanID, SearchDBID int64 StartedAt, Status, Visibility string }` | reviewTargetRecord holds lineage fields required to validate or compare a target run. |
| [`(*ReviewRepository).reviewTarget`](../src/database/reviews.go#L234) | method | 234-249 | `func (*ReviewRepository).reviewTarget(ctx context.Context, q queryRower, runID int64) (reviewTargetRecord, error)` | reviewTarget loads one planned run and its stable search lineage. |
| [`(*ReviewRepository).firstParentCandidate`](../src/database/reviews.go#L252) | method | 252-282 | `func (*ReviewRepository).firstParentCandidate(ctx context.Context, runID int64, target reviewTargetRecord, samePlan bool) (*ReviewContextCandidate, error)` | firstParentCandidate returns the newest eligible same-plan or same-search context. |
| [`(*ReviewRepository).ListParentCandidates`](../src/database/reviews.go#L285) | method | 285-340 | `func (*ReviewRepository).ListParentCandidates(ctx context.Context, runID int64, scope, cursorStartedAt string, cursorRunID int64, limit int, query string) ([]ReviewContextCandidate, error)` | ListParentCandidates returns bounded earlier contexts in stable descending run order. |
| [`(*ReviewRepository).CreateContext`](../src/database/reviews.go#L343) | method | 343-436 | `func (*ReviewRepository).CreateContext(ctx context.Context, runID int64, parentContextID *int64) (*ReviewContext, bool, error)` | CreateContext initializes one run context and freezes matching parent heads without copying version bodies. |
| [`(*ReviewRepository).GetWorkReview`](../src/database/reviews.go#L439) | method | 439-463 | `func (*ReviewRepository).GetWorkReview(ctx context.Context, contextID, workRevisionID int64) (*WorkReviewState, error)` | GetWorkReview returns the complete current state for one context work revision. |
| [`(*ReviewRepository).AppendWorkReview`](../src/database/reviews.go#L466) | method | 466-541 | `func (*ReviewRepository).AppendWorkReview(ctx context.Context, contextID, workRevisionID int64, expectedVersionID *int64, status string, substatuses []string, reason *string) (*WorkReviewState, bool, error)` | AppendWorkReview appends a complete immutable state and compare-and-swaps only the selected context head. |
| [`(*ReviewRepository).mutableWorkHead`](../src/database/reviews.go#L544) | method | 544-565 | `func (*ReviewRepository).mutableWorkHead(ctx context.Context, tx *sql.Tx, contextID, workRevisionID int64) (int64, int64, *int64, error)` | mutableWorkHead validates an editable context work and returns its run, work, and current version IDs. |
| [`(*ReviewRepository).ListWorkReviewVersions`](../src/database/reviews.go#L568) | method | 568-618 | `func (*ReviewRepository).ListWorkReviewVersions(ctx context.Context, contextID, workRevisionID, cursor int64, limit int) ([]WorkReviewVersion, error)` | ListWorkReviewVersions follows only the selected head's ancestor chain. |
| [`(*ReviewRepository).GetWorkReviewVersion`](../src/database/reviews.go#L621) | method | 621-636 | `func (*ReviewRepository).GetWorkReviewVersion(ctx context.Context, contextID, workRevisionID, versionID int64) (*WorkReviewVersion, error)` | GetWorkReviewVersion returns one full version only when it belongs to the selected head ancestry. |
| [`(*ReviewRepository).getWorkReviewVersion`](../src/database/reviews.go#L639) | method | 639-675 | `func (*ReviewRepository).getWorkReviewVersion(ctx context.Context, q reviewQuerier, id int64) (*WorkReviewVersion, error)` | getWorkReviewVersion reads one immutable version with canonical sub-statuses and reviewer attribution. |
| [`validateReviewState`](../src/database/reviews.go#L678) | function | 678-706 | `func validateReviewState(status string, substatuses []string, reason *string) ([]string, *string, error)` | validateReviewState normalizes one complete review state and enforces vocabulary compatibility. |
| [`insertReviewAudit`](../src/database/reviews.go#L709) | function | 709-711 | `func insertReviewAudit(ctx context.Context, tx *sql.Tx, runID int64, entityType, entityID string, action manifest.AuditAction, metadata any) error` | insertReviewAudit appends identifier-only review evidence within the caller's head-move transaction. |
| [`insertReviewChangeAudit`](../src/database/reviews.go#L714) | function | 714-737 | `func insertReviewChangeAudit(ctx context.Context, tx *sql.Tx, runID int64, entityType, entityID string, action manifest.AuditAction, before, after, metadata any) error` | insertReviewChangeAudit appends identifier metadata and optional bounded decision-state changes. |
| [`marshalReviewAuditValue`](../src/database/reviews.go#L740) | function | 740-749 | `func marshalReviewAuditValue(name string, value any) (any, error)` | marshalReviewAuditValue returns a nullable JSON payload for one review audit field. |
| [`nullablePointer`](../src/database/reviews.go#L752) | function | 752-757 | `func nullablePointer[T any](value *T) any` | nullablePointer converts an optional value into a SQL parameter. |
| [`nullInt64Pointer`](../src/database/reviews.go#L760) | function | 760-766 | `func nullInt64Pointer(value sql.NullInt64) *int64` | nullInt64Pointer converts a scanned nullable integer into an optional value. |
| [`optionalStringEqual`](../src/database/reviews.go#L769) | function | 769-771 | `func optionalStringEqual(left, right *string) bool` | optionalStringEqual compares nullable normalized text. |
| [`stringSlicesEqual`](../src/database/reviews.go#L774) | function | 774-784 | `func stringSlicesEqual(left, right []string) bool` | stringSlicesEqual compares canonical ordered string sets. |
| [`reviewerDisplay`](../src/database/reviews.go#L787) | function | 787-793 | `func reviewerDisplay(username, _ string) string` | reviewerDisplay exposes only the optional username and never places reviewer email in portable API responses. |
| [`ReviewNoteVersion`](../src/database/reviews.go#L796) | struct | 796-811 | ``type ReviewNoteVersion struct { ID int64 `json:"id"` NoteID int64 `json:"note_id"` ParentVersionID *int64 `json:"parent_version_id,omitempty"` CreatedInContextID int64 `json:"created_in_context_id"` State string `json:"state"` Body *string `json:"body"` BodyBytes int `json:"body_bytes"` BodyTruncated bool `json:"body_truncated"` Title string `json:"title"` Excerpt string `json:"excerpt"` LinkCount int `json:"link_count"` CreatedAt string `json:"created_at"` ReviewerDisplay string `json:"reviewer_display"` Links []ReviewLink `json:"links"` }`` | ReviewNoteVersion is one immutable active note snapshot or deletion tombstone. |
| [`ReviewNote`](../src/database/reviews.go#L814) | struct | 814-821 | ``type ReviewNote struct { ID int64 `json:"id"` WorkID int64 `json:"work_id"` WorkRevisionID int64 `json:"work_revision_id"` CreatedAt string `json:"created_at"` Version ReviewNoteVersion `json:"version"` InheritedFromContextID *int64 `json:"inherited_from_context_id,omitempty"` }`` | ReviewNote is one logical note with the selected context's current immutable head. |
| [`ReviewLink`](../src/database/reviews.go#L824) | struct | 824-837 | ``type ReviewLink struct { Ordinal int `json:"ordinal"` TargetType string `json:"target_type"` RawTarget string `json:"raw_target"` DisplayText *string `json:"display_text,omitempty"` UTF16Position int `json:"utf16_position"` UTF16Length int `json:"utf16_length"` Resolved bool `json:"resolved"` WorkRevisionID *int64 `json:"work_revision_id,omitempty"` NoteID *int64 `json:"note_id,omitempty"` AnchorID *string `json:"anchor_id,omitempty"` Page *int `json:"page,omitempty"` URL *string `json:"url,omitempty"` }`` | ReviewLink is a version-scoped custom link plus its context-sensitive resolution. |
| [`(*ReviewRepository).CreateNote`](../src/database/reviews.go#L840) | method | 840-891 | `func (*ReviewRepository).CreateNote(ctx context.Context, contextID, workRevisionID int64, body string) (*ReviewNote, error)` | CreateNote creates one logical note, immutable first version, head, links, and audit atomically. |
| [`(*ReviewRepository).AppendNoteVersion`](../src/database/reviews.go#L894) | method | 894-997 | `func (*ReviewRepository).AppendNoteVersion(ctx context.Context, contextID, noteID int64, expectedVersionID int64, state, body string) (*ReviewNote, bool, error)` | AppendNoteVersion appends an active edit or deletion tombstone with optimistic concurrency. |
| [`(*ReviewRepository).GetNote`](../src/database/reviews.go#L1000) | method | 1000-1002 | `func (*ReviewRepository).GetNote(ctx context.Context, contextID, noteID int64) (*ReviewNote, error)` | GetNote returns an explicitly addressed current note head, including tombstones. |
| [`(*ReviewRepository).getNote`](../src/database/reviews.go#L1005) | method | 1005-1031 | `func (*ReviewRepository).getNote(ctx context.Context, q reviewQuerier, contextID, noteID int64) (*ReviewNote, error)` | getNote reads one selected context head and resolves inherited attribution. |
| [`(*ReviewRepository).ListNotes`](../src/database/reviews.go#L1034) | method | 1034-1040 | `func (*ReviewRepository).ListNotes(ctx context.Context, contextID, workRevisionID, cursor int64, limit int, includeDeleted bool) ([]ReviewNote, error)` | ListNotes returns bounded current note heads for one context work, excluding tombstones unless requested. |
| [`(*ReviewRepository).ListNotesFiltered`](../src/database/reviews.go#L1043) | method | 1043-1085 | `func (*ReviewRepository).ListNotesFiltered(ctx context.Context, contextID int64, workRevisionID *int64, cursor int64, limit int, state, query string) ([]ReviewNote, error)` | ListNotesFiltered returns bounded current note-head summaries for one work or the complete run context. |
| [`(*ReviewRepository).ListNoteVersions`](../src/database/reviews.go#L1088) | method | 1088-1137 | `func (*ReviewRepository).ListNoteVersions(ctx context.Context, contextID, noteID, cursor int64, limit int) ([]ReviewNoteVersion, error)` | ListNoteVersions follows only the selected context note head's ancestors. |
| [`(*ReviewRepository).GetNoteVersion`](../src/database/reviews.go#L1140) | method | 1140-1154 | `func (*ReviewRepository).GetNoteVersion(ctx context.Context, contextID, noteID, versionID int64) (*ReviewNoteVersion, error)` | GetNoteVersion returns one full body and link set only when it belongs to the selected head ancestry. |
| [`(*ReviewRepository).getNoteVersion`](../src/database/reviews.go#L1157) | method | 1157-1186 | `func (*ReviewRepository).getNoteVersion(ctx context.Context, q reviewQuerier, contextID, versionID int64) (*ReviewNoteVersion, error)` | getNoteVersion reads one immutable note version and resolves its version-scoped links. |
| [`insertNoteLinks`](../src/database/reviews.go#L1189) | function | 1189-1199 | `func insertNoteLinks(ctx context.Context, tx *sql.Tx, versionID int64, links []notes.Link) error` | insertNoteLinks stores parser output against the exact immutable note version. |
| [`(*ReviewRepository).linksForVersion`](../src/database/reviews.go#L1202) | method | 1202-1233 | `func (*ReviewRepository).linksForVersion(ctx context.Context, q reviewQuerier, contextID, noteID, versionID int64) ([]ReviewLink, error)` | linksForVersion reads links in source order and resolves them against the selected context. |
| [`(*ReviewRepository).resolveLink`](../src/database/reviews.go#L1236) | method | 1236-1303 | `func (*ReviewRepository).resolveLink(ctx context.Context, q queryRower, contextID, sourceNoteID int64, link *ReviewLink) error` | resolveLink enriches a syntactically valid link without rewriting persisted link identity. |
| [`(*ReviewRepository).ListBacklinks`](../src/database/reviews.go#L1306) | method | 1306-1331 | `func (*ReviewRepository).ListBacklinks(ctx context.Context, contextID int64, targetType, targetID string, sourceWorkID, cursor int64, limit int) ([]ReviewNote, error)` | ListBacklinks returns links from current note heads only. |
| [`scanReviewNoteSummaries`](../src/database/reviews.go#L1334) | function | 1334-1364 | `func scanReviewNoteSummaries(rows *sql.Rows, contextID int64) ([]ReviewNote, error)` | scanReviewNoteSummaries reads bounded list projections without loading full bodies or resolving every link. |
| [`noteSummary`](../src/database/reviews.go#L1367) | function | 1367-1382 | `func noteSummary(body string) (string, string)` | noteSummary derives a safe title and excerpt from one stored body or bounded prefix. |
| [`truncateRunes`](../src/database/reviews.go#L1385) | function | 1385-1391 | `func truncateRunes(value string, limit int) string` | truncateRunes returns a Unicode-safe bounded label with an explicit truncation marker. |
| [`AnchorRectangle`](../src/database/reviews.go#L1394) | struct | 1394-1399 | ``type AnchorRectangle struct { X float64 `json:"x"` Y float64 `json:"y"` Width float64 `json:"width"` Height float64 `json:"height"` }`` | AnchorRectangle is one normalized highlight rectangle on a single PDF page. |
| [`ReviewAnchorVersion`](../src/database/reviews.go#L1402) | struct | 1402-1416 | ``type ReviewAnchorVersion struct { ID int64 `json:"id"` AnchorID string `json:"anchor_id"` ParentVersionID *int64 `json:"parent_version_id,omitempty"` CreatedInContextID int64 `json:"created_in_context_id"` WorkRevisionID int64 `json:"work_revision_id"` PDFContentHash string `json:"pdf_content_hash"` State string `json:"state"` Page *int `json:"page,omitempty"` SelectedText *string `json:"selected_text,omitempty"` SelectedTextTruncated bool `json:"selected_text_truncated"` Rectangles []AnchorRectangle `json:"rectangles,omitempty"` CreatedAt string `json:"created_at"` ReviewerDisplay string `json:"reviewer_display"` }`` | ReviewAnchorVersion is one immutable active highlight snapshot or deletion tombstone. |
| [`ReviewAnchor`](../src/database/reviews.go#L1419) | struct | 1419-1426 | ``type ReviewAnchor struct { ID string `json:"id"` Label string `json:"label"` WorkID int64 `json:"work_id"` CreatedAt string `json:"created_at"` Version ReviewAnchorVersion `json:"version"` InheritedFromContextID *int64 `json:"inherited_from_context_id,omitempty"` }`` | ReviewAnchor is one stable corpus-wide anchor with the selected context's current head. |
| [`(*ReviewRepository).CreateAnchor`](../src/database/reviews.go#L1429) | method | 1429-1485 | `func (*ReviewRepository).CreateAnchor(ctx context.Context, contextID, workRevisionID int64, label, contentHash string, page int, selectedText string, rectangles []AnchorRectangle) (*ReviewAnchor, error)` | CreateAnchor creates one generated logical anchor with a work-scoped label and immutable first version atomically. |
| [`(*ReviewRepository).AppendAnchorVersion`](../src/database/reviews.go#L1488) | method | 1488-1577 | `func (*ReviewRepository).AppendAnchorVersion(ctx context.Context, contextID int64, anchorID string, expectedVersionID int64, state, contentHash string, page int, selectedText string, rectangles []AnchorRectangle) (*ReviewAnchor, bool, error)` | AppendAnchorVersion appends an active replacement or tombstone using optimistic concurrency. |
| [`(*ReviewRepository).GetAnchor`](../src/database/reviews.go#L1580) | method | 1580-1582 | `func (*ReviewRepository).GetAnchor(ctx context.Context, contextID int64, anchorID string) (*ReviewAnchor, error)` | GetAnchor returns one selected-context logical anchor and its current head. |
| [`(*ReviewRepository).ListAnchors`](../src/database/reviews.go#L1585) | method | 1585-1621 | `func (*ReviewRepository).ListAnchors(ctx context.Context, contextID, workRevisionID int64, cursor string, limit int) ([]ReviewAnchor, error)` | ListAnchors returns bounded active current anchors for one context work. |
| [`(*ReviewRepository).getAnchor`](../src/database/reviews.go#L1624) | method | 1624-1647 | `func (*ReviewRepository).getAnchor(ctx context.Context, contextID int64, anchorID string) (*ReviewAnchor, error)` | getAnchor reads one selected context anchor head with inherited attribution. |
| [`(*ReviewRepository).ListAnchorVersions`](../src/database/reviews.go#L1650) | method | 1650-1687 | `func (*ReviewRepository).ListAnchorVersions(ctx context.Context, contextID int64, anchorID string, cursor int64, limit int) ([]ReviewAnchorVersion, error)` | ListAnchorVersions follows only the selected context anchor head's ancestors. |
| [`(*ReviewRepository).GetAnchorVersion`](../src/database/reviews.go#L1690) | method | 1690-1704 | `func (*ReviewRepository).GetAnchorVersion(ctx context.Context, contextID int64, anchorID string, versionID int64) (*ReviewAnchorVersion, error)` | GetAnchorVersion returns one full geometry version only when it belongs to the selected head ancestry. |
| [`scanReviewAnchor`](../src/database/reviews.go#L1707) | function | 1707-1719 | `func scanReviewAnchor(scanner interface{ Scan(...any) error }, contextID int64) (ReviewAnchor, error)` | scanReviewAnchor reads one bounded logical-anchor list projection. |
| [`scanReviewAnchorVersion`](../src/database/reviews.go#L1722) | function | 1722-1724 | `func scanReviewAnchorVersion(scanner interface{ Scan(...any) error }) (ReviewAnchorVersion, error)` | scanReviewAnchorVersion reads one bounded immutable anchor-version projection. |
| [`scanReviewAnchorVersionWithPrefix`](../src/database/reviews.go#L1727) | function | 1727-1755 | `func scanReviewAnchorVersionWithPrefix(scanner interface{ Scan(...any) error }, anchor *ReviewAnchor) (ReviewAnchorVersion, error)` | scanReviewAnchorVersionWithPrefix shares decoding for logical-head and history projections. |
| [`(*ReviewRepository).getAnchorVersion`](../src/database/reviews.go#L1758) | method | 1758-1791 | `func (*ReviewRepository).getAnchorVersion(ctx context.Context, q queryRower, versionID int64) (*ReviewAnchorVersion, error)` | getAnchorVersion reads one immutable geometry snapshot or tombstone. |
| [`validateAnchorVersion`](../src/database/reviews.go#L1794) | function | 1794-1835 | `func validateAnchorVersion(anchorID, contentHash, state string, page int, selectedText string, rectangles []AnchorRectangle) error` | validateAnchorVersion enforces safe identity, PDF binding, state, and normalized geometry. |
| [`newAnchorID`](../src/database/reviews.go#L1838) | function | 1838-1844 | `func newAnchorID() (string, error)` | newAnchorID returns an opaque global identifier compatible with the note-language anchor grammar. |
| [`anchorsEqual`](../src/database/reviews.go#L1847) | function | 1847-1863 | `func anchorsEqual(current *ReviewAnchorVersion, state, contentHash string, page int, selectedText string, rectangles []AnchorRectangle) bool` | anchorsEqual detects an identical save so the repository can avoid redundant history. |
| [`sameNullableID`](../src/database/reviews.go#L1866) | function | 1866-1871 | `func sameNullableID(left, right *int64) bool` | sameNullableID compares optional immutable identifiers by value. |
| [`IsReviewConflict`](../src/database/reviews.go#L1874) | function | 1874-1877 | `func IsReviewConflict(err error) bool` | IsReviewConflict reports whether an error is an optimistic head conflict. |
...
| [`(*WorkRepository).CreateByDOI`](../src/database/works.go#L41) | method | 41-83 | `func (*WorkRepository).CreateByDOI(doi string) (int64, error)` | CreateByDOI inserts a new work by DOI. The DOI is normalized (lowercased, URL prefix stripped) before storage. If the normalized DOI already exists, returns the existing work ID (INSERT OR IGNORE semantics). |
| [`(*WorkRepository).CreateWithoutDOI`](../src/database/works.go#L87) | method | 87-101 | `func (*WorkRepository).CreateWithoutDOI() (int64, error)` | CreateWithoutDOI inserts a new work without a DOI (e.g. a title-only record). Each call creates a distinct row; uncertain records are never globally merged. |
| [`(*WorkRepository).GetByID`](../src/database/works.go#L104) | method | 104-123 | `func (*WorkRepository).GetByID(id int64) (*Work, error)` | GetByID returns a work by its primary key, or nil if not found. |
| [`(*WorkRepository).GetByDOI`](../src/database/works.go#L129) | method | 129-154 | `func (*WorkRepository).GetByDOI(doi string) (*Work, error)` | GetByDOI returns a work by its DOI, or nil if not found. The DOI is normalized the same way as CreateByDOI so that "10.1000/x", "10.1000/X", and "https://doi.org/10.1000/x" all resolve correctly. |
| [`(*WorkRepository).ListByIDs`](../src/database/works.go#L157) | method | 157-201 | `func (*WorkRepository).ListByIDs(ids []int64) ([]*Work, error)` | ListByIDs returns works matching the given IDs, in ID order. |
| [`(*WorkRepository).Count`](../src/database/works.go#L204) | method | 204-213 | `func (*WorkRepository).Count() (int, error)` | Count returns the total number of works. |
| [`WorkIdentifierRepository`](../src/database/works.go#L216) | struct | 216-218 | `type WorkIdentifierRepository struct { db *Database }` | WorkIdentifierRepository provides CRUD for the work_identifiers table. |
| [`(*WorkIdentifierRepository).Insert`](../src/database/works.go#L223) | method | 223-281 | `func (*WorkIdentifierRepository).Insert(workID int64, namespace, identifier string) (int64, error)` | Insert adds a new identifier for a work. If the (namespace, identifier) pair already exists for the same work, returns the existing ID. If it belongs to a different work, returns an error to prevent silent ownership conflicts. |
| [`(*WorkIdentifierRepository).GetByID`](../src/database/works.go#L284) | method | 284-300 | `func (*WorkIdentifierRepository).GetByID(id int64) (*WorkIdentifier, error)` | GetByID returns a work identifier by its primary key, or nil if not found. |
| [`(*WorkIdentifierRepository).GetByWorkID`](../src/database/works.go#L303) | method | 303-330 | `func (*WorkIdentifierRepository).GetByWorkID(workID int64) ([]*WorkIdentifier, error)` | GetByWorkID returns all identifiers for a given work, ordered by ID. |
| [`(*WorkIdentifierRepository).GetByNamespaceAndIdentifier`](../src/database/works.go#L334) | method | 334-351 | `func (*WorkIdentifierRepository).GetByNamespaceAndIdentifier(namespace, identifier string) (*WorkIdentifier, error)` | GetByNamespaceAndIdentifier returns the work identifier record for the given namespace and identifier pair, or nil if not found. |
| [`(*WorkIdentifierRepository).CountByWorkID`](../src/database/works.go#L354) | method | 354-365 | `func (*WorkIdentifierRepository).CountByWorkID(workID int64) (int, error)` | CountByWorkID returns the number of identifiers for a given work. |
| [`NormalizeDOI`](../src/database/works.go#L369) | function | 369-375 | `func NormalizeDOI(doi string) string` | NormalizeDOI applies the canonical DOI representation used by the works table and the companion PDF store. |
| [`WorkRevision`](../src/database/works.go#L381) | struct | 381-400 | ``type WorkRevision struct { ID int64 `json:"id"` WorkID int64 `json:"work_id"` PipelineRunID int64 `json:"pipeline_run_id"` ProducerStage string `json:"producer_stage"` FieldSchemaVersion string `json:"field_schema_version"` PayloadHash string `json:"payload_hash"` Title string `json:"title"` Abstract string `json:"abstract"` Year int `json:"year"` Journal string `json:"journal"` Publisher string `json:"publisher"` Source string `json:"source"` Keywords string `json:"keywords"` // JSON array KeywordsPlus string `json:"keywords_plus"` // JSON array CitationCount int `json:"citation_count"` ReferenceCount int `json:"reference_count"` ExtensionData string `json:"extension_data"` // JSON object CreatedAt string `json:"created_at"` }`` | WorkRevision is an immutable snapshot of a work's typed core metadata and extension data at the point it was produced by a pipeline run stage. producer_stage records which pipeline stage created the revision (e.g. "parse", "enrich"). |
| [`RunWorkStage`](../src/database/works.go#L407) | struct | 407-416 | ``type RunWorkStage struct { ID int64 `json:"id"` PipelineRunID int64 `json:"pipeline_run_id"` WorkID int64 `json:"work_id"` StageName string `json:"stage_name"` Outcome string `json:"outcome"` Reason string `json:"reason"` CreatedAt string `json:"created_at"` UpdatedAt string `json:"updated_at"` }`` | RunWorkStage records what happened to one work at one pipeline stage within a single pipeline run. The (pipeline_run_id, work_id, stage_name) triplet is unique so the same stage cannot report two different outcomes for the same work in the same run. created_at is the first time the outcome was set; updated_at is the most recent time it changed (via ON CONFLICT DO UPDATE). |
| [`WorkRevisionRepository`](../src/database/works.go#L465) | struct | 465-467 | `type WorkRevisionRepository struct { db *Database }` | WorkRevisionRepository provides CRUD for the work_revisions table. |
| [`(*WorkRevisionRepository).Create`](../src/database/works.go#L473) | method | 473-521 | `func (*WorkRevisionRepository).Create(rev *WorkRevision) (int64, error)` | Create inserts a new immutable work revision and returns its ID. The payload hash is computed from the supplied core fields and extension data. If FieldSchemaVersion is empty, it defaults to "1". ProducerStage must be a known pipeline stage; legacy_unknown is rejected. |
| [`(*WorkRevisionRepository).GetByID`](../src/database/works.go#L524) | method | 524-532 | `func (*WorkRevisionRepository).GetByID(id int64) (*WorkRevision, error)` | GetByID returns a work revision by its primary key, or nil if not found. |
| [`(*WorkRevisionRepository).GetByWorkID`](../src/database/works.go#L536) | method | 536-565 | `func (*WorkRevisionRepository).GetByWorkID(workID int64) ([]*WorkRevision, error)` | GetByWorkID returns all revisions for a given work, ordered by ID ascending (chronological order). |
| [`(*WorkRevisionRepository).GetByRunID`](../src/database/works.go#L568) | method | 568-597 | `func (*WorkRevisionRepository).GetByRunID(runID int64) ([]*WorkRevision, error)` | GetByRunID returns all revisions created by a given pipeline run. |
| [`(*WorkRevisionRepository).CountByWorkID`](../src/database/works.go#L600) | method | 600-611 | `func (*WorkRevisionRepository).CountByWorkID(workID int64) (int, error)` | CountByWorkID returns the number of revisions for a given work. |
| [`RunWorkStageRepository`](../src/database/works.go#L614) | struct | 614-616 | `type RunWorkStageRepository struct { db *Database }` | RunWorkStageRepository provides CRUD for the run_work_stages table. |
| [`(*RunWorkStageRepository).SetOutcome`](../src/database/works.go#L623) | method | 623-654 | `func (*RunWorkStageRepository).SetOutcome(runID, workID int64, stageName, outcome, reason string) error` | SetOutcome inserts or updates a stage outcome for a given work in a run. Uses INSERT ... ON CONFLICT DO UPDATE so the row identity (id, created_at) is preserved across progressive outcome updates (e.g. "pending" -> "parsed"). Stage names and outcomes are validated against known stage/outcome pairs; impossible combinations (e.g. parse/valid) are rejected. |
| [`validProducerStage`](../src/database/works.go#L722) | function | 722-724 | `func validProducerStage(s string) bool` | validProducerStage reports whether the supplied producer stage is supported. |
| [`validStageName`](../src/database/works.go#L727) | function | 727-729 | `func validStageName(s string) bool` | validStageName reports whether the supplied stage name is supported. |
| [`validStageOutcomeForStage`](../src/database/works.go#L732) | function | 732-738 | `func validStageOutcomeForStage(stageName, outcome string) bool` | validStageOutcomeForStage returns true if outcome is allowed for the given stage. |
| [`(*RunWorkStageRepository).GetByRunAndWork`](../src/database/works.go#L742) | method | 742-748 | `func (*RunWorkStageRepository).GetByRunAndWork(runID, workID int64, stageName string) (*RunWorkStage, error)` | GetByRunAndWork returns the stage outcome for a specific work, run, and stage name, or nil if not found. |
| [`(*RunWorkStageRepository).GetByRunID`](../src/database/works.go#L751) | method | 751-775 | `func (*RunWorkStageRepository).GetByRunID(runID int64) ([]*RunWorkStage, error)` | GetByRunID returns all stage outcomes for a given pipeline run, ordered by ID. |
| [`(*RunWorkStageRepository).GetByWorkID`](../src/database/works.go#L778) | method | 778-802 | `func (*RunWorkStageRepository).GetByWorkID(workID int64) ([]*RunWorkStage, error)` | GetByWorkID returns all stage outcomes across runs for a given work, ordered by ID. |
| [`(*RunWorkStageRepository).CountByStageAndOutcome`](../src/database/works.go#L806) | method | 806-819 | `func (*RunWorkStageRepository).CountByStageAndOutcome(runID int64, stageName, outcome string) (int, error)` | CountByStageAndOutcome returns the number of works that reached a given stage outcome within a pipeline run. Useful for dashboard funnel counts. |
| [`scannable`](../src/database/works.go#L822) | interface | 822-824 | `type scannable interface { Scan(dest ...any) error }` | scannable defines the behavior required of scannable implementations. |
| [`scanWorkRevision`](../src/database/works.go#L827) | function | 827-863 | `func scanWorkRevision(row scannable) (*WorkRevision, error)` | scanWorkRevision decodes work revision from a database row. |
| [`scanRunWorkStage`](../src/database/works.go#L866) | function | 866-880 | `func scanRunWorkStage(row scannable) (*RunWorkStage, error)` | scanRunWorkStage decodes run work stage from a database row. |
| [`nullStrPtrVal`](../src/database/works.go#L883) | function | 883-888 | `func nullStrPtrVal(ns sql.NullString) string` | nullStrPtrVal returns a nullable SQL string's value or an empty string. |
| [`computeRevisionPayloadHash`](../src/database/works.go#L895) | function | 895-925 | `func computeRevisionPayloadHash(rev *WorkRevision) string` | computeRevisionPayloadHash computes a deterministic SHA-256 hex hash from the typed core fields and extension data of a WorkRevision. producer_stage is excluded because it is provenance metadata, not content. field_schema_version is included because it affects how the fields should be interpreted. |
...
| Symbol | Kind | Lines | Declaration, inputs, and outputs | Source description |
|---|---|---:|---|---|
| [`Client`](../src/enrich/client.go#L29) | struct | 29-33 | `type Client struct { cfg SourceConfig ticker *time.Ticker client *http.Client }` | Client fetches URLs concurrently with configurable rate limiting and exponential backoff on 429 responses. |
| [`NewClient`](../src/enrich/client.go#L41) | function | 41-54 | `func NewClient(cfg SourceConfig) (*Client, error)` | NewClient creates a Client for the given validated source config. |
| [`ValidateSourceConfig`](../src/enrich/client.go#L57) | function | 57-95 | `func ValidateSourceConfig(cfg SourceConfig) error` | ValidateSourceConfig verifies provider endpoints and request bounds before enrichment begins. |
| [`validProviderURL`](../src/enrich/client.go#L98) | function | 98-104 | `func validProviderURL(label, value string) error` | validProviderURL verifies an HTTP(S) provider endpoint has a host. |
| [`(*Client).Close`](../src/enrich/client.go#L107) | method | 107-109 | `func (*Client).Close()` | Close stops the rate-limit ticker. |
| [`FetchResult`](../src/enrich/client.go#L112) | struct | 112-116 | `type FetchResult struct { Body []byte StatusCode int Err error }` | FetchResult carries the response body (or error) for one URL fetch. |
| [`(*Client).FetchAll`](../src/enrich/client.go#L121) | method | 121-226 | `func (*Client).FetchAll(ctx context.Context, urls []string) map[string]*FetchResult` | FetchAll fetches all URLs concurrently using a goroutine pool. It respects rate limiting across all workers and applies exponential backoff on 429. Returns a map of original URL -> FetchResult in the same order as input. |
| [`(*Client).Fetch`](../src/enrich/client.go#L231) | method | 231-233 | `func (*Client).Fetch(ctx context.Context, url string) *FetchResult` | Fetch retrieves one URL through the same rate-limited path as FetchAll. Workspace cache policy uses it so individual cache misses do not bypass the provider's configured request limit. |
| [`(*Client).fetchOne`](../src/enrich/client.go#L236) | method | 236-334 | `func (*Client).fetchOne(ctx context.Context, url string) *FetchResult` | fetchOne performs a single HTTP GET with retries and exponential backoff. |
...
| [`TestNewClientRejectsInvalidSourceConfig`](../src/enrich/client_unit_test.go#L42) | test | 42-69 | `func TestNewClientRejectsInvalidSourceConfig(t *testing.T)` | TestNewClientRejectsInvalidSourceConfig verifies defensive validation for direct callers. |
| [`TestRequestLogTargetExcludesQuery`](../src/enrich/client_unit_test.go#L72) | test | 72-77 | `func TestRequestLogTargetExcludesQuery(t *testing.T)` | TestRequestLogTargetExcludesQuery verifies provider logs cannot contain request query data. |

### [`src/enrich/crossref.go`](../src/enrich/crossref.go)

| Symbol | Kind | Lines | Declaration, inputs, and outputs | Source description |
|---|---|---:|---|---|
| [`extractCrossrefEntry`](../src/enrich/crossref.go#L15) | function | 15-26 | `func extractCrossrefEntry(body []byte) map[string]any` | extractCrossrefEntry parses the Crossref API response and returns a generic map of extracted fields. |
| [`DecodeCrossrefResponse`](../src/enrich/crossref.go#L31) | function | 31-40 | `func DecodeCrossrefResponse(body []byte, doi string) *ArticleEnrichment` | DecodeCrossrefResponse converts a raw Crossref work response into the workspace enrichment representation. The workspace owns cache policy and persistence; this package remains responsible only for decoding. |
| [`crossrefEntryToArticle`](../src/enrich/crossref.go#L44) | function | 44-70 | `func crossrefEntryToArticle(entry map[string]any, doi string) *ArticleEnrichment` | crossrefEntryToArticle converts a Crossref API message map to an ArticleEnrichment. |
| [`extractCrossrefAuthors`](../src/enrich/crossref.go#L73) | function | 73-116 | `func extractCrossrefAuthors(authors []any) []EnrichedAuthor` | extractCrossrefAuthors parses the Crossref author array. |
| [`extractCrossrefReferences`](../src/enrich/crossref.go#L119) | function | 119-162 | `func extractCrossrefReferences(refs []any) []EnrichedReference` | extractCrossrefReferences parses the Crossref reference array. |
...
| Symbol | Kind | Lines | Declaration, inputs, and outputs | Source description |
|---|---|---:|---|---|
| [`SourceConfig`](../src/enrich/enrich.go#L15) | struct | 15-28 | `type SourceConfig struct { Name string BaseURL string UserAgent string ContactEmail string RatePerSecond int Concurrency int TimeoutSecs int MaxRetries int Fields []string ExtraURLs map[string]string BatchSize int FillMissingOnly bool }` | SourceConfig describes an enrichment source as declared in a SOMETHING configuration. |
| [`Config`](../src/enrich/enrich.go#L31) | struct | 31-33 | `type Config struct { Sources map[string]SourceConfig }` | Config holds all enrichment sources keyed by their configuration name. |
| [`EnrichedAuthor`](../src/enrich/enrich.go#L36) | struct | 36-49 | `type EnrichedAuthor struct { ORCID string FirstName string LastName string CitationName string Affiliation string DisplayName string WorksCount int CitedByCnt int HIndex int I10Index int Institution string Source string }` | EnrichedAuthor is the per-author result of enrichment. |
| [`EnrichedReference`](../src/enrich/enrich.go#L52) | struct | 52-59 | ``type EnrichedReference struct { Raw string `json:"raw"` DOI string `json:"doi"` Title string `json:"title"` Author string `json:"author"` Year int `json:"year"` Source string `json:"source"` }`` | EnrichedReference is enriched metadata for one cited reference. |
| [`ArticleEnrichment`](../src/enrich/enrich.go#L62) | struct | 62-71 | `type ArticleEnrichment struct { DOI string Title string Abstract string Authors []EnrichedAuthor Publisher string References []EnrichedReference CitationCount int ReferenceCount int }` | ArticleEnrichment holds gathered data for one article from one source. |
| [`GatherResult`](../src/enrich/enrich.go#L74) | struct | 74-81 | `type GatherResult struct { Source string FillMissingOnly bool Articles map[string]*ArticleEnrichment Authors map[string]*EnrichedAuthor AuthorMatches map[string]string DOINotFound []string }` | GatherResult is the pure output of one gather step. |
| [`(SourceConfig).AllowsField`](../src/enrich/enrich.go#L84) | method | 84-89 | `func (SourceConfig).AllowsField(field string) bool` | AllowsField reports whether the configured field selection permits a canonical field; an omitted list preserves all-field behavior. |

### [`src/enrich/openalex.go`](../src/enrich/openalex.go)

| Symbol | Kind | Lines | Declaration, inputs, and outputs | Source description |
|---|---|---:|---|---|
| [`DecodeOpenAlexResponse`](../src/enrich/openalex.go#L17) | function | 17-26 | `func DecodeOpenAlexResponse(body []byte, doi string) (*ArticleEnrichment, []string)` | DecodeOpenAlexResponse converts one raw work response. Reference metadata is populated separately by DecodeOpenAlexReferenceResponse so the workspace can cache work and reference lookups independently. |
| [`DecodeOpenAlexReferenceResponse`](../src/enrich/openalex.go#L30) | function | 30-57 | `func DecodeOpenAlexReferenceResponse(body []byte) map[string]EnrichedReference` | DecodeOpenAlexReferenceResponse converts an OpenAlex batch response to references keyed by OpenAlex work ID (for example, W123). |
| [`openalexEntryToArticle`](../src/enrich/openalex.go#L60) | function | 60-82 | `func openalexEntryToArticle(entry map[string]any, doi string) *ArticleEnrichment` | openalexEntryToArticle converts an OpenAlex work object to article enrichment fields. |
| [`reconstructAbstract`](../src/enrich/openalex.go#L85) | function | 85-111 | `func reconstructAbstract(index map[string]any) string` | reconstructAbstract rebuilds abstract text from OpenAlex's word-position index. |
| [`extractOpenAlexPublisher`](../src/enrich/openalex.go#L114) | function | 114-126 | `func extractOpenAlexPublisher(entry map[string]any) string` | extractOpenAlexPublisher returns the direct or primary-source publisher name from a work object. |
| [`extractOpenAlexAuthors`](../src/enrich/openalex.go#L129) | function | 129-164 | `func extractOpenAlexAuthors(authorships []any) []EnrichedAuthor` | extractOpenAlexAuthors converts usable OpenAlex authorships to enriched authors in source order. |
...
| [`DecodeORCIDRecordResponse`](../src/enrich/orcid.go#L27) | function | 27-34 | `func DecodeORCIDRecordResponse(body []byte, orcid string) *EnrichedAuthor` | DecodeORCIDRecordResponse converts an ORCID person record for an exact ORCID lookup. |
| [`decodeORCIDRecord`](../src/enrich/orcid.go#L37) | function | 37-73 | `func decodeORCIDRecord(body []byte) map[string]any` | decodeORCIDRecord decodes orcid record from the supplied payload. |
| [`ORCIDNameSearchURLs`](../src/enrich/orcid.go#L78) | function | 78-106 | `func ORCIDNameSearchURLs(source SourceConfig, name string) []string` | ORCIDNameSearchURLs returns the ordered exact-name search requests used by the workspace pipeline. Candidates from every successful query remain evidence for review and are never treated as identity proof. |
| [`ORCIDNameSearchCandidate`](../src/enrich/orcid.go#L110) | struct | 110-112 | `type ORCIDNameSearchCandidate struct { ORCID string }` | ORCIDNameSearchCandidate is provider evidence only, never proof that the citation author is the person identified by the returned ORCID. |
| [`DecodeORCIDNameSearchCandidates`](../src/enrich/orcid.go#L116) | function | 116-134 | `func DecodeORCIDNameSearchCandidates(body []byte) []ORCIDNameSearchCandidate` | DecodeORCIDNameSearchCandidates returns every usable ORCID in provider order. Workspace callers retain this ambiguity for human review. |
| [`orcidEntryToAuthor`](../src/enrich/orcid.go#L137) | function | 137-185 | `func orcidEntryToAuthor(entry map[string]any, orcid string) *EnrichedAuthor` | orcidEntryToAuthor converts a validated ORCID record to author enrichment fields. |
| [`orcidSearchLiteral`](../src/enrich/orcid.go#L188) | function | 188-199 | `func orcidSearchLiteral(value string) string` | orcidSearchLiteral returns a quoted ORCID search literal with query syntax escaped. |
...
| Symbol | Kind | Lines | Declaration, inputs, and outputs | Source description |
|---|---|---:|---|---|
| [`decodeJSONObject`](../src/enrich/payload_validation.go#L13) | function | 13-19 | `func decodeJSONObject(body []byte) map[string]any` | decodeJSONObject decodes one JSON object and returns nil for malformed input. |
| [`ValidateProviderPayload`](../src/enrich/payload_validation.go#L25) | function | 25-61 | `func ValidateProviderPayload(provider, namespace string, body []byte) error` | ValidateProviderPayload verifies that a successful provider response has the envelope expected by the workspace decoder. It deliberately validates only transport envelopes, not optional bibliographic fields: a valid provider record may legitimately omit a requested field. |
| [`requireObject`](../src/enrich/payload_validation.go#L64) | function | 64-74 | `func requireObject(object map[string]json.RawMessage, key string) error` | requireObject requires a valid object value. |
| [`requireArray`](../src/enrich/payload_validation.go#L77) | function | 77-87 | `func requireArray(object map[string]json.RawMessage, key string) error` | requireArray requires a valid array value. |
| [`requireArrayOrNull`](../src/enrich/payload_validation.go#L90) | function | 90-103 | `func requireArrayOrNull(object map[string]json.RawMessage, key string) error` | requireArrayOrNull requires a valid array or null value. |
| [`requireString`](../src/enrich/payload_validation.go#L106) | function | 106-116 | `func requireString(object map[string]json.RawMessage, key string) error` | requireString requires a valid string value. |
| [`ValidateWorkIdentity`](../src/enrich/payload_validation.go#L119) | function | 119-133 | `func ValidateWorkIdentity(provider string, body []byte, doi string) error` | ValidateWorkIdentity rejects metadata carrying a different structured DOI while permitting omitted optional identity. |
...
### [`src/internal/pathpolicy/pathpolicy.go`](../src/internal/pathpolicy/pathpolicy.go)

| Symbol | Kind | Lines | Declaration, inputs, and outputs | Source description |
|---|---|---:|---|---|
| [`ResolveExistingWithin`](../src/internal/pathpolicy/pathpolicy.go#L12) | function | 12-28 | `func ResolveExistingWithin(root, candidate string) (string, error)` | ResolveExistingWithin resolves an existing candidate and requires both its lexical and resolved paths to remain below root. |
| [`ResolveExistingComponentsWithin`](../src/internal/pathpolicy/pathpolicy.go#L31) | function | 31-47 | `func ResolveExistingComponentsWithin(root, candidate string) (string, error)` | ResolveExistingComponentsWithin resolves every existing candidate component while allowing a missing final file. |
| [`Within`](../src/internal/pathpolicy/pathpolicy.go#L50) | function | 50-53 | `func Within(root, path string) bool` | Within reports whether path is root itself or is contained below root. |
| [`resolveRootAndCandidate`](../src/internal/pathpolicy/pathpolicy.go#L56) | function | 56-73 | `func resolveRootAndCandidate(root, candidate string) (string, string, string, error)` | resolveRootAndCandidate returns absolute lexical and resolved root paths for one candidate. |
| [`resolveExistingComponents`](../src/internal/pathpolicy/pathpolicy.go#L76) | function | 76-100 | `func resolveExistingComponents(path string) (string, error)` | resolveExistingComponents resolves all existing components while retaining a not-yet-created final component. |

### [`src/internal/pathpolicy/pathpolicy_unit_test.go`](../src/internal/pathpolicy/pathpolicy_unit_test.go)

| Symbol | Kind | Lines | Declaration, inputs, and outputs | Source description |
|---|---|---:|---|---|
| [`TestResolveExistingWithinRejectsEscapes`](../src/internal/pathpolicy/pathpolicy_unit_test.go#L11) | test | 11-29 | `func TestResolveExistingWithinRejectsEscapes(t *testing.T)` | TestResolveExistingWithinRejectsEscapes verifies contained paths resolve while lexical and symbolic-link escapes fail. |
| [`TestResolveExistingComponentsWithinAllowsNewFinalFile`](../src/internal/pathpolicy/pathpolicy_unit_test.go#L32) | test | 32-41 | `func TestResolveExistingComponentsWithinAllowsNewFinalFile(t *testing.T)` | TestResolveExistingComponentsWithinAllowsNewFinalFile verifies a missing final file retains resolved containment. |

### [`src/internal/sqliteuri/sqliteuri.go`](../src/internal/sqliteuri/sqliteuri.go)

| Symbol | Kind | Lines | Declaration, inputs, and outputs | Source description |
|---|---|---:|---|---|
| [`File`](../src/internal/sqliteuri/sqliteuri.go#L8) | function | 8-10 | `func File(path string, parameters map[string][]string) string` | File returns a SQLite file URI for path with the supplied query parameters. Repeated parameters, including modernc SQLite pragmas, retain their values. |

### [`src/internal/sqliteuri/sqliteuri_unit_test.go`](../src/internal/sqliteuri/sqliteuri_unit_test.go)

| Symbol | Kind | Lines | Declaration, inputs, and outputs | Source description |
|---|---|---:|---|---|
| [`TestFileEncodesPathAndRepeatedParameters`](../src/internal/sqliteuri/sqliteuri_unit_test.go#L9) | test | 9-31 | `func TestFileEncodesPathAndRepeatedParameters(t *testing.T)` | TestFileEncodesPathAndRepeatedParameters verifies file paths and repeated pragmas retain their exact values. |

### [`src/internal/textlimit/textlimit.go`](../src/internal/textlimit/textlimit.go)

| Symbol | Kind | Lines | Declaration, inputs, and outputs | Source description |
|---|---|---:|---|---|
| [`UTF8Prefix`](../src/internal/textlimit/textlimit.go#L7) | function | 7-9 | `func UTF8Prefix(value string, limit int) string` | UTF8Prefix returns a valid UTF-8 prefix of value that fits within limit bytes. |
| [`UTF8PrefixBytes`](../src/internal/textlimit/textlimit.go#L12) | function | 12-27 | `func UTF8PrefixBytes(value []byte, limit int) []byte` | UTF8PrefixBytes returns a valid UTF-8 prefix of value that fits within limit bytes. |

### [`src/internal/textlimit/textlimit_unit_test.go`](../src/internal/textlimit/textlimit_unit_test.go)

| Symbol | Kind | Lines | Declaration, inputs, and outputs | Source description |
|---|---|---:|---|---|
| [`TestUTF8PrefixPreservesByteLimitsAndEncoding`](../src/internal/textlimit/textlimit_unit_test.go#L10) | test | 10-18 | `func TestUTF8PrefixPreservesByteLimitsAndEncoding(t *testing.T)` | TestUTF8PrefixPreservesByteLimitsAndEncoding verifies multibyte prefixes never produce invalid output. |
| [`TestUTF8PrefixBytesPreservesByteLimitsAndEncoding`](../src/internal/textlimit/textlimit_unit_test.go#L21) | test | 21-27 | `func TestUTF8PrefixBytesPreservesByteLimitsAndEncoding(t *testing.T)` | TestUTF8PrefixBytesPreservesByteLimitsAndEncoding verifies byte callers receive the same safe prefix. |
...
| Symbol | Kind | Lines | Declaration, inputs, and outputs | Source description |
|---|---|---:|---|---|
| [`version`](../src/main.go#L33) | function | 33-35 | `func version() string` | version returns the semantic version string. |
| [`usage`](../src/main.go#L38) | function | 38-100 | `func usage()` | usage writes the supported command syntax to standard error. |
| [`main`](../src/main.go#L103) | function | 103-127 | `func main()` | main dispatches the analysis command selected by process arguments and exits on command failure. |
| [`migrateMain`](../src/main.go#L130) | function | 130-143 | `func migrateMain()` | migrateMain applies pending metadata migrations to one existing database. |
| [`serveMain`](../src/main.go#L146) | function | 146-187 | `func serveMain()` | serveMain serves main. |
| [`validateLoopbackAddress`](../src/main.go#L190) | function | 190-200 | `func validateLoopbackAddress(address string) error` | validateLoopbackAddress accepts exact IP loopback authorities and rejects names and wildcard hosts. |
| [`frontendAssets`](../src/main.go#L203) | function | 203-212 | `func frontendAssets(dir string) (fs.FS, error)` | frontendAssets validates one frontend asset directory and exposes it as a file system. |
| [`runPipelineMain`](../src/main.go#L215) | function | 215-247 | `func runPipelineMain()` | runPipelineMain parses run flags, resolves workspace selections, and executes each pipeline workspace. |
| [`changeToRepositoryRoot`](../src/main.go#L250) | function | 250-256 | `func changeToRepositoryRoot()` | changeToRepositoryRoot moves one directory upward only when execution starts inside the module directory. |
| [`recoverMain`](../src/main.go#L259) | function | 259-273 | `func recoverMain()` | recoverMain recovers one abandoned attempt without migrating or executing a workspace. |
...
| [`TestWorkspaceAttemptRecordsDeclaredEnrichmentPolicy`](../src/main_test.go#L555) | test | 555-604 | `func TestWorkspaceAttemptRecordsDeclaredEnrichmentPolicy(t *testing.T)` | TestWorkspaceAttemptRecordsDeclaredEnrichmentPolicy verifies workspace attempt records declared enrichment policy. |
| [`TestWorkspacePipelineRecordsUnreadableSourcePreflightFailure`](../src/main_test.go#L607) | test | 607-695 | `func TestWorkspacePipelineRecordsUnreadableSourcePreflightFailure(t *testing.T)` | TestWorkspacePipelineRecordsUnreadableSourcePreflightFailure verifies workspace pipeline records unreadable source preflight failure. |
| [`TestWorkspacePipelineFailsForEmptyOrMalformedSource`](../src/main_test.go#L698) | test | 698-785 | `func TestWorkspacePipelineFailsForEmptyOrMalformedSource(t *testing.T)` | TestWorkspacePipelineFailsForEmptyOrMalformedSource verifies workspace pipeline fails for empty or malformed source. |
| [`TestWorkspacePipelineRecordsInformationalExpectedResultCounts`](../src/main_test.go#L788) | test | 788-839 | `func TestWorkspacePipelineRecordsInformationalExpectedResultCounts(t *testing.T)` | TestWorkspacePipelineRecordsInformationalExpectedResultCounts verifies workspace pipeline records informational expected result counts. |
| [`TestWorkspacePipelineRetainsSourceRecordsRejectedDuringCanonicalConversion`](../src/main_test.go#L842) | test | 842-877 | `func TestWorkspacePipelineRetainsSourceRecordsRejectedDuringCanonicalConversion(t *testing.T)` | TestWorkspacePipelineRetainsSourceRecordsRejectedDuringCanonicalConversion verifies workspace pipeline retains source records rejected during canonical conversion. |
| [`TestFrontendAssets`](../src/main_test.go#L880) | test | 880-904 | `func TestFrontendAssets(t *testing.T)` | TestFrontendAssets verifies frontend asset directory validation. |
| [`TestVersion`](../src/main_test.go#L907) | test | 907-915 | `func TestVersion(t *testing.T)` | TestVersion verifies the version command output format and current value. |
| [`TestValidateLoopbackAddress`](../src/main_test.go#L918) | test | 918-929 | `func TestValidateLoopbackAddress(t *testing.T)` | TestValidateLoopbackAddress verifies writable serving rejects names, wildcards, and remote IPs. |
| [`chdirToRepositoryRoot`](../src/main_test.go#L932) | function | 932-947 | `func chdirToRepositoryRoot(t *testing.T)` | chdirToRepositoryRoot supports the package test suite's chdir to repository root setup or assertions. |
| [`testWorkspaceRun`](../src/main_test.go#L950) | function | 950-968 | `func testWorkspaceRun(sourcePath string) *workspace.Run` | testWorkspaceRun supports the package test suite's test workspace run setup or assertions. |
...
| [`ValidateAttemptStatus`](../src/manifest/lifecycle.go#L26) | function | 26-33 | `func ValidateAttemptStatus(s string) error` | ValidateAttemptStatus returns an error if s is not a valid attempt status. |
| [`StageOutcome`](../src/manifest/lifecycle.go#L36) | type | 36 | `type StageOutcome string` | StageOutcome represents the result of a single pipeline stage. |
| [`ValidateStageOutcome`](../src/manifest/lifecycle.go#L55) | function | 55-62 | `func ValidateStageOutcome(s string) error` | ValidateStageOutcome returns an error if s is not a valid stage outcome. |
| [`CacheOutcome`](../src/manifest/lifecycle.go#L68) | type | 68 | `type CacheOutcome string` | CacheOutcome represents the result of a cache lookup operation. It describes what the cache layer returned, not where the data was ultimately resolved. The resolution source (network, prior-run snapshot, etc.) is tracked separately as a fetch/resolution event. |
| [`ValidateCacheOutcome`](../src/manifest/lifecycle.go#L82) | function | 82-89 | `func ValidateCacheOutcome(s string) error` | ValidateCacheOutcome returns an error if s is not a valid cache outcome. |
| [`RunVisibility`](../src/manifest/lifecycle.go#L92) | type | 92 | `type RunVisibility string` | RunVisibility represents the visibility state of a pipeline run. |
| [`ValidateRunVisibility`](../src/manifest/lifecycle.go#L106) | function | 106-113 | `func ValidateRunVisibility(s string) error` | ValidateRunVisibility returns an error if s is not a valid run visibility. |
| [`AuditAction`](../src/manifest/lifecycle.go#L116) | type | 116 | `type AuditAction string` | AuditAction identifies one kind of append-only audit event. |
| [`ValidateAuditAction`](../src/manifest/lifecycle.go#L175) | function | 175-191 | `func ValidateAuditAction(s string) error` | ValidateAuditAction returns an error if s is not a valid audit action. |
| [`AuditEvent`](../src/manifest/lifecycle.go#L197) | struct | 197-232 | ``type AuditEvent struct { // OccurredAt is the ISO 8601 UTC timestamp of the event. OccurredAt string `json:"occurred_at"` // Actor identifies the system component or user that produced the event // (e.g. "pipeline", "viewer", "admin"). Actor string `json:"actor"` // PipelineRunID links this event to a specific pipeline run attempt. // Zero means the event is not associated with a specific run. PipelineRunID int64 `json:"pipeline_run_id,omitempty"` // EntityType identifies the kind of entity affected (e.g. "article", // "author", "plan", "run", "source"). EntityType string `json:"entity_type"` // EntityID is the identifier of the affected entity. EntityID string `json:"entity_id"` // Action is the audit action name. Action AuditAction `json:"action"` // BeforeJSON is the JSON representation of the entity state before the // action, if applicable. BeforeJSON string `json:"before_json,omitempty"` // AfterJSON is the JSON representation of the entity state after the // action, if applicable. AfterJSON string `json:"after_json,omitempty"` // MetadataJSON is additional event-specific context as JSON. MetadataJSON string `json:"metadata_json,omitempty"` // CorrelationID is an optional idempotency key for deduplication. CorrelationID string `json:"correlation_id,omitempty"` }`` | AuditEvent is a single append-only audit record. Every event records when it occurred, which actor produced it, which pipeline run it belongs to, the affected entity, the action performed, optional before/after state, and a correlation id for deduplication. |
| [`RetentionPolicy`](../src/manifest/lifecycle.go#L237) | struct | 237-241 | ``type RetentionPolicy struct { // TrashRetentionDays is the minimum number of days a trashed run is kept // before it becomes eligible for purge. Zero means indefinite retention. TrashRetentionDays int `json:"trash_retention_days"` }`` | RetentionPolicy describes how long trashed run data is retained before it becomes eligible for purge. Audit events are never deleted; RetentionPolicy does not govern audit records. |
| [`PurgePolicy`](../src/manifest/lifecycle.go#L245) | struct | 245-254 | ``type PurgePolicy struct { // RequireVerification, when true, requires that purging a run first // verifies no shared artifacts or cache entries are referenced by other // runs. RequireVerification bool `json:"require_verification"` // KeepTombstone, when true, retains a lightweight purge event or tombstone // record instead of removing all evidence of the purged run. KeepTombstone bool `json:"keep_tombstone"` }`` | PurgePolicy describes the authorization and safety checks required before data is permanently removed. |
| [`DefaultRetentionPolicy`](../src/manifest/lifecycle.go#L257) | function | 257-261 | `func DefaultRetentionPolicy() RetentionPolicy` | DefaultRetentionPolicy returns the default retention policy. |
| [`DefaultPurgePolicy`](../src/manifest/lifecycle.go#L264) | function | 264-269 | `func DefaultPurgePolicy() PurgePolicy` | DefaultPurgePolicy returns the default purge policy. |

### [`src/manifest/lifecycle_unit_test.go`](../src/manifest/lifecycle_unit_test.go)

| Symbol | Kind | Lines | Declaration, inputs, and outputs | Source description |
|---|---|---:|---|---|
| [`TestValidateAttemptStatusInvalid`](../src/manifest/lifecycle_unit_test.go#L11) | test | 11-15 | `func TestValidateAttemptStatusInvalid(t *testing.T)` | TestValidateAttemptStatusInvalid verifies validate attempt status invalid. |
| [`TestValidateStageOutcomeInvalid`](../src/manifest/lifecycle_unit_test.go#L18) | test | 18-22 | `func TestValidateStageOutcomeInvalid(t *testing.T)` | TestValidateStageOutcomeInvalid verifies validate stage outcome invalid. |
| [`TestValidateStageOutcomeEmpty`](../src/manifest/lifecycle_unit_test.go#L25) | test | 25-29 | `func TestValidateStageOutcomeEmpty(t *testing.T)` | TestValidateStageOutcomeEmpty verifies validate stage outcome empty. |
| [`TestValidateCacheOutcomeInvalid`](../src/manifest/lifecycle_unit_test.go#L32) | test | 32-36 | `func TestValidateCacheOutcomeInvalid(t *testing.T)` | TestValidateCacheOutcomeInvalid verifies validate cache outcome invalid. |
| [`TestValidateCacheOutcomeEmpty`](../src/manifest/lifecycle_unit_test.go#L39) | test | 39-43 | `func TestValidateCacheOutcomeEmpty(t *testing.T)` | TestValidateCacheOutcomeEmpty verifies validate cache outcome empty. |
| [`TestCacheOutcomeConstants`](../src/manifest/lifecycle_unit_test.go#L46) | test | 46-59 | `func TestCacheOutcomeConstants(t *testing.T)` | TestCacheOutcomeConstants verifies cache outcome constants. |
| [`TestValidateRunVisibilityInvalid`](../src/manifest/lifecycle_unit_test.go#L62) | test | 62-66 | `func TestValidateRunVisibilityInvalid(t *testing.T)` | TestValidateRunVisibilityInvalid verifies validate run visibility invalid. |
| [`TestValidateRunVisibilityEmpty`](../src/manifest/lifecycle_unit_test.go#L69) | test | 69-73 | `func TestValidateRunVisibilityEmpty(t *testing.T)` | TestValidateRunVisibilityEmpty verifies validate run visibility empty. |
| [`TestRunVisibilityConstants`](../src/manifest/lifecycle_unit_test.go#L76) | test | 76-86 | `func TestRunVisibilityConstants(t *testing.T)` | TestRunVisibilityConstants verifies run visibility constants. |
| [`TestValidateAuditActionInvalid`](../src/manifest/lifecycle_unit_test.go#L89) | test | 89-93 | `func TestValidateAuditActionInvalid(t *testing.T)` | TestValidateAuditActionInvalid verifies validate audit action invalid. |
| [`TestValidateAuditActionEmpty`](../src/manifest/lifecycle_unit_test.go#L96) | test | 96-100 | `func TestValidateAuditActionEmpty(t *testing.T)` | TestValidateAuditActionEmpty verifies validate audit action empty. |
| [`TestAuditActionConstants`](../src/manifest/lifecycle_unit_test.go#L103) | test | 103-130 | `func TestAuditActionConstants(t *testing.T)` | TestAuditActionConstants verifies audit action constants. |
| [`TestAuditEventFields`](../src/manifest/lifecycle_unit_test.go#L133) | test | 133-165 | `func TestAuditEventFields(t *testing.T)` | TestAuditEventFields verifies audit event fields. |
| [`TestDefaultRetentionPolicy`](../src/manifest/lifecycle_unit_test.go#L168) | test | 168-173 | `func TestDefaultRetentionPolicy(t *testing.T)` | TestDefaultRetentionPolicy verifies default retention policy. |
| [`TestDefaultPurgePolicy`](../src/manifest/lifecycle_unit_test.go#L176) | test | 176-184 | `func TestDefaultPurgePolicy(t *testing.T)` | TestDefaultPurgePolicy verifies default purge policy. |
| [`TestRetentionPolicyCustom`](../src/manifest/lifecycle_unit_test.go#L187) | test | 187-194 | `func TestRetentionPolicyCustom(t *testing.T)` | TestRetentionPolicyCustom verifies retention policy custom. |
| [`TestPurgePolicyCustom`](../src/manifest/lifecycle_unit_test.go#L197) | test | 197-208 | `func TestPurgePolicyCustom(t *testing.T)` | TestPurgePolicyCustom verifies purge policy custom. |
...
| [`isInitials`](../src/normalization/normalization.go#L67) | function | 67-76 | `func isInitials(word string) bool` | isInitials reports whether a word is a supported compact or punctuated initial sequence. |
| [`isUpperAlpha`](../src/normalization/normalization.go#L79) | function | 79-86 | `func isUpperAlpha(s string) bool` | isUpperAlpha reports whether a non-empty string contains only ASCII uppercase letters. |
| [`smartTitle`](../src/normalization/normalization.go#L89) | function | 89-116 | `func smartTitle(word string) string` | smartTitle applies author-name casing rules to one word while preserving particles and compounds. |
| [`splitGivenFamily`](../src/normalization/normalization.go#L119) | function | 119-136 | `func splitGivenFamily(words []string) (given, family []string)` | splitGivenFamily splits given family into its component values. |
| [`wordToInitial`](../src/normalization/normalization.go#L139) | function | 139-156 | `func wordToInitial(word string) string` | wordToInitial converts a given-name word or existing initial sequence to dotted uppercase initials. |
| [`toInitials`](../src/normalization/normalization.go#L159) | function | 159-171 | `func toInitials(givenStr string) string` | toInitials converts whitespace-separated given names to canonical dotted initials. |
| [`normalizeFamilyWords`](../src/normalization/normalization.go#L174) | function | 174-200 | `func normalizeFamilyWords(words []string) []string` | normalizeFamilyWords normalizes family words. |
| [`NormalizeAuthorName`](../src/normalization/normalization.go#L205) | function | 205-263 | `func NormalizeAuthorName(name string) string` | NormalizeAuthorName normalizes a single author name to 'Lastname, F. M.' format. |
| [`splitOnComma`](../src/normalization/normalization.go#L266) | function | 266-280 | `func splitOnComma(name string) []string` | splitOnComma splits on comma into its component values. |
| [`SplitFirstLast`](../src/normalization/normalization.go#L283) | function | 283-289 | `func SplitFirstLast(normalized string) (first, last string)` | SplitFirstLast splits 'Lastname, F. M.' into (first_name, last_name). |
| [`expandAbbreviations`](../src/normalization/normalization.go#L335) | function | 335-341 | `func expandAbbreviations(text string) string` | expandAbbreviations expands recognized affiliation abbreviations case-insensitively. |
| [`affTitleWord`](../src/normalization/normalization.go#L344) | function | 344-386 | `func affTitleWord(word string, firstWord bool) string` | affTitleWord applies affiliation-specific casing while preserving punctuation and acronyms. |
| [`normalizeAffiliationCase`](../src/normalization/normalization.go#L391) | function | 391-425 | `func normalizeAffiliationCase(text string) string` | normalizeAffiliationCase normalizes affiliation case. |
| [`NormalizeAffiliation`](../src/normalization/normalization.go#L428) | function | 428-439 | `func NormalizeAffiliation(aff string) string` | NormalizeAffiliation normalizes an affiliation string. |
| [`isAcronym`](../src/normalization/normalization.go#L481) | function | 481-484 | `func isAcronym(word string) bool` | isAcronym reports whether a word is a short all-uppercase publisher acronym. |
| [`pubTitleCore`](../src/normalization/normalization.go#L487) | function | 487-509 | `func pubTitleCore(core string) string` | pubTitleCore applies publisher-specific casing to an unpunctuated word core. |
| [`pubTitleWord`](../src/normalization/normalization.go#L512) | function | 512-530 | `func pubTitleWord(word string) string` | pubTitleWord applies publisher casing while preserving surrounding punctuation. |
| [`detachLegalSuffix`](../src/normalization/normalization.go#L533) | function | 533-539 | `func detachLegalSuffix(text string) (string, string)` | detachLegalSuffix separates a recognized publisher legal suffix from the preceding name. |
| [`titleCasePublisher`](../src/normalization/normalization.go#L542) | function | 542-631 | `func titleCasePublisher(name string) string` | titleCasePublisher applies publisher-specific title casing across words and parenthesized text. |
| [`NormalizePublisher`](../src/normalization/normalization.go#L634) | function | 634-660 | `func NormalizePublisher(publisher string) string` | NormalizePublisher normalizes a publisher name. |
| [`journalIsAcronym`](../src/normalization/normalization.go#L712) | function | 712-714 | `func journalIsAcronym(word string) bool` | journalIsAcronym reports whether a word is in the recognized journal-acronym set. |
| [`stripJournalSubtitle`](../src/normalization/normalization.go#L719) | function | 719-721 | `func stripJournalSubtitle(name string) string` | stripJournalSubtitle removes a dash-delimited generic article subtitle from a journal name. |
| [`titleCaseJournal`](../src/normalization/normalization.go#L724) | function | 724-757 | `func titleCaseJournal(name string) string` | titleCaseJournal applies journal-specific casing across words, punctuation, and parenthesized text. |
| [`titleCaseJournalSegment`](../src/normalization/normalization.go#L760) | function | 760-799 | `func titleCaseJournalSegment(value string, parenthesized bool) string` | titleCaseJournalSegment cases one text segment while preserving its surrounding whitespace. |
| [`normalizedPunctuationWord`](../src/normalization/normalization.go#L802) | function | 802-807 | `func normalizedPunctuationWord(value string) string` | normalizedPunctuationWord removes a duplicate terminal period introduced by casing. |
| [`NormalizeJournal`](../src/normalization/normalization.go#L810) | function | 810-827 | `func NormalizeJournal(journal string) string` | NormalizeJournal normalizes a journal name. |
...
| [`Link`](../src/notes/parser.go#L36) | struct | 36-43 | ``type Link struct { Ordinal int `json:"ordinal"` TargetType string `json:"target_type"` RawTarget string `json:"raw_target"` DisplayText *string `json:"display_text"` Position int `json:"position"` Length int `json:"length"` }`` | Link is one syntactically valid custom link extracted from a note version. |
| [`Block`](../src/notes/parser.go#L46) | struct | 46-54 | ``type Block struct { Type string `json:"type"` Text string `json:"text,omitempty"` Level int `json:"level,omitempty"` Ordered bool `json:"ordered,omitempty"` Items []string `json:"items,omitempty"` Header []string `json:"header,omitempty"` Rows [][]string `json:"rows,omitempty"` }`` | Block is a normalized note block used by cross-language conformance fixtures. |
| [`Document`](../src/notes/parser.go#L57) | struct | 57-61 | ``type Document struct { Blocks []Block `json:"blocks"` Links []Link `json:"links"` Errors []SyntaxError `json:"errors"` }`` | Document is the normalized parser result. Any syntax error makes the document unsaveable. |
| [`Parse`](../src/notes/parser.go#L64) | function | 64-77 | `func Parse(body string) Document` | Parse returns all recognized blocks, links, and recoverable syntax errors. |
| [`parseBlocks`](../src/notes/parser.go#L80) | function | 80-198 | `func parseBlocks(body string, problems []SyntaxError) ([]Block, []SyntaxError)` | parseBlocks recognizes the bounded block grammar while suppressing links inside code fences. |
| [`heading`](../src/notes/parser.go#L201) | function | 201-209 | `func heading(line string) (int, string, bool)` | heading recognizes project headings from level one through four. |
| [`listItem`](../src/notes/parser.go#L212) | function | 212-220 | `func listItem(line string) (string, string, bool)` | listItem recognizes bullet and deliberately simple ordered-list lines. |
| [`isTableDelimiter`](../src/notes/parser.go#L223) | function | 223-230 | `func isTableDelimiter(line string) bool` | isTableDelimiter recognizes the complete delimiter grammar before interpreting pipe-bearing prose as a table. |
| [`hasUnescapedPipe`](../src/notes/parser.go#L233) | function | 233-246 | `func hasUnescapedPipe(line string) bool` | hasUnescapedPipe reports whether a line may begin the simple table grammar. |
| [`splitTableRow`](../src/notes/parser.go#L249) | function | 249-283 | `func splitTableRow(line string) []string` | splitTableRow returns safely unescaped cells or nil for a malformed row. |
| [`parseLinks`](../src/notes/parser.go#L286) | function | 286-316 | `func parseLinks(body string, problems []SyntaxError) ([]Link, []SyntaxError)` | parseLinks extracts custom links outside fenced code while retaining UTF-16 positions. |
| [`parseLineLinks`](../src/notes/parser.go#L319) | function | 319-349 | `func parseLineLinks(line string, base int, problems []SyntaxError) ([]Link, []SyntaxError)` | parseLineLinks extracts and validates each custom link from one source line. |
| [`findLinkEnd`](../src/notes/parser.go#L352) | function | 352-365 | `func findLinkEnd(line string, start int) int` | findLinkEnd locates the next unescaped closing delimiter. |
| [`decodeLink`](../src/notes/parser.go#L368) | function | 368-427 | `func decodeLink(input string) (Link, string)` | decodeLink converts one custom-link payload into canonical persisted identity. |
| [`splitEscaped`](../src/notes/parser.go#L430) | function | 430-467 | `func splitEscaped(input string) ([]string, error)` | splitEscaped separates link fields while preserving supported escaped delimiters. |
| [`normalizeDOI`](../src/notes/parser.go#L470) | function | 470-476 | `func normalizeDOI(value string) string` | normalizeDOI canonicalizes article-link DOI targets without database access. |
| [`utf16Length`](../src/notes/parser.go#L479) | function | 479-488 | `func utf16Length(value string) int` | utf16Length returns the browser-compatible code-unit length of a string. |
...
| [`resolveStorePath`](../src/pdfstore/store.go#L318) | function | 318-343 | `func resolveStorePath(metadataPath, relativePath string) (string, error)` | resolveStorePath resolves store path from the supplied context. |
| [`validateRelativeStorePath`](../src/pdfstore/store.go#L346) | function | 346-355 | `func validateRelativeStorePath(relativePath string) (string, error)` | validateRelativeStorePath rejects absolute or escaping companion-store paths and returns a clean relative path. |
...
| Symbol | Kind | Lines | Declaration, inputs, and outputs | Source description |
|---|---|---:|---|---|
| [`TestParseScopusQuery`](../src/searchterms/searchterms_unit_test.go#L14) | test | 14-33 | `func TestParseScopusQuery(t *testing.T)` | TestParseScopusQuery verifies the Scopus TITLE-ABS-KEY query shape. |
| [`TestParseWOSQuery`](../src/searchterms/searchterms_unit_test.go#L36) | test | 36-45 | `func TestParseWOSQuery(t *testing.T)` | TestParseWOSQuery verifies the WOS TS= query shape. |
| [`TestParseIEEEXploreQuery`](../src/searchterms/searchterms_unit_test.go#L48) | test | 48-59 | `func TestParseIEEEXploreQuery(t *testing.T)` | TestParseIEEEXploreQuery verifies the IEEE Xplore quoted field-label shape. |
| [`TestParseWildcardQuery`](../src/searchterms/searchterms_unit_test.go#L62) | test | 62-70 | `func TestParseWildcardQuery(t *testing.T)` | TestParseWildcardQuery verifies workspace 2's wildcard-heavy WOS query shape. |
| [`TestParseSkipsOperators`](../src/searchterms/searchterms_unit_test.go#L73) | test | 73-80 | `func TestParseSkipsOperators(t *testing.T)` | TestParseSkipsOperators verifies operators are skipped case-insensitively. |
| [`TestParseSkipsFieldPrefixes`](../src/searchterms/searchterms_unit_test.go#L83) | test | 83-90 | `func TestParseSkipsFieldPrefixes(t *testing.T)` | TestParseSkipsFieldPrefixes verifies bare field prefixes are skipped. |
| [`TestParseKeepsBareTokens`](../src/searchterms/searchterms_unit_test.go#L93) | test | 93-100 | `func TestParseKeepsBareTokens(t *testing.T)` | TestParseKeepsBareTokens verifies bare digit-only and single-letter tokens. |
| [`TestParseDeduplicatesCaseInsensitively`](../src/searchterms/searchterms_unit_test.go#L103) | test | 103-109 | `func TestParseDeduplicatesCaseInsensitively(t *testing.T)` | TestParseDeduplicatesCaseInsensitively verifies deduplication keeps the first spelling. |
| [`TestParseSkipsPunctuationOnly`](../src/searchterms/searchterms_unit_test.go#L112) | test | 112-118 | `func TestParseSkipsPunctuationOnly(t *testing.T)` | TestParseSkipsPunctuationOnly verifies punctuation-only tokens are skipped. |
| [`TestParseEmptyInput`](../src/searchterms/searchterms_unit_test.go#L121) | test | 121-128 | `func TestParseEmptyInput(t *testing.T)` | TestParseEmptyInput verifies empty and punctuation-only queries yield no terms. |
| [`TestParseSourcesAttribution`](../src/searchterms/searchterms_unit_test.go#L131) | test | 131-144 | `func TestParseSourcesAttribution(t *testing.T)` | TestParseSourcesAttribution verifies deterministic source attribution and cross-source deduplication. |
| [`TestParseSourcesEmptyQueries`](../src/searchterms/searchterms_unit_test.go#L147) | test | 147-152 | `func TestParseSourcesEmptyQueries(t *testing.T)` | TestParseSourcesEmptyQueries verifies empty queries contribute no terms. |
| [`TestMatchWholeWord`](../src/searchterms/searchterms_unit_test.go#L155) | test | 155-173 | `func TestMatchWholeWord(t *testing.T)` | TestMatchWholeWord verifies case-insensitive whole-word matching. |
| [`TestMatchPrefixWildcard`](../src/searchterms/searchterms_unit_test.go#L178) | test | 178-196 | `func TestMatchPrefixWildcard(t *testing.T)` | TestMatchPrefixWildcard verifies trailing-star prefix matching. Terms and text are stemmed before matching, so the wildcard prefix must be a stem (for example "optim*" matches "optimization" because both stem to "optim"). |
| [`TestMatchLeadingAndBothWildcards`](../src/searchterms/searchterms_unit_test.go#L199) | test | 199-217 | `func TestMatchLeadingAndBothWildcards(t *testing.T)` | TestMatchLeadingAndBothWildcards verifies suffix and substring wildcards. |
| [`TestMatchStemming`](../src/searchterms/searchterms_unit_test.go#L222) | test | 222-247 | `func TestMatchStemming(t *testing.T)` | TestMatchStemming verifies inflected forms match their base term after stemming, which is the deterministic contract that replaces the previous exact-phrase approximation. |
| [`TestMatchRegexMetacharacters`](../src/searchterms/searchterms_unit_test.go#L250) | test | 250-264 | `func TestMatchRegexMetacharacters(t *testing.T)` | TestMatchRegexMetacharacters verifies terms with regex metacharacters are escaped. |
| [`TestMatchFields`](../src/searchterms/searchterms_unit_test.go#L267) | test | 267-291 | `func TestMatchFields(t *testing.T)` | TestMatchFields verifies per-field matching with per-element keyword semantics. |
| [`TestMatchFieldsEmpty`](../src/searchterms/searchterms_unit_test.go#L294) | test | 294-300 | `func TestMatchFieldsEmpty(t *testing.T)` | TestMatchFieldsEmpty verifies empty fields produce empty lists. |

### [`src/server/audit.go`](../src/server/audit.go)

| Symbol | Kind | Lines | Declaration, inputs, and outputs | Source description |
|---|---|---:|---|---|
| [`(*Server).audit`](../src/server/audit.go#L24) | method | 24-240 | `func (*Server).audit(w http.ResponseWriter, r *http.Request)` | audit validates filters and returns a cursor-paginated audit timeline with summary and facets. |
| [`(*Server).auditRecordedData`](../src/server/audit.go#L243) | method | 243-294 | `func (*Server).auditRecordedData(w http.ResponseWriter, r *http.Request)` | auditRecordedData returns one privacy-scrubbed, byte-bounded payload only after explicit expansion. |
| [`boundAuditEventPayloads`](../src/server/audit.go#L297) | function | 297-325 | `func boundAuditEventPayloads(items []map[string]any, limit int)` | boundAuditEventPayloads removes private fields and keeps timeline pages within a fixed payload budget per field. |
| [`safeAuditJSON`](../src/server/audit.go#L328) | function | 328-346 | `func safeAuditJSON(raw any, limit int) (any, int, bool)` | safeAuditJSON decodes and recursively removes prose and contact fields before enforcing the byte budget. |
| [`scrubAuditValue`](../src/server/audit.go#L349) | function | 349-370 | `func scrubAuditValue(raw any) any` | scrubAuditValue recursively omits review prose, selected text, and reviewer contact fields. |
| [`auditMultiValues`](../src/server/audit.go#L373) | function | 373-393 | `func auditMultiValues(raw, parameter string) ([]string, error)` | auditMultiValues parses, deduplicates, and bounds a comma-separated audit facet filter. |
| [`auditInClause`](../src/server/audit.go#L396) | function | 396-404 | `func auditInClause(column string, values []string) (string, []any)` | auditInClause builds a parameterized SQL IN clause for validated audit facet values. |
| [`auditWhere`](../src/server/audit.go#L407) | function | 407-412 | `func auditWhere(clauses []string) string` | auditWhere joins audit predicates into an optional SQL WHERE clause. |
| [`(*Server).auditSummary`](../src/server/audit.go#L415) | method | 415-430 | `func (*Server).auditSummary(ctx context.Context, where string, args []any) (map[string]any, error)` | auditSummary counts filtered audit events by presentation category. |
| [`(*Server).auditFacet`](../src/server/audit.go#L433) | method | 433-458 | `func (*Server).auditFacet(ctx context.Context, column, scopeClause string, scopeArgs []any) ([]string, error)` | auditFacet returns distinct non-empty values for an allowlisted audit column and run scope. |
| [`nullableRunScope`](../src/server/audit.go#L461) | function | 461-466 | `func nullableRunScope(raw string) any` | nullableRunScope preserves an invariant null-or-string scope value in audit responses. |
| [`(*Server).auditRows`](../src/server/audit.go#L469) | method | 469-476 | `func (*Server).auditRows(ctx context.Context, condition string, args ...any) ([]map[string]any, error)` | auditRows returns audit event rows matching a caller-supplied parameterized condition. |
| [`(*Server).runArtifacts`](../src/server/audit.go#L479) | method | 479-676 | `func (*Server).runArtifacts(w http.ResponseWriter, r *http.Request)` | runArtifacts returns artifact metadata linked to the selected run. |
| [`nullablePositiveID`](../src/server/audit.go#L679) | function | 679-684 | `func nullablePositiveID(id int64) any` | nullablePositiveID preserves an invariant null-or-number response for optional focused records. |
| [`(*Server).artifactContent`](../src/server/audit.go#L687) | method | 687-722 | `func (*Server).artifactContent(w http.ResponseWriter, r *http.Request)` | artifactContent streams one stored artifact blob with a safe content disposition. |
| [`(*Server).artifactInspection`](../src/server/audit.go#L725) | method | 725-776 | `func (*Server).artifactInspection(w http.ResponseWriter, r *http.Request)` | artifactInspection returns bounded metadata and preview content for one artifact. |
...
| Symbol | Kind | Lines | Declaration, inputs, and outputs | Source description |
|---|---|---:|---|---|
| [`scopedRowsDefinition`](../src/server/corpus.go#L18) | struct | 18-26 | `type scopedRowsDefinition struct { columns []string from string where string groupBy string search string sortFields map[string]string uniqueOrder string }` | scopedRowsDefinition defines the safe projection, joins, filters, and sorting for one corpus section. |
| [`(*Server).runCorpus`](../src/server/corpus.go#L78) | method | 78-146 | `func (*Server).runCorpus(w http.ResponseWriter, r *http.Request)` | runCorpus returns one context-scoped corpus section for the selected run. |
| [`(*Server).attachArticleTermMatches`](../src/server/corpus.go#L149) | method | 149-175 | `func (*Server).attachArticleTermMatches(ctx context.Context, runID int64, items []map[string]any) error` | attachArticleTermMatches adds bounded stored search-term evidence to article collection rows. |
| [`corpusSelectColumns`](../src/server/corpus.go#L178) | function | 178-191 | `func corpusSelectColumns(kind string) string` | corpusSelectColumns returns the fixed safe projection for a browsable corpus section. |
| [`(*Server).runStages`](../src/server/corpus.go#L194) | method | 194-259 | `func (*Server).runStages(w http.ResponseWriter, r *http.Request)` | runStages returns detailed work-stage outcomes for the selected run. |
| [`(*Server).runStageSummaries`](../src/server/corpus.go#L262) | method | 262-321 | `func (*Server).runStageSummaries(ctx context.Context, runID int64) ([]map[string]any, error)` | runStageSummaries returns aggregate outcome counts by pipeline stage. |
| [`scopedRowsRequest`](../src/server/corpus.go#L324) | function | 324-360 | `func scopedRowsRequest(r *http.Request, fields map[string]string, fallback string, additionalQueryKeys ...string) (int, int, string, string, string, error)` | scopedRowsRequest parses and validates the context, filters, sorting, and pagination for a corpus request. |
...
| Symbol | Kind | Lines | Declaration, inputs, and outputs | Source description |
|---|---|---:|---|---|
| [`(*Server).articleDetail`](../src/server/details.go#L41) | method | 41-133 | `func (*Server).articleDetail(w http.ResponseWriter, r *http.Request)` | articleDetail treats the numeric route identifier as an immutable work revision ID. It intentionally does not expose the retired mutable articles projection. |
| [`(*Server).authorDetail`](../src/server/details.go#L136) | method | 136-184 | `func (*Server).authorDetail(w http.ResponseWriter, r *http.Request)` | authorDetail returns one author occurrence with its articles, audit evidence, and optional run-scoped identity candidates. |
| [`(*Server).referenceDetail`](../src/server/details.go#L187) | method | 187-220 | `func (*Server).referenceDetail(w http.ResponseWriter, r *http.Request)` | referenceDetail returns one reference mention with its citing and resolved-work context. |
| [`(*Server).articleEnrichmentSummary`](../src/server/details.go#L223) | method | 223-266 | `func (*Server).articleEnrichmentSummary(ctx context.Context, workID, runID int64) (map[string]any, error)` | articleEnrichmentSummary returns a bounded set of provider and field labels without transferring event payloads. |
| [`(*Server).articleDetailCollection`](../src/server/details.go#L269) | method | 269-300 | `func (*Server).articleDetailCollection(w http.ResponseWriter, r *http.Request)` | articleDetailCollection returns one bounded page of a large article relationship or event collection. |
| [`(*Server).authorDetailCollection`](../src/server/details.go#L303) | method | 303-339 | `func (*Server).authorDetailCollection(w http.ResponseWriter, r *http.Request)` | authorDetailCollection returns one bounded page of run-owned author relationships or evidence. |
| [`(*Server).articleDetailWorkID`](../src/server/details.go#L342) | method | 342-350 | `func (*Server).articleDetailWorkID(ctx context.Context, revisionID, runID int64) (int64, error)` | articleDetailWorkID validates one visible article revision and returns its owning work. |
| [`(*Server).detailCollectionEnvelope`](../src/server/details.go#L353) | method | 353-397 | `func (*Server).detailCollectionEnvelope(ctx context.Context, kind, fromWhere, orderID string, args []any, cursorID int64, descending bool, limit int) (map[string]any, error)` | detailCollectionEnvelope executes one ID-keyset query with an exact count and one-row continuation sentinel. |
| [`(*Server).articleDetailCollectionData`](../src/server/details.go#L400) | method | 400-436 | `func (*Server).articleDetailCollectionData(ctx context.Context, revisionID, workID, runID int64, collection, cursorKind string, cursorID int64, limit int) (map[string]any, error)` | articleDetailCollectionData defines the fixed projections for article detail subresources. |
| [`(*Server).authorDetailCollectionData`](../src/server/details.go#L439) | method | 439-495 | `func (*Server).authorDetailCollectionData(ctx context.Context, authorID, runID int64, collection, cursorKind string, cursorID int64, limit int) (map[string]any, error)` | authorDetailCollectionData defines the fixed projections for author detail subresources. |
| [`articleAuditCondition`](../src/server/details.go#L498) | function | 498-513 | `func articleAuditCondition(workID, runID int64) (string, []any)` | articleAuditCondition returns the run-scoped, privacy-safe logical-work event predicate and arguments. |
| [`(*Server).rows`](../src/server/details.go#L516) | method | 516-523 | `func (*Server).rows(ctx context.Context, query string, args ...any) ([]map[string]any, error)` | rows executes a read-only query and converts every result row to a field map. |
| [`(*Server).oneRow`](../src/server/details.go#L526) | method | 526-535 | `func (*Server).oneRow(ctx context.Context, query string, args ...any) (map[string]any, error)` | oneRow returns the first mapped query row, or nil when the query returns no rows. |
| [`stringID`](../src/server/details.go#L538) | function | 538 | `func stringID(id int64) string` | stringID formats a numeric database identifier in base 10. |
...
| Symbol | Kind | Lines | Declaration, inputs, and outputs | Source description |
|---|---|---:|---|---|
| [`(*Server).runEvaluation`](../src/server/evaluation.go#L41) | method | 41-248 | `func (*Server).runEvaluation(w http.ResponseWriter, r *http.Request)` | runEvaluation lists the selected run's normalized articles and overlays their state from the independently bound PDF inventory. |
| [`(*Server).evaluationQueueNavigation`](../src/server/evaluation.go#L251) | method | 251-309 | `func (*Server).evaluationQueueNavigation(ctx context.Context, runID, currentRevisionID int64, from, where string, args []any, sortField, order string) (map[string]any, error)` | evaluationQueueNavigation returns adjacent unreviewed revisions within the active queue filters. |
| [`(*Server).availablePDFDOIs`](../src/server/evaluation.go#L312) | method | 312-331 | `func (*Server).availablePDFDOIs(ctx context.Context) ([]string, error)` | availablePDFDOIs returns the bounded identity projection used for evaluation inventory filters. |
| [`(*Server).evaluationReviewSummary`](../src/server/evaluation.go#L334) | method | 334-390 | `func (*Server).evaluationReviewSummary(ctx context.Context, runID, contextID int64, availableDOIsJSON string) (map[string]any, error)` | evaluationReviewSummary returns invariant queue progress independently of page rows and filters. |
| [`(*Server).overlayPDFInventory`](../src/server/evaluation.go#L393) | method | 393-439 | `func (*Server).overlayPDFInventory(ctx context.Context, items []map[string]any) error` | overlayPDFInventory overlays companion PDF availability onto evaluation rows by normalized DOI. |

### [`src/server/evaluation_integration_test.go`](../src/server/evaluation_integration_test.go)

| Symbol | Kind | Lines | Declaration, inputs, and outputs | Source description |
|---|---|---:|---|---|
| [`TestCorpusArticleReviewProjection`](../src/server/evaluation_integration_test.go#L16) | test | 16-70 | `func TestCorpusArticleReviewProjection(t *testing.T)` | TestCorpusArticleReviewProjection verifies one article projection, combined filters, and numeric queue ordering. |
| [`TestEvaluationListsOnlyNormalizedArticlesWithInventoryState`](../src/server/evaluation_integration_test.go#L73) | test | 73-144 | `func TestEvaluationListsOnlyNormalizedArticlesWithInventoryState(t *testing.T)` | TestEvaluationListsOnlyNormalizedArticlesWithInventoryState verifies evaluation lists only normalized articles with inventory state. |
...
| [`(*Server).graphArticles`](../src/server/graph.go#L98) | method | 98-159 | `func (*Server).graphArticles(ctx context.Context, r *http.Request, runID int64, limit int) ([]map[string]any, int, error)` | graphArticles selects normalized, valid article nodes matching the request filters and limit. |
...
| [`(*Server).graphEdgesWithinBudget`](../src/server/graph.go#L167) | method | 167-280 | `func (*Server).graphEdgesWithinBudget(ctx context.Context, mode string, articles []map[string]any, relatedBudget, edgeBudget int) ([]map[string]any, []map[string]any, bool, error)` | graphEdgesWithinBudget reads no more than one sentinel row beyond the remaining response budget. |
| [`(*Server).graphResearchNetwork`](../src/server/graph.go#L283) | method | 283-478 | `func (*Server).graphResearchNetwork(ctx context.Context, articles []map[string]any, relatedBudget, edgeBudget int) ([]map[string]any, []map[string]any, bool, error)` | graphResearchNetwork combines authorship, reference, citation, coauthor, and bibliographic-coupling relationships. |
| [`placeholders`](../src/server/graph.go#L481) | function | 481-488 | `func placeholders(ids []int64) (string, []any)` | placeholders returns a comma-separated SQL placeholder list and matching identifier arguments. |
| [`graphFilters`](../src/server/graph.go#L491) | function | 491-499 | `func graphFilters(r *http.Request, runID int64, mode string, limit int) map[string]any` | graphFilters returns the effective non-empty graph filters for response metadata. |
...
| [`(*Server).hierarchySummary`](../src/server/hierarchy.go#L63) | method | 63-88 | `func (*Server).hierarchySummary(ctx context.Context) (map[string]any, error)` | hierarchySummary returns current workspace totals and the latest planned run. |
| [`(*Server).latestHierarchyRun`](../src/server/hierarchy.go#L91) | method | 91-107 | `func (*Server).latestHierarchyRun(ctx context.Context) (any, error)` | latestHierarchyRun returns the newest run with complete ancestry when one exists. |
| [`(*Server).hierarchySearches`](../src/server/hierarchy.go#L110) | method | 110-201 | `func (*Server).hierarchySearches(ctx context.Context, r *http.Request) (map[string]any, error)` | hierarchySearches returns one bounded server-searchable page of search summaries. |
| [`(*Server).hierarchyRevisions`](../src/server/hierarchy.go#L204) | method | 204-283 | `func (*Server).hierarchyRevisions(ctx context.Context, r *http.Request) (map[string]any, error)` | hierarchyRevisions returns one bounded page of revision summaries for a selected search. |
| [`(*Server).hierarchyPlans`](../src/server/hierarchy.go#L286) | method | 286-354 | `func (*Server).hierarchyPlans(ctx context.Context, r *http.Request) (map[string]any, error)` | hierarchyPlans returns one bounded searchable page of execution plans for a selected revision. |
| [`(*Server).hierarchyAttempts`](../src/server/hierarchy.go#L357) | method | 357-432 | `func (*Server).hierarchyAttempts(ctx context.Context, r *http.Request) (map[string]any, error)` | hierarchyAttempts returns one bounded searchable page of non-trashed attempts for a selected plan. |
| [`(*Server).hierarchyRuns`](../src/server/hierarchy.go#L435) | method | 435-519 | `func (*Server).hierarchyRuns(ctx context.Context, r *http.Request) (map[string]any, error)` | hierarchyRuns returns one bounded filtered page of run attempts with complete ancestry. |
| [`hierarchyScanner`](../src/server/hierarchy.go#L522) | interface | 522-524 | `type hierarchyScanner interface { Scan(...any) error }` | hierarchyScanner is the shared Scan contract for one row or rows cursor. |
| [`scanHierarchyRun`](../src/server/hierarchy.go#L527) | function | 527-551 | `func scanHierarchyRun(scanner hierarchyScanner) (map[string]any, error)` | scanHierarchyRun scans one run and its complete search ancestry. |
| [`hierarchyPage`](../src/server/hierarchy.go#L554) | function | 554-573 | `func hierarchyPage(kind, scope string, items []map[string]any) map[string]any` | hierarchyPage trims the lookahead row and emits an opaque continuation cursor. |
| [`hierarchyDate`](../src/server/hierarchy.go#L576) | function | 576-588 | `func hierarchyDate(raw, name string) (string, error)` | hierarchyDate validates one inclusive-from or exclusive-before calendar boundary. |
| [`optionalHierarchyID`](../src/server/hierarchy.go#L591) | function | 591-597 | `func optionalHierarchyID(r *http.Request, name string) (int64, error)` | optionalHierarchyID parses an optional selected item identifier for exact membership validation. |
| [`hierarchyRequiredID`](../src/server/hierarchy.go#L600) | function | 600-606 | `func hierarchyRequiredID(r *http.Request, name string) (int64, error)` | hierarchyRequiredID parses one required parent identifier after the handler-level allowlist check. |
| [`hierarchyScope`](../src/server/hierarchy.go#L609) | function | 609-612 | `func hierarchyScope(parts ...string) string` | hierarchyScope hashes the section filters so a cursor cannot cross result sets. |
| [`decodeHierarchyCursor`](../src/server/hierarchy.go#L615) | function | 615-628 | `func decodeHierarchyCursor(raw, kind, scope string) (hierarchyCursor, error)` | decodeHierarchyCursor validates an opaque cursor against its owning filtered collection. |
| [`nullableInt64`](../src/server/hierarchy.go#L631) | function | 631-636 | `func nullableInt64(value sql.NullInt64) any` | nullableInt64 converts a nullable identifier to the invariant JSON null-or-number shape. |
| [`nullableString`](../src/server/hierarchy.go#L639) | function | 639-644 | `func nullableString(value sql.NullString) any` | nullableString converts an optional stored string to the invariant JSON null-or-string shape. |
...
| Symbol | Kind | Lines | Declaration, inputs, and outputs | Source description |
|---|---|---:|---|---|
| [`(*Server).runIdentityEvidence`](../src/server/identity_evidence.go#L21) | method | 21-109 | `func (*Server).runIdentityEvidence(w http.ResponseWriter, r *http.Request)` | runIdentityEvidence exposes name-derived ORCID evidence without presenting it as an author identity. The endpoint is unavailable for databases created before the evidence migration, preserving the viewer's read-only behavior. |
| [`(*Server).identityEvidenceStats`](../src/server/identity_evidence.go#L112) | method | 112-128 | `func (*Server).identityEvidenceStats(ctx context.Context, runID int64) (map[string]int64, error)` | identityEvidenceStats counts candidate and resolution states for the selected context. |
| [`(*Server).attachIdentityCandidatePreviews`](../src/server/identity_evidence.go#L131) | method | 131-192 | `func (*Server).attachIdentityCandidatePreviews(ctx context.Context, resolutions []map[string]any) error` | attachIdentityCandidatePreviews batches a small ranked preview for every visible resolution. |
| [`(*Server).identityCandidates`](../src/server/identity_evidence.go#L195) | method | 195-273 | `func (*Server).identityCandidates(w http.ResponseWriter, r *http.Request)` | identityCandidates returns one cursor-paginated ranked candidate page for a run-owned resolution. |
...
| [`TestIdentityEvidenceRevisionLinks`](../src/server/identity_evidence_integration_test.go#L32) | test | 32-109 | `func TestIdentityEvidenceRevisionLinks(t *testing.T)` | TestIdentityEvidenceRevisionLinks verifies coherent article pairs and bounded author evidence across immutable snapshots. |
| [`TestIdentityCandidatePages`](../src/server/identity_evidence_integration_test.go#L113) | test | 113-209 | `func TestIdentityCandidatePages(t *testing.T)` | TestIdentityCandidatePages verifies bounded previews, stable cursor traversal, run ownership, and collection-bound cursor validation. |

### [`src/server/overview.go`](../src/server/overview.go)

| Symbol | Kind | Lines | Declaration, inputs, and outputs | Source description |
|---|---|---:|---|---|
| [`(*Server).sourceResultCounts`](../src/server/overview.go#L56) | method | 56-71 | `func (*Server).sourceResultCounts(ctx context.Context, runID int64) ([]map[string]any, error)` | sourceResultCounts returns the stored source inventory and result-count evidence for a run. |
| [`(*Server).sourceFilterCounts`](../src/server/overview.go#L74) | method | 74-128 | `func (*Server).sourceFilterCounts(ctx context.Context, runID int64) ([]map[string]any, []map[string]any, error)` | sourceFilterCounts decodes stored per-source filter stages and reports malformed evidence without exposing its raw content. |
| [`(*Server).health`](../src/server/overview.go#L131) | method | 131-170 | `func (*Server).health(w http.ResponseWriter, r *http.Request)` | health reports database readability and the discovered table inventory. |
| [`(*Server).tableNames`](../src/server/overview.go#L173) | method | 173-186 | `func (*Server).tableNames() []string` | tableNames returns discovered table names in deterministic order. |
| [`(*Server).searches`](../src/server/overview.go#L189) | method | 189-272 | `func (*Server).searches(w http.ResponseWriter, r *http.Request)` | searches returns a bounded compatibility view of searches and their newest revisions. |
| [`(*Server).plans`](../src/server/overview.go#L275) | method | 275-311 | `func (*Server).plans(w http.ResponseWriter, r *http.Request)` | plans returns a bounded compatibility view of execution plans for one revision. |
| [`(*Server).runs`](../src/server/overview.go#L314) | method | 314-380 | `func (*Server).runs(w http.ResponseWriter, r *http.Request)` | runs returns pipeline attempts filtered by research context and visibility. |
| [`(*Server).runContext`](../src/server/overview.go#L383) | method | 383-481 | `func (*Server).runContext(w http.ResponseWriter, r *http.Request)` | runContext returns the canonical complete ancestry and lifecycle for one run. |
| [`(*Server).overview`](../src/server/overview.go#L484) | method | 484-604 | `func (*Server).overview(w http.ResponseWriter, r *http.Request)` | overview returns captured metrics, coverage, relationships, and source evidence for a run. |
| [`metricGroup`](../src/server/overview.go#L607) | function | 607-620 | `func metricGroup(metrics map[string]map[string]any, names ...string) map[string]any` | metricGroup selects named metrics and marks absent captures as unavailable. |
| [`sourceBreakdown`](../src/server/overview.go#L623) | function | 623-643 | `func sourceBreakdown(metrics []map[string]any, totals map[string]int64) map[string]any` | sourceBreakdown calculates each source's share of captured input records. |
| [`enrichmentFieldBreakdown`](../src/server/overview.go#L648) | function | 648-662 | `func enrichmentFieldBreakdown(byName map[string]map[string]any) map[string]any` | enrichmentFieldBreakdown extracts per-field enrichment counts from the byName metric map. Metrics named "enriched_fields_<field>" with no source are included. |
| [`enrichmentProviderBreakdown`](../src/server/overview.go#L667) | function | 667-682 | `func enrichmentProviderBreakdown(metrics []map[string]any) map[string]any` | enrichmentProviderBreakdown extracts per-provider enrichment counts from the raw metrics list. Metrics named "enriched_fields" with a non-empty source are included. |
| [`normalizationFieldBreakdown`](../src/server/overview.go#L685) | function | 685-734 | `func normalizationFieldBreakdown(metrics []map[string]any) map[string]map[string]any` | normalizationFieldBreakdown groups normalization outcome metrics by field and derives percentages. |
| [`metricDenominator`](../src/server/overview.go#L737) | function | 737-749 | `func metricDenominator(metric string, values map[string]int64) (int64, bool)` | metricDenominator returns the captured population against which a metric is measured. |
| [`(*Server).currentCoverage`](../src/server/overview.go#L752) | method | 752-761 | `func (*Server).currentCoverage(ctx context.Context, runID int64) (map[string]any, error)` | currentCoverage returns work-revision and journal coverage for a run. |
| [`(*Server).relationshipTotals`](../src/server/overview.go#L764) | method | 764-785 | `func (*Server).relationshipTotals(ctx context.Context, runID int64) (map[string]any, error)` | relationshipTotals counts canonical works, authorships, references, and resolved citations for a run. |
| [`percent`](../src/server/overview.go#L788) | function | 788-794 | `func percent(value, denominator int64) *float64` | percent returns value as a percentage of denominator, or nil when denominator is zero. |
| [`requiredQueryID`](../src/server/overview.go#L797) | function | 797-806 | `func requiredQueryID(r *http.Request, name string) (int64, error)` | requiredQueryID validates the endpoint query allowlist and returns one required positive identifier. |
| [`validateKnownQuery`](../src/server/overview.go#L809) | function | 809-823 | `func validateKnownQuery(r *http.Request, allowed ...string) error` | validateKnownQuery rejects semicolon syntax and query parameters outside the endpoint allowlist. |
| [`parseOptionalInt`](../src/server/overview.go#L826) | function | 826-835 | `func parseOptionalInt(raw, name string) (int64, error)` | parseOptionalInt parses a named decimal query value for an endpoint diagnostic. |
...
| [`(*Server).workPDF`](../src/server/pdf.go#L81) | method | 81-146 | `func (*Server).workPDF(w http.ResponseWriter, r *http.Request)` | workPDF streams the validated PDF associated with the requested work revision. |
...
| Symbol | Kind | Lines | Declaration, inputs, and outputs | Source description |
|---|---|---:|---|---|
| [`(*Server).runReviewContext`](../src/server/reviews.go#L30) | method | 30-64 | `func (*Server).runReviewContext(w http.ResponseWriter, r *http.Request)` | runReviewContext returns the initialized context or the deterministic proposed parent. |
| [`(*Server).reviewContextCandidates`](../src/server/reviews.go#L67) | method | 67-119 | `func (*Server).reviewContextCandidates(w http.ResponseWriter, r *http.Request)` | reviewContextCandidates returns bounded eligible parent contexts. |
| [`(*Server).createReviewContext`](../src/server/reviews.go#L122) | method | 122-158 | `func (*Server).createReviewContext(w http.ResponseWriter, r *http.Request)` | createReviewContext explicitly initializes a completed run's review context. |
| [`(*Server).articleReview`](../src/server/reviews.go#L161) | method | 161-231 | `func (*Server).articleReview(w http.ResponseWriter, r *http.Request)` | articleReview returns current review state or an uninitialized default without manufacturing a head. |
| [`(*Server).updateArticleReview`](../src/server/reviews.go#L234) | method | 234-267 | `func (*Server).updateArticleReview(w http.ResponseWriter, r *http.Request)` | updateArticleReview appends one complete immutable article-review state. |
| [`(*Server).articleReviewVersions`](../src/server/reviews.go#L270) | method | 270-308 | `func (*Server).articleReviewVersions(w http.ResponseWriter, r *http.Request)` | articleReviewVersions returns bounded immutable ancestors from the selected context head. |
| [`(*Server).articleReviewVersion`](../src/server/reviews.go#L311) | method | 311-335 | `func (*Server).articleReviewVersion(w http.ResponseWriter, r *http.Request)` | articleReviewVersion returns one full immutable decision version from the selected ancestry. |
| [`(*Server).articleNotes`](../src/server/reviews.go#L338) | method | 338-380 | `func (*Server).articleNotes(w http.ResponseWriter, r *http.Request)` | articleNotes returns bounded current active note heads. |
| [`(*Server).runNotes`](../src/server/reviews.go#L383) | method | 383-425 | `func (*Server).runNotes(w http.ResponseWriter, r *http.Request)` | runNotes returns a searchable run-scoped index of active and removed note heads. |
| [`(*Server).createArticleNote`](../src/server/reviews.go#L428) | method | 428-454 | `func (*Server).createArticleNote(w http.ResponseWriter, r *http.Request)` | createArticleNote creates a logical note and first immutable version. |
| [`(*Server).note`](../src/server/reviews.go#L457) | method | 457-477 | `func (*Server).note(w http.ResponseWriter, r *http.Request)` | note returns an explicitly addressed current head, including a tombstone. |
| [`(*Server).noteVersions`](../src/server/reviews.go#L480) | method | 480-513 | `func (*Server).noteVersions(w http.ResponseWriter, r *http.Request)` | noteVersions returns bounded immutable note ancestors. |
| [`(*Server).noteVersion`](../src/server/reviews.go#L516) | method | 516-537 | `func (*Server).noteVersion(w http.ResponseWriter, r *http.Request)` | noteVersion returns one full immutable note body and resolved link set from the selected ancestry. |
| [`(*Server).createNoteVersion`](../src/server/reviews.go#L540) | method | 540-579 | `func (*Server).createNoteVersion(w http.ResponseWriter, r *http.Request)` | createNoteVersion creates an active edit or deletion tombstone. |
| [`(*Server).articleAnchors`](../src/server/reviews.go#L582) | method | 582-634 | `func (*Server).articleAnchors(w http.ResponseWriter, r *http.Request)` | articleAnchors returns bounded current active PDF anchors. |
| [`(*Server).createArticleAnchor`](../src/server/reviews.go#L637) | method | 637-672 | `func (*Server).createArticleAnchor(w http.ResponseWriter, r *http.Request)` | createArticleAnchor creates a logical anchor and its first immutable geometry version. |
| [`(*Server).anchorVersions`](../src/server/reviews.go#L675) | method | 675-718 | `func (*Server).anchorVersions(w http.ResponseWriter, r *http.Request)` | anchorVersions returns bounded immutable anchor ancestors. |
| [`(*Server).anchorVersion`](../src/server/reviews.go#L721) | method | 721-742 | `func (*Server).anchorVersion(w http.ResponseWriter, r *http.Request)` | anchorVersion returns one full immutable anchor version from the selected ancestry. |
| [`(*Server).createAnchorVersion`](../src/server/reviews.go#L745) | method | 745-825 | `func (*Server).createAnchorVersion(w http.ResponseWriter, r *http.Request)` | createAnchorVersion creates a replacement anchor version or tombstone using the currently selected PDF hash. |
| [`(*Server).reviewBacklinks`](../src/server/reviews.go#L828) | method | 828-888 | `func (*Server).reviewBacklinks(w http.ResponseWriter, r *http.Request)` | reviewBacklinks returns bounded current-version backlinks. |
| [`reviewRunRecord`](../src/server/reviews.go#L891) | struct | 891 | `type reviewRunRecord struct{ Status, Visibility string }` | reviewRunRecord contains the lifecycle fields that gate local review. |
| [`(*Server).loadReviewRun`](../src/server/reviews.go#L894) | method | 894-901 | `func (*Server).loadReviewRun(ctx context.Context, runID int64) (reviewRunRecord, error)` | loadReviewRun returns lifecycle fields without rejecting read-only historical contexts. |
| [`(*Server).requireReviewableRun`](../src/server/reviews.go#L904) | method | 904-917 | `func (*Server).requireReviewableRun(ctx context.Context, runID int64) (reviewRunRecord, error)` | requireReviewableRun rejects missing, failed, running, or trashed run contexts. |
| [`(*Server).requireContextForRead`](../src/server/reviews.go#L920) | method | 920-936 | `func (*Server).requireContextForRead(ctx context.Context, runID int64) (*database.ReviewContext, error)` | requireContextForRead returns an existing context for active or read-only historical runs. |
| [`(*Server).requireInitializedContext`](../src/server/reviews.go#L939) | method | 939-955 | `func (*Server).requireInitializedContext(ctx context.Context, runID int64) (*database.ReviewContext, error)` | requireInitializedContext returns the explicitly created context for an eligible run. |
| [`(*Server).reviewArticlePDF`](../src/server/reviews.go#L958) | method | 958-970 | `func (*Server).reviewArticlePDF(ctx context.Context, runID, workRevisionID int64) (int64, map[string]any, error)` | reviewArticlePDF validates exact run ownership and returns stable work plus inventory state. |
| [`(*Server).requireAvailableArticlePDF`](../src/server/reviews.go#L973) | method | 973-987 | `func (*Server).requireAvailableArticlePDF(ctx context.Context, runID, workRevisionID int64) (int64, string, error)` | requireAvailableArticlePDF gates an exact revision mutation on selected PDF bytes. |
| [`(*Server).requireAvailableWorkPDF`](../src/server/reviews.go#L990) | method | 990-1012 | `func (*Server).requireAvailableWorkPDF(ctx context.Context, runID, workID int64) (int64, string, error)` | requireAvailableWorkPDF gates PDF anchor mutation on run membership and matching PDF bytes. |
| [`(*Server).reviewSummaryCounts`](../src/server/reviews.go#L1015) | method | 1015-1034 | `func (*Server).reviewSummaryCounts(ctx context.Context, contextID, workID int64) (map[string]int, error)` | reviewSummaryCounts returns current note, anchor, and decision-version summary counts. |
| [`reviewArticleIDs`](../src/server/reviews.go#L1037) | function | 1037-1044 | `func reviewArticleIDs(r *http.Request) (int64, int64, error)` | reviewArticleIDs parses the positive run and revision identifiers from one review route. |
| [`(*Server).reviewNoteRequest`](../src/server/reviews.go#L1047) | method | 1047-1059 | `func (*Server).reviewNoteRequest(r *http.Request) (int64, int64, *database.ReviewContext, context.Context, context.CancelFunc, error)` | reviewNoteRequest prepares a bounded request context and resolves one logical note route. |
| [`(*Server).reviewAnchorRequest`](../src/server/reviews.go#L1062) | method | 1062-1074 | `func (*Server).reviewAnchorRequest(r *http.Request) (int64, string, *database.ReviewContext, context.Context, context.CancelFunc, error)` | reviewAnchorRequest prepares a bounded request context and resolves one safe anchor route. |
| [`reviewCursor`](../src/server/reviews.go#L1077) | struct | 1077-1082 | ``type reviewCursor struct { Kind string `json:"k"` ID int64 `json:"i,omitempty"` Text string `json:"s,omitempty"` StartedAt string `json:"t,omitempty"` }`` | reviewCursor is the endpoint-bound opaque keyset carried between review collection pages. |
| [`reviewLimit`](../src/server/reviews.go#L1085) | function | 1085-1095 | `func reviewLimit(r *http.Request) (int, error)` | reviewLimit validates the public page-size boundary. |
| [`reviewIDPage`](../src/server/reviews.go#L1098) | function | 1098-1111 | `func reviewIDPage(r *http.Request, kind string) (int64, int, error)` | reviewIDPage decodes an endpoint-bound numeric keyset cursor and page limit. |
| [`decodeReviewCursor`](../src/server/reviews.go#L1114) | function | 1114-1127 | `func decodeReviewCursor(raw, kind string) (reviewCursor, error)` | decodeReviewCursor validates an opaque cursor against its owning collection. |
| [`reviewIDItems`](../src/server/reviews.go#L1130) | function | 1130-1140 | `func reviewIDItems[T any](items []T, limit int, kind string, id func(T) int64) ([]T, bool, *string)` | reviewIDItems trims a limit-plus-one result and encodes the last visible numeric key. |
| [`decodeMutationJSON`](../src/server/reviews.go#L1143) | function | 1143-1181 | `func decodeMutationJSON(w http.ResponseWriter, r *http.Request, destination any) error` | decodeMutationJSON enforces media type, body bound, single value, and known JSON fields. |
| [`mapReviewError`](../src/server/reviews.go#L1184) | function | 1184-1258 | `func mapReviewError(err error) error` | mapReviewError converts repository conflicts and validation failures into stable API problems. |
| [`setMutableResponseHeaders`](../src/server/reviews.go#L1261) | function | 1261-1263 | `func setMutableResponseHeaders(w http.ResponseWriter)` | setMutableResponseHeaders prevents caching context-sensitive review responses. |
...
| Symbol | Kind | Lines | Declaration, inputs, and outputs | Source description |
|---|---|---:|---|---|
| [`encodeCursor`](../src/server/server.go#L46) | function | 46-49 | `func encodeCursor(cursor any) string` | encodeCursor serializes one endpoint-bound keyset without exposing its structure. |
| [`Server`](../src/server/server.go#L70) | struct | 70-79 | `type Server struct { db *sql.DB writeDB *database.Database pdfDB *sql.DB pdfPath string pdfCacheMu sync.Mutex pdfCache *cachedPDF tables map[string]tableInfo AssetsFS fs.FS // serves frontend assets from this filesystem }` | Server serves one existing workspace database. db remains a query-only connection while writeDB owns bounded local review and lifecycle mutations. AssetsFS is the frontend asset file system served at the web root; it must be set because the binary does not embed frontend assets. |
| [`tableInfo`](../src/server/server.go#L82) | struct | 82-85 | ``type tableInfo struct { Name string `json:"name"` Columns []columnInfo `json:"columns"` }`` | tableInfo stores the discovered columns for one browsable SQLite table. |
| [`columnInfo`](../src/server/server.go#L88) | struct | 88-92 | ``type columnInfo struct { Name string `json:"name"` Type string `json:"type"` PrimaryKey bool `json:"primary_key"` }`` | columnInfo records a SQLite column's name, declared type, and primary-key position. |
| [`(*Server).tableHasColumns`](../src/server/server.go#L95) | method | 95-110 | `func (*Server).tableHasColumns(table string, required ...string) bool` | tableHasColumns reports whether a discovered table contains every requested column. |
| [`Open`](../src/server/server.go#L113) | function | 113-168 | `func Open(path string) (*Server, error)` | Open opens an existing database without creating it or modifying it. |
| [`(*Server).Close`](../src/server/server.go#L171) | method | 171-187 | `func (*Server).Close() error` | Close releases resources owned by the receiver. |
| [`(*Server).PDFStoreBound`](../src/server/server.go#L190) | method | 190 | `func (*Server).PDFStoreBound() bool` | PDFStoreBound reports whether a readable companion PDF database is attached. |
| [`(*Server).Handler`](../src/server/server.go#L193) | method | 193-258 | `func (*Server).Handler() http.Handler` | Handler returns the local API and frontend handler. |
| [`(*Server).serveViewPage`](../src/server/server.go#L261) | method | 261-265 | `func (*Server).serveViewPage(file string) http.HandlerFunc` | serveViewPage serves one view HTML document from the assets filesystem. |
| [`apiResponseByteBudget`](../src/server/server.go#L269) | function | 269-280 | `func apiResponseByteBudget(path string) int` | apiResponseByteBudget returns the serialized JSON budget for one API route. Binary PDF and artifact-content routes stream outside this JSON contract. |
| [`apiQueryBudget`](../src/server/server.go#L283) | function | 283-294 | `func apiQueryBudget(path string) int` | apiQueryBudget returns the maximum SQL statements one API request may execute. |
| [`responseBudgetWriter`](../src/server/server.go#L297) | struct | 297-303 | `type responseBudgetWriter struct { header http.Header body bytes.Buffer status int limit int exceeded bool }` | responseBudgetWriter buffers one bounded JSON response before it is committed. |
| [`(*responseBudgetWriter).Header`](../src/server/server.go#L306) | method | 306 | `func (*responseBudgetWriter).Header() http.Header` | Header implements http.ResponseWriter. |
| [`(*responseBudgetWriter).WriteHeader`](../src/server/server.go#L309) | method | 309-313 | `func (*responseBudgetWriter).WriteHeader(status int)` | WriteHeader records the first response status without committing it. |
| [`(*responseBudgetWriter).Write`](../src/server/server.go#L316) | method | 316-330 | `func (*responseBudgetWriter).Write(data []byte) (int, error)` | Write retains at most the configured byte budget and reports a complete write. |
| [`withAPIResponseBudgets`](../src/server/server.go#L333) | function | 333-377 | `func withAPIResponseBudgets(next http.Handler) http.Handler` | withAPIResponseBudgets enforces route-specific serialized JSON limits. |
| [`(*Server).verifyReviewSchema`](../src/server/server.go#L380) | method | 380-407 | `func (*Server).verifyReviewSchema(ctx context.Context) error` | verifyReviewSchema rejects an unmigrated metadata database before writable controls are served. |
| [`(*Server).openBoundPDFStore`](../src/server/server.go#L410) | method | 410-482 | `func (*Server).openBoundPDFStore(ctx context.Context, metadataDir string) error` | openBoundPDFStore resolves and opens the companion PDF database declared by metadata. |
| [`pdfTableColumns`](../src/server/server.go#L485) | function | 485-505 | `func pdfTableColumns(ctx context.Context, db *sql.DB, table string) (map[string]bool, error)` | pdfTableColumns returns the discovered columns for a companion PDF table. |
| [`(*Server).HTTPServer`](../src/server/server.go#L508) | method | 508-517 | `func (*Server).HTTPServer(addr string) *http.Server` | HTTPServer returns a conservatively configured local HTTP server. |
| [`enforceLoopbackAuthority`](../src/server/server.go#L520) | function | 520-533 | `func enforceLoopbackAuthority(authority string, next http.Handler) http.Handler` | enforceLoopbackAuthority rejects invalid Host authorities before routing local viewer requests. |
| [`(*Server).discoverTables`](../src/server/server.go#L536) | method | 536-567 | `func (*Server).discoverTables(ctx context.Context) error` | discoverTables reads the SQLite schema and returns tables eligible for read-only browsing. |
| [`(*Server).columns`](../src/server/server.go#L570) | method | 570-588 | `func (*Server).columns(ctx context.Context, table string) ([]columnInfo, error)` | columns returns ordered metadata for the requested table's columns. |
| [`quoteIdentifier`](../src/server/server.go#L591) | function | 591-593 | `func quoteIdentifier(identifier string) string` | quoteIdentifier quotes a validated SQLite identifier and escapes embedded quotes. |
| [`(*Server).hasTable`](../src/server/server.go#L596) | method | 596 | `func (*Server).hasTable(name string) bool` | hasTable reports whether a table was discovered as browsable. |
| [`(*Server).hasColumn`](../src/server/server.go#L599) | method | 599-610 | `func (*Server).hasColumn(table, column string) bool` | hasColumn reports whether a discovered table contains a named column. |
| [`apiError`](../src/server/server.go#L613) | struct | 613-619 | ``type apiError struct { Error struct { Code string `json:"code"` Message string `json:"message"` Details any `json:"details,omitempty"` } `json:"error"` }`` | apiError is the stable JSON envelope returned for client-visible failures. |
| [`apiProblem`](../src/server/server.go#L622) | struct | 622-626 | `type apiProblem struct { Code, Message string Status int Details any }` | apiProblem carries an HTTP status and safe client-facing error message. |
| [`(*apiProblem).Error`](../src/server/server.go#L629) | method | 629 | `func (*apiProblem).Error() string` | Error returns the receiver's diagnostic message. |
| [`badRequest`](../src/server/server.go#L632) | function | 632-634 | `func badRequest(message string) error` | badRequest constructs an API problem with HTTP status 400. |
| [`notFound`](../src/server/server.go#L637) | function | 637-639 | `func notFound(message string) error` | notFound constructs an API problem with HTTP status 404. |
| [`withJSONErrors`](../src/server/server.go#L642) | function | 642-650 | `func withJSONErrors(next http.Handler) http.Handler` | withJSONErrors converts handler-returned errors into the server's JSON error response. |
| [`writeJSON`](../src/server/server.go#L653) | function | 653-657 | `func writeJSON(w http.ResponseWriter, status int, value any)` | writeJSON writes a JSON response with the supplied HTTP status. |
| [`writeError`](../src/server/server.go#L660) | function | 660-662 | `func writeError(w http.ResponseWriter, status int, code, message string)` | writeError writes the stable JSON error envelope. |
| [`writeDetailedError`](../src/server/server.go#L665) | function | 665-671 | `func writeDetailedError(w http.ResponseWriter, status int, code, message string, details any)` | writeDetailedError writes the stable JSON error envelope with optional structured details. |
| [`(*Server).respond`](../src/server/server.go#L674) | method | 674-686 | `func (*Server).respond(w http.ResponseWriter, r *http.Request, value any, err error)` | respond maps successful values, client problems, and internal failures to safe JSON responses. |
| [`queryContext`](../src/server/server.go#L689) | function | 689-691 | `func queryContext(r *http.Request) (context.Context, context.CancelFunc)` | queryContext derives a request context bounded by the server query timeout. |
| [`positiveID`](../src/server/server.go#L694) | function | 694-700 | `func positiveID(raw string) (int64, error)` | positiveID parses a strictly positive decimal identifier or returns a bad-request problem. |
| [`rowsAsMaps`](../src/server/server.go#L703) | function | 703-729 | `func rowsAsMaps(rows *sql.Rows) ([]map[string]any, error)` | rowsAsMaps scans SQL rows into maps keyed by result-column name. |
...
| Symbol | Kind | Lines | Declaration, inputs, and outputs | Source description |
|---|---|---:|---|---|
| [`TestAPIResponseByteBudgets`](../src/server/server_unit_test.go#L22) | test | 22-66 | `func TestAPIResponseByteBudgets(t *testing.T)` | TestAPIResponseByteBudgets verifies route classification and hard serialized limits. |
| [`TestTruncateUTF8BytesPreservesByteLimitAndEncoding`](../src/server/server_unit_test.go#L69) | test | 69-78 | `func TestTruncateUTF8BytesPreservesByteLimitAndEncoding(t *testing.T)` | TestTruncateUTF8BytesPreservesByteLimitAndEncoding verifies table-cell truncation uses bytes safely. |
| [`TestAPIQueryBudgets`](../src/server/server_unit_test.go#L81) | test | 81-137 | `func TestAPIQueryBudgets(t *testing.T)` | TestAPIQueryBudgets verifies deterministic route ceilings, driver accounting, and hard rejection. |
| [`TestQueryBudgetDriverInterfaces`](../src/server/server_unit_test.go#L140) | test | 140-195 | `func TestQueryBudgetDriverInterfaces(t *testing.T)` | TestQueryBudgetDriverInterfaces verifies statement, transaction, and optional driver paths. |
| [`TestHelper_quoteIdentifier`](../src/server/server_unit_test.go#L198) | test | 198-220 | `func TestHelper_quoteIdentifier(t *testing.T)` | TestHelper_quoteIdentifier verifies helper quote identifier. |
| [`TestHelper_positiveID`](../src/server/server_unit_test.go#L223) | test | 223-267 | `func TestHelper_positiveID(t *testing.T)` | TestHelper_positiveID verifies helper positive id. |
| [`TestHelper_stringID`](../src/server/server_unit_test.go#L270) | test | 270-287 | `func TestHelper_stringID(t *testing.T)` | TestHelper_stringID verifies helper string id. |
| [`TestHelper_nullableValue`](../src/server/server_unit_test.go#L290) | test | 290-310 | `func TestHelper_nullableValue(t *testing.T)` | TestHelper_nullableValue verifies helper nullable value. |
| [`TestHelper_badRequest`](../src/server/server_unit_test.go#L313) | test | 313-331 | `func TestHelper_badRequest(t *testing.T)` | TestHelper_badRequest verifies helper bad request. |
| [`TestHelper_notFound`](../src/server/server_unit_test.go#L334) | test | 334-349 | `func TestHelper_notFound(t *testing.T)` | TestHelper_notFound verifies helper not found. |
| [`TestHelper_metricGroup`](../src/server/server_unit_test.go#L352) | test | 352-374 | `func TestHelper_metricGroup(t *testing.T)` | TestHelper_metricGroup verifies helper metric group. |
| [`TestHelper_sourceBreakdown`](../src/server/server_unit_test.go#L377) | test | 377-403 | `func TestHelper_sourceBreakdown(t *testing.T)` | TestHelper_sourceBreakdown verifies helper source breakdown. |
| [`TestHelper_sourceBreakdown_noDenominator`](../src/server/server_unit_test.go#L406) | test | 406-418 | `func TestHelper_sourceBreakdown_noDenominator(t *testing.T)` | TestHelper_sourceBreakdown_noDenominator verifies helper source breakdown no denominator. |
| [`TestHelper_enrichmentFieldBreakdown`](../src/server/server_unit_test.go#L421) | test | 421-448 | `func TestHelper_enrichmentFieldBreakdown(t *testing.T)` | TestHelper_enrichmentFieldBreakdown verifies helper enrichment field breakdown. |
| [`TestHelper_enrichmentProviderBreakdown`](../src/server/server_unit_test.go#L451) | test | 451-473 | `func TestHelper_enrichmentProviderBreakdown(t *testing.T)` | TestHelper_enrichmentProviderBreakdown verifies helper enrichment provider breakdown. |
| [`TestHelper_normalizationFieldBreakdown`](../src/server/server_unit_test.go#L476) | test | 476-515 | `func TestHelper_normalizationFieldBreakdown(t *testing.T)` | TestHelper_normalizationFieldBreakdown verifies helper normalization field breakdown. |
| [`TestHelper_normalizationFieldBreakdown_emptyProcessed`](../src/server/server_unit_test.go#L518) | test | 518-537 | `func TestHelper_normalizationFieldBreakdown_emptyProcessed(t *testing.T)` | TestHelper_normalizationFieldBreakdown_emptyProcessed verifies helper normalization field breakdown empty processed. |
| [`TestHelper_normalizationFieldBreakdown_skipsMissingStatuses`](../src/server/server_unit_test.go#L540) | test | 540-551 | `func TestHelper_normalizationFieldBreakdown_skipsMissingStatuses(t *testing.T)` | TestHelper_normalizationFieldBreakdown_skipsMissingStatuses verifies helper normalization field breakdown skips missing statuses. |
| [`TestHelper_metricDenominator`](../src/server/server_unit_test.go#L554) | test | 554-588 | `func TestHelper_metricDenominator(t *testing.T)` | TestHelper_metricDenominator verifies helper metric denominator. |
| [`TestHelper_metricDenominator_zeroThreshold`](../src/server/server_unit_test.go#L591) | test | 591-603 | `func TestHelper_metricDenominator_zeroThreshold(t *testing.T)` | TestHelper_metricDenominator_zeroThreshold verifies helper metric denominator zero threshold. |
| [`TestHelper_percent`](../src/server/server_unit_test.go#L606) | test | 606-634 | `func TestHelper_percent(t *testing.T)` | TestHelper_percent verifies helper percent. |
| [`TestHelper_normalizedArtifactContentType`](../src/server/server_unit_test.go#L637) | test | 637-657 | `func TestHelper_normalizedArtifactContentType(t *testing.T)` | TestHelper_normalizedArtifactContentType verifies helper normalized artifact content type. |
| [`TestMapReviewErrorPreservesUnknownFailures`](../src/server/server_unit_test.go#L660) | test | 660-665 | `func TestMapReviewErrorPreservesUnknownFailures(t *testing.T)` | TestMapReviewErrorPreservesUnknownFailures verifies unclassified repository failures reach the safe internal-error responder. |
| [`TestHelper_jsonArtifactContentType`](../src/server/server_unit_test.go#L668) | test | 668-686 | `func TestHelper_jsonArtifactContentType(t *testing.T)` | TestHelper_jsonArtifactContentType verifies helper json artifact content type. |
| [`TestHelper_inlineArtifactContentType`](../src/server/server_unit_test.go#L689) | test | 689-712 | `func TestHelper_inlineArtifactContentType(t *testing.T)` | TestHelper_inlineArtifactContentType verifies helper inline artifact content type. |
| [`TestHelper_auditMultiValues`](../src/server/server_unit_test.go#L715) | test | 715-799 | `func TestHelper_auditMultiValues(t *testing.T)` | TestHelper_auditMultiValues verifies helper audit multi values. |
| [`TestHelper_auditInClause`](../src/server/server_unit_test.go#L802) | test | 802-835 | `func TestHelper_auditInClause(t *testing.T)` | TestHelper_auditInClause verifies helper audit in clause. |
| [`TestHelper_auditWhere`](../src/server/server_unit_test.go#L838) | test | 838-855 | `func TestHelper_auditWhere(t *testing.T)` | TestHelper_auditWhere verifies helper audit where. |
| [`TestHelper_corpusSelectColumns`](../src/server/server_unit_test.go#L858) | test | 858-876 | `func TestHelper_corpusSelectColumns(t *testing.T)` | TestHelper_corpusSelectColumns verifies helper corpus select columns. |
| [`TestHelper_scopedPagination`](../src/server/server_unit_test.go#L879) | test | 879-922 | `func TestHelper_scopedPagination(t *testing.T)` | TestHelper_scopedPagination verifies helper scoped pagination. |
| [`TestHelper_placeholders`](../src/server/server_unit_test.go#L925) | test | 925-958 | `func TestHelper_placeholders(t *testing.T)` | TestHelper_placeholders verifies helper placeholders. |
| [`TestHelper_parseOptionalInt`](../src/server/server_unit_test.go#L961) | test | 961-1013 | `func TestHelper_parseOptionalInt(t *testing.T)` | TestHelper_parseOptionalInt verifies helper parse optional int. |
| [`ptr`](../src/server/server_unit_test.go#L1016) | function | 1016-1018 | `func ptr(v float64) *float64` | ptr supports the package test suite's ptr setup or assertions. |

### [`src/server/tables.go`](../src/server/tables.go)

| Symbol | Kind | Lines | Declaration, inputs, and outputs | Source description |
|---|---|---:|---|---|
| [`tableProjection`](../src/server/tables.go#L27) | struct | 27-31 | ``type tableProjection struct { Columns []columnInfo `json:"columns"` OmittedColumns map[string]string `json:"omitted_columns,omitempty"` RedactedFields []string `json:"redacted_fields,omitempty"` }`` | tableProjection describes the columns that Advanced may return without exposing raw binary or sensitive evidence. |
| [`tableSummary`](../src/server/tables.go#L34) | struct | 34-37 | ``type tableSummary struct { Name string `json:"name"` tableProjection }`` | tableSummary is the schema-only discovery shape; row counts are computed only for the selected table. |
| [`(*Server).tablesHandler`](../src/server/tables.go#L40) | method | 40-51 | `func (*Server).tablesHandler(w http.ResponseWriter, r *http.Request)` | tablesHandler returns metadata for every discovered browsable table. |
| [`(*Server).tableRows`](../src/server/tables.go#L54) | method | 54-177 | `func (*Server).tableRows(w http.ResponseWriter, r *http.Request)` | tableRows returns a bounded page from one validated browsable table. |
| [`tableRowsAsMaps`](../src/server/tables.go#L180) | function | 180-202 | `func tableRowsAsMaps(rows *sql.Rows) ([]map[string]any, error)` | tableRowsAsMaps scans table-browser rows without converting byte prefixes before UTF-8 truncation. |
| [`safeTableProjection`](../src/server/tables.go#L205) | function | 205-225 | `func safeTableProjection(info tableInfo) tableProjection` | safeTableProjection excludes binary values, redacts sensitive evidence, and caps overly wide schemas. |
...
| [`Pprint`](../src/something/api.go#L53) | function | 53-91 | `func Pprint(v any, indent int) string` | Pprint pretty-prints a resolved SOMETHING value for diagnostic display. Its JSON-like output is not a re-parseable SOMETHING serializer; the something-printer tool owns that source-format contract. |
...
| [`TestEvalForExpandsExpressionsIndependently`](../src/something/directive_generator_functional_test.go#L24) | test | 24-52 | `func TestEvalForExpandsExpressionsIndependently(t *testing.T)` | TestEvalForExpandsExpressionsIndependently verifies each loop and macro expansion receives a fresh expression tree rather than reusing prior values. |
| [`TestEvalInsert_Functional`](../src/something/directive_generator_functional_test.go#L55) | test | 55-60 | `func TestEvalInsert_Functional(t *testing.T)` | TestEvalInsert_Functional verifies eval insert functional. |
| [`TestEvalInsertInterpolation_Functional`](../src/something/directive_generator_functional_test.go#L63) | test | 63-68 | `func TestEvalInsertInterpolation_Functional(t *testing.T)` | TestEvalInsertInterpolation_Functional verifies eval insert interpolation functional. |
| [`TestEvalInsertMultipleValues_Functional`](../src/something/directive_generator_functional_test.go#L71) | test | 71-76 | `func TestEvalInsertMultipleValues_Functional(t *testing.T)` | TestEvalInsertMultipleValues_Functional verifies eval insert multiple values functional. |
| [`TestEvalIteration_Functional`](../src/something/directive_generator_functional_test.go#L79) | test | 79-91 | `func TestEvalIteration_Functional(t *testing.T)` | TestEvalIteration_Functional verifies eval iteration functional. |
| [`TestEvalIterationPeek_Functional`](../src/something/directive_generator_functional_test.go#L94) | test | 94-100 | `func TestEvalIterationPeek_Functional(t *testing.T)` | TestEvalIterationPeek_Functional verifies eval iteration peek functional. |
| [`TestEvalAsLvalue_Functional`](../src/something/directive_generator_functional_test.go#L103) | test | 103-108 | `func TestEvalAsLvalue_Functional(t *testing.T)` | TestEvalAsLvalue_Functional verifies eval as lvalue functional. |
| [`TestEvalAsLvalueFromVar_Functional`](../src/something/directive_generator_functional_test.go#L111) | test | 111-116 | `func TestEvalAsLvalueFromVar_Functional(t *testing.T)` | TestEvalAsLvalueFromVar_Functional verifies eval as lvalue from var functional. |
| [`TestEvalScopeFromIteration_Functional`](../src/something/directive_generator_functional_test.go#L119) | test | 119-133 | `func TestEvalScopeFromIteration_Functional(t *testing.T)` | TestEvalScopeFromIteration_Functional verifies eval scope from iteration functional. |
| [`TestEvalNestedIterationScope_Functional`](../src/something/directive_generator_functional_test.go#L136) | test | 136-156 | `func TestEvalNestedIterationScope_Functional(t *testing.T)` | TestEvalNestedIterationScope_Functional verifies eval nested iteration scope functional. |
| [`TestEvalIterationWithLabel_Functional`](../src/something/directive_generator_functional_test.go#L159) | test | 159-170 | `func TestEvalIterationWithLabel_Functional(t *testing.T)` | TestEvalIterationWithLabel_Functional verifies eval iteration with label functional. |
| [`TestEvalIterationLabelsHaveIndependentCounters_Functional`](../src/something/directive_generator_functional_test.go#L173) | test | 173-193 | `func TestEvalIterationLabelsHaveIndependentCounters_Functional(t *testing.T)` | TestEvalIterationLabelsHaveIndependentCounters_Functional verifies eval iteration labels have independent counters functional. |
| [`TestEvalBareScopeDoesNotDoubleConsumeIterationCounters_Functional`](../src/something/directive_generator_functional_test.go#L196) | test | 196-202 | `func TestEvalBareScopeDoesNotDoubleConsumeIterationCounters_Functional(t *testing.T)` | TestEvalBareScopeDoesNotDoubleConsumeIterationCounters_Functional verifies eval bare scope does not double consume iteration counters functional. |
| [`TestEvalForMappingIteration_Functional`](../src/something/directive_generator_functional_test.go#L205) | test | 205-212 | `func TestEvalForMappingIteration_Functional(t *testing.T)` | TestEvalForMappingIteration_Functional verifies eval for mapping iteration functional. |
| [`TestEvalForMapIntKey_Functional`](../src/something/directive_generator_functional_test.go#L215) | test | 215-221 | `func TestEvalForMapIntKey_Functional(t *testing.T)` | TestEvalForMapIntKey_Functional verifies eval for map int key functional. |
| [`TestEvalForSourceArray_Functional`](../src/something/directive_generator_functional_test.go#L224) | test | 224-230 | `func TestEvalForSourceArray_Functional(t *testing.T)` | TestEvalForSourceArray_Functional verifies eval for source array functional. |
| [`TestEvalScopeBodyForDecl_Functional`](../src/something/directive_generator_functional_test.go#L233) | test | 233-240 | `func TestEvalScopeBodyForDecl_Functional(t *testing.T)` | TestEvalScopeBodyForDecl_Functional verifies eval scope body for decl functional. |
| [`TestEvalScopeBodyInsertDecl_Functional`](../src/something/directive_generator_functional_test.go#L243) | test | 243-250 | `func TestEvalScopeBodyInsertDecl_Functional(t *testing.T)` | TestEvalScopeBodyInsertDecl_Functional verifies eval scope body insert decl functional. |
| [`TestEvalScopeBodyIterationTyped_Functional`](../src/something/directive_generator_functional_test.go#L253) | test | 253-268 | `func TestEvalScopeBodyIterationTyped_Functional(t *testing.T)` | TestEvalScopeBodyIterationTyped_Functional verifies eval scope body iteration typed functional. |
| [`TestEvalScopeBodyAsLvalueTyped_Functional`](../src/something/directive_generator_functional_test.go#L271) | test | 271-280 | `func TestEvalScopeBodyAsLvalueTyped_Functional(t *testing.T)` | TestEvalScopeBodyAsLvalueTyped_Functional verifies eval scope body as lvalue typed functional. |
| [`TestEvalResolveAsLvalueFromString_Functional`](../src/something/directive_generator_functional_test.go#L283) | test | 283-289 | `func TestEvalResolveAsLvalueFromString_Functional(t *testing.T)` | TestEvalResolveAsLvalueFromString_Functional verifies eval resolve as lvalue from string functional. |
| [`TestEvalPrivIteration_Functional`](../src/something/directive_generator_functional_test.go#L292) | test | 292-300 | `func TestEvalPrivIteration_Functional(t *testing.T)` | TestEvalPrivIteration_Functional verifies eval priv iteration functional. |
| [`TestEvalPrivAsLvalue_Functional`](../src/something/directive_generator_functional_test.go#L303) | test | 303-309 | `func TestEvalPrivAsLvalue_Functional(t *testing.T)` | TestEvalPrivAsLvalue_Functional verifies eval priv as lvalue functional. |
| [`TestEvalPrivIterationInScope_Functional`](../src/something/directive_generator_functional_test.go#L312) | test | 312-321 | `func TestEvalPrivIterationInScope_Functional(t *testing.T)` | TestEvalPrivIterationInScope_Functional verifies eval priv iteration in scope functional. |
| [`TestEvalPrivAsLvalueInScope_Functional`](../src/something/directive_generator_functional_test.go#L324) | test | 324-331 | `func TestEvalPrivAsLvalueInScope_Functional(t *testing.T)` | TestEvalPrivAsLvalueInScope_Functional verifies eval priv as lvalue in scope functional. |
| [`TestResolveAsLvalueNameAll_Functional`](../src/something/directive_generator_functional_test.go#L334) | test | 334-341 | `func TestResolveAsLvalueNameAll_Functional(t *testing.T)` | TestResolveAsLvalueNameAll_Functional verifies resolve as lvalue name all functional. |
| [`TestResolveAsLvalueNameError_Functional`](../src/something/directive_generator_functional_test.go#L344) | test | 344-348 | `func TestResolveAsLvalueNameError_Functional(t *testing.T)` | TestResolveAsLvalueNameError_Functional verifies resolve as lvalue name error functional. |
...
| Symbol | Kind | Lines | Declaration, inputs, and outputs | Source description |
|---|---|---:|---|---|
| [`EnumValue`](../src/something/evaluator.go#L13) | struct | 13-16 | `type EnumValue struct { Ordinal int EnumName string }` | EnumValue preserves enum identity internally while expressions are evaluated. |
| [`runtimeBinding`](../src/something/evaluator.go#L19) | struct | 19-23 | `type runtimeBinding struct { value any typeRef TypeRef private bool }` | runtimeBinding stores one evaluated value with its type and visibility. |
| [`runtimeEnvironment`](../src/something/evaluator.go#L26) | struct | 26-30 | `type runtimeEnvironment struct { parent *runtimeEnvironment bindings map[string]*runtimeBinding types map[string]TypeRef }` | runtimeEnvironment stores lexically nested runtime bindings and named types. |
| [`newRuntimeEnvironment`](../src/something/evaluator.go#L33) | function | 33-39 | `func newRuntimeEnvironment(parent *runtimeEnvironment) *runtimeEnvironment` | newRuntimeEnvironment creates an empty runtime scope linked to its parent. |
| [`(*runtimeEnvironment).lookupBinding`](../src/something/evaluator.go#L42) | method | 42-49 | `func (*runtimeEnvironment).lookupBinding(name string) (*runtimeBinding, bool)` | lookupBinding searches the current and enclosing scopes for a value binding. |
| [`(*runtimeEnvironment).lookupType`](../src/something/evaluator.go#L52) | method | 52-59 | `func (*runtimeEnvironment).lookupType(name string) (TypeRef, bool)` | lookupType searches the current and enclosing scopes for a named type. |
| [`runtimeObject`](../src/something/evaluator.go#L62) | struct | 62-66 | `type runtimeObject struct { environment *runtimeEnvironment kind PrimitiveKind typeName string }` | runtimeObject carries scope, namespace, or setup fields in a nested runtime environment. |
| [`runtimeMapping`](../src/something/evaluator.go#L69) | struct | 69-73 | `type runtimeMapping struct { entries []runtimeMappingEntry keyType TypeRef valueType TypeRef }` | runtimeMapping preserves ordered key-value entries and their resolved types. |
| [`runtimeMappingEntry`](../src/something/evaluator.go#L76) | struct | 76-79 | `type runtimeMappingEntry struct { key any value any }` | runtimeMappingEntry stores one evaluated mapping key and value. |
| [`(*runtimeMapping).lookup`](../src/something/evaluator.go#L82) | method | 82-89 | `func (*runtimeMapping).lookup(key any) (any, bool)` | lookup returns the value associated with a runtime-equivalent mapping key. |
| [`(*runtimeMapping).update`](../src/something/evaluator.go#L92) | method | 92-100 | `func (*runtimeMapping).update(key, value any) bool` | update replaces the value for a runtime-equivalent mapping key and reports whether it existed. |
| [`runtimeKeysEqual`](../src/something/evaluator.go#L103) | function | 103-118 | `func runtimeKeysEqual(left, right any) bool` | runtimeKeysEqual compares evaluated mapping keys while preserving enum ordinal compatibility. |
| [`Evaluator`](../src/something/evaluator.go#L121) | struct | 121-125 | `type Evaluator struct { checked *CheckedProgram filepath string state *runtimeState }` | Evaluator evaluates a checked program into its public configuration map. |
| [`runtimeState`](../src/something/evaluator.go#L128) | struct | 128-135 | `type runtimeState struct { filepath string root *runtimeEnvironment current *runtimeEnvironment types map[*Assignment]TypeRef assertions map[string][]Statement // type name -> assertion bodies instanceLocation *SourceLocation // set when evaluating assertions for a specific instance }` | runtimeState owns evaluator scopes, checked assignment types, assertions, and source context. |
| [`newRuntimeState`](../src/something/evaluator.go#L138) | function | 138-141 | `func newRuntimeState(filepath string, types map[*Assignment]TypeRef) *runtimeState` | newRuntimeState creates evaluator state with an empty root environment. |
| [`NewEvaluator`](../src/something/evaluator.go#L144) | function | 144-153 | `func NewEvaluator(checked *CheckedProgram, filepath string) *Evaluator` | NewEvaluator constructs evaluator. |
| [`(*Evaluator).evaluate`](../src/something/evaluator.go#L156) | method | 156-162 | `func (*Evaluator).evaluate() map[string]any` | evaluate executes a checked program and returns its public configuration map. |
| [`(*runtimeState).err`](../src/something/evaluator.go#L165) | method | 165-167 | `func (*runtimeState).err(message string, location *SourceLocation, suggestion string)` | err panics with a source-located SomethingError for a runtime semantic failure. |
| [`(*runtimeState).evaluateStatements`](../src/something/evaluator.go#L170) | method | 170-185 | `func (*runtimeState).evaluateStatements(statements []Statement)` | evaluateStatements evaluates statements against the current evaluator state. |
| [`(*runtimeState).evaluateAssertDirective`](../src/something/evaluator.go#L188) | method | 188-200 | `func (*runtimeState).evaluateAssertDirective(assertion *AssertDirective)` | evaluateAssertDirective evaluates assert directive against the current evaluator state. |
| [`(*runtimeState).evaluateIfDirective`](../src/something/evaluator.go#L203) | method | 203-215 | `func (*runtimeState).evaluateIfDirective(ifDir *IfDirective)` | evaluateIfDirective evaluates if directive against the current evaluator state. |
| [`(*runtimeState).evaluateErrorDirective`](../src/something/evaluator.go#L218) | method | 218-230 | `func (*runtimeState).evaluateErrorDirective(errDir *ErrorDirective)` | evaluateErrorDirective evaluates error directive against the current evaluator state. |
| [`(*runtimeState).evaluateAssignment`](../src/something/evaluator.go#L233) | method | 233-254 | `func (*runtimeState).evaluateAssignment(assignment *Assignment)` | evaluateAssignment evaluates assignment against the current evaluator state. |
| [`(*runtimeState).evaluateEnumDefinition`](../src/something/evaluator.go#L257) | method | 257-272 | `func (*runtimeState).evaluateEnumDefinition(assignment *Assignment, definition *EnumDefinition)` | evaluateEnumDefinition evaluates enum definition against the current evaluator state. |
| [`(*runtimeState).evaluateSetupDefinition`](../src/something/evaluator.go#L275) | method | 275-286 | `func (*runtimeState).evaluateSetupDefinition(assignment *Assignment, definition *SetupDefinition)` | evaluateSetupDefinition evaluates setup definition against the current evaluator state. |
| [`requireIdentifierTarget`](../src/something/evaluator.go#L289) | function | 289-295 | `func requireIdentifierTarget(state *runtimeState, target LValue, location *SourceLocation) string` | requireIdentifierTarget requires a valid identifier target value. |
| [`(*runtimeState).evaluateObjectAssignment`](../src/something/evaluator.go#L298) | method | 298-310 | `func (*runtimeState).evaluateObjectAssignment(assignment *Assignment, statements []Statement, kind PrimitiveKind)` | evaluateObjectAssignment evaluates object assignment against the current evaluator state. |
| [`(*runtimeState).assignValue`](../src/something/evaluator.go#L313) | method | 313-332 | `func (*runtimeState).assignValue(assignment *Assignment, value any, typeRef TypeRef)` | assignValue declares or reassigns an evaluated assignment target. |
| [`(*runtimeState).declareMember`](../src/something/evaluator.go#L335) | method | 335-350 | `func (*runtimeState).declareMember(target *MemberLValue, value any, typeRef TypeRef, private bool, location *SourceLocation)` | declareMember adds a previously undeclared field to a scope or namespace value. |
| [`(*runtimeState).reassign`](../src/something/evaluator.go#L353) | method | 353-396 | `func (*runtimeState).reassign(target LValue, value any, location *SourceLocation)` | reassign replaces an existing variable, field, array element, or mapping entry. |
| [`(*runtimeState).resolveLValueContainer`](../src/something/evaluator.go#L399) | method | 399-413 | `func (*runtimeState).resolveLValueContainer(root string, accesses []Access, location *SourceLocation) (any, TypeRef, Access)` | resolveLValueContainer resolves l value container from the supplied context. |
| [`(*runtimeState).evaluateExpression`](../src/something/evaluator.go#L416) | method | 416-464 | `func (*runtimeState).evaluateExpression(expression Expression, expected TypeRef) any` | evaluateExpression evaluates expression against the current evaluator state. |
| [`(*runtimeState).evaluateString`](../src/something/evaluator.go#L467) | method | 467-515 | `func (*runtimeState).evaluateString(expression *StringExpression) string` | evaluateString evaluates string against the current evaluator state. |
| [`(*runtimeState).evaluateInterpolationReference`](../src/something/evaluator.go#L518) | method | 518-521 | `func (*runtimeState).evaluateInterpolationReference(name string, location *SourceLocation) any` | evaluateInterpolationReference evaluates interpolation reference against the current evaluator state. |
| [`(*runtimeState).evaluateBinaryOp`](../src/something/evaluator.go#L524) | method | 524-576 | `func (*runtimeState).evaluateBinaryOp(expression *BinaryOpExpression) any` | evaluateBinaryOp evaluates binary op against the current evaluator state. |
| [`(*runtimeState).evaluateUnaryOp`](../src/something/evaluator.go#L579) | method | 579-586 | `func (*runtimeState).evaluateUnaryOp(expression *UnaryOpExpression) any` | evaluateUnaryOp evaluates unary op against the current evaluator state. |
| [`(*runtimeState).evaluateMatch`](../src/something/evaluator.go#L589) | method | 589-605 | `func (*runtimeState).evaluateMatch(expression *MatchExpression) any` | evaluateMatch evaluates match against the current evaluator state. |
| [`(*runtimeState).evaluateLen`](../src/something/evaluator.go#L608) | method | 608-619 | `func (*runtimeState).evaluateLen(expression *LenExpression) any` | evaluateLen evaluates len against the current evaluator state. |
| [`(*runtimeState).evaluateIntrinsic`](../src/something/evaluator.go#L622) | method | 622-635 | `func (*runtimeState).evaluateIntrinsic(expression *IntrinsicExpression) any` | evaluateIntrinsic evaluates an intrinsic call against the current evaluator state. |
| [`runtimeValuesEqual`](../src/something/evaluator.go#L638) | function | 638-665 | `func runtimeValuesEqual(left, right any) bool` | runtimeValuesEqual compares two runtime values for equality. |
| [`compareValues`](../src/something/evaluator.go#L668) | function | 668-720 | `func compareValues(left, right any, location *SourceLocation, state *runtimeState) int` | compareValues performs ordered comparison returning -1/0/1. |
| [`dottedReference`](../src/something/evaluator.go#L723) | function | 723-730 | `func dottedReference(path string, location *SourceLocation) (string, []Access)` | dottedReference converts a dotted path into a root name and field-access sequence. |
| [`(*runtimeState).evaluateReference`](../src/something/evaluator.go#L733) | method | 733-776 | `func (*runtimeState).evaluateReference(reference *ReferenceExpression, expected TypeRef) any` | evaluateReference evaluates reference against the current evaluator state. |
| [`(*runtimeState).enumMember`](../src/something/evaluator.go#L779) | method | 779-787 | `func (*runtimeState).enumMember(enumType *EnumType, member string, location *SourceLocation) *EnumValue` | enumMember returns the typed ordinal for a named enum member or raises a diagnostic. |
| [`(*runtimeState).expectedEnum`](../src/something/evaluator.go#L790) | method | 790-794 | `func (*runtimeState).expectedEnum(expected TypeRef) *EnumType` | expectedEnum resolves an expected type and returns it only when it is an enum. |
| [`(*runtimeState).resolveAccess`](../src/something/evaluator.go#L797) | method | 797-800 | `func (*runtimeState).resolveAccess(current any, access Access, location *SourceLocation) any` | resolveAccess resolves access from the supplied context. |
| [`(*runtimeState).applyAccesses`](../src/something/evaluator.go#L803) | method | 803-810 | `func (*runtimeState).applyAccesses(value any, typeRef TypeRef, accesses []Access, location *SourceLocation) any` | applyAccesses resolves a sequence of member accesses on an evaluated value. |
| [`(*runtimeState).resolveTypedAccess`](../src/something/evaluator.go#L813) | method | 813-868 | `func (*runtimeState).resolveTypedAccess(current any, currentType TypeRef, access Access, location *SourceLocation) (any, TypeRef)` | resolveTypedAccess resolves typed access from the supplied context. |
| [`(*runtimeState).collectionIndexType`](../src/something/evaluator.go#L871) | method | 871-885 | `func (*runtimeState).collectionIndexType(typeRef TypeRef, location *SourceLocation) TypeRef` | collectionIndexType returns the accepted index or key type of a collection. |
| [`(*runtimeState).collectionElementType`](../src/something/evaluator.go#L888) | method | 888-898 | `func (*runtimeState).collectionElementType(typeRef TypeRef, location *SourceLocation) TypeRef` | collectionElementType returns the element or value type of an array, enum-keyed array, or mapping. |
| [`(*runtimeState).integerIndex`](../src/something/evaluator.go#L901) | method | 901-911 | `func (*runtimeState).integerIndex(index any, location *SourceLocation) int` | integerIndex converts an integer or enum value to an array index. |
| [`(*runtimeState).evaluateArray`](../src/something/evaluator.go#L914) | method | 914-932 | `func (*runtimeState).evaluateArray(expression *ArrayExpression, expected TypeRef) any` | evaluateArray evaluates array against the current evaluator state. |
| [`(*runtimeState).evaluateMapping`](../src/something/evaluator.go#L935) | method | 935-967 | `func (*runtimeState).evaluateMapping(expression *MappingExpression, expected TypeRef) any` | evaluateMapping evaluates mapping against the current evaluator state. |
| [`(*runtimeState).evaluateStruct`](../src/something/evaluator.go#L970) | method | 970-1032 | `func (*runtimeState).evaluateStruct(expression *StructExpression, expected TypeRef) any` | evaluateStruct evaluates struct against the current evaluator state. |
| [`(*runtimeState).resolveType`](../src/something/evaluator.go#L1035) | method | 1035-1055 | `func (*runtimeState).resolveType(typeRef TypeRef, location *SourceLocation) TypeRef` | resolveType resolves type from the supplied context. |
| [`(*runtimeState).runtimeType`](../src/something/evaluator.go#L1058) | method | 1058-1105 | `func (*runtimeState).runtimeType(value any) TypeRef` | runtimeType derives a SOMETHING type reference from an evaluated value. |
| [`(*runtimeState).publicMap`](../src/something/evaluator.go#L1108) | method | 1108-1117 | `func (*runtimeState).publicMap(environment *runtimeEnvironment) map[string]any` | publicMap materializes the non-private bindings in a runtime environment. |
| [`materializeValue`](../src/something/evaluator.go#L1120) | function | 1120-1173 | `func materializeValue(value any, includePrivate bool) any` | materializeValue recursively converts internal runtime values to public Go values. |
| [`runtimeTypeName`](../src/something/evaluator.go#L1176) | function | 1176-1197 | `func runtimeTypeName(value any) string` | runtimeTypeName returns the user-facing type name of an evaluated value. |
| [`mapKeys`](../src/something/evaluator.go#L1200) | function | 1200-1206 | `func mapKeys(values map[string]any) []string` | mapKeys returns all keys from a string-keyed runtime map. |
| [`isValidTimestamp`](../src/something/evaluator.go#L1209) | function | 1209-1232 | `func isValidTimestamp(value string) bool` | isValidTimestamp reports whether a string matches the supported timestamp shape. |
| [`typeRefDisplayName`](../src/something/evaluator.go#L1235) | function | 1235-1240 | `func typeRefDisplayName(typeRef TypeRef) string` | typeRefDisplayName returns a diagnostic name for a type reference. |
| [`isAlphaNum`](../src/something/evaluator.go#L1243) | function | 1243-1245 | `func isAlphaNum(value byte) bool` | isAlphaNum reports whether a byte is an ASCII letter or digit. |
| [`indexOf`](../src/something/evaluator.go#L1248) | function | 1248-1255 | `func indexOf(values []string, expected string) int` | indexOf returns the first matching string index, or minus one when absent. |
...
| [`TestEvalMapping`](../src/something/evaluator_functional_test.go#L200) | test | 200-209 | `func TestEvalMapping(t *testing.T)` | TestEvalMapping verifies eval mapping. |
| [`TestEvalMappingKeyAccess`](../src/something/evaluator_functional_test.go#L212) | test | 212-217 | `func TestEvalMappingKeyAccess(t *testing.T)` | TestEvalMappingKeyAccess verifies eval mapping key access. |
| [`TestEvalCombinedFieldAndIndexAccess`](../src/something/evaluator_functional_test.go#L220) | test | 220-231 | `func TestEvalCombinedFieldAndIndexAccess(t *testing.T)` | TestEvalCombinedFieldAndIndexAccess verifies eval combined field and index access. |
| [`TestEvalQualifiedEnumMappingKey`](../src/something/evaluator_functional_test.go#L234) | test | 234-239 | `func TestEvalQualifiedEnumMappingKey(t *testing.T)` | TestEvalQualifiedEnumMappingKey verifies eval qualified enum mapping key. |
| [`TestEvalEmptyMappingUsesStringKeys`](../src/something/evaluator_functional_test.go#L242) | test | 242-247 | `func TestEvalEmptyMappingUsesStringKeys(t *testing.T)` | TestEvalEmptyMappingUsesStringKeys verifies eval empty mapping uses string keys. |
| [`TestEvalMappingEnumKeys`](../src/something/evaluator_functional_test.go#L250) | test | 250-259 | `func TestEvalMappingEnumKeys(t *testing.T)` | TestEvalMappingEnumKeys verifies eval mapping enum keys. |
| [`TestEvalMappingEnumKeyWithDotPrefix`](../src/something/evaluator_functional_test.go#L262) | test | 262-272 | `func TestEvalMappingEnumKeyWithDotPrefix(t *testing.T)` | TestEvalMappingEnumKeyWithDotPrefix verifies eval mapping enum key with dot prefix. |
| [`TestEvalMappingKeyAccessEnumIndex`](../src/something/evaluator_functional_test.go#L275) | test | 275-280 | `func TestEvalMappingKeyAccessEnumIndex(t *testing.T)` | TestEvalMappingKeyAccessEnumIndex verifies eval mapping key access enum index. |
| [`TestEvalMappingCompositeKeys`](../src/something/evaluator_functional_test.go#L283) | test | 283-289 | `func TestEvalMappingCompositeKeys(t *testing.T)` | TestEvalMappingCompositeKeys verifies eval mapping composite keys. |
| [`TestEvalMappingCompositeStringKeys`](../src/something/evaluator_functional_test.go#L292) | test | 292-299 | `func TestEvalMappingCompositeStringKeys(t *testing.T)` | TestEvalMappingCompositeStringKeys verifies eval mapping composite string keys. |
| [`TestEvalMappingStringKeyWithDot`](../src/something/evaluator_functional_test.go#L302) | test | 302-309 | `func TestEvalMappingStringKeyWithDot(t *testing.T)` | TestEvalMappingStringKeyWithDot verifies eval mapping string key with dot. |
| [`TestEvalEnumPlain`](../src/something/evaluator_functional_test.go#L314) | test | 314-319 | `func TestEvalEnumPlain(t *testing.T)` | TestEvalEnumPlain verifies eval enum plain. |
| [`TestEvalEnumShorthand`](../src/something/evaluator_functional_test.go#L322) | test | 322-327 | `func TestEvalEnumShorthand(t *testing.T)` | TestEvalEnumShorthand verifies eval enum shorthand. |
| [`TestEvalEnumTaggedValue`](../src/something/evaluator_functional_test.go#L330) | test | 330-335 | `func TestEvalEnumTaggedValue(t *testing.T)` | TestEvalEnumTaggedValue verifies eval enum tagged value. |
| [`TestEvalEnumTaggedStructValue`](../src/something/evaluator_functional_test.go#L338) | test | 338-347 | `func TestEvalEnumTaggedStructValue(t *testing.T)` | TestEvalEnumTaggedStructValue verifies eval enum tagged struct value. |
| [`TestEvalEnumIndexedVariable`](../src/something/evaluator_functional_test.go#L350) | test | 350-356 | `func TestEvalEnumIndexedVariable(t *testing.T)` | TestEvalEnumIndexedVariable verifies eval enum indexed variable. |
| [`TestEvalEnumQualifiedAccess`](../src/something/evaluator_functional_test.go#L359) | test | 359-364 | `func TestEvalEnumQualifiedAccess(t *testing.T)` | TestEvalEnumQualifiedAccess verifies eval enum qualified access. |
| [`TestEvalEnumValueViaStructField`](../src/something/evaluator_functional_test.go#L367) | test | 367-376 | `func TestEvalEnumValueViaStructField(t *testing.T)` | TestEvalEnumValueViaStructField verifies eval enum value via struct field. |
| [`TestEvalEnumValueNoValueType`](../src/something/evaluator_functional_test.go#L379) | test | 379-383 | `func TestEvalEnumValueNoValueType(t *testing.T)` | TestEvalEnumValueNoValueType verifies eval enum value no value type. |
| [`TestEvalEnumValueOrdinalInResult`](../src/something/evaluator_functional_test.go#L386) | test | 386-392 | `func TestEvalEnumValueOrdinalInResult(t *testing.T)` | TestEvalEnumValueOrdinalInResult verifies eval enum value ordinal in result. |
| [`TestEvalOptionalEnumDefaultSupportsTaggedValueAccess`](../src/something/evaluator_functional_test.go#L395) | test | 395-405 | `func TestEvalOptionalEnumDefaultSupportsTaggedValueAccess(t *testing.T)` | TestEvalOptionalEnumDefaultSupportsTaggedValueAccess verifies eval optional enum default supports tagged value access. |
| [`TestEvalStruct`](../src/something/evaluator_functional_test.go#L410) | test | 410-419 | `func TestEvalStruct(t *testing.T)` | TestEvalStruct verifies eval struct. |
| [`TestEvalNestedStruct`](../src/something/evaluator_functional_test.go#L422) | test | 422-429 | `func TestEvalNestedStruct(t *testing.T)` | TestEvalNestedStruct verifies eval nested struct. |
| [`TestEvalScope`](../src/something/evaluator_functional_test.go#L434) | test | 434-439 | `func TestEvalScope(t *testing.T)` | TestEvalScope verifies eval scope. |
| [`TestEvalScopePrivateVar`](../src/something/evaluator_functional_test.go#L442) | test | 442-450 | `func TestEvalScopePrivateVar(t *testing.T)` | TestEvalScopePrivateVar verifies eval scope private var. |
| [`TestEvalPrivateVar`](../src/something/evaluator_functional_test.go#L453) | test | 453-461 | `func TestEvalPrivateVar(t *testing.T)` | TestEvalPrivateVar verifies eval private var. |
| [`TestEvalScopeVarDeclNamedType`](../src/something/evaluator_functional_test.go#L464) | test | 464-470 | `func TestEvalScopeVarDeclNamedType(t *testing.T)` | TestEvalScopeVarDeclNamedType verifies eval scope var decl named type. |
| [`TestEvalScopeBodyNestedScope`](../src/something/evaluator_functional_test.go#L473) | test | 473-479 | `func TestEvalScopeBodyNestedScope(t *testing.T)` | TestEvalScopeBodyNestedScope verifies eval scope body nested scope. |
| [`TestEvalDotAccessEnumField`](../src/something/evaluator_functional_test.go#L482) | test | 482-487 | `func TestEvalDotAccessEnumField(t *testing.T)` | TestEvalDotAccessEnumField verifies eval dot access enum field. |
| [`TestEvalResolveDotPathUndefinedField`](../src/something/evaluator_functional_test.go#L490) | test | 490-494 | `func TestEvalResolveDotPathUndefinedField(t *testing.T)` | TestEvalResolveDotPathUndefinedField verifies eval resolve dot path undefined field. |
| [`TestEvalResolveDotPathNonDict`](../src/something/evaluator_functional_test.go#L497) | test | 497-501 | `func TestEvalResolveDotPathNonDict(t *testing.T)` | TestEvalResolveDotPathNonDict verifies eval resolve dot path non dict. |
| [`TestEvalResolveDotPathVariableKey`](../src/something/evaluator_functional_test.go#L504) | test | 504-510 | `func TestEvalResolveDotPathVariableKey(t *testing.T)` | TestEvalResolveDotPathVariableKey verifies eval resolve dot path variable key. |
| [`TestEvalResolveDotPathVariableKeyUndefined`](../src/something/evaluator_functional_test.go#L513) | test | 513-517 | `func TestEvalResolveDotPathVariableKeyUndefined(t *testing.T)` | TestEvalResolveDotPathVariableKeyUndefined verifies eval resolve dot path variable key undefined. |
| [`TestEvalUndefinedVar`](../src/something/evaluator_functional_test.go#L522) | test | 522-526 | `func TestEvalUndefinedVar(t *testing.T)` | TestEvalUndefinedVar verifies eval undefined var. |
| [`TestEvalVariableWithSemicolons`](../src/something/evaluator_functional_test.go#L529) | test | 529-534 | `func TestEvalVariableWithSemicolons(t *testing.T)` | TestEvalVariableWithSemicolons verifies eval variable with semicolons. |
| [`TestEvalNegativeInteger`](../src/something/evaluator_functional_test.go#L539) | test | 539-544 | `func TestEvalNegativeInteger(t *testing.T)` | TestEvalNegativeInteger verifies eval negative integer. |
| [`TestEvalNegativeFloat`](../src/something/evaluator_functional_test.go#L547) | test | 547-552 | `func TestEvalNegativeFloat(t *testing.T)` | TestEvalNegativeFloat verifies eval negative float. |
| [`TestParseIntLiteral`](../src/something/evaluator_functional_test.go#L557) | test | 557-564 | `func TestParseIntLiteral(t *testing.T)` | TestParseIntLiteral verifies parse int literal. |
| [`TestParseFloatLiteral`](../src/something/evaluator_functional_test.go#L567) | test | 567-574 | `func TestParseFloatLiteral(t *testing.T)` | TestParseFloatLiteral verifies parse float literal. |
| [`TestIsInt`](../src/something/evaluator_functional_test.go#L577) | test | 577-587 | `func TestIsInt(t *testing.T)` | TestIsInt verifies is int. |
| [`TestIsValidTimestampWithMicros`](../src/something/evaluator_functional_test.go#L590) | test | 590-595 | `func TestIsValidTimestampWithMicros(t *testing.T)` | TestIsValidTimestampWithMicros verifies is valid timestamp with micros. |
| [`TestMapKeysErrorPath`](../src/something/evaluator_functional_test.go#L598) | test | 598-603 | `func TestMapKeysErrorPath(t *testing.T)` | TestMapKeysErrorPath verifies map keys error path. |
| [`TestTypeNameOfAll`](../src/something/evaluator_functional_test.go#L608) | test | 608-622 | `func TestTypeNameOfAll(t *testing.T)` | TestTypeNameOfAll verifies type name of all. |
| [`TestTypeNameOfFloat`](../src/something/evaluator_functional_test.go#L625) | test | 625-630 | `func TestTypeNameOfFloat(t *testing.T)` | TestTypeNameOfFloat verifies type name of float. |
| [`TestIsValidTimestampAll`](../src/something/evaluator_functional_test.go#L635) | test | 635-649 | `func TestIsValidTimestampAll(t *testing.T)` | TestIsValidTimestampAll verifies is valid timestamp all. |
| [`TestGetLocationIterationDecl`](../src/something/evaluator_functional_test.go#L654) | test | 654-659 | `func TestGetLocationIterationDecl(t *testing.T)` | TestGetLocationIterationDecl verifies get location iteration decl. |
| [`TestGetLocationAsLvalueDecl`](../src/something/evaluator_functional_test.go#L662) | test | 662-667 | `func TestGetLocationAsLvalueDecl(t *testing.T)` | TestGetLocationAsLvalueDecl verifies get location as lvalue decl. |
| [`TestValidExprKindsDefault`](../src/something/evaluator_functional_test.go#L672) | test | 672-679 | `func TestValidExprKindsDefault(t *testing.T)` | TestValidExprKindsDefault verifies valid expr kinds default. |
| [`TestValidExprKindsForTypeReturnNil`](../src/something/evaluator_functional_test.go#L682) | test | 682-689 | `func TestValidExprKindsForTypeReturnNil(t *testing.T)` | TestValidExprKindsForTypeReturnNil verifies valid expr kinds for type return nil. |
| [`TestEvalIterationPeekInScope`](../src/something/evaluator_functional_test.go#L694) | test | 694-705 | `func TestEvalIterationPeekInScope(t *testing.T)` | TestEvalIterationPeekInScope verifies eval iteration peek in scope. |
...
| [`TestRuntimeTypeNameDefault`](../src/something/evaluator_unit_test.go#L34) | test | 34-39 | `func TestRuntimeTypeNameDefault(t *testing.T)` | TestRuntimeTypeNameDefault verifies the runtime type name default. |
| [`TestIndexOfNotFound`](../src/something/evaluator_unit_test.go#L42) | test | 42-48 | `func TestIndexOfNotFound(t *testing.T)` | TestIndexOfNotFound verifies index of not found. |
| [`TestNewEvaluatorNilCheckedProgram`](../src/something/evaluator_unit_test.go#L52) | test | 52-57 | `func TestNewEvaluatorNilCheckedProgram(t *testing.T)` | TestNewEvaluatorNilCheckedProgram verifies a nil checked program evaluates consistently with the evaluator's existing empty-result behavior. |
...
| [`TestIfBodiesHaveChildScopes`](../src/something/pipeline_functional_test.go#L761) | test | 761-784 | `func TestIfBodiesHaveChildScopes(t *testing.T)` | TestIfBodiesHaveChildScopes verifies conditional declarations do not escape while reassignments of enclosing bindings retain their established meaning. |
| [`TestIfBodiesRemainScopedInAssertions`](../src/something/pipeline_functional_test.go#L788) | test | 788-806 | `func TestIfBodiesRemainScopedInAssertions(t *testing.T)` | TestIfBodiesRemainScopedInAssertions verifies conditional declarations do not become setup fields while assignments to a setup field still apply. |
| [`TestErrorDirectivePanics`](../src/something/pipeline_functional_test.go#L809) | test | 809-813 | `func TestErrorDirectivePanics(t *testing.T)` | TestErrorDirectivePanics verifies error directive panics. |
| [`TestErrorDirectiveUsesInterpolation`](../src/something/pipeline_functional_test.go#L816) | test | 816-820 | `func TestErrorDirectiveUsesInterpolation(t *testing.T)` | TestErrorDirectiveUsesInterpolation verifies error directive uses interpolation. |
| [`TestComparisonOperators`](../src/something/pipeline_functional_test.go#L823) | test | 823-843 | `func TestComparisonOperators(t *testing.T)` | TestComparisonOperators verifies comparison operators. |
| [`TestBooleanOperators`](../src/something/pipeline_functional_test.go#L846) | test | 846-864 | `func TestBooleanOperators(t *testing.T)` | TestBooleanOperators verifies boolean operators. |
| [`TestShortCircuitAnd`](../src/something/pipeline_functional_test.go#L867) | test | 867-884 | `func TestShortCircuitAnd(t *testing.T)` | TestShortCircuitAnd verifies short circuit and. |
| [`TestShortCircuitOr`](../src/something/pipeline_functional_test.go#L887) | test | 887-904 | `func TestShortCircuitOr(t *testing.T)` | TestShortCircuitOr verifies short circuit or. |
| [`TestParenthesizedGrouping`](../src/something/pipeline_functional_test.go#L907) | test | 907-914 | `func TestParenthesizedGrouping(t *testing.T)` | TestParenthesizedGrouping verifies parenthesized grouping. |
| [`TestMatchExpression`](../src/something/pipeline_functional_test.go#L917) | test | 917-925 | `func TestMatchExpression(t *testing.T)` | TestMatchExpression verifies match expression. |
| [`TestMatchInvalidRegex`](../src/something/pipeline_functional_test.go#L928) | test | 928-932 | `func TestMatchInvalidRegex(t *testing.T)` | TestMatchInvalidRegex verifies match invalid regex. |
| [`TestLenOnArray`](../src/something/pipeline_functional_test.go#L935) | test | 935-943 | `func TestLenOnArray(t *testing.T)` | TestLenOnArray verifies len on array. |
| [`TestLenOnEmptyArray`](../src/something/pipeline_functional_test.go#L946) | test | 946-954 | `func TestLenOnEmptyArray(t *testing.T)` | TestLenOnEmptyArray verifies len on empty array. |
| [`TestLenOnMapping`](../src/something/pipeline_functional_test.go#L957) | test | 957-965 | `func TestLenOnMapping(t *testing.T)` | TestLenOnMapping verifies len on mapping. |
| [`TestOperatorPrecedence`](../src/something/pipeline_functional_test.go#L968) | test | 968-976 | `func TestOperatorPrecedence(t *testing.T)` | TestOperatorPrecedence verifies operator precedence. |
| [`TestStringEscapingDoubleBrace`](../src/something/pipeline_functional_test.go#L979) | test | 979-987 | `func TestStringEscapingDoubleBrace(t *testing.T)` | TestStringEscapingDoubleBrace verifies string escaping double brace. |
| [`TestNestedIfInAssert`](../src/something/pipeline_functional_test.go#L990) | test | 990-1009 | `func TestNestedIfInAssert(t *testing.T)` | TestNestedIfInAssert verifies nested if in assert. |
| [`TestUnknownDirectiveErrorMessage`](../src/something/pipeline_functional_test.go#L1012) | test | 1012-1016 | `func TestUnknownDirectiveErrorMessage(t *testing.T)` | TestUnknownDirectiveErrorMessage verifies unknown directive error message. |
...
| [`(*TypeChecker).checkIfDirective`](../src/something/typechecker.go#L233) | method | 233-241 | `func (*TypeChecker).checkIfDirective(ifDir *IfDirective)` | checkIfDirective checks if directive against the current invariants. |
| [`(*TypeChecker).checkErrorDirective`](../src/something/typechecker.go#L244) | method | 244-247 | `func (*TypeChecker).checkErrorDirective(errDir *ErrorDirective)` | checkErrorDirective checks error directive against the current invariants. |
| [`(*TypeChecker).checkSetupDefinition`](../src/something/typechecker.go#L250) | method | 250-274 | `func (*TypeChecker).checkSetupDefinition(assignment *Assignment, definition *SetupDefinition)` | checkSetupDefinition checks setup definition against the current invariants. |
| [`(*TypeChecker).checkScopeAssignment`](../src/something/typechecker.go#L277) | method | 277-324 | `func (*TypeChecker).checkScopeAssignment(assignment *Assignment, scope *ScopeExpression, namespace bool)` | checkScopeAssignment checks scope assignment against the current invariants. |
| [`(*TypeChecker).checkNamespaceAssignment`](../src/something/typechecker.go#L327) | method | 327-330 | `func (*TypeChecker).checkNamespaceAssignment(assignment *Assignment, namespace *NamespaceExpression)` | checkNamespaceAssignment checks namespace assignment against the current invariants. |
| [`(*TypeChecker).requireTypeTarget`](../src/something/typechecker.go#L333) | method | 333-339 | `func (*TypeChecker).requireTypeTarget(assignment *Assignment) string` | requireTypeTarget requires a valid type target value. |
| [`(*TypeChecker).ensureTypeNameAvailable`](../src/something/typechecker.go#L342) | method | 342-349 | `func (*TypeChecker).ensureTypeNameAvailable(name string, location *SourceLocation)` | ensureTypeNameAvailable rejects duplicate or shadowed named type declarations. |
| [`(*TypeChecker).declareTarget`](../src/something/typechecker.go#L352) | method | 352-385 | `func (*TypeChecker).declareTarget(assignment *Assignment, typeRef TypeRef)` | declareTarget records a new assignment target and enforces declaration rules. |
| [`(*TypeChecker).resolveExistingTarget`](../src/something/typechecker.go#L388) | method | 388-403 | `func (*TypeChecker).resolveExistingTarget(target LValue, location *SourceLocation) TypeRef` | resolveExistingTarget resolves existing target from the supplied context. |
| [`(*TypeChecker).resolveStaticContainer`](../src/something/typechecker.go#L406) | method | 406-419 | `func (*TypeChecker).resolveStaticContainer(root string, accesses []Access, location *SourceLocation) (TypeRef, Access)` | resolveStaticContainer resolves static container from the supplied context. |
| [`(*TypeChecker).expressionType`](../src/something/typechecker.go#L422) | method | 422-484 | `func (*TypeChecker).expressionType(expression Expression, expected TypeRef) TypeRef` | expressionType resolves and validates the static type of an expression. |
| [`(*TypeChecker).applyAccessesType`](../src/something/typechecker.go#L487) | method | 487-493 | `func (*TypeChecker).applyAccessesType(base TypeRef, accesses []Access, location *SourceLocation) TypeRef` | applyAccessesType resolves the type reached by member accesses on a base type. |
| [`(*TypeChecker).intrinsicExpressionType`](../src/something/typechecker.go#L497) | method | 497-510 | `func (*TypeChecker).intrinsicExpressionType(expression *IntrinsicExpression) TypeRef` | intrinsicExpressionType checks an intrinsic call's arguments and returns its declared return type. |
| [`(*TypeChecker).checkBinaryOpType`](../src/something/typechecker.go#L513) | method | 513-540 | `func (*TypeChecker).checkBinaryOpType(expression *BinaryOpExpression) TypeRef` | checkBinaryOpType checks binary op type against the current invariants. |
| [`(*TypeChecker).isComparableType`](../src/something/typechecker.go#L543) | method | 543-549 | `func (*TypeChecker).isComparableType(typeRef TypeRef) bool` | isComparableType reports whether a resolved type supports ordered comparison. |
| [`(*TypeChecker).checkStringReferences`](../src/something/typechecker.go#L552) | method | 552-577 | `func (*TypeChecker).checkStringReferences(expression *StringExpression)` | checkStringReferences checks string references against the current invariants. |
| [`(*TypeChecker).constantString`](../src/something/typechecker.go#L580) | method | 580-596 | `func (*TypeChecker).constantString(expression *StringExpression) (string, bool)` | constantString returns a statically known string expression when one is available. |
| [`(*TypeChecker).referenceType`](../src/something/typechecker.go#L599) | method | 599-638 | `func (*TypeChecker).referenceType(reference *ReferenceExpression, expected TypeRef) TypeRef` | referenceType resolves the type reached by a root binding and its member accesses. |
| [`(*TypeChecker).accessType`](../src/something/typechecker.go#L641) | method | 641-701 | `func (*TypeChecker).accessType(base TypeRef, access Access, location *SourceLocation, assignment bool) TypeRef` | accessType validates one field or index access and returns the resulting type. |
| [`(*TypeChecker).arrayExpressionType`](../src/something/typechecker.go#L704) | method | 704-743 | `func (*TypeChecker).arrayExpressionType(expression *ArrayExpression, expected TypeRef) TypeRef` | arrayExpressionType checks array elements and returns the resolved array type. |
| [`(*TypeChecker).mappingExpressionType`](../src/something/typechecker.go#L746) | method | 746-772 | `func (*TypeChecker).mappingExpressionType(expression *MappingExpression, expected TypeRef) TypeRef` | mappingExpressionType checks mapping keys and values and returns the resolved mapping type. |
| [`(*TypeChecker).requireMappingKeyType`](../src/something/typechecker.go#L775) | method | 775-785 | `func (*TypeChecker).requireMappingKeyType(typeRef TypeRef, location *SourceLocation)` | requireMappingKeyType requires a valid mapping key type value. |
| [`(*TypeChecker).structExpressionType`](../src/something/typechecker.go#L788) | method | 788-833 | `func (*TypeChecker).structExpressionType(expression *StructExpression, expected TypeRef) TypeRef` | structExpressionType checks named struct fields and returns the resolved setup type. |
| [`(*TypeChecker).resolveType`](../src/something/typechecker.go#L836) | method | 836-863 | `func (*TypeChecker).resolveType(typeRef TypeRef, location *SourceLocation) TypeRef` | resolveType resolves type from the supplied context. |
| [`(*TypeChecker).requireAssignable`](../src/something/typechecker.go#L866) | method | 866-871 | `func (*TypeChecker).requireAssignable(expected, actual TypeRef, location *SourceLocation, context string)` | requireAssignable requires a valid assignable value. |
| [`(*TypeChecker).assignable`](../src/something/typechecker.go#L874) | method | 874-921 | `func (*TypeChecker).assignable(expected, actual TypeRef) bool` | assignable reports whether an actual type may be assigned to an expected type. |
| [`(*TypeChecker).structurallyAssignable`](../src/something/typechecker.go#L924) | method | 924-935 | `func (*TypeChecker).structurallyAssignable(expected, actual map[string]*BindingType) bool` | structurallyAssignable reports whether two compound types have compatible structure. |
| [`enumHasMember`](../src/something/typechecker.go#L938) | function | 938-945 | `func enumHasMember(enumType *EnumType, name string) bool` | enumHasMember reports whether a resolved enum contains a named member. |
| [`(*TypeChecker).detectDependencyCycles`](../src/something/typechecker.go#L949) | method | 949-984 | `func (*TypeChecker).detectDependencyCycles()` | detectDependencyCycles reports direct and indirect value or type cycles before source-order checking reports a less useful forward-reference error. |
| [`(*TypeChecker).collectCycleNodes`](../src/something/typechecker.go#L987) | method | 987-1022 | `func (*TypeChecker).collectCycleNodes(statements []Statement, prefix string, graph map[string][]string, locations map[string]*SourceLocation)` | collectCycleNodes collects cycle nodes from the supplied inputs. |
| [`(*TypeChecker).collectCycleEdges`](../src/something/typechecker.go#L1025) | method | 1025-1100 | `func (*TypeChecker).collectCycleEdges(statements []Statement, prefix string, graph map[string][]string, visibleValues, visibleTypes map[string]string)` | collectCycleEdges collects cycle edges from the supplied inputs. |
| [`copyCycleNames`](../src/something/typechecker.go#L1103) | function | 1103-1109 | `func copyCycleNames(source map[string]string) map[string]string` | copyCycleNames copies cycle names into an independent value. |
| [`cycleTargetName`](../src/something/typechecker.go#L1112) | function | 1112-1130 | `func cycleTargetName(target LValue, prefix string) string` | cycleTargetName returns the declared root name participating in dependency analysis. |
| [`typeDependencies`](../src/something/typechecker.go#L1133) | function | 1133-1146 | `func typeDependencies(typeRef TypeRef) []string` | typeDependencies collects named types referenced by a type expression. |
| [`expressionDependencies`](../src/something/typechecker.go#L1149) | function | 1149-1236 | `func expressionDependencies(expression Expression) []string` | expressionDependencies collects root bindings referenced by an expression. |
| [`referencePath`](../src/something/typechecker.go#L1239) | function | 1239-1251 | `func referencePath(reference *ReferenceExpression) string` | referencePath renders a reference root and accesses for dependency diagnostics. |
...
| [`TestEvalStructTypeMismatch_Functional`](../src/something/typechecker_functional_test.go#L151) | test | 151-155 | `func TestEvalStructTypeMismatch_Functional(t *testing.T)` | TestEvalStructTypeMismatch_Functional verifies eval struct type mismatch functional. |
| [`TestEvalStructDefault_Functional`](../src/something/typechecker_functional_test.go#L158) | test | 158-164 | `func TestEvalStructDefault_Functional(t *testing.T)` | TestEvalStructDefault_Functional verifies eval struct default functional. |
| [`TestResolveTypeRefArray_Functional`](../src/something/typechecker_functional_test.go#L167) | test | 167-175 | `func TestResolveTypeRefArray_Functional(t *testing.T)` | TestResolveTypeRefArray_Functional verifies resolve type ref array functional. |
| [`TestResolveTypeRefMapping_Functional`](../src/something/typechecker_functional_test.go#L178) | test | 178-187 | `func TestResolveTypeRefMapping_Functional(t *testing.T)` | TestResolveTypeRefMapping_Functional verifies resolve type ref mapping functional. |
| [`TestResolveTypeRefEnumKey_Functional`](../src/something/typechecker_functional_test.go#L190) | test | 190-201 | `func TestResolveTypeRefEnumKey_Functional(t *testing.T)` | TestResolveTypeRefEnumKey_Functional verifies resolve type ref enum key functional. |
| [`TestTypeRefDisplayNameAll_Functional`](../src/something/typechecker_functional_test.go#L204) | test | 204-214 | `func TestTypeRefDisplayNameAll_Functional(t *testing.T)` | TestTypeRefDisplayNameAll_Functional verifies type ref display name all functional. |
| [`TestTypeRefDisplayNameMapping_Functional`](../src/something/typechecker_functional_test.go#L217) | test | 217-222 | `func TestTypeRefDisplayNameMapping_Functional(t *testing.T)` | TestTypeRefDisplayNameMapping_Functional verifies type ref display name mapping functional. |
| [`TestTypeRefDisplayNameArray_Functional`](../src/something/typechecker_functional_test.go#L225) | test | 225-230 | `func TestTypeRefDisplayNameArray_Functional(t *testing.T)` | TestTypeRefDisplayNameArray_Functional verifies type ref display name array functional. |
| [`TestTypeRefDisplayNameEnumKey_Functional`](../src/something/typechecker_functional_test.go#L233) | test | 233-238 | `func TestTypeRefDisplayNameEnumKey_Functional(t *testing.T)` | TestTypeRefDisplayNameEnumKey_Functional verifies type ref display name enum key functional. |
| [`TestValidExprKindsForTypeAll_Functional`](../src/something/typechecker_functional_test.go#L241) | test | 241-251 | `func TestValidExprKindsForTypeAll_Functional(t *testing.T)` | TestValidExprKindsForTypeAll_Functional verifies valid expr kinds for type all functional. |
| [`TestValidExprKindsPrimString_Functional`](../src/something/typechecker_functional_test.go#L254) | test | 254-259 | `func TestValidExprKindsPrimString_Functional(t *testing.T)` | TestValidExprKindsPrimString_Functional verifies valid expr kinds prim string functional. |
| [`TestValidExprKindsPrimFloat_Functional`](../src/something/typechecker_functional_test.go#L262) | test | 262-267 | `func TestValidExprKindsPrimFloat_Functional(t *testing.T)` | TestValidExprKindsPrimFloat_Functional verifies valid expr kinds prim float functional. |
| [`TestValidExprKindsPrimTimestamp_Functional`](../src/something/typechecker_functional_test.go#L270) | test | 270-275 | `func TestValidExprKindsPrimTimestamp_Functional(t *testing.T)` | TestValidExprKindsPrimTimestamp_Functional verifies valid expr kinds prim timestamp functional. |
| [`TestValidExprKindsPrimBoolean_Functional`](../src/something/typechecker_functional_test.go#L278) | test | 278-283 | `func TestValidExprKindsPrimBoolean_Functional(t *testing.T)` | TestValidExprKindsPrimBoolean_Functional verifies valid expr kinds prim boolean functional. |
| [`TestFormatUnknownTypeSuggestionNoSetups_Functional`](../src/something/typechecker_functional_test.go#L286) | test | 286-290 | `func TestFormatUnknownTypeSuggestionNoSetups_Functional(t *testing.T)` | TestFormatUnknownTypeSuggestionNoSetups_Functional verifies format unknown type suggestion no setups functional. |
| [`TestResolveTypeRefUnknown_Functional`](../src/something/typechecker_functional_test.go#L293) | test | 293-298 | `func TestResolveTypeRefUnknown_Functional(t *testing.T)` | TestResolveTypeRefUnknown_Functional verifies resolve type ref unknown functional. |
...
| [`TestSortedKeys`](../src/something/typechecker_unit_test.go#L317) | test | 317-331 | `func TestSortedKeys(t *testing.T)` | TestSortedKeys verifies deterministic string-keyed map ordering. |
| [`TestSortedKeysEmpty`](../src/something/typechecker_unit_test.go#L334) | test | 334-339 | `func TestSortedKeysEmpty(t *testing.T)` | TestSortedKeysEmpty verifies sorting an empty map. |
| [`TestSortedKeysNil`](../src/something/typechecker_unit_test.go#L342) | test | 342-347 | `func TestSortedKeysNil(t *testing.T)` | TestSortedKeysNil verifies sorting a nil map. |
...
| [`walkMode`](../src/something/utils.go#L124) | type | 124 | `type walkMode int` | walkMode selects whether iteration segments require one match, a selected match, or all matches. |
| [`sortedKeys`](../src/something/utils.go#L133) | function | 133-140 | `func sortedKeys[T any](values map[string]T) []string` | sortedKeys returns string-keyed map keys in deterministic order. |
| [`walkFrom`](../src/something/utils.go#L144) | function | 144-199 | `func walkFrom(current any, segments []string, mode walkMode, index int) ([]any, error)` | walkFrom continues a traversal from the current value while preserving the existing public error types and path text for every selection mode. |
| [`checkType`](../src/something/utils.go#L202) | function | 202-237 | `func checkType(val any, expected string, path []string) error` | checkType checks type against the current invariants. |
| [`valueAt`](../src/something/utils.go#L240) | function | 240-250 | `func valueAt(data map[string]any, path []string, mode walkMode, index int, expected string) (any, error)` | valueAt selects and checks one typed value for the Once and Index accessors. |
| [`valuesAt`](../src/something/utils.go#L253) | function | 253-264 | `func valuesAt(data map[string]any, path []string, expected string) ([]any, error)` | valuesAt selects and checks every typed value for the All accessors. |
| [`GetStringOnce`](../src/something/utils.go#L267) | function | 267-273 | `func GetStringOnce(data map[string]any, path ...string) (string, error)` | GetStringOnce returns a string value at path. |
| [`GetIntegerOnce`](../src/something/utils.go#L276) | function | 276-282 | `func GetIntegerOnce(data map[string]any, path ...string) (int, error)` | GetIntegerOnce returns an integer value at path. |
| [`GetFloatOnce`](../src/something/utils.go#L285) | function | 285-294 | `func GetFloatOnce(data map[string]any, path ...string) (float64, error)` | GetFloatOnce returns a float value at path. |
| [`GetBoolOnce`](../src/something/utils.go#L297) | function | 297-303 | `func GetBoolOnce(data map[string]any, path ...string) (bool, error)` | GetBoolOnce returns a boolean value at path. |
| [`GetTimestampOnce`](../src/something/utils.go#L306) | function | 306-312 | `func GetTimestampOnce(data map[string]any, path ...string) (string, error)` | GetTimestampOnce returns a timestamp (string) value at path. |
| [`GetListOnce`](../src/something/utils.go#L315) | function | 315-321 | `func GetListOnce(data map[string]any, path ...string) ([]any, error)` | GetListOnce returns a list value at path. |
| [`GetMappingOnce`](../src/something/utils.go#L324) | function | 324-330 | `func GetMappingOnce(data map[string]any, path ...string) (map[string]any, error)` | GetMappingOnce returns a mapping (dict) value at path. |
| [`GetStructOnce`](../src/something/utils.go#L333) | function | 333-339 | `func GetStructOnce(data map[string]any, path ...string) (map[string]any, error)` | GetStructOnce returns a struct (dict) value at path. |
| [`GetScopeOnce`](../src/something/utils.go#L342) | function | 342-348 | `func GetScopeOnce(data map[string]any, path ...string) (map[string]any, error)` | GetScopeOnce returns a scope (dict) value at path. |
| [`GetEnumOnce`](../src/something/utils.go#L351) | function | 351-357 | `func GetEnumOnce(data map[string]any, path ...string) (int, error)` | GetEnumOnce returns an enum (integer ordinal) value at path. |
| [`GetStringIndex`](../src/something/utils.go#L360) | function | 360-366 | `func GetStringIndex(data map[string]any, index int, path ...string) (string, error)` | GetStringIndex returns a string at path, using index to select among iterations. |
| [`GetIntegerIndex`](../src/something/utils.go#L369) | function | 369-375 | `func GetIntegerIndex(data map[string]any, index int, path ...string) (int, error)` | GetIntegerIndex returns an integer at path, using index to select among iterations. |
| [`GetFloatIndex`](../src/something/utils.go#L378) | function | 378-387 | `func GetFloatIndex(data map[string]any, index int, path ...string) (float64, error)` | GetFloatIndex returns a float at path, using index to select among iterations. |
| [`GetBoolIndex`](../src/something/utils.go#L390) | function | 390-396 | `func GetBoolIndex(data map[string]any, index int, path ...string) (bool, error)` | GetBoolIndex returns a boolean at path, using index to select among iterations. |
| [`GetTimestampIndex`](../src/something/utils.go#L399) | function | 399-405 | `func GetTimestampIndex(data map[string]any, index int, path ...string) (string, error)` | GetTimestampIndex returns a timestamp at path, using index to select among iterations. |
| [`GetListIndex`](../src/something/utils.go#L408) | function | 408-414 | `func GetListIndex(data map[string]any, index int, path ...string) ([]any, error)` | GetListIndex returns a list at path, using index to select among iterations. |
| [`GetMappingIndex`](../src/something/utils.go#L417) | function | 417-423 | `func GetMappingIndex(data map[string]any, index int, path ...string) (map[string]any, error)` | GetMappingIndex returns a mapping at path, using index to select among iterations. |
| [`GetStructIndex`](../src/something/utils.go#L426) | function | 426-432 | `func GetStructIndex(data map[string]any, index int, path ...string) (map[string]any, error)` | GetStructIndex returns a struct at path, using index to select among iterations. |
| [`GetScopeIndex`](../src/something/utils.go#L435) | function | 435-441 | `func GetScopeIndex(data map[string]any, index int, path ...string) (map[string]any, error)` | GetScopeIndex returns a scope at path, using index to select among iterations. |
| [`GetEnumIndex`](../src/something/utils.go#L444) | function | 444-450 | `func GetEnumIndex(data map[string]any, index int, path ...string) (int, error)` | GetEnumIndex returns an enum at path, using index to select among iterations. |
| [`GetStringAll`](../src/something/utils.go#L453) | function | 453-463 | `func GetStringAll(data map[string]any, path ...string) ([]string, error)` | GetStringAll returns all string values reachable at path. |
| [`GetIntegerAll`](../src/something/utils.go#L466) | function | 466-476 | `func GetIntegerAll(data map[string]any, path ...string) ([]int, error)` | GetIntegerAll returns all integer values reachable at path. |
| [`GetFloatAll`](../src/something/utils.go#L479) | function | 479-493 | `func GetFloatAll(data map[string]any, path ...string) ([]float64, error)` | GetFloatAll returns all float values reachable at path. |
| [`GetBoolAll`](../src/something/utils.go#L496) | function | 496-506 | `func GetBoolAll(data map[string]any, path ...string) ([]bool, error)` | GetBoolAll returns all boolean values reachable at path. |
| [`GetTimestampAll`](../src/something/utils.go#L509) | function | 509-519 | `func GetTimestampAll(data map[string]any, path ...string) ([]string, error)` | GetTimestampAll returns all timestamp values reachable at path. |
| [`GetListAll`](../src/something/utils.go#L522) | function | 522-524 | `func GetListAll(data map[string]any, path ...string) ([]any, error)` | GetListAll returns all list values reachable at path. |
| [`GetMappingAll`](../src/something/utils.go#L527) | function | 527-537 | `func GetMappingAll(data map[string]any, path ...string) ([]map[string]any, error)` | GetMappingAll returns all mapping values reachable at path. |
| [`GetStructAll`](../src/something/utils.go#L540) | function | 540-550 | `func GetStructAll(data map[string]any, path ...string) ([]map[string]any, error)` | GetStructAll returns all struct values reachable at path. |
| [`GetScopeAll`](../src/something/utils.go#L553) | function | 553-563 | `func GetScopeAll(data map[string]any, path ...string) ([]map[string]any, error)` | GetScopeAll returns all scope values reachable at path. |
| [`GetEnumAll`](../src/something/utils.go#L566) | function | 566-576 | `func GetEnumAll(data map[string]any, path ...string) ([]int, error)` | GetEnumAll returns all enum (int) values reachable at path. |
...
| Symbol | Kind | Lines | Declaration, inputs, and outputs | Source description |
|---|---|---:|---|---|
| [`walkOnce`](../src/something/utils_unit_test.go#L13) | function | 13-19 | `func walkOnce(data map[string]any, segments []string) (any, error)` | walkOnce returns the single value reached by the test path. |
| [`walkIndex`](../src/something/utils_unit_test.go#L22) | function | 22-28 | `func walkIndex(data map[string]any, index int, segments []string) (any, error)` | walkIndex returns the indexed value reached by the test path. |
| [`walkAll`](../src/something/utils_unit_test.go#L31) | function | 31-33 | `func walkAll(data map[string]any, segments []string) ([]any, error)` | walkAll returns every value reached by the test path. |
| [`TestMatchIterationKeysPlain`](../src/something/utils_unit_test.go#L36) | test | 36-42 | `func TestMatchIterationKeysPlain(t *testing.T)` | TestMatchIterationKeysPlain verifies match iteration keys plain. |
| [`TestMatchIterationKeysPlainMissing`](../src/something/utils_unit_test.go#L45) | test | 45-51 | `func TestMatchIterationKeysPlainMissing(t *testing.T)` | TestMatchIterationKeysPlainMissing verifies match iteration keys plain missing. |
| [`TestMatchIterationKeysWildcard`](../src/something/utils_unit_test.go#L54) | test | 54-66 | `func TestMatchIterationKeysWildcard(t *testing.T)` | TestMatchIterationKeysWildcard verifies match iteration keys wildcard. |
| [`TestMatchIterationKeysWildcardWithSuffix`](../src/something/utils_unit_test.go#L69) | test | 69-78 | `func TestMatchIterationKeysWildcardWithSuffix(t *testing.T)` | TestMatchIterationKeysWildcardWithSuffix verifies match iteration keys wildcard with suffix. |
| [`TestMatchIterationKeysWildcardNoMatch`](../src/something/utils_unit_test.go#L81) | test | 81-87 | `func TestMatchIterationKeysWildcardNoMatch(t *testing.T)` | TestMatchIterationKeysWildcardNoMatch verifies match iteration keys wildcard no match. |
| [`TestWalkOnceSimple`](../src/something/utils_unit_test.go#L90) | test | 90-99 | `func TestWalkOnceSimple(t *testing.T)` | TestWalkOnceSimple verifies walk once simple. |
| [`TestWalkOnceNested`](../src/something/utils_unit_test.go#L102) | test | 102-111 | `func TestWalkOnceNested(t *testing.T)` | TestWalkOnceNested verifies walk once nested. |
| [`TestWalkOnceIteration`](../src/something/utils_unit_test.go#L114) | test | 114-124 | `func TestWalkOnceIteration(t *testing.T)` | TestWalkOnceIteration verifies walk once iteration. |
| [`TestWalkOnceIterationWithSuffix`](../src/something/utils_unit_test.go#L127) | test | 127-136 | `func TestWalkOnceIterationWithSuffix(t *testing.T)` | TestWalkOnceIterationWithSuffix verifies walk once iteration with suffix. |
| [`TestWalkOncePathNotValid`](../src/something/utils_unit_test.go#L139) | test | 139-148 | `func TestWalkOncePathNotValid(t *testing.T)` | TestWalkOncePathNotValid verifies walk once path not valid. |
| [`TestWalkOncePathCannotBeReached`](../src/something/utils_unit_test.go#L151) | test | 151-160 | `func TestWalkOncePathCannotBeReached(t *testing.T)` | TestWalkOncePathCannotBeReached verifies walk once path cannot be reached. |
| [`TestWalkOnceMultipleMatches`](../src/something/utils_unit_test.go#L163) | test | 163-172 | `func TestWalkOnceMultipleMatches(t *testing.T)` | TestWalkOnceMultipleMatches verifies walk once multiple matches. |
| [`TestWalkIndexSimple`](../src/something/utils_unit_test.go#L175) | test | 175-184 | `func TestWalkIndexSimple(t *testing.T)` | TestWalkIndexSimple verifies walk index simple. |
| [`TestWalkIndexIteration`](../src/something/utils_unit_test.go#L187) | test | 187-196 | `func TestWalkIndexIteration(t *testing.T)` | TestWalkIndexIteration verifies walk index iteration. |
| [`TestWalkIndexIterationLast`](../src/something/utils_unit_test.go#L199) | test | 199-208 | `func TestWalkIndexIterationLast(t *testing.T)` | TestWalkIndexIterationLast verifies walk index iteration last. |
| [`TestWalkIndexOutOfBounds`](../src/something/utils_unit_test.go#L211) | test | 211-220 | `func TestWalkIndexOutOfBounds(t *testing.T)` | TestWalkIndexOutOfBounds verifies walk index out of bounds. |
| [`TestWalkIndexNegative`](../src/something/utils_unit_test.go#L223) | test | 223-232 | `func TestWalkIndexNegative(t *testing.T)` | TestWalkIndexNegative verifies walk index negative. |
| [`TestWalkIndexPathNotValid`](../src/something/utils_unit_test.go#L235) | test | 235-244 | `func TestWalkIndexPathNotValid(t *testing.T)` | TestWalkIndexPathNotValid verifies walk index path not valid. |
| [`TestWalkAllSimple`](../src/something/utils_unit_test.go#L247) | test | 247-256 | `func TestWalkAllSimple(t *testing.T)` | TestWalkAllSimple verifies walk all simple. |
| [`TestWalkAllIteration`](../src/something/utils_unit_test.go#L259) | test | 259-271 | `func TestWalkAllIteration(t *testing.T)` | TestWalkAllIteration verifies walk all iteration. |
| [`TestWalkAllIterationWithSuffix`](../src/something/utils_unit_test.go#L274) | test | 274-283 | `func TestWalkAllIterationWithSuffix(t *testing.T)` | TestWalkAllIterationWithSuffix verifies walk all iteration with suffix. |
| [`TestWalkAllEmptyResult`](../src/something/utils_unit_test.go#L286) | test | 286-295 | `func TestWalkAllEmptyResult(t *testing.T)` | TestWalkAllEmptyResult verifies walk all empty result. |
| [`TestWalkAllPathNotValid`](../src/something/utils_unit_test.go#L298) | test | 298-307 | `func TestWalkAllPathNotValid(t *testing.T)` | TestWalkAllPathNotValid verifies walk all path not valid. |
| [`TestGetStringOnce`](../src/something/utils_unit_test.go#L310) | test | 310-319 | `func TestGetStringOnce(t *testing.T)` | TestGetStringOnce verifies get string once. |
| [`TestGetStringOnceTypeError`](../src/something/utils_unit_test.go#L322) | test | 322-331 | `func TestGetStringOnceTypeError(t *testing.T)` | TestGetStringOnceTypeError verifies get string once type error. |
| [`TestGetIntegerOnce`](../src/something/utils_unit_test.go#L334) | test | 334-343 | `func TestGetIntegerOnce(t *testing.T)` | TestGetIntegerOnce verifies get integer once. |
| [`TestGetFloatOnce`](../src/something/utils_unit_test.go#L346) | test | 346-355 | `func TestGetFloatOnce(t *testing.T)` | TestGetFloatOnce verifies get float once. |
| [`TestGetFloatOnceFromInt`](../src/something/utils_unit_test.go#L358) | test | 358-367 | `func TestGetFloatOnceFromInt(t *testing.T)` | TestGetFloatOnceFromInt verifies get float once from int. |
| [`TestGetBoolOnce`](../src/something/utils_unit_test.go#L370) | test | 370-379 | `func TestGetBoolOnce(t *testing.T)` | TestGetBoolOnce verifies get bool once. |
| [`TestGetTimestampOnce`](../src/something/utils_unit_test.go#L382) | test | 382-391 | `func TestGetTimestampOnce(t *testing.T)` | TestGetTimestampOnce verifies get timestamp once. |
| [`TestGetListOnce`](../src/something/utils_unit_test.go#L394) | test | 394-403 | `func TestGetListOnce(t *testing.T)` | TestGetListOnce verifies get list once. |
| [`TestGetMappingOnce`](../src/something/utils_unit_test.go#L406) | test | 406-415 | `func TestGetMappingOnce(t *testing.T)` | TestGetMappingOnce verifies get mapping once. |
| [`TestGetStructOnce`](../src/something/utils_unit_test.go#L418) | test | 418-427 | `func TestGetStructOnce(t *testing.T)` | TestGetStructOnce verifies get struct once. |
| [`TestGetScopeOnce`](../src/something/utils_unit_test.go#L430) | test | 430-441 | `func TestGetScopeOnce(t *testing.T)` | TestGetScopeOnce verifies get scope once. |
| [`TestGetEnumOnce`](../src/something/utils_unit_test.go#L444) | test | 444-453 | `func TestGetEnumOnce(t *testing.T)` | TestGetEnumOnce verifies get enum once. |
| [`TestGetEnumOnceTypeError`](../src/something/utils_unit_test.go#L456) | test | 456-462 | `func TestGetEnumOnceTypeError(t *testing.T)` | TestGetEnumOnceTypeError verifies get enum once type error. |
| [`TestGetStringIndex`](../src/something/utils_unit_test.go#L465) | test | 465-474 | `func TestGetStringIndex(t *testing.T)` | TestGetStringIndex verifies get string index. |
| [`TestGetIntegerIndex`](../src/something/utils_unit_test.go#L477) | test | 477-486 | `func TestGetIntegerIndex(t *testing.T)` | TestGetIntegerIndex verifies get integer index. |
| [`TestGetFloatIndex`](../src/something/utils_unit_test.go#L489) | test | 489-501 | `func TestGetFloatIndex(t *testing.T)` | TestGetFloatIndex verifies get float index. |
| [`TestGetBoolIndex`](../src/something/utils_unit_test.go#L504) | test | 504-515 | `func TestGetBoolIndex(t *testing.T)` | TestGetBoolIndex verifies get bool index. |
| [`TestGetTimestampIndex`](../src/something/utils_unit_test.go#L518) | test | 518-529 | `func TestGetTimestampIndex(t *testing.T)` | TestGetTimestampIndex verifies get timestamp index. |
| [`TestGetListIndex`](../src/something/utils_unit_test.go#L532) | test | 532-543 | `func TestGetListIndex(t *testing.T)` | TestGetListIndex verifies get list index. |
| [`TestGetMappingIndex`](../src/something/utils_unit_test.go#L546) | test | 546-557 | `func TestGetMappingIndex(t *testing.T)` | TestGetMappingIndex verifies get mapping index. |
| [`TestGetStructIndex`](../src/something/utils_unit_test.go#L560) | test | 560-571 | `func TestGetStructIndex(t *testing.T)` | TestGetStructIndex verifies get struct index. |
| [`TestGetScopeIndex`](../src/something/utils_unit_test.go#L574) | test | 574-585 | `func TestGetScopeIndex(t *testing.T)` | TestGetScopeIndex verifies get scope index. |
| [`TestGetEnumIndex`](../src/something/utils_unit_test.go#L588) | test | 588-599 | `func TestGetEnumIndex(t *testing.T)` | TestGetEnumIndex verifies get enum index. |
| [`TestGetStringIndexOutOfBounds`](../src/something/utils_unit_test.go#L602) | test | 602-611 | `func TestGetStringIndexOutOfBounds(t *testing.T)` | TestGetStringIndexOutOfBounds verifies get string index out of bounds. |
| [`TestGetStringAll`](../src/something/utils_unit_test.go#L614) | test | 614-626 | `func TestGetStringAll(t *testing.T)` | TestGetStringAll verifies get string all. |
| [`TestGetIntegerAll`](../src/something/utils_unit_test.go#L629) | test | 629-641 | `func TestGetIntegerAll(t *testing.T)` | TestGetIntegerAll verifies get integer all. |
| [`TestGetFloatAll`](../src/something/utils_unit_test.go#L644) | test | 644-656 | `func TestGetFloatAll(t *testing.T)` | TestGetFloatAll verifies get float all. |
| [`TestGetBoolAll`](../src/something/utils_unit_test.go#L659) | test | 659-674 | `func TestGetBoolAll(t *testing.T)` | TestGetBoolAll verifies get bool all. |
| [`TestGetTimestampAll`](../src/something/utils_unit_test.go#L677) | test | 677-689 | `func TestGetTimestampAll(t *testing.T)` | TestGetTimestampAll verifies get timestamp all. |
| [`TestGetListAll`](../src/something/utils_unit_test.go#L692) | test | 692-704 | `func TestGetListAll(t *testing.T)` | TestGetListAll verifies get list all. |
| [`TestGetMappingAll`](../src/something/utils_unit_test.go#L707) | test | 707-719 | `func TestGetMappingAll(t *testing.T)` | TestGetMappingAll verifies get mapping all. |
| [`TestGetStructAll`](../src/something/utils_unit_test.go#L722) | test | 722-734 | `func TestGetStructAll(t *testing.T)` | TestGetStructAll verifies get struct all. |
| [`TestGetScopeAll`](../src/something/utils_unit_test.go#L737) | test | 737-749 | `func TestGetScopeAll(t *testing.T)` | TestGetScopeAll verifies get scope all. |
| [`TestGetEnumAll`](../src/something/utils_unit_test.go#L752) | test | 752-767 | `func TestGetEnumAll(t *testing.T)` | TestGetEnumAll verifies get enum all. |
| [`TestGetStringAllEmpty`](../src/something/utils_unit_test.go#L770) | test | 770-779 | `func TestGetStringAllEmpty(t *testing.T)` | TestGetStringAllEmpty verifies get string all empty. |
| [`TestPathNotValidError`](../src/something/utils_unit_test.go#L782) | test | 782-788 | `func TestPathNotValidError(t *testing.T)` | TestPathNotValidError verifies path not valid error. |
| [`TestPathCannotBeReachedError`](../src/something/utils_unit_test.go#L791) | test | 791-797 | `func TestPathCannotBeReachedError(t *testing.T)` | TestPathCannotBeReachedError verifies path cannot be reached error. |
| [`TestOutOfBoundsError`](../src/something/utils_unit_test.go#L800) | test | 800-806 | `func TestOutOfBoundsError(t *testing.T)` | TestOutOfBoundsError verifies out of bounds error. |
...
| [`readPolicy`](../src/tools/coveragecheck/main.go#L116) | function | 116-201 | `func readPolicy(policyPath string) (policy, error)` | readPolicy reads policy from the supplied source. |
| [`readProfile`](../src/tools/coveragecheck/main.go#L204) | function | 204-273 | `func readProfile(input io.Reader) (map[string]coverage, map[string]coverage, string, error)` | readProfile reads profile from the supplied source. |
| [`writeCoverageTable`](../src/tools/coveragecheck/main.go#L276) | function | 276-298 | `func writeCoverageTable(output io.Writer, title string, minimums map[string]float64, actual map[string]coverage, failures *[]string)` | writeCoverageTable writes coverage table to the supplied destination. |
...
| [`TestReadPolicyRejectsNonStringExcludedPackage`](../src/tools/coveragecheck/main_functional_test.go#L57) | test | 57-75 | `func TestReadPolicyRejectsNonStringExcludedPackage(t *testing.T)` | TestReadPolicyRejectsNonStringExcludedPackage verifies malformed policy lists return a descriptive error instead of panicking during conversion. |
...
| [`TestCoveragePercentTreatsEmptyScopeAsComplete`](../src/tools/coveragecheck/main_unit_test.go#L49) | test | 49-53 | `func TestCoveragePercentTreatsEmptyScopeAsComplete(t *testing.T)` | TestCoveragePercentTreatsEmptyScopeAsComplete verifies the documented empty scope policy is represented by the returned percentage. |
...
| Symbol | Kind | Lines | Declaration, inputs, and outputs | Source description |
|---|---|---:|---|---|
| [`options`](../src/tools/prepare-osf/main.go#L30) | struct | 30 | `type options struct{ DB, Config, Out string }` | options contains source and new-output paths accepted by the export command. |
| [`hashMapping`](../src/tools/prepare-osf/main.go#L33) | struct | 33-36 | ``type hashMapping struct { Original string `json:"original"` Sanitized string `json:"sanitized"` }`` | hashMapping records a reviewer-bearing raw configuration hash and its sanitized replacement. |
| [`exportManifest`](../src/tools/prepare-osf/main.go#L39) | struct | 39-46 | ``type exportManifest struct { CreatedAt string `json:"created_at"` SourceSchemaVersions map[string]string `json:"source_schema_versions"` SanitizedSchemaVersions map[string]string `json:"sanitized_schema_versions"` Files map[string]string `json:"files"` ConfigurationMappings []hashMapping `json:"configuration_hash_mappings"` BrowserDraftDisclaimer string `json:"browser_draft_disclaimer"` }`` | exportManifest records schema identity, copied file hashes, sanitization mappings, and limitations. |
| [`main`](../src/tools/prepare-osf/main.go#L49) | function | 49-59 | `func main()` | main parses the copy-only export command and reports failures without publishing partial output. |
| [`prepare`](../src/tools/prepare-osf/main.go#L62) | function | 62-213 | `func prepare(ctx context.Context, input options) error` | prepare validates source ownership, sanitizes temporary snapshots, and atomically publishes a new bundle. |
| [`sanitizeMetadata`](../src/tools/prepare-osf/main.go#L216) | function | 216-334 | `func sanitizeMetadata(ctx context.Context, db *sql.DB) ([]hashMapping, string, error)` | sanitizeMetadata redacts reviewer identity and rewires copied content-addressed configuration artifacts. |
| [`validateArtifactBlobs`](../src/tools/prepare-osf/main.go#L337) | function | 337-357 | `func validateArtifactBlobs(ctx context.Context, tx *sql.Tx) error` | validateArtifactBlobs recomputes every copied artifact size and SHA-256 identity. |
| [`sanitizeReviewerAssignments`](../src/tools/prepare-osf/main.go#L360) | function | 360-425 | `func sanitizeReviewerAssignments(source []byte) ([]byte, bool, error)` | sanitizeReviewerAssignments replaces only provable inline reviewer values and fails closed otherwise. |
| [`validateReviewerLiterals`](../src/tools/prepare-osf/main.go#L428) | function | 428-463 | `func validateReviewerLiterals(text string) error` | validateReviewerLiterals accepts only direct identity strings whose complete source is inside the redacted object. |
| [`tokenKind`](../src/tools/prepare-osf/main.go#L466) | type | 466 | `type tokenKind int` | tokenKind distinguishes source identifiers from skipped strings, comments, multiline data, and punctuation. |
| [`nextCodeToken`](../src/tools/prepare-osf/main.go#L475) | function | 475-491 | `func nextCodeToken(text string, start int) (int, tokenKind)` | nextCodeToken returns the next executable SOMETHING source position without inspecting data tokens. |
| [`somethingMultilineEnd`](../src/tools/prepare-osf/main.go#L494) | function | 494-528 | `func somethingMultilineEnd(text string, start int) (int, bool)` | somethingMultilineEnd skips one SOMETHING multiline literal including its named closing delimiter. |
| [`skipTrivia`](../src/tools/prepare-osf/main.go#L531) | function | 531-547 | `func skipTrivia(text string, start int) int` | skipTrivia advances over whitespace and line comments. |
| [`matchingBrace`](../src/tools/prepare-osf/main.go#L550) | function | 550-580 | `func matchingBrace(text string, open int) (int, error)` | matchingBrace finds an inline reviewer value boundary while ignoring quoted data and comments. |
| [`quotedEnd`](../src/tools/prepare-osf/main.go#L583) | function | 583-595 | `func quotedEnd(text string, start int) (int, error)` | quotedEnd returns the position after an escaped single or double quoted token. |
| [`hasWordAt`](../src/tools/prepare-osf/main.go#L598) | function | 598-604 | `func hasWordAt(text string, start int, word string) bool` | hasWordAt matches one identifier without accepting a longer identifier prefix. |
| [`isIdentifierByte`](../src/tools/prepare-osf/main.go#L607) | function | 607-609 | `func isIdentifierByte(value byte) bool` | isIdentifierByte reports whether a byte may continue the identifiers used by the sanitizer. |
| [`copyAndSanitizeConfiguration`](../src/tools/prepare-osf/main.go#L612) | function | 612-671 | `func copyAndSanitizeConfiguration(source, destinationRoot string) (string, error)` | copyAndSanitizeConfiguration copies one include tree without permitting lexical or symbolic-link escape. |
| [`localIncludes`](../src/tools/prepare-osf/main.go#L674) | function | 674-701 | `func localIncludes(source string) ([]string, error)` | localIncludes extracts literal local include paths outside comments, strings, and multiline data. |
| [`snapshotSQLite`](../src/tools/prepare-osf/main.go#L704) | function | 704-710 | `func snapshotSQLite(ctx context.Context, source *sql.DB, destination string) error` | snapshotSQLite uses VACUUM INTO to capture a WAL-consistent source without overwriting a destination. |
| [`schemaVersion`](../src/tools/prepare-osf/main.go#L713) | function | 713-720 | `func schemaVersion(ctx context.Context, path string) (string, error)` | schemaVersion opens one SQLite file read-only and returns its latest recorded migration filename. |
| [`schemaVersionDB`](../src/tools/prepare-osf/main.go#L723) | function | 723-729 | `func schemaVersionDB(ctx context.Context, db *sql.DB) (string, error)` | schemaVersionDB returns the latest recorded migration filename from an open SQLite connection. |
| [`exportedFileHashes`](../src/tools/prepare-osf/main.go#L732) | function | 732-752 | `func exportedFileHashes(root string) (map[string]string, error)` | exportedFileHashes hashes every regular copied file and rejects unsupported file types. |
| [`safeCompanionPath`](../src/tools/prepare-osf/main.go#L756) | function | 756-768 | `func safeCompanionPath(metadataPath, relative string) (string, error)` | safeCompanionPath resolves a clean bundle-relative PDF binding and rejects a resolved target that escapes the metadata bundle through a symbolic link. |
| [`existingFile`](../src/tools/prepare-osf/main.go#L771) | function | 771-788 | `func existingFile(path string) (string, error)` | existingFile returns a resolved absolute path only for an existing regular file. |
| [`samePath`](../src/tools/prepare-osf/main.go#L791) | function | 791-798 | `func samePath(left, right string) bool` | samePath detects aliases using filesystem identity when both targets exist. |
| [`sqliteReadURI`](../src/tools/prepare-osf/main.go#L801) | function | 801-806 | `func sqliteReadURI(path string) string` | sqliteReadURI returns an existing-only read URI with the project busy timeout. |
| [`sqliteWritableURI`](../src/tools/prepare-osf/main.go#L809) | function | 809-813 | `func sqliteWritableURI(path string) string` | sqliteWritableURI returns a writable SQLite URI with the project pragmas. |
| [`fileHash`](../src/tools/prepare-osf/main.go#L816) | function | 816-827 | `func fileHash(path string) (string, error)` | fileHash streams one file into a lowercase SHA-256 digest. |
| [`mustRelative`](../src/tools/prepare-osf/main.go#L830) | function | 830-836 | `func mustRelative(root, path string) string` | mustRelative returns the previously validated path relative to its export root. |

### [`src/tools/prepare-osf/main_integration_test.go`](../src/tools/prepare-osf/main_integration_test.go)

| Symbol | Kind | Lines | Declaration, inputs, and outputs | Source description |
|---|---|---:|---|---|
| [`TestPrepareCopiesAndSanitizesWithoutMutatingSources`](../src/tools/prepare-osf/main_integration_test.go#L25) | test | 25-120 | `func TestPrepareCopiesAndSanitizesWithoutMutatingSources(t *testing.T)` | TestPrepareCopiesAndSanitizesWithoutMutatingSources verifies the atomic copy-only export boundary. |
| [`mustExec`](../src/tools/prepare-osf/main_integration_test.go#L123) | function | 123-130 | `func mustExec(t *testing.T, db *sql.DB, query string, args ...any) sql.Result` | mustExec runs one fixture statement and fails the current test on error. |
| [`TestSanitizeReviewerAssignments`](../src/tools/prepare-osf/main_integration_test.go#L133) | test | 133-145 | `func TestSanitizeReviewerAssignments(t *testing.T)` | TestSanitizeReviewerAssignments verifies inline redaction and fail-closed references. |
| [`TestSanitizeReviewerAfterCommentedMultiline`](../src/tools/prepare-osf/main_integration_test.go#L148) | test | 148-157 | `func TestSanitizeReviewerAfterCommentedMultiline(t *testing.T)` | TestSanitizeReviewerAfterCommentedMultiline verifies redaction continues after a multiline block whose closing tag has a trailing comment. |
| [`TestConfigurationCopyRejectsSymlinkEscape`](../src/tools/prepare-osf/main_integration_test.go#L160) | test | 160-177 | `func TestConfigurationCopyRejectsSymlinkEscape(t *testing.T)` | TestConfigurationCopyRejectsSymlinkEscape verifies that configuration includes cannot follow links outside the copied root. |
| [`TestSafeCompanionPathRejectsSymlinkEscape`](../src/tools/prepare-osf/main_integration_test.go#L181) | test | 181-197 | `func TestSafeCompanionPathRejectsSymlinkEscape(t *testing.T)` | TestSafeCompanionPathRejectsSymlinkEscape verifies a bundle-local link cannot select a companion database outside the selected metadata directory. |
| [`TestPrepareRealPipelineReviewerEvidence`](../src/tools/prepare-osf/main_integration_test.go#L200) | test | 200-258 | `func TestPrepareRealPipelineReviewerEvidence(t *testing.T)` | TestPrepareRealPipelineReviewerEvidence verifies preflight references, hashes, and exported file bytes are sanitized together. |
| [`TestSanitizeRejectsIndirectFieldExpressions`](../src/tools/prepare-osf/main_integration_test.go#L261) | test | 261-268 | `func TestSanitizeRejectsIndirectFieldExpressions(t *testing.T)` | TestSanitizeRejectsIndirectFieldExpressions verifies references and interpolation fail closed without publishing identity declarations. |
...
### [`src/workspace/backend_findings_integration_test.go`](../src/workspace/backend_findings_integration_test.go)

| Symbol | Kind | Lines | Declaration, inputs, and outputs | Source description |
|---|---|---:|---|---|
| [`TestReviewDOIRequestPath`](../src/workspace/backend_findings_integration_test.go#L29) | test | 29-57 | `func TestReviewDOIRequestPath(t *testing.T)` | TestReviewDOIRequestPath checks DOI punctuation remains identifier data in provider requests. |
| [`TestReviewRetryExhaustion`](../src/workspace/backend_findings_integration_test.go#L60) | test | 60-82 | `func TestReviewRetryExhaustion(t *testing.T)` | TestReviewRetryExhaustion checks an exhausted request does not enter another backoff. |
| [`reviewRoot`](../src/workspace/backend_findings_integration_test.go#L85) | function | 85-101 | `func reviewRoot(t *testing.T) string` | reviewRoot locates the repository from either the Makefile or Go test working directory. |
| [`reviewTransport`](../src/workspace/backend_findings_integration_test.go#L104) | type | 104 | `type reviewTransport func(*http.Request) (*http.Response, error)` | reviewTransport intercepts provider traffic without opening a socket or contacting live services. |
| [`(reviewTransport).RoundTrip`](../src/workspace/backend_findings_integration_test.go#L107) | method | 107-109 | `func (reviewTransport).RoundTrip(request *http.Request) (*http.Response, error)` | RoundTrip supplies a synthetic response to the real rate-limited provider client. |
| [`reviewProvider`](../src/workspace/backend_findings_integration_test.go#L112) | function | 112-124 | `func reviewProvider(t *testing.T, run *workspace.Run, body string)` | reviewProvider configures a bounded synthetic Crossref client and records its manifest fields. |
| [`TestReviewProviderFieldSelection`](../src/workspace/backend_findings_integration_test.go#L127) | test | 127-141 | `func TestReviewProviderFieldSelection(t *testing.T)` | TestReviewProviderFieldSelection checks that requesting only titles does not overwrite publishers. |
| [`TestReviewUnstructuredReferencePreserved`](../src/workspace/backend_findings_integration_test.go#L144) | test | 144-163 | `func TestReviewUnstructuredReferencePreserved(t *testing.T)` | TestReviewUnstructuredReferencePreserved checks provider reference text survives decoding and persistence. |
| [`TestReviewOversizedLinkAllocation`](../src/workspace/backend_findings_integration_test.go#L166) | test | 166-179 | `func TestReviewOversizedLinkAllocation(t *testing.T)` | TestReviewOversizedLinkAllocation checks oversized link rejection occurs before excessive copying. |
| [`TestReviewRawSourcePreserved`](../src/workspace/backend_findings_integration_test.go#L182) | test | 182-204 | `func TestReviewRawSourcePreserved(t *testing.T)` | TestReviewRawSourcePreserved checks raw source evidence is captured before text sanitization. |
| [`reviewDB`](../src/workspace/backend_findings_integration_test.go#L207) | function | 207-218 | `func reviewDB(t *testing.T) (*database.Database, string)` | reviewDB creates a synthetic database using production migrations in an isolated test directory. |
| [`reviewRun`](../src/workspace/backend_findings_integration_test.go#L221) | function | 221-234 | `func reviewRun(t *testing.T, dir, prefix string) *workspace.Run` | reviewRun creates a synthetic source and minimal supported pipeline configuration. |
| [`TestReviewStageInputPreserved`](../src/workspace/backend_findings_integration_test.go#L237) | test | 237-277 | `func TestReviewStageInputPreserved(t *testing.T)` | TestReviewStageInputPreserved reproduces F-01 against a real pipeline run. |
| [`TestReviewNoteSingleColumnTable`](../src/workspace/backend_findings_integration_test.go#L280) | test | 280-288 | `func TestReviewNoteSingleColumnTable(t *testing.T)` | TestReviewNoteSingleColumnTable checks that a valid single-column table is accepted. |
| [`TestReviewNotePipeParagraphNotTable`](../src/workspace/backend_findings_integration_test.go#L291) | test | 291-299 | `func TestReviewNotePipeParagraphNotTable(t *testing.T)` | TestReviewNotePipeParagraphNotTable checks that prose containing pipes is not rejected as a table. |
| [`TestReviewPriorRunCacheStable`](../src/workspace/backend_findings_integration_test.go#L302) | test | 302-344 | `func TestReviewPriorRunCacheStable(t *testing.T)` | TestReviewPriorRunCacheStable reproduces F-02 with a private snapshot and a global refresh. |
| [`TestReviewStructuredDOIPreserved`](../src/workspace/backend_findings_integration_test.go#L347) | test | 347-357 | `func TestReviewStructuredDOIPreserved(t *testing.T)` | TestReviewStructuredDOIPreserved checks punctuation in an explicit DOI field. |
| [`TestReviewCSVByteOrderMark`](../src/workspace/backend_findings_integration_test.go#L360) | test | 360-373 | `func TestReviewCSVByteOrderMark(t *testing.T)` | TestReviewCSVByteOrderMark checks normal UTF-8 BOM handling in the first header. |
| [`TestReviewOpenAlexPublisherIsNotJournal`](../src/workspace/backend_findings_integration_test.go#L376) | test | 376-382 | `func TestReviewOpenAlexPublisherIsNotJournal(t *testing.T)` | TestReviewOpenAlexPublisherIsNotJournal checks the distinct publisher and journal fields. |
| [`TestReviewInterruptedAttemptRecoverable`](../src/workspace/backend_findings_integration_test.go#L385) | test | 385-412 | `func TestReviewInterruptedAttemptRecoverable(t *testing.T)` | TestReviewInterruptedAttemptRecoverable checks whether fresh can recover an abandoned attempt. |
| [`TestReviewCancellationAndActiveRecovery`](../src/workspace/backend_findings_integration_test.go#L415) | test | 415-470 | `func TestReviewCancellationAndActiveRecovery(t *testing.T)` | TestReviewCancellationAndActiveRecovery verifies active writers block recovery and cancellation leaves an audited retryable attempt. |
| [`TestReviewMissingCSVHeaderFails`](../src/workspace/backend_findings_integration_test.go#L473) | test | 473-487 | `func TestReviewMissingCSVHeaderFails(t *testing.T)` | TestReviewMissingCSVHeaderFails verifies a missing required mapped column cannot complete an empty corpus. |
| [`TestReviewProviderIdentityMismatch`](../src/workspace/backend_findings_integration_test.go#L490) | test | 490-501 | `func TestReviewProviderIdentityMismatch(t *testing.T)` | TestReviewProviderIdentityMismatch verifies another work's response is rejected before cache publication. |
| [`TestReviewMetadataAndIdentityInputs`](../src/workspace/backend_findings_integration_test.go#L504) | test | 504-550 | `func TestReviewMetadataAndIdentityInputs(t *testing.T)` | TestReviewMetadataAndIdentityInputs verifies independent immutable inputs for both enrichment stages. |
...
| [`validateCachePayload`](../src/workspace/cache.go#L301) | function | 301-309 | `func validateCachePayload(request cacheRequest, body []byte) error` | validateCachePayload checks both the transport envelope and any provider-supplied work identity before caching or replay. |
...
| [`TestWorkspaceCacheStaleNegativeRelooksUp`](../src/workspace/cache_integration_test.go#L147) | test | 147-179 | `func TestWorkspaceCacheStaleNegativeRelooksUp(t *testing.T)` | TestWorkspaceCacheStaleNegativeRelooksUp verifies workspace cache stale negative relooks up. |
| [`TestWorkspaceCacheNetworkFallback`](../src/workspace/cache_integration_test.go#L182) | test | 182-214 | `func TestWorkspaceCacheNetworkFallback(t *testing.T)` | TestWorkspaceCacheNetworkFallback verifies workspace cache network fallback. |
| [`TestWorkspaceCacheRejectsMalformedSuccessAndRecovers`](../src/workspace/cache_integration_test.go#L217) | test | 217-246 | `func TestWorkspaceCacheRejectsMalformedSuccessAndRecovers(t *testing.T)` | TestWorkspaceCacheRejectsMalformedSuccessAndRecovers verifies workspace cache rejects malformed success and recovers. |
| [`TestWorkspaceCacheSkipsStoredMalformedPayload`](../src/workspace/cache_integration_test.go#L249) | test | 249-280 | `func TestWorkspaceCacheSkipsStoredMalformedPayload(t *testing.T)` | TestWorkspaceCacheSkipsStoredMalformedPayload verifies workspace cache skips stored malformed payload. |
| [`TestWorkspaceCacheORCIDNameFailureAndEmptyMatchPolicies`](../src/workspace/cache_integration_test.go#L283) | test | 283-316 | `func TestWorkspaceCacheORCIDNameFailureAndEmptyMatchPolicies(t *testing.T)` | TestWorkspaceCacheORCIDNameFailureAndEmptyMatchPolicies verifies workspace cache orcid name failure and empty match policies. |
| [`TestWorkspaceMetricUnavailableForOlderRun`](../src/workspace/cache_integration_test.go#L319) | test | 319-326 | `func TestWorkspaceMetricUnavailableForOlderRun(t *testing.T)` | TestWorkspaceMetricUnavailableForOlderRun verifies workspace metric unavailable for older run. |
| [`TestWorkspaceCrossrefCacheUsesSQLitePolicy`](../src/workspace/cache_integration_test.go#L329) | test | 329-352 | `func TestWorkspaceCrossrefCacheUsesSQLitePolicy(t *testing.T)` | TestWorkspaceCrossrefCacheUsesSQLitePolicy verifies workspace crossref cache uses sq lite policy. |
| [`TestWorkspaceOpenAlexReferenceCacheUsesSQLitePolicy`](../src/workspace/cache_integration_test.go#L355) | test | 355-384 | `func TestWorkspaceOpenAlexReferenceCacheUsesSQLitePolicy(t *testing.T)` | TestWorkspaceOpenAlexReferenceCacheUsesSQLitePolicy verifies the OpenAlex reference cache uses SQLite policy. |
| [`openWorkspaceCacheTestForDB`](../src/workspace/cache_integration_test.go#L387) | function | 387-394 | `func openWorkspaceCacheTestForDB(t *testing.T, db *database.Database, policy manifest.CachePolicy) (*database.Database, *workspaceCache, int64)` | openWorkspaceCacheTestForDB supports the package test suite's open workspace cache test for db setup or assertions. |
| [`TestWorkspaceGlobalRefreshPreservesConsumedResponses`](../src/workspace/cache_integration_test.go#L397) | test | 397-427 | `func TestWorkspaceGlobalRefreshPreservesConsumedResponses(t *testing.T)` | TestWorkspaceGlobalRefreshPreservesConsumedResponses verifies global-only hits and negative responses remain stable in named-run replay. |

### [`src/workspace/config.go`](../src/workspace/config.go)

| Symbol | Kind | Lines | Declaration, inputs, and outputs | Source description |
|---|---|---:|---|---|
| [`Config`](../src/workspace/config.go#L25) | struct | 25-28 | `type Config struct { OriginalBytes []byte Runs []*Run }` | Config is one immutable evaluation of a workspace configuration file. OriginalBytes are retained so a run can persist exactly what was evaluated. |
| [`Run`](../src/workspace/config.go#L31) | struct | 31-35 | `type Run struct { Manifest *manifest.ResolvedManifest Enrichment *enrich.Config Reviewer Reviewer }` | Run is one declared workspace iteration. |
| [`Reviewer`](../src/workspace/config.go#L38) | struct | 38-41 | ``type Reviewer struct { Username string `json:"username"` Email string `json:"email"` }`` | Reviewer is the optional identity captured with each newly created pipeline run. |
| [`Selector`](../src/workspace/config.go#L45) | function | 45-47 | `func Selector(searchID, revision string) string` | Selector identifies one workspace iteration by its stable search ID and revision label. Its CLI form is search_id@search_revision. |
| [`Load`](../src/workspace/config.go#L51) | function | 51-95 | `func Load(path string) (*Config, error)` | Load reads a workspace configuration exactly once, evaluates those bytes, and converts every workspace iteration to typed run configuration. |
| [`(*Config).Select`](../src/workspace/config.go#L99) | method | 99-135 | `func (*Config).Select(selectors []string) ([]*Run, error)` | Select returns all runs when selectors is empty. Otherwise it returns the requested iterations in declaration order and rejects unknown selectors. |
| [`parseRun`](../src/workspace/config.go#L138) | function | 138-202 | `func parseRun(entry map[string]any, configDir string) (*Run, error)` | parseRun parses run from the supplied input. |
| [`parseReviewer`](../src/workspace/config.go#L205) | function | 205-232 | `func parseReviewer(workspace map[string]any) (Reviewer, error)` | parseReviewer normalizes the optional reviewer setup without adding it to execution manifests. |
| [`parseCachePolicy`](../src/workspace/config.go#L239) | function | 239-309 | `func parseCachePolicy(entry map[string]any, reusePolicy string) (manifest.CachePolicy, error)` | parseCachePolicy parses cache policy from the supplied input. |
| [`parseSources`](../src/workspace/config.go#L312) | function | 312-389 | `func parseSources(entry map[string]any, configDir string) ([]manifest.SourceManifest, error)` | parseSources parses sources from the supplied input. |
| [`parseProviders`](../src/workspace/config.go#L392) | function | 392-456 | `func parseProviders(entry map[string]any) ([]manifest.EnrichmentProvider, *enrich.Config, error)` | parseProviders parses providers from the supplied input. |
| [`requiredPath`](../src/workspace/config.go#L459) | function | 459-468 | `func requiredPath(values map[string]any, name, configDir string) (string, error)` | requiredPath returns a required path resolved relative to the configuration directory. |
| [`stringList`](../src/workspace/config.go#L471) | function | 471-485 | `func stringList(values map[string]any, name string) ([]string, error)` | stringList reads a required list of non-empty strings. |
| [`stringMap`](../src/workspace/config.go#L488) | function | 488-490 | `func stringMap(values map[string]any, name string) (map[string]string, error)` | stringMap reads a required mapping whose keys and values are strings. |
| [`optionalStringMap`](../src/workspace/config.go#L493) | function | 493-495 | `func optionalStringMap(values map[string]any, name string) (map[string]string, error)` | optionalStringMap reads the optional string map value. |
| [`requiredStringMap`](../src/workspace/config.go#L498) | function | 498-516 | `func requiredStringMap(values map[string]any, name string, required bool) (map[string]string, error)` | requiredStringMap validates a required or optional mapping of non-empty strings. |
| [`validateCacheLayers`](../src/workspace/config.go#L519) | function | 519-543 | `func validateCacheLayers(layers []string, write bool) error` | validateCacheLayers validates cache-layer names, uniqueness, and read or write eligibility. |
| [`validateRunLayer`](../src/workspace/config.go#L546) | function | 546-557 | `func validateRunLayer(reads []string) error` | validateRunLayer checks that a resolved run:N layer has a valid run ID. |
| [`enumIntList`](../src/workspace/config.go#L561) | function | 561-578 | `func enumIntList(values map[string]any, name string, memberNames []string) ([]int, error)` | enumIntList reads a list of int enum ordinals from a map and validates they are within range of the provided member names list. |
| [`parseRawDataFilters`](../src/workspace/config.go#L583) | function | 583-613 | `func parseRawDataFilters(source map[string]any, name string) ([]manifest.RawDataFilter, error)` | parseRawDataFilters reads the "filters" field as a list of raw_data_filters structs, each containing a "filters" array of available_filters enum ordinals and a "count" integer. |
...
| [`TestLoadRejectsInvalidProviderRequestBounds`](../src/workspace/config_unit_test.go#L47) | test | 47-68 | `func TestLoadRejectsInvalidProviderRequestBounds(t *testing.T)` | TestLoadRejectsInvalidProviderRequestBounds verifies config loading rejects unsafe provider clients. |
| [`TestLoadNormalizesReviewerWithoutChangingManifestIdentity`](../src/workspace/config_unit_test.go#L71) | test | 71-100 | `func TestLoadNormalizesReviewerWithoutChangingManifestIdentity(t *testing.T)` | TestLoadNormalizesReviewerWithoutChangingManifestIdentity verifies optional attribution is trimmed and excluded from plan fingerprints. |
| [`TestProductionWorkspaceConfigLoads`](../src/workspace/config_unit_test.go#L103) | test | 103-145 | `func TestProductionWorkspaceConfigLoads(t *testing.T)` | TestProductionWorkspaceConfigLoads verifies production workspace config loads. |
| [`TestSelectUsesDeclaredOrderAndRejectsUnknown`](../src/workspace/config_unit_test.go#L148) | test | 148-163 | `func TestSelectUsesDeclaredOrderAndRejectsUnknown(t *testing.T)` | TestSelectUsesDeclaredOrderAndRejectsUnknown verifies select uses declared order and rejects unknown. |
| [`TestLoadRetainsEvaluatedBytesAfterConfigChanges`](../src/workspace/config_unit_test.go#L166) | test | 166-181 | `func TestLoadRetainsEvaluatedBytesAfterConfigChanges(t *testing.T)` | TestLoadRetainsEvaluatedBytesAfterConfigChanges verifies load retains evaluated bytes after config changes. |
| [`TestLoadRejectsUnsupportedWorkspacePolicies`](../src/workspace/config_unit_test.go#L184) | test | 184-203 | `func TestLoadRejectsUnsupportedWorkspacePolicies(t *testing.T)` | TestLoadRejectsUnsupportedWorkspacePolicies verifies load rejects unsupported workspace policies. |
| [`writeConfig`](../src/workspace/config_unit_test.go#L206) | function | 206-213 | `func writeConfig(t *testing.T, text string) string` | writeConfig supports the package test suite's write config setup or assertions. |
| [`testConfig`](../src/workspace/config_unit_test.go#L216) | function | 216-353 | `func testConfig() string` | testConfig supports the package test suite's test config setup or assertions. |
| [`TestConfigHelpers`](../src/workspace/config_unit_test.go#L356) | test | 356-420 | `func TestConfigHelpers(t *testing.T)` | TestConfigHelpers verifies config helpers. |
...
### [`src/workspace/pipeline_lock_other.go`](../src/workspace/pipeline_lock_other.go)

| Symbol | Kind | Lines | Declaration, inputs, and outputs | Source description |
|---|---|---:|---|---|
| [`lockPipelineDatabase`](../src/workspace/pipeline_lock_other.go#L11) | function | 11-13 | `func lockPipelineDatabase(path string, create bool) (*os.File, error)` | lockPipelineDatabase rejects execution when the platform cannot enforce the pipeline ownership lock. |

### [`src/workspace/pipeline_lock_unix.go`](../src/workspace/pipeline_lock_unix.go)

| Symbol | Kind | Lines | Declaration, inputs, and outputs | Source description |
|---|---|---:|---|---|
| [`lockPipelineDatabase`](../src/workspace/pipeline_lock_unix.go#L13) | function | 13-30 | `func lockPipelineDatabase(path string, create bool) (*os.File, error)` | lockPipelineDatabase holds a nonblocking process lock on the database inode until the descriptor closes. |

### [`src/workspace/recovery.go`](../src/workspace/recovery.go)

| Symbol | Kind | Lines | Declaration, inputs, and outputs | Source description |
|---|---|---:|---|---|
| [`RecoverAbandonedRun`](../src/workspace/recovery.go#L9) | function | 9-21 | `func RecoverAbandonedRun(ctx context.Context, dbPath string, runID int64) error` | RecoverAbandonedRun marks one running attempt failed only while holding exclusive pipeline ownership. |
...
| Symbol | Kind | Lines | Declaration, inputs, and outputs | Source description |
|---|---|---:|---|---|
| [`databaseRegistryPath`](../src/workspace/store.go#L32) | function | 32-47 | `func databaseRegistryPath() string` | databaseRegistryPath returns the path to the database registry config at <repo-root>/config/database.something. It walks up from CWD looking for go.mod (src/go.mod), then goes one level up to find the config dir. |
| [`RunPipeline`](../src/workspace/store.go#L51) | function | 51-53 | `func RunPipeline(dbPath string, originalConfig []byte, run *Run, fresh bool) error` | RunPipeline is the immutable workspace pipeline. It does not use the deprecated mutable corpus repositories. |
| [`RunPipelineContext`](../src/workspace/store.go#L56) | function | 56-335 | `func RunPipelineContext(ctx context.Context, dbPath string, originalConfig []byte, run *Run, fresh bool) (runErr error)` | RunPipelineContext executes one attempt under an exclusive process lock and finalizes cancellation as failure. |
| [`normalizedInventoryWork`](../src/workspace/store.go#L338) | struct | 338-342 | `type normalizedInventoryWork struct { DOI string WorkID int64 PipelineRunID int64 }` | normalizedInventoryWork identifies a persisted normalized work for companion PDF registration. |
| [`syncNormalizedPDFInventory`](../src/workspace/store.go#L349) | function | 349-391 | `func syncNormalizedPDFInventory(ctx context.Context, db *database.Database, dbPath, registryPath string) (int, int, error)` | syncNormalizedPDFInventory reconciles the companion inventory from the authoritative normalized revisions. It is safe to call after a new run or when a completed execution plan is reused. |
| [`normalizedInventoryBatch`](../src/workspace/store.go#L395) | function | 395-421 | `func normalizedInventoryBatch(ctx context.Context, db *database.Database, afterWorkID int64) ([]normalizedInventoryWork, error)` | normalizedInventoryBatch reads one bounded, deterministic inventory page and closes the metadata cursor before callers touch the companion database. |
| [`articlesWithFailedIdentityEvidence`](../src/workspace/store.go#L424) | function | 424-438 | `func articlesWithFailedIdentityEvidence(articles []*article.Article, evidence []uncertainORCIDSearchEvidence) []*article.Article` | articlesWithFailedIdentityEvidence selects articles whose identity provider search failed. |
| [`resultCountComparison`](../src/workspace/store.go#L441) | function | 441-450 | `func resultCountComparison(expected, observed int) string` | resultCountComparison classifies an observed source count as below, above, or matching its expectation. |
| [`loadWorkspaceEntries`](../src/workspace/store.go#L453) | function | 453-462 | `func loadWorkspaceEntries(fileType, path, source string) ([]map[string]string, error)` | loadWorkspaceEntries loads workspace entries from the supplied source. |
| [`cloneStringMap`](../src/workspace/store.go#L465) | function | 465-471 | `func cloneStringMap(in map[string]string) map[string]string` | cloneStringMap clones string map into an independent value. |
| [`persistWorkspaceStage`](../src/workspace/store.go#L474) | function | 474-498 | `func persistWorkspaceStage(db *database.Database, runID int64, articles []*article.Article, producerStage, stage, outcome string, reasons map[string][]string) (map[string]int64, map[string]int64, error)` | persistWorkspaceStage persists workspace stage through the owning repository. |
| [`setWorkspaceStageOutcome`](../src/workspace/store.go#L501) | function | 501-515 | `func setWorkspaceStageOutcome(db *database.Database, runID int64, articles []*article.Article, stage, outcome, reason string) error` | setWorkspaceStageOutcome sets workspace stage outcome using the supplied values. |
| [`setRunMetrics`](../src/workspace/store.go#L518) | function | 518-525 | `func setRunMetrics(db *database.Database, runID int64, metrics ...any) error` | setRunMetrics sets run metrics using the supplied values. |
| [`snapshotArticles`](../src/workspace/store.go#L528) | function | 528-542 | `func snapshotArticles(articles []*article.Article) []*article.Article` | snapshotArticles preserves the complete stage input before in-place article and relationship mutations. |
| [`recordWorkspaceStage`](../src/workspace/store.go#L545) | function | 545-576 | `func recordWorkspaceStage(db *database.Database, run *Run, runID int64, name string, input any, output any) error` | recordWorkspaceStage records workspace stage. |
| [`persistWorkSnapshot`](../src/workspace/store.go#L579) | function | 579-620 | `func persistWorkSnapshot(db *database.Database, runID int64, a *article.Article, producerStage, extensionData string) (int64, int64, error)` | persistWorkSnapshot persists work snapshot through the owning repository. |
| [`workspaceRevisionExtension`](../src/workspace/store.go#L623) | struct | 623-627 | ``type workspaceRevisionExtension struct { NormalizedJournal string `json:"normalized_journal,omitempty"` ValidationReasons []string `json:"validation_reasons,omitempty"` NormalizedAuthors map[string]string `json:"normalized_authors,omitempty"` }`` | workspaceRevisionExtension stores normalized values and validation evidence outside canonical revision columns. |
| [`workspaceExtension`](../src/workspace/store.go#L630) | function | 630-639 | `func workspaceExtension(a *article.Article, reasons []string) string` | workspaceExtension serializes normalized journal, author names, and validation reasons for a revision. |
| [`validateWorkspaceArticles`](../src/workspace/store.go#L642) | function | 642-654 | `func validateWorkspaceArticles(articles []*article.Article) (valid, discarded []*article.Article, reasons map[string][]string)` | validateWorkspaceArticles partitions articles by validation outcome and records discard reasons by DOI. |
| [`normalizationFieldResult`](../src/workspace/store.go#L657) | struct | 657-661 | `type normalizationFieldResult struct { DOI string Field string Outcome string }` | normalizationFieldResult records one field's normalization outcome for an article DOI. |
| [`normalizeWorkspaceArticles`](../src/workspace/store.go#L680) | function | 680-702 | `func normalizeWorkspaceArticles(articles []*article.Article) []normalizationFieldResult` | normalizeWorkspaceArticles applies each normalizer and records one outcome for every checked field. Journal normalization is retained in revision extension data, so its result is measured here but not assigned to Journal. |
| [`normalizationResult`](../src/workspace/store.go#L705) | function | 705-713 | `func normalizationResult(doi, field, input, output string) normalizationFieldResult` | normalizationResult classifies a field as changed, already canonical, or unavailable. |
| [`recordNormalizationMetrics`](../src/workspace/store.go#L719) | function | 719-748 | `func recordNormalizationMetrics(db *database.Database, runID int64, processedArticles int, results []normalizationFieldResult) error` | recordNormalizationMetrics stores mutually exclusive outcomes for each checked field. No per-field audit events are emitted: the immutable normalized revision already records the output, and the prior revision is retained as the corresponding input evidence. |
| [`sumNormalizationOutcomes`](../src/workspace/store.go#L751) | function | 751-757 | `func sumNormalizationOutcomes(outcomes map[string]int) int` | sumNormalizationOutcomes totals the recognized mutually exclusive normalization outcomes. |
| [`enrichWorkspaceMetadata`](../src/workspace/store.go#L762) | function | 762-795 | `func enrichWorkspaceMetadata(ctx context.Context, db *database.Database, runID int64, run *Run, articles []*article.Article) (int, []fieldChange, error)` | enrichWorkspaceMetadata applies article-level providers. Its output is persisted before identity enrichment so an ORCID provider failure cannot erase the metadata and authorship evidence already gathered for the run. |
| [`enrichWorkspaceIdentity`](../src/workspace/store.go#L800) | function | 800-812 | `func enrichWorkspaceIdentity(ctx context.Context, db *database.Database, runID int64, run *Run, articles []*article.Article) (int, []fieldChange, []uncertainORCIDSearchEvidence, error)` | enrichWorkspaceIdentity applies exact observed-ORCID profiles and records name-search evidence. The caller persists this evidence against the prior enrich_metadata snapshot, including when this function returns an error. |
| [`gatherCachedCrossref`](../src/workspace/store.go#L815) | function | 815-841 | `func gatherCachedCrossref(ctx context.Context, cache *workspaceCache, source enrich.SourceConfig, articles []*article.Article) (*enrich.GatherResult, error)` | gatherCachedCrossref gathers cached crossref from the supplied inputs. |
| [`gatherCachedOpenAlex`](../src/workspace/store.go#L844) | function | 844-890 | `func gatherCachedOpenAlex(ctx context.Context, cache *workspaceCache, source enrich.SourceConfig, articles []*article.Article) (*enrich.GatherResult, error)` | gatherCachedOpenAlex gathers cached open alex from the supplied inputs. |
| [`resolveCachedOpenAlexReferences`](../src/workspace/store.go#L893) | function | 893-932 | `func resolveCachedOpenAlexReferences(ctx context.Context, cache *workspaceCache, client *enrich.Client, source enrich.SourceConfig, refIDsByDOI map[string][]string) (map[string]enrich.EnrichedReference, error)` | resolveCachedOpenAlexReferences resolves cached open alex references from the supplied context. |
| [`uncertainORCIDCandidate`](../src/workspace/store.go#L935) | struct | 935-941 | `type uncertainORCIDCandidate struct { ORCID string ProviderDisplay string QueryURL string PayloadArtifactID int64 ProviderRank int }` | uncertainORCIDCandidate stores one unconfirmed ORCID name-search result and its provenance. |
| [`uncertainORCIDSearchEvidence`](../src/workspace/store.go#L944) | struct | 944-951 | `type uncertainORCIDSearchEvidence struct { DOI string AuthorIndex int CitationName string Candidates []uncertainORCIDCandidate Status string ErrorMessage string }` | uncertainORCIDSearchEvidence records candidate-search results or provider failure for one author occurrence. |
| [`enrichCachedORCID`](../src/workspace/store.go#L954) | function | 954-1026 | `func enrichCachedORCID(ctx context.Context, cache *workspaceCache, orcidSource, openAlexSource enrich.SourceConfig, hasOpenAlex bool, articles []*article.Article) (int, []fieldChange, []uncertainORCIDSearchEvidence, error)` | enrichCachedORCID gathers confirmed ORCID records and uncertain name-search evidence through the workspace cache. |
| [`resolveCachedORCIDProfile`](../src/workspace/store.go#L1029) | function | 1029-1057 | `func resolveCachedORCIDProfile(ctx context.Context, cache *workspaceCache, orcidSource enrich.SourceConfig, orcidClient *enrich.Client, openAlexSource enrich.SourceConfig, openAlexClient *enrich.Client, hasOpenAlex bool, orcid string) (*enrich.EnrichedAuthor, error)` | resolveCachedORCIDProfile resolves cached orcid profile from the supplied context. |
| [`resolveCachedORCIDNameCandidates`](../src/workspace/store.go#L1060) | function | 1060-1090 | `func resolveCachedORCIDNameCandidates(ctx context.Context, cache *workspaceCache, source enrich.SourceConfig, client *enrich.Client, name string) ([]uncertainORCIDCandidate, error)` | resolveCachedORCIDNameCandidates resolves cached orcid name candidates from the supplied context. |
| [`persistUncertainORCIDEvidence`](../src/workspace/store.go#L1093) | function | 1093-1133 | `func persistUncertainORCIDEvidence(db *database.Database, runID int64, revisionIDs map[string]int64, evidence []uncertainORCIDSearchEvidence) error` | persistUncertainORCIDEvidence persists uncertain orcid evidence through the owning repository. |
| [`normalizeCacheName`](../src/workspace/store.go#L1136) | function | 1136-1138 | `func normalizeCacheName(name string) string` | normalizeCacheName normalizes cache name. |
| [`authorProfileProvenance`](../src/workspace/store.go#L1141) | struct | 1141-1144 | `type authorProfileProvenance struct { DOI string Index int }` | authorProfileProvenance identifies the persisted article location of one author. |
| [`applyAuthorProfile`](../src/workspace/store.go#L1148) | function | 1148-1185 | `func applyAuthorProfile(authors []*article.Author, profile *enrich.EnrichedAuthor, matchedORCID string, provenance map[*article.Author]authorProfileProvenance) (bool, []fieldChange)` | applyAuthorProfile fills missing author values and returns exactly the fields it changed, using the supplied provenance for audit and metric evidence. |
| [`fieldChange`](../src/workspace/store.go#L1188) | struct | 1188-1192 | `type fieldChange struct { DOI string Field string Provider string }` | fieldChange records one enriched field change for audit purposes. |
| [`applyArticleEnrichment`](../src/workspace/store.go#L1195) | function | 1195-1245 | `func applyArticleEnrichment(articles map[string]*article.Article, result *enrich.GatherResult) (int, []fieldChange)` | applyArticleEnrichment applies article enrichment to the supplied state. |
| [`usableEnrichedAuthors`](../src/workspace/store.go#L1248) | function | 1248-1261 | `func usableEnrichedAuthors(enriched []enrich.EnrichedAuthor) []article.Author` | usableEnrichedAuthors converts enriched authors that contain a non-empty citation name. |
| [`applyConfiguredArticleEnrichment`](../src/workspace/store.go#L1264) | function | 1264-1295 | `func applyConfiguredArticleEnrichment(articles map[string]*article.Article, source enrich.SourceConfig, result *enrich.GatherResult) (int, []fieldChange)` | applyConfiguredArticleEnrichment applies configured article enrichment to the supplied state. |
| [`emitFieldEnrichedAuditEvents`](../src/workspace/store.go#L1299) | function | 1299-1327 | `func emitFieldEnrichedAuditEvents(db *database.Database, runID int64, revisionIDs map[string]int64, changes []fieldChange) error` | emitFieldEnrichedAuditEvents records one field_enriched audit event per field change. revisionIDs maps DOI -> work_revision ID. |
| [`recordFieldEnrichmentMetrics`](../src/workspace/store.go#L1333) | function | 1333-1357 | `func recordFieldEnrichmentMetrics(db *database.Database, runID int64, changes []fieldChange) error` | recordFieldEnrichmentMetrics records per-field and per-provider enrichment metrics to pipeline_run_metrics. Per-author-index fields (e.g. author_orcid_0, author_first_name_1) are aggregated into a single count per field type (author_orcid, author_first_name, etc.). |
| [`completePipelineRun`](../src/workspace/store.go#L1360) | function | 1360-1366 | `func completePipelineRun(db *database.Database, runID int64) error` | completePipelineRun completes pipeline run and records its terminal state. |
| [`recordValidationAudit`](../src/workspace/store.go#L1369) | function | 1369-1393 | `func recordValidationAudit(db *database.Database, runID int64, articles []*article.Article, reasons map[string][]string) error` | recordValidationAudit records validation audit. |
...
| [`TestApplyAuthorProfileRecordsOnlyObservedFieldChanges`](../src/workspace/store_integration_test.go#L310) | test | 310-349 | `func TestApplyAuthorProfileRecordsOnlyObservedFieldChanges(t *testing.T)` | TestApplyAuthorProfileRecordsOnlyObservedFieldChanges verifies apply author profile records only observed field changes. |
| [`TestEmitFieldEnrichedAuditEvents`](../src/workspace/store_integration_test.go#L352) | test | 352-398 | `func TestEmitFieldEnrichedAuditEvents(t *testing.T)` | TestEmitFieldEnrichedAuditEvents verifies emit field enriched audit events. |
| [`TestRecordFieldEnrichmentMetrics`](../src/workspace/store_integration_test.go#L401) | test | 401-431 | `func TestRecordFieldEnrichmentMetrics(t *testing.T)` | TestRecordFieldEnrichmentMetrics verifies record field enrichment metrics. |
| [`TestEmitFieldEnrichedSkipsUnknownDOI`](../src/workspace/store_integration_test.go#L434) | test | 434-462 | `func TestEmitFieldEnrichedSkipsUnknownDOI(t *testing.T)` | TestEmitFieldEnrichedSkipsUnknownDOI verifies emit field enriched skips unknown doi. |
...
| [`StartWorkspaceAttempt`](../src/workspace/support.go#L84) | function | 84-251 | `func StartWorkspaceAttempt(db *database.Database, originalConfig []byte, run *Run, fresh bool) (int64, error)` | StartWorkspaceAttempt snapshots configuration and inputs, reuses or creates a plan, and starts an eligible pipeline attempt. |
| [`recordPreflightStep`](../src/workspace/support.go#L254) | function | 254-306 | `func recordPreflightStep(db *database.Database, runID, configArtifactID, inputManifestArtifactID int64, inputFingerprint, outputFingerprint string, reusedFromRunID *int64, failureSummary string) error` | recordPreflightStep records preflight step. |
| [`buildInputManifest`](../src/workspace/support.go#L309) | function | 309-329 | `func buildInputManifest(resolved *manifest.ResolvedManifest) (*manifest.InputManifest, error)` | buildInputManifest hashes configured source files and returns both captured evidence and aggregate read failure. |
| [`persistArtifact`](../src/workspace/support.go#L332) | function | 332-335 | `func persistArtifact(db *database.Database, runID int64, data []byte, contentType string) (int64, error)` | persistArtifact stores content-addressed artifact metadata and bytes for a run. |
| [`contentHash`](../src/workspace/support.go#L338) | function | 338-341 | `func contentHash(data []byte) string` | contentHash returns the lowercase hexadecimal SHA-256 digest of data. |
| [`freshReason`](../src/workspace/support.go#L344) | function | 344-352 | `func freshReason(explicitFresh bool, reusePolicy string) string` | freshReason returns the persisted reason for starting a non-reused attempt. |
| [`loadCSVEntries`](../src/workspace/support.go#L355) | function | 355-396 | `func loadCSVEntries(path, source string) ([]map[string]string, error)` | loadCSVEntries loads csv entries from the supplied source. |
| [`loadBibEntries`](../src/workspace/support.go#L399) | function | 399-421 | `func loadBibEntries(path, source string) ([]map[string]string, error)` | loadBibEntries loads bib entries from the supplied source. |
...
| [`mutate`](../frontend/src/api.tsx#L92) | function | 92 | `function mutate(path: string, method: string, body: unknown)` | Sends a same-origin JSON mutation and returns its decoded response. |
| [`tables`](../frontend/src/api.tsx#L105) | function | 105 | `async function tables()` | Loads and caches the discovered database table list. |

### [`frontend/src/components/article-table.tsx`](../frontend/src/components/article-table.tsx)

| Symbol | Kind | Lines | Declaration, inputs, and outputs | Source description |
|---|---|---:|---|---|
| [`termMatchMarkup`](../frontend/src/components/article-table.tsx#L78) | function | 78 | `function termMatchMarkup(row: WireRecord)` | Renders the stored search-term coverage for one article row. |
...
| [`AuditEntity`](../frontend/src/components/audit-events.tsx#L79) | function | 79 | `function AuditEntity(props: { event: AuditEventRecord })` | Renders a context-preserving link or label for the affected audit entity. |
| [`eventSummary`](../frontend/src/components/audit-events.tsx#L120) | function | 120 | `function eventSummary(event: AuditEventRecord, metadata: WireRecord, before: WireRecord, after: WireRecord)` | Returns a concise human-readable summary of an audit event. |
| [`auditClock`](../frontend/src/components/audit-events.tsx#L195) | function | 195 | `function auditClock(timestamp: string \| undefined)` | Formats the time within a UTC date group without repeating the full date on every row. |
| [`ReviewDecisionState`](../frontend/src/components/audit-events.tsx#L208) | function | 208 | `function ReviewDecisionState(props: { label: string; state: WireRecord })` | Renders one complete previous or new review-decision state. |
| [`ReviewDecisionChange`](../frontend/src/components/audit-events.tsx#L245) | function | 245 | `function ReviewDecisionChange(props: { event: AuditEventRecord; before: WireRecord; after: WireRecord })` | Renders the visible before-and-after decision comparison for review audit events. |
| [`EventDetails`](../frontend/src/components/audit-events.tsx#L257) | function | 257 | `function EventDetails(props: { event: AuditEventRecord; metadata: WireRecord; before: WireRecord; after: WireRecord })` | Renders expandable facts and JSON payloads for an audit event. |
| [`RecordedData`](../frontend/src/components/audit-events.tsx#L307) | function | 307 | `function RecordedData(props: { data: AuditRecordedData })` | Renders one lazy audit recorded-data response. |
| [`bindAuditRecordedData`](../frontend/src/components/audit-events.tsx#L338) | function | 338 | `function bindAuditRecordedData(root: ParentNode = document)` | Binds one-shot, run-scoped loading for every visible Recorded data disclosure. |
| [`AuditEventMarkup`](../frontend/src/components/audit-events.tsx#L363) | function | 363 | `function AuditEventMarkup(props: { event: AuditEventRecord })` | Renders the complete escaped markup for one audit event. |
| [`AuditStream`](../frontend/src/components/audit-events.tsx#L427) | function | 427 | `function AuditStream(props: { events: AuditEventRecord[]; emptyMessage?: string })` | Renders audit events grouped by local date as a timeline. |
| [`RecordAuditInvestigation`](../frontend/src/components/audit-events.tsx#L465) | function | 465 | `function RecordAuditInvestigation(props: { events: AuditEventRecord[]; collection?: DetailCollectionPage<AuditEventRecord>; endpoint?: string; cursorKey?: string })` | Renders the record audit investigation controls and initial event batch. |
| [`bindRecordAuditInvestigation`](../frontend/src/components/audit-events.tsx#L522) | function | 522 | `function bindRecordAuditInvestigation(events: AuditEventRecord[])` | Binds DOM behavior for record audit investigation. |
| [`apply`](../frontend/src/components/audit-events.tsx#L538) | function | 538 | `function apply()` | Applies the current filter controls to the visible event batch. |
| [`resetAndApply`](../frontend/src/components/audit-events.tsx#L563) | function | 563 | `function resetAndApply()` | Resets the visible batch limit and reapplies the filters. |
...
| [`mountBacklinks`](../frontend/src/components/backlinks.tsx#L38) | function | 38 | `async function mountBacklinks(host: HTMLElement, options: BacklinkOptions)` | Loads and renders every requested backlink page without discarding prior rows. |
| [`render`](../frontend/src/components/backlinks.tsx#L45) | function | 45 | `function render()` | Renders loaded source-note summaries and an explicit continuation control. |
| [`loadPage`](../frontend/src/components/backlinks.tsx#L66) | function | 66 | `async function loadPage()` | Appends one opaque-cursor backlink page and retains visible evidence on failure. |
...
| [`renderDropdownLoading`](../frontend/src/components/context-selector.tsx#L123) | function | 123 | `function renderDropdownLoading(key: string)` | Renders a concise selector loading announcement. |
| [`syncDropdown`](../frontend/src/components/context-selector.tsx#L131) | function | 131 | `function syncDropdown(key: string)` | Synchronizes the custom selector presentation with its native select source. |
| [`selectOptions`](../frontend/src/components/context-selector.tsx#L148) | function | 148 | `function selectOptions(key: string, items: SelectorItem[], selected: string, config: DropdownConfig)` | Populates one native select from a bounded server page and synchronizes its presentation. |
| [`appendOptions`](../frontend/src/components/context-selector.tsx#L167) | function | 167 | `function appendOptions(key: string, items: SelectorItem[], config: DropdownConfig)` | Appends a bounded continuation page without duplicating option identifiers. |
| [`fetchOptionPage`](../frontend/src/components/context-selector.tsx#L184) | function | 184 | `async function fetchOptionPage(config: DropdownConfig, query: string, cursor: string)` | Fetches one validated hierarchy page and retains successful pages for later route renders. |
...
| [`scheduleDropdownSearch`](../frontend/src/components/context-selector.tsx#L224) | function | 224 | `function scheduleDropdownSearch(key: string)` | Schedules a server search after the user pauses typing. |
| [`initializeDropdown`](../frontend/src/components/context-selector.tsx#L236) | function | 236 | `function initializeDropdown(key: string)` | Initializes one keyboard-operable searchable selector around its native select. |
| [`focusContextSelector`](../frontend/src/components/context-selector.tsx#L352) | function | 352 | `function focusContextSelector()` | Scrolls to and focuses the visible search selector trigger. |
| [`showLoading`](../frontend/src/components/context-selector.tsx#L360) | function | 360 | `function showLoading(key: string)` | Shows one local selector-loading state without replacing the current page. |
| [`showDropdownError`](../frontend/src/components/context-selector.tsx#L370) | function | 370 | `function showDropdownError(key: string, message: string)` | Shows an inline loading failure beside one context selector. |
| [`hideDropdownError`](../frontend/src/components/context-selector.tsx#L383) | function | 383 | `function hideDropdownError(key: string)` | Removes an inline loading failure from one context selector. |
| [`reconcileSelectedRun`](../frontend/src/components/context-selector.tsx#L389) | function | 389 | `async function reconcileSelectedRun()` | Reconciles a selected run to its server-owned complete ancestry. |
| [`withSelectedItem`](../frontend/src/components/context-selector.tsx#L415) | function | 415 | `function withSelectedItem(page: HierarchyPage<HierarchyItem>, canonicalItem: SelectorItem \| null \| undefined)` | Adds one exact selected item to a page when it falls outside the first result window. |
| [`selectorParam`](../frontend/src/components/context-selector.tsx#L427) | function | 427 | `function selectorParam(key: string)` | Returns the URL parameter owned by one selector level. |
| [`hydrateLevel`](../frontend/src/components/context-selector.tsx#L436) | function | 436 | `async function hydrateLevel(key: string, config: DropdownConfig, canonicalItem: SelectorItem \| null \| undefined, clearInvalid: Record<string, unknown>)` | Loads and validates one hierarchy level, including sole-child selection. |
| [`hydrateSelectors`](../frontend/src/components/context-selector.tsx#L460) | function | 460 | `async function hydrateSelectors()` | Loads the bounded context hierarchy required by the currently selected URL values. |
...
| [`bindTableControls`](../frontend/src/components/data-table.tsx#L222) | function | 222 | `function bindTableControls(tableName: string, context?: DataTableContext)` | Binds DOM behavior for table controls. |
...
| Symbol | Kind | Lines | Declaration, inputs, and outputs | Source description |
|---|---|---:|---|---|
| [`nodeSearchText`](../frontend/src/components/graph.tsx#L28) | function | 28 | `function nodeSearchText(node: GraphNode)` | Returns the lowercased searchable text for one graph node. |
| [`GraphField`](../frontend/src/components/graph.tsx#L102) | function | 102 | `function GraphField(props: { name: string; label: string; type?: JSX.RWInputType })` | Renders an escaped graph-filter input with its current URL value. |
| [`graphQuery`](../frontend/src/components/graph.tsx#L113) | function | 113 | `function graphQuery()` | Returns the current graph-filter values keyed by query parameter. |
| [`graphLink`](../frontend/src/components/graph.tsx#L122) | function | 122 | `function graphLink(node: GraphNode)` | Returns a context-preserving detail link target for a graph node when one exists. |
| [`endpointID`](../frontend/src/components/graph.tsx#L136) | function | 136 | `function endpointID(endpoint: string \| number \| GraphNode)` | Returns an edge endpoint identifier from either an identifier or resolved node object. |
| [`graphClusters`](../frontend/src/components/graph.tsx#L142) | function | 142 | `function graphClusters(sourceNodes: GraphNode[], sourceEdges: GraphEdge[])` | Finds deterministic connected components and maps graph nodes to cluster identifiers. |
| [`legendEntry`](../frontend/src/components/graph.tsx#L183) | function | 183 | `function legendEntry(markClass: ClassNames, label: string)` | Renders one legend entry with a colored mark and its label. |
| [`GraphResult`](../frontend/src/components/graph.tsx#L193) | function | 193 | `function GraphResult(props: { data: GraphResponse })` | Renders the interactive graph viewport, legend, and relationship table. |
| [`nodeSize`](../frontend/src/components/graph.tsx#L319) | function | 319 | `function nodeSize(node: GraphNode, degree: number, maxDegree: number)` | Calculates a node radius from entity type and visible degree. |
| [`hash`](../frontend/src/components/graph.tsx#L331) | function | 331 | `function hash(value: unknown)` | Returns a deterministic unsigned hash for stable graph placement. |
| [`palette`](../frontend/src/components/graph.tsx#L340) | function | 340 | `function palette()` | Reads graph colors from active CSS custom properties with safe fallbacks. |
| [`get`](../frontend/src/components/graph.tsx#L343) | function | 343 | `function get(name: string, fallback: string)` | Returns a CSS custom property value with a fallback. |
| [`drawDiamond`](../frontend/src/components/graph.tsx#L364) | function | 364 | `function drawDiamond(context: CanvasRenderingContext2D, x: number, y: number, radius: number)` | Draws a diamond path for reference nodes. |
| [`drawTriangle`](../frontend/src/components/graph.tsx#L374) | function | 374 | `function drawTriangle(context: CanvasRenderingContext2D, x: number, y: number, radius: number)` | Draws a triangle path for referenced-author nodes. |
| [`relationshipLabel`](../frontend/src/components/graph.tsx#L383) | function | 383 | `function relationshipLabel(edge: GraphEdge)` | Returns the user-facing label for a graph edge type and its relevant metadata. |
| [`destroyGraph`](../frontend/src/components/graph.tsx#L408) | function | 408 | `function destroyGraph()` | Destroys the active graph simulation, observers, and animation frame. |
| [`layoutNode`](../frontend/src/components/graph.tsx#L428) | function | 428 | `function layoutNode(node: GraphNode, clusters: { byID: Map<string \| number, number>; components: ClusterSummary[] }, degree: Map<string \| number, number>, maxDegree: number)` | Resolves one node's initial layout position and cluster metadata. |
| [`mountGraph`](../frontend/src/components/graph.tsx#L446) | function | 446 | `function mountGraph(data: GraphResponse)` | Mounts the interactive graph viewport and its force-layout simulation. |
| [`resize`](../frontend/src/components/graph.tsx#L585) | function | 585 | `function resize()` | Resizes the backing canvas for its layout size and device pixel ratio. |
| [`graphBounds`](../frontend/src/components/graph.tsx#L606) | function | 606 | `function graphBounds(nodes: GraphNode[])` | Returns the radius-aware world-coordinate bounds of graph nodes. |
| [`clusterOverview`](../frontend/src/components/graph.tsx#L632) | function | 632 | `function clusterOverview(nodes: GraphNode[])` | Returns connected-cluster sizes ordered from largest to smallest. |
| [`drawClusterBubble`](../frontend/src/components/graph.tsx#L650) | function | 650 | `function drawClusterBubble(context: CanvasRenderingContext2D, cluster: ClusterSummary, colors: ReturnType<typeof palette>, x: number, y: number, radius: number)` | Draws one overview bubble and its label, returning its layout entry. |
| [`drawClusterOverview`](../frontend/src/components/graph.tsx#L679) | function | 679 | `function drawClusterOverview(context: CanvasRenderingContext2D, clusters: ClusterSummary[], colors: ReturnType<typeof palette>, width: number, height: number, offset: { x: number; y: number }, legendInset: number)` | Draws the connected-cluster overview bubbles and returns their layout. |
| [`fitGraph`](../frontend/src/components/graph.tsx#L706) | function | 706 | `function fitGraph(graph: GraphState)` | Adjusts the graph transform to fit all node bounds in the canvas. |
| [`runLayout`](../frontend/src/components/graph.tsx#L723) | function | 723 | `function runLayout(graph: GraphState, status: HTMLElement \| null)` | Advances the force simulation in animation-frame batches and finalizes spatial state. |
| [`next`](../frontend/src/components/graph.tsx#L746) | function | 746 | `function next()` | Advances and redraws the next batch of force-layout ticks. |
| [`draw`](../frontend/src/components/graph.tsx#L780) | function | 780 | `function draw(graph: GraphState)` | Draws the current graph state to the backing canvas. |
| [`drawArrow`](../frontend/src/components/graph.tsx#L954) | function | 954 | `function drawArrow(context: CanvasRenderingContext2D, source: GraphNode, target: GraphNode, radius: number, color: string)` | Draws a directional arrowhead at the target end of a citation edge. |
| [`graphCoordinates`](../frontend/src/components/graph.tsx#L968) | function | 968 | `function graphCoordinates(graph: GraphState, event: MouseEvent)` | Converts a pointer event from canvas coordinates to graph world coordinates. |
| [`zoomViewAt`](../frontend/src/components/graph.tsx#L977) | function | 977 | `function zoomViewAt(view: { x: number; y: number; scale: number }, screenPoint: { x: number; y: number }, nextScale: number)` | Returns a zoom transform that keeps the selected screen point stationary. |
| [`nearestOverviewCluster`](../frontend/src/components/graph.tsx#L988) | function | 988 | `function nearestOverviewCluster(graph: GraphState, event: MouseEvent)` | Returns the overview cluster hit by a pointer event, when any. |
| [`focusCluster`](../frontend/src/components/graph.tsx#L998) | function | 998 | `function focusCluster(graph: GraphState, clusterID: number)` | Focuses the viewport on one connected cluster. |
| [`buildSpatialIndex`](../frontend/src/components/graph.tsx#L1016) | function | 1016 | `function buildSpatialIndex(nodes: GraphNode[])` | Builds a spatial index over node positions for fast hit testing. |
| [`nearbyNodes`](../frontend/src/components/graph.tsx#L1031) | function | 1031 | `function nearbyNodes(index: { cellSize: number; cells: Map<string, GraphNode[]> }, point: { x: number; y: number })` | Returns nodes in the spatial-index cell surrounding a graph point. |
| [`nearestNode`](../frontend/src/components/graph.tsx#L1045) | function | 1045 | `function nearestNode(graph: GraphState, point: { x: number; y: number })` | Returns the closest selectable node within its hit radius. |
| [`bindInteractions`](../frontend/src/components/graph.tsx#L1071) | function | 1071 | `function bindInteractions(graph: GraphState, status: HTMLElement \| null, selectionPanel: HTMLElement \| null, zoomIndicator: HTMLElement \| null)` | Binds pointer, keyboard, and toolbar interactions for the graph viewport. |
| [`setSelection`](../frontend/src/components/graph.tsx#L1076) | function | 1076 | `function setSelection(id: string \| number \| null)` | Sets the selected node and refreshes the inspection panel and edge table. |
| [`updateZoomDisplay`](../frontend/src/components/graph.tsx#L1102) | function | 1102 | `function updateZoomDisplay()` | Updates the visible zoom percentage indicator. |
| [`bindGraphSearch`](../frontend/src/components/graph.tsx#L1278) | function | 1278 | `function bindGraphSearch(graph: GraphState, setSelection: (id: string \| number \| null) => void)` | Binds graph node search, highlighting matching nodes by name or DOI. |
| [`updateResults`](../frontend/src/components/graph.tsx#L1287) | function | 1287 | `function updateResults()` | Renders the bounded, keyboard-operable equivalent of canvas search highlighting. |
| [`bindGraphTheme`](../frontend/src/components/graph.tsx#L1323) | function | 1323 | `function bindGraphTheme(graph: GraphState)` | Repaints the canvas when either explicit or operating-system theme state changes. |
| [`bindGraphExport`](../frontend/src/components/graph.tsx#L1336) | function | 1336 | `function bindGraphExport(graph: GraphState, data: GraphResponse)` | Binds graph export as PNG, downloading the canvas as a PNG image. |
| [`bindGraphExpand`](../frontend/src/components/graph.tsx#L1370) | function | 1370 | `function bindGraphExpand(graph: GraphState)` | Binds the graph expand button and fullscreen lifecycle. |
| [`closeFallback`](../frontend/src/components/graph.tsx#L1379) | function | 1379 | `function closeFallback()` | Leaves the CSS fallback state and restores document and opener state. |
| [`updateLabel`](../frontend/src/components/graph.tsx#L1385) | function | 1385 | `function updateLabel()` | Updates the expand button label and refits the graph after a size change. |
| [`SelectionMarkup`](../frontend/src/components/graph.tsx#L1439) | function | 1439 | `function SelectionMarkup(props: { node: GraphNode \| undefined; neighbours: number })` | Renders the selected-node inspection panel. |
| [`NodeMarkup`](../frontend/src/components/graph.tsx#L1483) | function | 1483 | `function NodeMarkup(props: { node: GraphNode })` | Renders a linked or plain label for a graph node. |
| [`renderEdgePage`](../frontend/src/components/graph.tsx#L1491) | function | 1491 | `function renderEdgePage(graph: GraphState)` | Renders the paginated relationship table for the current selection or full graph. |
| [`edgeDetails`](../frontend/src/components/graph.tsx#L1563) | function | 1563 | `function edgeDetails(edge: GraphEdge)` | Returns relationship-specific details for a graph edge row. |
...
| Symbol | Kind | Lines | Declaration, inputs, and outputs | Source description |
|---|---|---:|---|---|
| [`draftKey`](../frontend/src/components/note-editor.tsx#L48) | function | 48 | `function draftKey(corpusID: string, runID: Identifier, workRevisionID: Identifier, noteID?: Identifier \| null, expectedVersionID?: Identifier \| null)` | Builds a browser-local draft key scoped to the opaque corpus and immutable head. |
| [`readDraft`](../frontend/src/components/note-editor.tsx#L53) | function | 53 | `function readDraft(key: string, storage?: Storage)` | Reads a draft without assuming browser storage is available. |
| [`writeDraft`](../frontend/src/components/note-editor.tsx#L62) | function | 62 | `function writeDraft(key: string, value: string, storage?: Storage)` | Writes a draft and reports storage failure without discarding editor content. |
| [`clearDraft`](../frontend/src/components/note-editor.tsx#L72) | function | 72 | `function clearDraft(key: string, storage?: Storage)` | Removes only the exact draft associated with a successful save. |
| [`lineDiff`](../frontend/src/components/note-editor.tsx#L90) | function | 90 | `function lineDiff(previous: unknown, current: unknown, limit?: number)` | Produces a bounded line comparison or complete side-by-side fallback. |
| [`noteCardMarkup`](../frontend/src/components/note-editor.tsx#L154) | function | 154 | `function noteCardMarkup(note: ReviewNoteRecord, editable: boolean)` | Renders one active note card with its metadata, content, and actions. |
| [`versionComparisonMarkup`](../frontend/src/components/note-editor.tsx#L199) | function | 199 | `function versionComparisonMarkup(previous: string, version: ReviewNoteVersion)` | Renders one immutable version comparison row for the note history. |
| [`mountNoteEditor`](../frontend/src/components/note-editor.tsx#L225) | function | 225 | `async function mountNoteEditor(host: HTMLElement, options: NoteEditorOptions)` | Mounts the note editor and current immutable note list for one article. |
| [`isDirty`](../frontend/src/components/note-editor.tsx#L368) | function | 368 | `function isDirty()` | Reports whether the current in-memory body differs from its saved or deliberately cleared baseline. |
| [`key`](../frontend/src/components/note-editor.tsx#L374) | function | 374 | `function key()` | Returns the draft key for the current new-note or immutable note head. |
| [`renderPreview`](../frontend/src/components/note-editor.tsx#L378) | function | 378 | `function renderPreview()` | Parses and safely renders current textarea content while displaying diagnostics. |
| [`updateByteCount`](../frontend/src/components/note-editor.tsx#L393) | function | 393 | `function updateByteCount()` | Updates the UTF-8 evidence limit without parsing the document. |
| [`flushDraft`](../frontend/src/components/note-editor.tsx#L400) | function | 400 | `function flushDraft()` | Persists and renders the in-memory draft after the typing debounce or a forced flush. |
| [`resetEditor`](../frontend/src/components/note-editor.tsx#L413) | function | 413 | `function resetEditor()` | Returns the editor to new-note mode and restores only its matching draft. |
| [`loadFullNote`](../frontend/src/components/note-editor.tsx#L425) | function | 425 | `async function loadFullNote(note: ReviewNoteRecord)` | Loads one complete current note head only when an action needs its body and resolved links. |
| [`renderNoteList`](../frontend/src/components/note-editor.tsx#L431) | function | 431 | `function renderNoteList()` | Renders loaded note pages and binds body, edit, history, removal, and backlink controls. |
| [`showNoteListError`](../frontend/src/components/note-editor.tsx#L514) | function | 514 | `function showNoteListError(messageText: string)` | Displays one local list failure without discarding already loaded notes. |
| [`loadNotes`](../frontend/src/components/note-editor.tsx#L520) | function | 520 | `async function loadNotes(reset = true)` | Loads one bounded active, removed, or combined note page and preserves prior rows on continuation. |
| [`showBacklinks`](../frontend/src/components/note-editor.tsx#L538) | function | 538 | `async function showBacklinks(note: ReviewNoteRecord, row: HTMLElement)` | Loads paged inbound links for one note target without replacing its card. |
| [`showHistory`](../frontend/src/components/note-editor.tsx#L547) | function | 547 | `async function showHistory(note: ReviewNoteRecord)` | Displays one selected head's paged immutable ancestry and optional restoration control. |
| [`renderHistory`](../frontend/src/components/note-editor.tsx#L553) | function | 553 | `function renderHistory()` | Renders loaded summaries and loads complete bodies only when a disclosure opens. |
| [`loadHistoryPage`](../frontend/src/components/note-editor.tsx#L653) | function | 653 | `async function loadHistoryPage()` | Appends one version-summary page while preserving already loaded ancestry. |
| [`focusNote`](../frontend/src/components/note-editor.tsx#L663) | function | 663 | `async function focusNote(noteID: Identifier)` | Resolves a URL-focused active or deleted note and exposes its history. |
| [`loadAnchorChoices`](../frontend/src/components/note-editor.tsx#L708) | function | 708 | `async function loadAnchorChoices(reset: boolean)` | Appends one page of stable anchor identities and human labels for link insertion. |
| [`loadAnchorChoicesSafely`](../frontend/src/components/note-editor.tsx#L732) | function | 732 | `async function loadAnchorChoicesSafely(reset: boolean)` | Loads anchor choices with a local diagnostic while keeping the note editor usable. |
...
| [`parseListItem`](../frontend/src/components/note-parser.tsx#L203) | function | 203 | `function parseListItem(line: string)` | Parses one supported list marker and its plain item text. |
| [`extractLinks`](../frontend/src/components/note-parser.tsx#L210) | function | 210 | `function extractLinks(text: string, baseOffset: number, links: NoteLink[], errors: NoteDiagnostic[])` | Extracts syntactically valid custom links and positional diagnostics from plain text. |
| [`decodeLink`](../frontend/src/components/note-parser.tsx#L251) | function | 251 | `function decodeLink(input: string)` | Decodes one custom-link payload into its canonical persisted identity. |
| [`isEscaped`](../frontend/src/components/note-parser.tsx#L299) | function | 299 | `function isEscaped(text: string, index: number)` | Reports whether the character at an index has an odd backslash prefix. |
| [`splitEscaped`](../frontend/src/components/note-parser.tsx#L306) | function | 306 | `function splitEscaped(input: string)` | Returns the first unescaped delimiter index or minus one. |
| [`splitTableRow`](../frontend/src/components/note-parser.tsx#L331) | function | 331 | `function splitTableRow(line: string)` | Splits one simple table row while preserving escaped vertical bars. |
| [`isTableDelimiter`](../frontend/src/components/note-parser.tsx#L367) | function | 367 | `function isTableDelimiter(line: string)` | Recognizes the complete delimiter row before interpreting prose as a table. |
| [`hasUnescapedPipe`](../frontend/src/components/note-parser.tsx#L372) | function | 372 | `function hasUnescapedPipe(line: string)` | Reports whether a line contains an unescaped table separator. |
| [`normalizeDOI`](../frontend/src/components/note-parser.tsx#L380) | function | 380 | `function normalizeDOI(value: string)` | Canonicalizes article-link DOI targets without database access. |
| [`NoteDocument`](../frontend/src/components/note-parser.tsx#L393) | function | 393 | `function NoteDocument(props: { document: { blocks: NoteBlock[] }; resolvedLinks?: ResolvedNoteLink[] \| null })` | Renders a parsed note as escaped HTML with context-preserving resolved links. |
| [`textWithBreaks`](../frontend/src/components/note-parser.tsx#L400) | function | 400 | `function textWithBreaks(text: string)` | Splits escaped text on newlines, inserting br elements between lines. |
| [`inline`](../frontend/src/components/note-parser.tsx#L413) | function | 413 | `function inline(text: string)` | Renders escaped inline text and associates parsed links with stored resolutions. |
| [`renderLink`](../frontend/src/components/note-parser.tsx#L484) | function | 484 | `function renderLink(label: string, source: NoteLink, resolved?: ResolvedNoteLink)` | Renders one safe resolved link or an accessible unresolved label. |
| [`resolutionMatches`](../frontend/src/components/note-parser.tsx#L512) | function | 512 | `function resolutionMatches(source: NoteLink, resolved?: ResolvedNoteLink)` | Confirms that a persisted ordinal resolution still describes the exact parsed draft link identity. |
...
| [`open`](../frontend/src/components/review-context-dialog.tsx#L213) | function | 213 | `async function open()` | Opens the modal and loads the first eligible-parent page once. |
...
| [`updateSubstatuses`](../frontend/src/components/review-panel.tsx#L454) | function | 454 | `function updateSubstatuses()` | Enables sub-status choices only for the two compatible terminal statuses. |
| [`renderDecisionHistory`](../frontend/src/components/review-panel.tsx#L564) | function | 564 | `function renderDecisionHistory()` | Renders every loaded decision-summary page and lazy full-reason controls. |
| [`loadDecisionHistoryPage`](../frontend/src/components/review-panel.tsx#L614) | function | 614 | `async function loadDecisionHistoryPage()` | Appends one opaque decision-history page without replacing prior rows. |
| [`renderAnchorCandidate`](../frontend/src/components/review-panel.tsx#L658) | function | 658 | `function renderAnchorCandidate()` | Converts one current PDF text selection into an accessible anchor creation form. |
| [`loadAnchors`](../frontend/src/components/review-panel.tsx#L730) | function | 730 | `async function loadAnchors(reset = true)` | Loads bounded active anchor heads, textual controls, and content-matched highlights. |
| [`showAnchorHistory`](../frontend/src/components/review-panel.tsx#L854) | function | 854 | `async function showAnchorHistory(anchorID: string, anchorLabel?: string)` | Displays bounded immutable active and tombstone ancestry for a focused anchor. |
| [`renderHistory`](../frontend/src/components/review-panel.tsx#L860) | function | 860 | `function renderHistory()` | Renders all loaded immutable anchor summaries and their continuation controls. |
| [`loadHistoryPage`](../frontend/src/components/review-panel.tsx#L963) | function | 963 | `async function loadHistoryPage()` | Appends one anchor-version cursor page without duplicating existing history. |
...
| [`loadPage`](../frontend/src/components/run-notes-index.tsx#L85) | function | 85 | `async function loadPage(reset: boolean)` | Loads one bounded note-summary page and keeps prior rows on continuation failure. |
...
| [`initMobileNavToggle`](../frontend/src/components/shell.tsx#L53) | function | 53 | `function initMobileNavToggle()` | Initialize mobile nav toggle. Shows/hides the primary navigation on small screens. |
| [`closeNavigation`](../frontend/src/components/shell.tsx#L61) | function | 61 | `function closeNavigation(restoreFocus: boolean)` | Closes the mobile navigation and optionally restores its opener focus. |
| [`handleToggle`](../frontend/src/components/shell.tsx#L70) | function | 70 | `function handleToggle()` | Toggles the mobile navigation disclosure. |
...
| Symbol | Kind | Lines | Declaration, inputs, and outputs | Source description |
|---|---|---:|---|---|
| [`setURL`](../frontend/src/router.tsx#L15) | function | 15 | `function setURL(updates: Record<string, unknown>, replace: boolean)` | Pushes or replaces URL state and immediately renders the resulting route. |
| [`replaceState`](../frontend/src/router.tsx#L33) | function | 33 | `function replaceState(updates: Record<string, unknown>)` | Replaces URL state without rendering, for in-page state that does not change the route. |
| [`navigationAllowed`](../frontend/src/router.tsx#L40) | function | 40 | `function navigationAllowed()` | Gives mounted editors one cancelable opportunity to protect unsaved local input. |
| [`bindFocusContext`](../frontend/src/router.tsx#L45) | function | 45 | `function bindFocusContext()` | Binds DOM behavior for focus context. |
| [`syncPrimaryNavigation`](../frontend/src/router.tsx#L55) | function | 55 | `function syncPrimaryNavigation(current: string)` | Synchronizes primary navigation. |
| [`syncShell`](../frontend/src/router.tsx#L72) | function | 72 | `function syncShell(current: string)` | Synchronizes shell visibility and the page-level breadcrumb before a view renders. |
| [`renderView`](../frontend/src/router.tsx#L122) | function | 122 | `async function renderView()` | Asynchronously renders view. |
| [`render`](../frontend/src/router.tsx#L137) | function | 137 | `async function render(options?: { focusTitle?: boolean; resetScroll?: boolean })` | Asynchronously renders the associated state. |
...
| [`isStateObject`](../frontend/src/state.tsx#L190) | function | 190 | `function isStateObject(raw: unknown)` | Validates an unknown value as a plain string-valued state object. |
| [`saveState`](../frontend/src/state.tsx#L196) | function | 196 | `function saveState()` | Persists the current viewerState to sessionStorage when storage is available. |
| [`loadState`](../frontend/src/state.tsx#L205) | function | 205 | `function loadState()` | Reads and validates the persisted viewerState, returning null when absent or invalid. |
| [`restoreState`](../frontend/src/state.tsx#L218) | function | 218 | `function restoreState(state: Record<string, string>)` | Assigns the viewerState and mirrors it to sessionStorage. |
| [`pathFor`](../frontend/src/state.tsx#L224) | function | 224 | `function pathFor(state: Record<string, string>)` | Returns the clean application path for one state object's view. |
| [`params`](../frontend/src/state.tsx#L229) | function | 229 | `function params()` | Returns the current URL search parameters backed by the viewerState. |
| [`value`](../frontend/src/state.tsx#L234) | function | 234 | `function value(name: string)` | Returns a named URL parameter or an empty string. |
| [`view`](../frontend/src/state.tsx#L239) | function | 239 | `function view()` | Returns the selected viewer view. |
| [`section`](../frontend/src/state.tsx#L244) | function | 244 | `function section(name: string, fallback: string)` | Returns a named section parameter or its fallback. |
| [`filterState`](../frontend/src/state.tsx#L249) | function | 249 | `function filterState(params: URLSearchParams, view: string, includeCanonical: boolean)` | Deletes keys a view may not carry from a parameter collection. |
| [`stateFor`](../frontend/src/state.tsx#L263) | function | 263 | `function stateFor(updates?: Record<string, unknown>)` | Builds the filtered destination state from updates applied to the current viewerState. |
| [`link`](../frontend/src/state.tsx#L297) | function | 297 | `function link(updates?: Record<string, unknown>)` | Builds an internal path-only URL from canonical context and destination-owned state only. |
| [`linkTargetFor`](../frontend/src/state.tsx#L302) | function | 302 | `function linkTargetFor(updates?: Record<string, unknown>)` | Builds an internal URL and its matching serialized navigation state. |
| [`detailLinkFor`](../frontend/src/state.tsx#L308) | function | 308 | `function detailLinkFor(kind: "article" \| "author" \| "reference", id: unknown)` | Returns a context-preserving detail link target for one record kind. |
| [`initViewerState`](../frontend/src/state.tsx#L318) | function | 318 | `function initViewerState()` | Adopts persisted state at boot, corrects the view from the pathname, and attaches it to the initial entry. |
| [`currentDetailOrigin`](../frontend/src/state.tsx#L332) | function | 332 | `function currentDetailOrigin()` | Serializes the current supported collection route for use by a detail link. |
| [`detailOrigin`](../frontend/src/state.tsx#L343) | function | 343 | `function detailOrigin()` | Validates the stored detail origin against route ownership and visible canonical context. |
| [`asJSON`](../frontend/src/state.tsx#L369) | function | 369 | `function asJSON(item: unknown)` | Formats a value for JSON-oriented display. |
| [`list`](../frontend/src/state.tsx#L375) | function | 375 | `function list(data: unknown, keys?: string[])` | Returns the first matching array in an API response. |
| [`appendUnique`](../frontend/src/state.tsx#L387) | function | 387 | `function appendUnique(items: T[], incoming: readonly T[], idKey: (item: T) => string \| number)` | Appends only the incoming items whose id is not already present in the target array. |
| [`columnNamesOf`](../frontend/src/state.tsx#L399) | function | 399 | `function columnNamesOf(columns: readonly (string \| ColumnInfo)[])` | Returns the ordered names of columns, accepting string or object column entries. |
| [`pickID`](../frontend/src/state.tsx#L407) | function | 407 | `function pickID(item: object \| null \| undefined)` | Returns the first supported identifier present on an item. |
| [`text`](../frontend/src/state.tsx#L417) | function | 417 | `function text(item: object \| null \| undefined, fields: string[])` | Returns the first non-empty display field on an item. |
| [`numericEvidence`](../frontend/src/state.tsx#L428) | function | 428 | `function numericEvidence(raw: unknown)` | Classifies numeric evidence without conflating missing or malformed values with recorded zero. |
| [`number`](../frontend/src/state.tsx#L439) | function | 439 | `function number(raw: unknown)` | Converts numeric evidence to a number, returning NaN when it is unavailable or invalid. |
| [`formatNumber`](../frontend/src/state.tsx#L444) | function | 444 | `function formatNumber(raw: unknown)` | Formats number. |
| [`percent`](../frontend/src/state.tsx#L452) | function | 452 | `function percent(raw: unknown, denominator: unknown)` | Formats a count as a percentage of its denominator. |
| [`formatTime`](../frontend/src/state.tsx#L462) | function | 462 | `function formatTime(raw: unknown)` | Formats time. |
| [`formatDate`](../frontend/src/state.tsx#L470) | function | 470 | `function formatDate(raw: unknown)` | Formats a timestamp as one UTC calendar date for grouping and display. |
| [`formatDuration`](../frontend/src/state.tsx#L478) | function | 478 | `function formatDuration(startedAt: unknown, finishedAt: unknown)` | Formats the elapsed time between two recorded timestamps. |
| [`formatBytes`](../frontend/src/state.tsx#L496) | function | 496 | `function formatBytes(raw: unknown)` | Formats bytes. |
| [`humanLabel`](../frontend/src/state.tsx#L512) | function | 512 | `function humanLabel(raw: unknown)` | Converts a machine-oriented identifier to a title-cased display label. |
| [`parseObject`](../frontend/src/state.tsx#L520) | function | 520 | `function parseObject(raw: unknown)` | Parses object. |
| [`statusClass`](../frontend/src/state.tsx#L539) | function | 539 | `function statusClass(raw: unknown)` | Maps a recorded status to its semantic color class. |
| [`StatusChip`](../frontend/src/state.tsx#L557) | function | 557 | `function StatusChip(props: { raw: unknown })` | Renders one status chip with its semantic color class. |
| [`metricEntries`](../frontend/src/state.tsx#L563) | function | 563 | `function metricEntries(group: MetricValue[] \| Record<string, MetricValue> \| null \| undefined)` | Normalizes array- or object-backed metrics to display-name and value pairs. |
| [`selectedRun`](../frontend/src/state.tsx#L576) | function | 576 | `function selectedRun()` | Returns the pipeline run selected by the current URL context. |
| [`showError`](../frontend/src/state.tsx#L584) | function | 584 | `function showError(error: unknown)` | Shows error. |
| [`clearError`](../frontend/src/state.tsx#L590) | function | 590 | `function clearError()` | Clears error. |
| [`busy`](../frontend/src/state.tsx#L596) | function | 596 | `function busy(isBusy: boolean)` | Shows or hides the global loading indicator. |
| [`contextChange`](../frontend/src/state.tsx#L603) | function | 603 | `function contextChange(updates: Record<string, unknown>)` | Adds route and focus cleanup required when a parent research context changes. |
| [`PageHeader`](../frontend/src/state.tsx#L624) | function | 624 | `function PageHeader(props: { kicker: string; title: string; description: string; extra?: JSX.Element })` | Renders the standard page header with escaped copy and optional actions. |
| [`Breadcrumb`](../frontend/src/state.tsx#L645) | function | 645 | `function Breadcrumb(props: { items: BreadcrumbItem[] })` | Renders escaped breadcrumb markup for an ordered page hierarchy. |
| [`setBreadcrumb`](../frontend/src/state.tsx#L667) | function | 667 | `function setBreadcrumb(items: BreadcrumbItem[])` | Replaces the shell breadcrumb with the supplied ordered page hierarchy. |
| [`EmptyState`](../frontend/src/state.tsx#L673) | function | 673 | `function EmptyState(props: { title: string; detail: string; action?: JSX.Element })` | Renders a complete empty-view state with the standard page header. |
| [`EmptyPanel`](../frontend/src/state.tsx#L686) | function | 686 | `function EmptyPanel(props: { title: string; detail: string; action?: JSX.Element })` | Renders a compact empty-state panel. |
| [`Panel`](../frontend/src/state.tsx#L697) | function | 697 | `function Panel(props: { title: string; description: string; body: JSX.Element; classes?: readonly ClassName[] })` | Renders the standard titled content panel. |
| [`Table`](../frontend/src/state.tsx#L716) | function | 716 | `function Table(props: { title: string; description: string; columns: TableColumn[]; rows: WireRecord[]; classes?: readonly ClassName[] })` | Renders an escaped data table inside the standard panel wrapper. |
| [`Subnav`](../frontend/src/state.tsx#L753) | function | 753 | `function Subnav(props: { items: Array<[string, string]>; current: string; key: string })` | Renders context-preserving tab navigation for a keyed section. |
| [`FilterChips`](../frontend/src/state.tsx#L773) | function | 773 | `function FilterChips(props: { filters: Record<string, unknown> \| null; labels?: Record<string, string>; options?: FilterChipOptions })` | Renders removable filter chips with a clear-all action. |
| [`MetricCard`](../frontend/src/state.tsx#L822) | function | 822 | `function MetricCard(props: { name: string; metric: MetricValue \| null \| undefined; href?: string; state?: Record<string, string> })` | Renders a metric card with availability, denominator, and optional navigation. |
| [`FlowStage`](../frontend/src/state.tsx#L875) | function | 875 | `function FlowStage(props: { label: string; raw: unknown; base: unknown; previous: unknown; modifier?: ClassName; stageKey: string; options: FlowStageOptions })` | Renders one retention-flow stage with counts, percentages, and optional links. |
| [`sourceFilterStageSummary`](../frontend/src/state.tsx#L998) | function | 998 | `function sourceFilterStageSummary(items: SourceFilterCount[])` | Combines cumulative source filter counts into ordered cross-source stages. |
| [`RetentionPhase`](../frontend/src/state.tsx#L1057) | function | 1057 | `function RetentionPhase(props: { title: string; description: string; summary: string; children: JSX.Element; phase: "source" \| "pipeline" \| "corpus" })` | Renders one titled phase in the retention-flow presentation. |
| [`RetentionFlow`](../frontend/src/state.tsx#L1075) | function | 1075 | `function RetentionFlow(props: { overview: OverviewResponse })` | Renders the three-phase source-selection, pipeline-processing, and corpus-enrichment flow for an overview payload. |
| [`Breakdown`](../frontend/src/state.tsx#L1227) | function | 1227 | `function Breakdown(props: { title: string; source: Record<string, MetricEvidence>; valueLabel?: string; useTotal?: boolean })` | Renders a metric breakdown table with relative bars and optional total percentages. |
| [`valueRender`](../frontend/src/state.tsx#L1254) | function | 1254 | `function valueRender(row: WireRecord)` | Renders one breakdown value with availability and optional percentage. |
| [`barRender`](../frontend/src/state.tsx#L1276) | function | 1276 | `function barRender(row: WireRecord)` | Renders an accessible relative-volume bar for one breakdown row. |
| [`SourceResultCountSummary`](../frontend/src/state.tsx#L1316) | function | 1316 | `function SourceResultCountSummary(props: { items: SourceResultCount[] \| null; classes?: readonly ClassName[] })` | Renders the expected-versus-observed source export count table. |
| [`count`](../frontend/src/state.tsx#L1319) | function | 1319 | `function count(raw: unknown)` | Formats a source count or its unavailable state. |
| [`comparison`](../frontend/src/state.tsx#L1327) | function | 1327 | `function comparison(raw: unknown)` | Renders a status chip for a source-count comparison. |
| [`date`](../frontend/src/state.tsx#L1335) | function | 1335 | `function date(raw: unknown)` | Renders an export date or its unavailable state. |
| [`SourceSearchQueries`](../frontend/src/state.tsx#L1390) | function | 1390 | `function SourceSearchQueries(props: { items: SourceResultCount[] \| null; classes?: readonly ClassName[] })` | Renders expandable exact-query markup for source exports. |
| [`Cell`](../frontend/src/state.tsx#L1422) | function | 1422 | `function Cell(props: { item: unknown; column: string; tableName?: string; options?: CellOptions })` | Renders and links a table cell according to its column and table context. |
| [`bindCopyButtons`](../frontend/src/state.tsx#L1465) | function | 1465 | `function bindCopyButtons()` | Bind copy-to-clipboard behavior for [data-copy-text] buttons. Shows "Copied!" feedback for 2 seconds, falls back to prompt(). |
...
| Symbol | Kind | Lines | Declaration, inputs, and outputs | Source description |
|---|---|---:|---|---|
| [`columnNames`](../frontend/src/views/corpus.tsx#L107) | function | 107 | `function columnNames(table: TableInfo \| undefined)` | Returns the ordered union of column names present in result rows. |
| [`IdentityEvidenceTable`](../frontend/src/views/corpus.tsx#L113) | function | 113 | `function IdentityEvidenceTable(props: { data: IdentityEvidenceResponse; context: DataTableContext & { perPage: number } })` | Renders the column definition used for identity evidence rows. |
| [`clippedLabel`](../frontend/src/views/corpus.tsx#L210) | function | 210 | `function clippedLabel(title: unknown)` | Renders the clipped label text for a record title. |
| [`clippedRecordLink`](../frontend/src/views/corpus.tsx#L215) | function | 215 | `function clippedRecordLink(kind: string, id: unknown, title: unknown)` | Renders a context-preserving record link with a clipped label. |
| [`clippedRecordText`](../frontend/src/views/corpus.tsx#L221) | function | 221 | `function clippedRecordText(title: unknown)` | Renders escaped record text clipped to the requested length. |
| [`corpusColumnConfig`](../frontend/src/views/corpus.tsx#L226) | function | 226 | `function corpusColumnConfig(current: string)` | Returns section-specific labels and renderers for corpus columns. |
| [`corpusView`](../frontend/src/views/corpus.tsx#L333) | function | 333 | `async function corpusView()` | Asynchronously implements corpus view for the viewer. |
| [`bindCorpusCollection`](../frontend/src/views/corpus.tsx#L530) | function | 530 | `function bindCorpusCollection()` | Binds the collection selector and clears filters owned by the previous collection. |
...
| [`backToCorpus`](../frontend/src/views/detail.tsx#L93) | function | 93 | `function backToCorpus(kind: string)` | Returns the context-preserving corpus return target for a detail view. |
| [`recorded`](../frontend/src/views/detail.tsx#L110) | function | 110 | `function recorded(raw: unknown, fallback?: JSX.Element)` | Renders a recorded value or its unavailable presentation. |
| [`propertyGrid`](../frontend/src/views/detail.tsx#L128) | function | 128 | `function propertyGrid(entries: DetailEntry[], baseClass: ClassName, classes?: readonly ClassName[])` | Renders definition-list markup for labeled record properties. |
| [`summaryStrip`](../frontend/src/views/detail.tsx#L148) | function | 148 | `function summaryStrip(entries: DetailEntry[])` | Renders compact summary-fact markup for a detail record. |
| [`mappingValue`](../frontend/src/views/detail.tsx#L154) | function | 154 | `function mappingValue(raw: unknown)` | Converts a stored mapping representation to a displayable object. |
| [`extensionMapping`](../frontend/src/views/detail.tsx#L180) | function | 180 | `function extensionMapping(raw: unknown)` | Renders the parsed extension mapping stored on a work revision. |
| [`keywordValues`](../frontend/src/views/detail.tsx#L187) | function | 187 | `function keywordValues(raw: unknown)` | Returns normalized keyword values from stored array or delimited input. |
| [`keywordMarkup`](../frontend/src/views/detail.tsx#L214) | function | 214 | `function keywordMarkup(raw: unknown)` | Renders label markup for normalized keyword values. |
| [`rawRecord`](../frontend/src/views/detail.tsx#L239) | function | 239 | `function rawRecord(record: WireRecord, excluded: string[])` | Renders expandable JSON markup for a raw record. |
| [`CollectionMarkup`](../frontend/src/views/detail.tsx#L271) | function | 271 | `function CollectionMarkup(props: { collectionKey: string; state: CollectionState })` | Renders expandable markup for a related-record collection. |
| [`mountCollection`](../frontend/src/views/detail.tsx#L333) | function | 333 | `function mountCollection(key: string, title: string, description: string, columns: Array<{ label: string; render: (row: WireRecord) => JSX.Element }>, source: DetailCollectionPage<WireRecord>, endpoint: string, cursorKey: string)` | Mounts collection. |
| [`loadCollectionPage`](../frontend/src/views/detail.tsx#L358) | function | 358 | `async function loadCollectionPage(key: string, cursor: string, rememberCurrent: boolean)` | Loads one cursor page while preserving the prior visible page after a local failure. |
| [`renderCollection`](../frontend/src/views/detail.tsx#L387) | function | 387 | `function renderCollection(key: string)` | Renders collection. |
| [`stageReasonMarkup`](../frontend/src/views/detail.tsx#L405) | function | 405 | `function stageReasonMarkup(raw: unknown)` | Renders escaped validation or failure reason markup for a stage outcome. |
| [`SearchTermCoveragePanel`](../frontend/src/views/detail.tsx#L423) | function | 423 | `function SearchTermCoveragePanel(props: { matches: TermMatchSummary \| null; record: ArticleRecord })` | Renders the search term coverage panel for an article revision. |
| [`ArticleView`](../frontend/src/views/detail.tsx#L559) | function | 559 | `function ArticleView(props: { record: ArticleRecord; data: ArticleDetailResponse })` | Renders the article detail view from its immutable revision payload. |
| [`PDFStatusPanel`](../frontend/src/views/detail.tsx#L699) | function | 699 | `function PDFStatusPanel(props: { record: ArticleRecord; pdf: PDFStatus })` | Renders PDF inventory and download-status markup for an article. |
| [`IdentityCandidateList`](../frontend/src/views/detail.tsx#L748) | function | 748 | `function IdentityCandidateList(props: { candidates: IdentityCandidate[] })` | Renders one ranked ORCID candidate list without implying confirmed identity. |
| [`AuthorIdentityEvidence`](../frontend/src/views/detail.tsx#L777) | function | 777 | `function AuthorIdentityEvidence(props: { evidence: IdentityResolution[] })` | Renders candidate ORCID evidence associated with the selected author occurrence. |
| [`IdentityCollectionMarkup`](../frontend/src/views/detail.tsx#L825) | function | 825 | `function IdentityCollectionMarkup(props: { state: CollectionState })` | Renders one cursor page of author identity resolutions with local continuation status. |
| [`bindIdentityCandidatePages`](../frontend/src/views/detail.tsx#L839) | function | 839 | `function bindIdentityCandidatePages(runID: string)` | Binds on-demand traversal for paged author identity candidates. |
| [`AuthorView`](../frontend/src/views/detail.tsx#L875) | function | 875 | `function AuthorView(props: { record: AuthorRecord; data: AuthorDetailResponse })` | Renders the author occurrence detail view with related articles and audit evidence. |
| [`ReferenceView`](../frontend/src/views/detail.tsx#L949) | function | 949 | `function ReferenceView(props: { record: ReferenceRecord })` | Renders the reference mention detail view with citation context. |
| [`detailView`](../frontend/src/views/detail.tsx#L1047) | function | 1047 | `async function detailView(kind: string)` | Asynchronously implements detail view for the viewer. |

### [`frontend/src/views/evaluation.tsx`](../frontend/src/views/evaluation.tsx)

| Symbol | Kind | Lines | Declaration, inputs, and outputs | Source description |
|---|---|---:|---|---|
| [`titleLink`](../frontend/src/views/evaluation.tsx#L48) | function | 48 | `function titleLink(row: WireRecord)` | Renders a queue-preserving article link for an evaluation row. |
| [`inventoriedDate`](../frontend/src/views/evaluation.tsx#L59) | function | 59 | `function inventoriedDate(row: WireRecord)` | Renders the recorded PDF inventory date or an unavailable label. |
| [`reviewSource`](../frontend/src/views/evaluation.tsx#L68) | function | 68 | `function reviewSource(row: WireRecord, initialized: boolean)` | Renders explicit review-lineage state from the invariant server response. |
| [`facetOptions`](../frontend/src/views/evaluation.tsx#L80) | function | 80 | `function facetOptions(items: EvaluationFacet[], selected: string)` | Renders one select option from an aggregate facet value. |
| [`articleCollectionView`](../frontend/src/views/evaluation.tsx#L88) | function | 88 | `async function articleCollectionView(heading: JSX.Element, refresh: () => Promise<void>)` | Renders the Corpus article collection with metadata, PDF inventory, and review controls. |

### [`frontend/src/views/home.tsx`](../frontend/src/views/home.tsx)

| Symbol | Kind | Lines | Declaration, inputs, and outputs | Source description |
|---|---|---:|---|---|
| [`deepdiveLink`](../frontend/src/views/home.tsx#L49) | function | 49 | `function deepdiveLink(searchID: Identifier, revisionID: Identifier, planID: Identifier, runID: Identifier)` | Returns a clean Deepdive link target for one complete research context. |
| [`hasContext`](../frontend/src/views/home.tsx#L64) | function | 64 | `function hasContext(item: HierarchyRun)` | Returns whether one hierarchy item contains a complete planned-run context. |
| [`ContinueAction`](../frontend/src/views/home.tsx#L69) | function | 69 | `function ContinueAction(props: { searchID: Identifier \| null; revisionID: Identifier \| null; planID: Identifier \| null; runID: Identifier \| null })` | Renders one direct action for a search's latest complete run. |
| [`SearchCard`](../frontend/src/views/home.tsx#L84) | function | 84 | `function SearchCard(props: { search: HierarchySearch })` | Renders one bounded search-history summary with lazy revision discovery. |
| [`SectionError`](../frontend/src/views/home.tsx#L129) | function | 129 | `function SectionError(props: { title: string; failure: unknown })` | Renders one hierarchy API failure without hiding successful sibling sections. |
| [`RunTable`](../frontend/src/views/home.tsx#L140) | function | 140 | `function RunTable(props: { runs: HierarchyRun[]; hasMore: boolean })` | Renders one bounded page of run attempts and lifecycle controls. |
| [`HomeFilters`](../frontend/src/views/home.tsx#L235) | function | 235 | `function HomeFilters()` | Renders server-backed Home filters with explicit visibility and calendar scope. |
| [`RunDialog`](../frontend/src/views/home.tsx#L327) | function | 327 | `function RunDialog()` | Renders the native lifecycle confirmation dialog. |
| [`loadRevisions`](../frontend/src/views/home.tsx#L369) | function | 369 | `async function loadRevisions(searchID: string, cursor: string, host: HTMLElement)` | Loads and renders one bounded revision page inside an open search disclosure. |
| [`bindHomeDiscovery`](../frontend/src/views/home.tsx#L417) | function | 417 | `function bindHomeDiscovery()` | Binds search disclosures and cursor paging for the bounded Home sections. |
| [`bindRunLifecycle`](../frontend/src/views/home.tsx#L460) | function | 460 | `function bindRunLifecycle()` | Binds native-dialog confirmation and mutation behavior for Home lifecycle controls. |
| [`close`](../frontend/src/views/home.tsx#L476) | function | 476 | `function close()` | Dismisses the lifecycle dialog and restores focus to its exact opener. |
| [`open`](../frontend/src/views/home.tsx#L486) | function | 486 | `function open(button: HTMLButtonElement)` | Configures and opens the lifecycle dialog for the selected run action. |
| [`homeView`](../frontend/src/views/home.tsx#L574) | function | 574 | `async function homeView(lifecycleMessage = "")` | Renders the workspace Home page from independently recoverable bounded hierarchy requests. |
...
| Symbol | Kind | Lines | Declaration, inputs, and outputs | Source description |
|---|---|---:|---|---|
| [`renderTime`](../frontend/src/views/provenance.tsx#L96) | function | 96 | `function renderTime(_row: WireRecord, raw: unknown)` | Renders a formatted timestamp cell for a data-table column. |
| [`selectedValues`](../frontend/src/views/provenance.tsx#L101) | function | 101 | `function selectedValues(raw: unknown)` | Returns the selected comma-separated values for an audit facet. |
| [`AuditMultiSelect`](../frontend/src/views/provenance.tsx#L110) | function | 110 | `function AuditMultiSelect(props: { name: string; label: string; options: Array<string \| AuditFacet>; selectedRaw: unknown })` | Renders a multi-select control for one audit facet. |
| [`auditQuery`](../frontend/src/views/provenance.tsx#L143) | function | 143 | `function auditQuery(cursor: string)` | Builds API query parameters from the active audit filters. |
| [`AuditFilterSummary`](../frontend/src/views/provenance.tsx#L162) | function | 162 | `function AuditFilterSummary()` | Renders markup summarizing active audit filters and their removal links. |
| [`AuditFilters`](../frontend/src/views/provenance.tsx#L181) | function | 181 | `function AuditFilters(props: { facets: AuditResponse["facets"] })` | Renders the complete audit filter form. |
...
| [`stageStatus`](../frontend/src/views/provenance.tsx#L634) | function | 634 | `function stageStatus(summary: StageSummary \| undefined, step: RunStep \| undefined)` | Returns the effective display status for a work-stage record. |
| [`StageFlow`](../frontend/src/views/provenance.tsx#L661) | function | 661 | `function StageFlow(props: { summaries: StageSummary[]; steps: RunStep[] })` | Renders ordered stage-flow markup for one work. |
| [`StagesView`](../frontend/src/views/provenance.tsx#L753) | function | 753 | `function StagesView(props: { data: StagesResponse })` | Renders work-stage evidence and pagination markup. |
| [`RunView`](../frontend/src/views/provenance.tsx#L847) | function | 847 | `function RunView(props: { artifactData: ArtifactsResponse })` | Renders stored run details and exact configuration links. |
| [`provenanceView`](../frontend/src/views/provenance.tsx#L938) | function | 938 | `async function provenanceView()` | Asynchronously implements provenance view for the viewer. |
| [`bindAuditControls`](../frontend/src/views/provenance.tsx#L1032) | function | 1032 | `function bindAuditControls()` | Binds DOM behavior for audit controls. |
...
| [`renderArtifactInspector`](../frontend/src/views/provenance.tsx#L1233) | function | 1233 | `function renderArtifactInspector()` | Renders artifact inspector. |
| [`copyArtifactText`](../frontend/src/views/provenance.tsx#L1325) | function | 1325 | `async function copyArtifactText(text: string)` | Asynchronously copies artifact text. |
...
### [`frontend/tests/unit/helpers/fetch.ts`](../frontend/tests/unit/helpers/fetch.ts)

| Symbol | Kind | Lines | Declaration, inputs, and outputs | Source description |
|---|---|---:|---|---|
| [`apiResponse`](../frontend/tests/unit/helpers/fetch.ts#L4) | function | 4 | `function apiResponse(data: unknown)` | Builds one successful API response envelope. |
...
| Symbol | Kind | Lines | Declaration, inputs, and outputs | Source description |
|---|---|---:|---|---|
| [`setLocation`](../frontend/tests/unit/views/evaluation.test.ts#L11) | function | 11 | `function setLocation(values: Record<string, string>)` | Sets the Evaluation viewer state used by one unit test. |
| [`evaluationResponse`](../frontend/tests/unit/views/evaluation.test.ts#L16) | function | 16 | `function evaluationResponse(overrides: Record<string, any> = {})` | Builds one invariant Evaluation response with optional overrides. |

### [`frontend/tests/unit/views/home.test.ts`](../frontend/tests/unit/views/home.test.ts)

| Symbol | Kind | Lines | Declaration, inputs, and outputs | Source description |
|---|---|---:|---|---|
| [`hierarchyResponse`](../frontend/tests/unit/views/home.test.ts#L14) | function | 14 | `function hierarchyResponse(rawURL: string)` | Returns the hierarchy fixture section selected by one request URL. |

### [`frontend/tests/unit/views/provenance.test.ts`](../frontend/tests/unit/views/provenance.test.ts)

| Symbol | Kind | Lines | Declaration, inputs, and outputs | Source description |
|---|---|---:|---|---|
| [`setLocation`](../frontend/tests/unit/views/provenance.test.ts#L12) | function | 12 | `function setLocation(values: Record<string, string>)` | Sets viewer state. |
...
| [`workspaceExpanded`](../frontend/tests/viewer.spec.ts#L1005) | function | 1005 | `async function workspaceExpanded(page: Page)` | Returns whether the reading workspace is expanded by either the Fullscreen API or the fallback class. |
| [`pageFilter`](../frontend/tests/viewer.spec.ts#L1075) | function | 1075 | `async function pageFilter(page: Page)` | Returns the computed filter of the rendered PDF page. |
...
| Symbol | Kind | Lines | Declaration, inputs, and outputs | Source description |
|---|---|---:|---|---|
| [`audit rows align compact times and inline records without loading raw payloads`](../frontend/tests/ui-quality.spec.ts#L41) | test | 41 | `test("audit rows align compact times and inline records without loading raw payloads", callback)` | audit rows align compact times and inline records without loading raw payloads |
| [`detail breadcrumbs remain concise and identify the parent collection`](../frontend/tests/ui-quality.spec.ts#L76) | test | 76 | `test('detail breadcrumbs remain concise and identify the parent collection', callback)` | detail breadcrumbs remain concise and identify the parent collection |
| [`article reading and review share the desktop workspace and stack on mobile`](../frontend/tests/ui-quality.spec.ts#L88) | test | 88 | `test('article reading and review share the desktop workspace and stack on mobile', callback)` | article reading and review share the desktop workspace and stack on mobile |
| [`mobile and medium layouts fit the viewport while tables retain their own scroller`](../frontend/tests/ui-quality.spec.ts#L109) | test | 109 | `test('mobile and medium layouts fit the viewport while tables retain their own scroller', callback)` | mobile and medium layouts fit the viewport while tables retain their own scroller |
| [`320px and short-landscape layouts keep controls and evidence reachable`](../frontend/tests/ui-quality.spec.ts#L149) | test | 149 | `test('320px and short-landscape layouts keep controls and evidence reachable', callback)` | 320px and short-landscape layouts keep controls and evidence reachable |
| [`200 percent reflow, text spacing, and focused-input viewport changes preserve actions`](../frontend/tests/ui-quality.spec.ts#L171) | test | 171 | `test('200 percent reflow, text spacing, and focused-input viewport changes preserve actions', callback)` | 200 percent reflow, text spacing, and focused-input viewport changes preserve actions |
| [`skip link, errors, and reduced motion are announced`](../frontend/tests/ui-quality.spec.ts#L198) | test | 198 | `test('skip link, errors, and reduced motion are announced', callback)` | skip link, errors, and reduced motion are announced |
| [`the overview page document has no axe violations`](../frontend/tests/ui-quality.spec.ts#L223) | test | 223 | `test('the overview page document has no axe violations', callback)` | the overview page document has no axe violations |
| [`${name} has no axe violations`](../frontend/tests/ui-quality.spec.ts#L248) | test | 248 | ``test(`${name} has no axe violations`, callback)`` | ${name} has no axe violations |
| [`review setup dialog has no axe violations and can be dismissed`](../frontend/tests/ui-quality.spec.ts#L255) | test | 255 | `test('review setup dialog has no axe violations and can be dismissed', callback)` | review setup dialog has no axe violations and can be dismissed |
| [`open context selector and mobile navigation remain keyboard-accessible`](../frontend/tests/ui-quality.spec.ts#L266) | test | 266 | `test('open context selector and mobile navigation remain keyboard-accessible', callback)` | open context selector and mobile navigation remain keyboard-accessible |
| [`expanded graph remains keyboard-accessible and restores opener focus`](../frontend/tests/ui-quality.spec.ts#L281) | test | 281 | `test('expanded graph remains keyboard-accessible and restores opener focus', callback)` | expanded graph remains keyboard-accessible and restores opener focus |
| [`artifact truncation and error states remain explicit and accessible`](../frontend/tests/ui-quality.spec.ts#L297) | test | 297 | `test('artifact truncation and error states remain explicit and accessible', callback)` | artifact truncation and error states remain explicit and accessible |
| [`${name} light`](../frontend/tests/ui-quality.spec.ts#L347) | test | 347 | ``test(`${name} light`, callback)`` | ${name} light |
| [`overview dark`](../frontend/tests/ui-quality.spec.ts#L356) | test | 356 | `test('overview dark', callback)` | overview dark |
| [`provenance audit dark`](../frontend/tests/ui-quality.spec.ts#L362) | test | 362 | `test('provenance audit dark', callback)` | provenance audit dark |
| [`article review setup light`](../frontend/tests/ui-quality.spec.ts#L368) | test | 368 | `test('article review setup light', callback)` | article review setup light |
| [`article review setup dark`](../frontend/tests/ui-quality.spec.ts#L375) | test | 375 | `test('article review setup dark', callback)` | article review setup dark |
| [`artifact preview light`](../frontend/tests/ui-quality.spec.ts#L383) | test | 383 | `test('artifact preview light', callback)` | artifact preview light |
...
| [`returns the full body when no data key`](../frontend/tests/unit/api.test.ts#L56) | test | 56 | `it('returns the full body when no data key', callback)` | returns the full body when no data key |
| [`throws on non-ok response with error message`](../frontend/tests/unit/api.test.ts#L72) | test | 72 | `it('throws on non-ok response with error message', callback)` | throws on non-ok response with error message |
| [`throws on non-ok response without error message`](../frontend/tests/unit/api.test.ts#L89) | test | 89 | `it('throws on non-ok response without error message', callback)` | throws on non-ok response without error message |
| [`throws on invalid JSON response`](../frontend/tests/unit/api.test.ts#L106) | test | 106 | `it('throws on invalid JSON response', callback)` | throws on invalid JSON response |
| [`aborts when state.controller is aborted`](../frontend/tests/unit/api.test.ts#L123) | test | 123 | `it('aborts when state.controller is aborted', callback)` | aborts when state.controller is aborted |
| [`sends same-origin JSON mutations`](../frontend/tests/unit/api.test.ts#L142) | test | 142 | `it('sends same-origin JSON mutations', callback)` | sends same-origin JSON mutations |
| [`fetches tables on first call and caches them`](../frontend/tests/unit/api.test.ts#L169) | test | 169 | `it('fetches tables on first call and caches them', callback)` | fetches tables on first call and caches them |
...
| Symbol | Kind | Lines | Declaration, inputs, and outputs | Source description |
|---|---|---:|---|---|
| [`shows run completion as a successful outcome with compact UTC time`](../frontend/tests/unit/components/audit-events.test.ts#L13) | test | 13 | `it("shows run completion as a successful outcome with compact UTC time", callback)` | shows run completion as a successful outcome with compact UTC time |
| [`summarizes validation results and only exposes recorded operational facts`](../frontend/tests/unit/components/audit-events.test.ts#L34) | test | 34 | `it("summarizes validation results and only exposes recorded operational facts", callback)` | summarizes validation results and only exposes recorded operational facts |
| [`does not invent summaries or results for sparse historical events`](../frontend/tests/unit/components/audit-events.test.ts#L61) | test | 61 | `it("does not invent summaries or results for sparse historical events", callback)` | does not invent summaries or results for sparse historical events |
| [`classifies review and PDF evidence independently from pipeline events`](../frontend/tests/unit/components/audit-events.test.ts#L70) | test | 70 | `it('classifies review and PDF evidence independently from pipeline events', callback)` | classifies review and PDF evidence independently from pipeline events |
| [`shows outcome and run context from recorded payloads`](../frontend/tests/unit/components/audit-events.test.ts#L76) | test | 76 | `it('shows outcome and run context from recorded payloads', callback)` | shows outcome and run context from recorded payloads |
| [`shows the complete previous and new review decision states`](../frontend/tests/unit/components/audit-events.test.ts#L93) | test | 93 | `it('shows the complete previous and new review decision states', callback)` | shows the complete previous and new review decision states |
| [`does not invent decision details for historical review events without state payloads`](../frontend/tests/unit/components/audit-events.test.ts#L114) | test | 114 | `it('does not invent decision details for historical review events without state payloads', callback)` | does not invent decision details for historical review events without state payloads |
| [`renders chronological list semantics without exposing review prose or contact fields`](../frontend/tests/unit/components/audit-events.test.ts#L129) | test | 129 | `it('renders chronological list semantics without exposing review prose or contact fields', callback)` | renders chronological list semantics without exposing review prose or contact fields |

### [`frontend/tests/unit/components/backlinks.test.ts`](../frontend/tests/unit/components/backlinks.test.ts)

| Symbol | Kind | Lines | Declaration, inputs, and outputs | Source description |
|---|---|---:|---|---|
| [`traverses more than one hundred inbound notes without duplicates`](../frontend/tests/unit/components/backlinks.test.ts#L9) | test | 9 | `it("traverses more than one hundred inbound notes without duplicates", callback)` | traverses more than one hundred inbound notes without duplicates |
...
| Symbol | Kind | Lines | Declaration, inputs, and outputs | Source description |
|---|---|---:|---|---|
| [`renders a labeled input field`](../frontend/tests/unit/components/graph.test.ts#L14) | test | 14 | `it('renders a labeled input field', callback)` | renders a labeled input field |
| [`uses provided type`](../frontend/tests/unit/components/graph.test.ts#L22) | test | 22 | `it('uses provided type', callback)` | uses provided type |
| [`escapes HTML in label`](../frontend/tests/unit/components/graph.test.ts#L27) | test | 27 | `it('escapes HTML in label', callback)` | escapes HTML in label |
| [`returns an object with all graph filter values`](../frontend/tests/unit/components/graph.test.ts#L37) | test | 37 | `it('returns an object with all graph filter values', callback)` | returns an object with all graph filter values |
| [`links to article view for article nodes`](../frontend/tests/unit/components/graph.test.ts#L50) | test | 50 | `it('links to article view for article nodes', callback)` | links to article view for article nodes |
...
| [`links to reference view for other nodes`](../frontend/tests/unit/components/graph.test.ts#L62) | test | 62 | `it('links to reference view for other nodes', callback)` | links to reference view for other nodes |
| [`does not fabricate a detail route for a raw referenced-author string`](../frontend/tests/unit/components/graph.test.ts#L68) | test | 68 | `it('does not fabricate a detail route for a raw referenced-author string', callback)` | does not fabricate a detail route for a raw referenced-author string |
| [`renders graph result with nodes and edges`](../frontend/tests/unit/components/graph.test.ts#L76) | test | 76 | `it('renders graph result with nodes and edges', callback)` | renders graph result with nodes and edges |
| [`includes truncation warning when data is truncated`](../frontend/tests/unit/components/graph.test.ts#L90) | test | 90 | `it('includes truncation warning when data is truncated', callback)` | includes truncation warning when data is truncated |
| [`handles missing counts`](../frontend/tests/unit/components/graph.test.ts#L102) | test | 102 | `it('handles missing counts', callback)` | handles missing counts |
| [`includes node search input in toolbar`](../frontend/tests/unit/components/graph.test.ts#L109) | test | 109 | `it('includes node search input in toolbar', callback)` | includes node search input in toolbar |
| [`includes zoom indicator in toolbar`](../frontend/tests/unit/components/graph.test.ts#L116) | test | 116 | `it('includes zoom indicator in toolbar', callback)` | includes zoom indicator in toolbar |
| [`includes export PNG button in toolbar`](../frontend/tests/unit/components/graph.test.ts#L123) | test | 123 | `it('includes export PNG button in toolbar', callback)` | includes export PNG button in toolbar |
| [`assigns deterministic connected components`](../frontend/tests/unit/components/graph.test.ts#L133) | test | 133 | `it('assigns deterministic connected components', callback)` | assigns deterministic connected components |
| [`keeps the world position beneath the pointer fixed while zooming`](../frontend/tests/unit/components/graph.test.ts#L145) | test | 145 | `it('keeps the world position beneath the pointer fixed while zooming', callback)` | keeps the world position beneath the pointer fixed while zooming |
| [`does nothing when no active graph`](../frontend/tests/unit/components/graph.test.ts#L160) | test | 160 | `it('does nothing when no active graph', callback)` | does nothing when no active graph |
| [`does nothing when canvas is missing`](../frontend/tests/unit/components/graph.test.ts#L169) | test | 169 | `it('does nothing when canvas is missing', callback)` | does nothing when canvas is missing |
...
| [`builds matching path and serialized state from one update`](../frontend/tests/unit/state.test.ts#L567) | test | 567 | `it('builds matching path and serialized state from one update', callback)` | builds matching path and serialized state from one update |
| [`removes keys with empty values but keeps the Home default`](../frontend/tests/unit/state.test.ts#L574) | test | 574 | `it('removes keys with empty values but keeps the Home default', callback)` | removes keys with empty values but keeps the Home default |
| [`removes keys with null values but keeps the Home default`](../frontend/tests/unit/state.test.ts#L580) | test | 580 | `it('removes keys with null values but keeps the Home default', callback)` | removes keys with null values but keeps the Home default |
| [`ensures view defaults to Home`](../frontend/tests/unit/state.test.ts#L586) | test | 586 | `it('ensures view defaults to Home', callback)` | ensures view defaults to Home |
| [`uses the current view path when no updates are supplied`](../frontend/tests/unit/state.test.ts#L592) | test | 592 | `it('uses the current view path when no updates are supplied', callback)` | uses the current view path when no updates are supplied |
| [`uses the home path for an unsupported destination`](../frontend/tests/unit/state.test.ts#L598) | test | 598 | `it('uses the home path for an unsupported destination', callback)` | uses the home path for an unsupported destination |
| [`keeps only canonical context and destination-owned route state`](../frontend/tests/unit/state.test.ts#L604) | test | 604 | `it("keeps only canonical context and destination-owned route state", callback)` | keeps only canonical context and destination-owned route state |
| [`returns detail context changes to the corresponding collection without record focus`](../frontend/tests/unit/state.test.ts#L618) | test | 618 | `it("returns detail context changes to the corresponding collection without record focus", callback)` | returns detail context changes to the corresponding collection without record focus |
| [`clears article-local note, anchor, and page focus when the article changes`](../frontend/tests/unit/state.test.ts#L631) | test | 631 | `it("clears article-local note, anchor, and page focus when the article changes", callback)` | clears article-local note, anchor, and page focus when the article changes |
| [`returns a detail origin through the origin view path`](../frontend/tests/unit/state.test.ts#L640) | test | 640 | `it("returns a detail origin through the origin view path", callback)` | returns a detail origin through the origin view path |
| [`renders a page header with kicker, title, description`](../frontend/tests/unit/state.test.ts#L670) | test | 670 | `it('renders a page header with kicker, title, description', callback)` | renders a page header with kicker, title, description |
| [`includes extra content when provided`](../frontend/tests/unit/state.test.ts#L680) | test | 680 | `it('includes extra content when provided', callback)` | includes extra content when provided |
| [`escapes HTML in inputs`](../frontend/tests/unit/state.test.ts#L685) | test | 685 | `it('escapes HTML in inputs', callback)` | escapes HTML in inputs |
| [`returns empty string when no context is set`](../frontend/tests/unit/state.test.ts#L695) | test | 695 | `it('returns empty string when no context is set', callback)` | returns empty string when no context is set |
| [`renders an explicit ordered page hierarchy`](../frontend/tests/unit/state.test.ts#L699) | test | 699 | `it('renders an explicit ordered page hierarchy', callback)` | renders an explicit ordered page hierarchy |
| [`marks only the final item as the current page and mounts it in the shell`](../frontend/tests/unit/state.test.ts#L708) | test | 708 | `it('marks only the final item as the current page and mounts it in the shell', callback)` | marks only the final item as the current page and mounts it in the shell |
| [`renders an empty state with page header and panel`](../frontend/tests/unit/state.test.ts#L720) | test | 720 | `it('renders an empty state with page header and panel', callback)` | renders an empty state with page header and panel |
| [`renders a table with header and rows`](../frontend/tests/unit/state.test.ts#L731) | test | 731 | `it('renders a table with header and rows', callback)` | renders a table with header and rows |
| [`renders empty state when no rows`](../frontend/tests/unit/state.test.ts#L743) | test | 743 | `it('renders empty state when no rows', callback)` | renders empty state when no rows |
| [`renders subnavigation links`](../frontend/tests/unit/state.test.ts#L752) | test | 752 | `it('renders subnavigation links', callback)` | renders subnavigation links |
| [`renders a metric card with value`](../frontend/tests/unit/state.test.ts#L766) | test | 766 | `it('renders a metric card with value', callback)` | renders a metric card with value |
| [`renders unavailable state`](../frontend/tests/unit/state.test.ts#L773) | test | 773 | `it('renders unavailable state', callback)` | renders unavailable state |
| [`includes href when provided`](../frontend/tests/unit/state.test.ts#L778) | test | 778 | `it('includes href when provided', callback)` | includes href when provided |
| [`shows percentage when denominator is present`](../frontend/tests/unit/state.test.ts#L783) | test | 783 | `it('shows percentage when denominator is present', callback)` | shows percentage when denominator is present |
| [`renders a flow stage with count`](../frontend/tests/unit/state.test.ts#L793) | test | 793 | `it('renders a flow stage with count', callback)` | renders a flow stage with count |
| [`shows input baseline for null previous`](../frontend/tests/unit/state.test.ts#L801) | test | 801 | `it('shows input baseline for null previous', callback)` | shows input baseline for null previous |
| [`shows diff from prior`](../frontend/tests/unit/state.test.ts#L806) | test | 806 | `it('shows diff from prior', callback)` | shows diff from prior |
| [`handles unavailable state`](../frontend/tests/unit/state.test.ts#L811) | test | 811 | `it('handles unavailable state', callback)` | handles unavailable state |
| [`handles null raw`](../frontend/tests/unit/state.test.ts#L816) | test | 816 | `it('handles null raw', callback)` | handles null raw |
| [`renders not recorded when input is missing`](../frontend/tests/unit/state.test.ts#L825) | test | 825 | `it('renders not recorded when input is missing', callback)` | renders not recorded when input is missing |
| [`renders not recorded when input is unavailable`](../frontend/tests/unit/state.test.ts#L830) | test | 830 | `it('renders not recorded when input is unavailable', callback)` | renders not recorded when input is unavailable |
| [`renders full retention flow with valid data`](../frontend/tests/unit/state.test.ts#L835) | test | 835 | `it('renders full retention flow with valid data', callback)` | renders full retention flow with valid data |
| [`uses the initial unfiltered source total for every retention percentage`](../frontend/tests/unit/state.test.ts#L853) | test | 853 | `it('uses the initial unfiltered source total for every retention percentage', callback)` | uses the initial unfiltered source total for every retention percentage |
| [`does not repeat a shorter source sequence into later aggregate stages`](../frontend/tests/unit/state.test.ts#L899) | test | 899 | `it('does not repeat a shorter source sequence into later aggregate stages', callback)` | does not repeat a shorter source sequence into later aggregate stages |
| [`renders not recorded for empty entries`](../frontend/tests/unit/state.test.ts#L924) | test | 924 | `it('renders not recorded for empty entries', callback)` | renders not recorded for empty entries |
| [`renders a breakdown table with entries`](../frontend/tests/unit/state.test.ts#L929) | test | 929 | `it('renders a breakdown table with entries', callback)` | renders a breakdown table with entries |
| [`renders with total when useTotal is true`](../frontend/tests/unit/state.test.ts#L937) | test | 937 | `it('renders with total when useTotal is true', callback)` | renders with total when useTotal is true |
| [`renders a table with source counts`](../frontend/tests/unit/state.test.ts#L948) | test | 948 | `it('renders a table with source counts', callback)` | renders a table with source counts |
| [`handles empty items`](../frontend/tests/unit/state.test.ts#L959) | test | 959 | `it('handles empty items', callback)` | handles empty items |
| [`renders NULL for null/undefined`](../frontend/tests/unit/state.test.ts#L968) | test | 968 | `it('renders NULL for null/undefined', callback)` | renders NULL for null/undefined |
| [`renders short values inline`](../frontend/tests/unit/state.test.ts#L974) | test | 974 | `it('renders short values inline', callback)` | renders short values inline |
| [`truncates long values with details`](../frontend/tests/unit/state.test.ts#L980) | test | 980 | `it('truncates long values with details', callback)` | truncates long values with details |
| [`creates article link for article_id column`](../frontend/tests/unit/state.test.ts#L987) | test | 987 | `it('creates article link for article_id column', callback)` | creates article link for article_id column |
| [`creates author link for author_id column`](../frontend/tests/unit/state.test.ts#L993) | test | 993 | `it('creates author link for author_id column', callback)` | creates author link for author_id column |
| [`creates reference link for reference_id column`](../frontend/tests/unit/state.test.ts#L999) | test | 999 | `it('creates reference link for reference_id column', callback)` | creates reference link for reference_id column |
| [`creates article link for work_revision id with tableName`](../frontend/tests/unit/state.test.ts#L1005) | test | 1005 | `it('creates article link for work_revision id with tableName', callback)` | creates article link for work_revision id with tableName |
| [`creates author link for author_occurrences id with tableName`](../frontend/tests/unit/state.test.ts#L1010) | test | 1010 | `it('creates author link for author_occurrences id with tableName', callback)` | creates author link for author_occurrences id with tableName |
| [`creates reference link for reference_mentions id with tableName`](../frontend/tests/unit/state.test.ts#L1015) | test | 1015 | `it('creates reference link for reference_mentions id with tableName', callback)` | creates reference link for reference_mentions id with tableName |
| [`copies text and shows Copied! feedback`](../frontend/tests/unit/state.test.ts#L1028) | test | 1028 | `it('copies text and shows Copied! feedback', callback)` | copies text and shows Copied! feedback |
| [`falls back to prompt when clipboard fails`](../frontend/tests/unit/state.test.ts#L1057) | test | 1057 | `it('falls back to prompt when clipboard fails', callback)` | falls back to prompt when clipboard fails |

### [`frontend/tests/unit/views/advanced.test.ts`](../frontend/tests/unit/views/advanced.test.ts)

| Symbol | Kind | Lines | Declaration, inputs, and outputs | Source description |
|---|---|---:|---|---|
| [`renders a safe table projection without cached discovery counts`](../frontend/tests/unit/views/advanced.test.ts#L27) | test | 27 | `it("renders a safe table projection without cached discovery counts", callback)` | renders a safe table projection without cached discovery counts |
| [`replaces an invalid table key with a valid table and explains the correction`](../frontend/tests/unit/views/advanced.test.ts#L51) | test | 51 | `it("replaces an invalid table key with a valid table and explains the correction", callback)` | replaces an invalid table key with a valid table and explains the correction |
| [`keeps the Advanced shell visible when the selected table request fails`](../frontend/tests/unit/views/advanced.test.ts#L71) | test | 71 | `it("keeps the Advanced shell visible when the selected table request fails", callback)` | keeps the Advanced shell visible when the selected table request fails |
...
| Symbol | Kind | Lines | Declaration, inputs, and outputs | Source description |
|---|---|---:|---|---|
| [`requires a selected run without requesting evaluation data`](../frontend/tests/unit/views/evaluation.test.ts#L55) | test | 55 | `it("requires a selected run without requesting evaluation data", callback)` | requires a selected run without requesting evaluation data |
| [`renders queue progress, explicit lineage, facets, and inventory states`](../frontend/tests/unit/views/evaluation.test.ts#L71) | test | 71 | `it("renders queue progress, explicit lineage, facets, and inventory states", callback)` | renders queue progress, explicit lineage, facets, and inventory states |
| [`uses top-level initialized state when a filtered page has no rows`](../frontend/tests/unit/views/evaluation.test.ts#L102) | test | 102 | `it("uses top-level initialized state when a filtered page has no rows", callback)` | uses top-level initialized state when a filtered page has no rows |
| [`submits every queue filter as destination-owned URL state`](../frontend/tests/unit/views/evaluation.test.ts#L121) | test | 121 | `it("submits every queue filter as destination-owned URL state", callback)` | submits every queue filter as destination-owned URL state |

### [`frontend/tests/unit/views/home.test.ts`](../frontend/tests/unit/views/home.test.ts)

| Symbol | Kind | Lines | Declaration, inputs, and outputs | Source description |
|---|---|---:|---|---|
| [`renders bounded hierarchy metrics, filters, direct actions, and lazy revision pages`](../frontend/tests/unit/views/home.test.ts#L93) | test | 93 | `it("renders bounded hierarchy metrics, filters, direct actions, and lazy revision pages", callback)` | renders bounded hierarchy metrics, filters, direct actions, and lazy revision pages |
| [`uses a native dialog and restores focus to the lifecycle opener after cancellation`](../frontend/tests/unit/views/home.test.ts#L136) | test | 136 | `it("uses a native dialog and restores focus to the lifecycle opener after cancellation", callback)` | uses a native dialog and restores focus to the lifecycle opener after cancellation |
| [`retains successful sibling sections when run discovery fails`](../frontend/tests/unit/views/home.test.ts#L150) | test | 150 | `it("retains successful sibling sections when run discovery fails", callback)` | retains successful sibling sections when run discovery fails |
...
| Symbol | Kind | Lines | Declaration, inputs, and outputs | Source description |
|---|---|---:|---|---|
| [`shows one contextual empty panel for run-scoped sections without a run`](../frontend/tests/unit/views/provenance.test.ts#L23) | test | 23 | `it('shows one contextual empty panel for run-scoped sections without a run', callback)` | shows one contextual empty panel for run-scoped sections without a run |
| [`renders server-filtered audit evidence with visible active filters`](../frontend/tests/unit/views/provenance.test.ts#L30) | test | 30 | `it('renders server-filtered audit evidence with visible active filters', callback)` | renders server-filtered audit evidence with visible active filters |
| [`updates URL state when audit filters are submitted`](../frontend/tests/unit/views/provenance.test.ts#L55) | test | 55 | `it('updates URL state when audit filters are submitted', callback)` | updates URL state when audit filters are submitted |
| [`appends older audit events without duplicates and preserves open details`](../frontend/tests/unit/views/provenance.test.ts#L74) | test | 74 | `it('appends older audit events without duplicates and preserves open details', callback)` | appends older audit events without duplicates and preserves open details |
| [`windows long appended audit histories without removing open evidence`](../frontend/tests/unit/views/provenance.test.ts#L116) | test | 116 | `it('windows long appended audit histories without removing open evidence', callback)` | windows long appended audit histories without removing open evidence |
| [`renders bounded artifact preview metadata and truncation guidance`](../frontend/tests/unit/views/provenance.test.ts#L131) | test | 131 | `it('renders bounded artifact preview metadata and truncation guidance', callback)` | renders bounded artifact preview metadata and truncation guidance |
| [`renders cache search and complete pagination controls`](../frontend/tests/unit/views/provenance.test.ts#L161) | test | 161 | `it('renders cache search and complete pagination controls', callback)` | renders cache search and complete pagination controls |
| [`renders stage progression before paginated details`](../frontend/tests/unit/views/provenance.test.ts#L182) | test | 182 | `it('renders stage progression before paginated details', callback)` | renders stage progression before paginated details |
| [`renders run identity and configuration snapshots`](../frontend/tests/unit/views/provenance.test.ts#L205) | test | 205 | `it('renders run identity and configuration snapshots', callback)` | renders run identity and configuration snapshots |
...
| Symbol | Kind | Lines | Declaration, inputs, and outputs | Source description |
|---|---|---:|---|---|
| [`health endpoint returns readable status`](../frontend/tests/viewer.spec.ts#L79) | test | 79 | `test('health endpoint returns readable status', callback)` | health endpoint returns readable status |
| [`page loads and renders navigation`](../frontend/tests/viewer.spec.ts#L88) | test | 88 | `test('page loads and renders navigation', callback)` | page loads and renders navigation |
| [`primary navigation links are present`](../frontend/tests/viewer.spec.ts#L101) | test | 101 | `test('primary navigation links are present', callback)` | primary navigation links are present |
| [`primary navigation preserves the selected research context`](../frontend/tests/viewer.spec.ts#L111) | test | 111 | `test('primary navigation preserves the selected research context', callback)` | primary navigation preserves the selected research context |
| [`primary navigation loads view pages and browser history returns to the prior document`](../frontend/tests/viewer.spec.ts#L125) | test | 125 | `test('primary navigation loads view pages and browser history returns to the prior document', callback)` | primary navigation loads view pages and browser history returns to the prior document |
| [`cancelable draft protection blocks a native view-page transition`](../frontend/tests/viewer.spec.ts#L144) | test | 144 | `test('cancelable draft protection blocks a native view-page transition', callback)` | cancelable draft protection blocks a native view-page transition |
| [`a view page URL loads its identified document directly`](../frontend/tests/viewer.spec.ts#L160) | test | 160 | `test('a view page URL loads its identified document directly', callback)` | a view page URL loads its identified document directly |
| [`research context is displayed after selecting a run`](../frontend/tests/viewer.spec.ts#L168) | test | 168 | `test('research context is displayed after selecting a run', callback)` | research context is displayed after selecting a run |
| [`a selected run owns the complete displayed ancestry across crossed URLs and reload`](../frontend/tests/viewer.spec.ts#L187) | test | 187 | `test('a selected run owns the complete displayed ancestry across crossed URLs and reload', callback)` | a selected run owns the complete displayed ancestry across crossed URLs and reload |
| [`run-only, stale descendant, trashed, and history states remain deterministic and visibly focused`](../frontend/tests/viewer.spec.ts#L207) | test | 207 | `test('run-only, stale descendant, trashed, and history states remain deterministic and visibly focused', callback)` | run-only, stale descendant, trashed, and history states remain deterministic and visibly focused |
| [`displays run metrics for a completed run`](../frontend/tests/viewer.spec.ts#L244) | test | 244 | `test('displays run metrics for a completed run', callback)` | displays run metrics for a completed run |
| [`displays retention funnel for completed run`](../frontend/tests/viewer.spec.ts#L251) | test | 251 | `test('displays retention funnel for completed run', callback)` | displays retention funnel for completed run |
| [`uses one initial raw-result baseline across source and pipeline retention stages`](../frontend/tests/viewer.spec.ts#L258) | test | 258 | `test('uses one initial raw-result baseline across source and pipeline retention stages', callback)` | uses one initial raw-result baseline across source and pipeline retention stages |
| [`displays informational source export-count comparisons`](../frontend/tests/viewer.spec.ts#L303) | test | 303 | `test('displays informational source export-count comparisons', callback)` | displays informational source export-count comparisons |
| [`shows no enrichment badge for enrichment-disabled run`](../frontend/tests/viewer.spec.ts#L311) | test | 311 | `test('shows no enrichment badge for enrichment-disabled run', callback)` | shows no enrichment badge for enrichment-disabled run |
| [`displays enrichment field and provider breakdowns`](../frontend/tests/viewer.spec.ts#L318) | test | 318 | `test('displays enrichment field and provider breakdowns', callback)` | displays enrichment field and provider breakdowns |
| [`articles section loads and shows article rows`](../frontend/tests/viewer.spec.ts#L331) | test | 331 | `test('articles section loads and shows article rows', callback)` | articles section loads and shows article rows |
| [`articles section supports search filtering`](../frontend/tests/viewer.spec.ts#L343) | test | 343 | `test('articles section supports search filtering', callback)` | articles section supports search filtering |
| [`articles expansion shows matched search terms`](../frontend/tests/viewer.spec.ts#L351) | test | 351 | `test('articles expansion shows matched search terms', callback)` | articles expansion shows matched search terms |
| [`articles expansion shows no search terms recorded for a run without queries`](../frontend/tests/viewer.spec.ts#L363) | test | 363 | `test('articles expansion shows no search terms recorded for a run without queries', callback)` | articles expansion shows no search terms recorded for a run without queries |
| [`authors section loads and shows author rows`](../frontend/tests/viewer.spec.ts#L372) | test | 372 | `test('authors section loads and shows author rows', callback)` | authors section loads and shows author rows |
| [`references section loads and shows reference rows`](../frontend/tests/viewer.spec.ts#L379) | test | 379 | `test('references section loads and shows reference rows', callback)` | references section loads and shows reference rows |
| [`sources section loads and shows source records`](../frontend/tests/viewer.spec.ts#L393) | test | 393 | `test('sources section loads and shows source records', callback)` | sources section loads and shows source records |
| [`identity evidence links to author-scoped candidate details without expanding candidates inside the table`](../frontend/tests/viewer.spec.ts#L409) | test | 409 | `test('identity evidence links to author-scoped candidate details without expanding candidates inside the table', callback)` | identity evidence links to author-scoped candidate details without expanding candidates inside the table |
| [`identity evidence supports page selection, page size, sorting, and search`](../frontend/tests/viewer.spec.ts#L423) | test | 423 | `test("identity evidence supports page selection, page size, sorting, and search", callback)` | identity evidence supports page selection, page size, sorting, and search |
| [`corpus supports pagination`](../frontend/tests/viewer.spec.ts#L503) | test | 503 | `test('corpus supports pagination', callback)` | corpus supports pagination |
| [`corpus ignores a sort field that is unsupported by its selected section`](../frontend/tests/viewer.spec.ts#L522) | test | 522 | `test('corpus ignores a sort field that is unsupported by its selected section', callback)` | corpus ignores a sort field that is unsupported by its selected section |
| [`graph loads and renders for a completed run`](../frontend/tests/viewer.spec.ts#L533) | test | 533 | `test('graph loads and renders for a completed run', callback)` | graph loads and renders for a completed run |
| [`graph supports mode switching to citation`](../frontend/tests/viewer.spec.ts#L542) | test | 542 | `test('graph supports mode switching to citation', callback)` | graph supports mode switching to citation |
| [`graph supports text search filter`](../frontend/tests/viewer.spec.ts#L549) | test | 549 | `test('graph supports text search filter', callback)` | graph supports text search filter |
| [`graph filters are applied explicitly and remain visible in the URL`](../frontend/tests/viewer.spec.ts#L556) | test | 556 | `test('graph filters are applied explicitly and remain visible in the URL', callback)` | graph filters are applied explicitly and remain visible in the URL |
| [`graph node search input is present and functional`](../frontend/tests/viewer.spec.ts#L564) | test | 564 | `test('graph node search input is present and functional', callback)` | graph node search input is present and functional |
| [`graph export downloads a valid PNG`](../frontend/tests/viewer.spec.ts#L580) | test | 580 | `test('graph export downloads a valid PNG', callback)` | graph export downloads a valid PNG |
| [`graph legend is anchored at the bottom left of the canvas`](../frontend/tests/viewer.spec.ts#L596) | test | 596 | `test('graph legend is anchored at the bottom left of the canvas', callback)` | graph legend is anchored at the bottom left of the canvas |
| [`background click clears a selected graph node`](../frontend/tests/viewer.spec.ts#L609) | test | 609 | `test('background click clears a selected graph node', callback)` | background click clears a selected graph node |
| [`clicking the selected graph node again clears it`](../frontend/tests/viewer.spec.ts#L618) | test | 618 | `test('clicking the selected graph node again clears it', callback)` | clicking the selected graph node again clears it |
| [`secondary-button drag pans without changing graph selection`](../frontend/tests/viewer.spec.ts#L645) | test | 645 | `test('secondary-button drag pans without changing graph selection', callback)` | secondary-button drag pans without changing graph selection |
| [`audit section loads and shows events`](../frontend/tests/viewer.spec.ts#L663) | test | 663 | `test('audit section loads and shows events', callback)` | audit section loads and shows events |
| [`audit stream filters events by category on the server`](../frontend/tests/viewer.spec.ts#L675) | test | 675 | `test('audit stream filters events by category on the server', callback)` | audit stream filters events by category on the server |
| [`audit stream exposes mirrored PDF events`](../frontend/tests/viewer.spec.ts#L695) | test | 695 | `test('audit stream exposes mirrored PDF events', callback)` | audit stream exposes mirrored PDF events |
| [`artifacts section identifies configuration snapshots and offers downloads`](../frontend/tests/viewer.spec.ts#L703) | test | 703 | `test('artifacts section identifies configuration snapshots and offers downloads', callback)` | artifacts section identifies configuration snapshots and offers downloads |
| [`cache uses section supports shared filtering and pagination controls`](../frontend/tests/viewer.spec.ts#L725) | test | 725 | `test('cache uses section supports shared filtering and pagination controls', callback)` | cache uses section supports shared filtering and pagination controls |
| [`stages section loads`](../frontend/tests/viewer.spec.ts#L742) | test | 742 | `test('stages section loads', callback)` | stages section loads |
| [`run details section loads`](../frontend/tests/viewer.spec.ts#L754) | test | 754 | `test('run details section loads', callback)` | run details section loads |
| [`keeps review filters, expansion, and article return navigation in Corpus`](../frontend/tests/viewer.spec.ts#L766) | test | 766 | `test("keeps review filters, expansion, and article return navigation in Corpus", callback)` | keeps review filters, expansion, and article return navigation in Corpus |
| [`opens an existing Evaluation route as the Corpus article collection`](../frontend/tests/viewer.spec.ts#L800) | test | 800 | `test("opens an existing Evaluation route as the Corpus article collection", callback)` | opens an existing Evaluation route as the Corpus article collection |
| [`lists only normalized articles with manual inventory status`](../frontend/tests/viewer.spec.ts#L808) | test | 808 | `test('lists only normalized articles with manual inventory status', callback)` | lists only normalized articles with manual inventory status |
| [`preserves research context in the Corpus navigation link`](../frontend/tests/viewer.spec.ts#L826) | test | 826 | `test('preserves research context in the Corpus navigation link', callback)` | preserves research context in the Corpus navigation link |
| [`lists available tables`](../frontend/tests/viewer.spec.ts#L842) | test | 842 | `test('lists available tables', callback)` | lists available tables |
| [`displays rows from a selected table`](../frontend/tests/viewer.spec.ts#L849) | test | 849 | `test('displays rows from a selected table', callback)` | displays rows from a selected table |
| [`table browser supports pagination`](../frontend/tests/viewer.spec.ts#L856) | test | 856 | `test('table browser supports pagination', callback)` | table browser supports pagination |
| [`ignores a sort field that does not belong to the selected table`](../frontend/tests/viewer.spec.ts#L865) | test | 865 | `test('ignores a sort field that does not belong to the selected table', callback)` | ignores a sort field that does not belong to the selected table |
| [`Home shows all research hierarchy totals and manages trashed runs through a modal`](../frontend/tests/viewer.spec.ts#L875) | test | 875 | `test('Home shows all research hierarchy totals and manages trashed runs through a modal', callback)` | Home shows all research hierarchy totals and manages trashed runs through a modal |
| [`article detail shows revision metadata`](../frontend/tests/viewer.spec.ts#L898) | test | 898 | `test('article detail shows revision metadata', callback)` | article detail shows revision metadata |
| [`article detail shows the search term coverage panel`](../frontend/tests/viewer.spec.ts#L911) | test | 911 | `test('article detail shows the search term coverage panel', callback)` | article detail shows the search term coverage panel |
| [`article detail shows no search terms recorded for a run without queries`](../frontend/tests/viewer.spec.ts#L923) | test | 923 | `test('article detail shows no search terms recorded for a run without queries', callback)` | article detail shows no search terms recorded for a run without queries |
| [`article detail opens the bound PDF without discarding research context`](../frontend/tests/viewer.spec.ts#L936) | test | 936 | `test('article detail opens the bound PDF without discarding research context', callback)` | article detail opens the bound PDF without discarding research context |
| [`article detail shows an absent PDF without an open action`](../frontend/tests/viewer.spec.ts#L951) | test | 951 | `test('article detail shows an absent PDF without an open action', callback)` | article detail shows an absent PDF without an open action |
| [`article detail preserves the originating corpus state`](../frontend/tests/viewer.spec.ts#L957) | test | 957 | `test('article detail preserves the originating corpus state', callback)` | article detail preserves the originating corpus state |
| [`article detail shows authors and references`](../frontend/tests/viewer.spec.ts#L972) | test | 972 | `test('article detail shows authors and references', callback)` | article detail shows authors and references |
| [`author detail shows author information`](../frontend/tests/viewer.spec.ts#L980) | test | 980 | `test('author detail shows author information', callback)` | author detail shows author information |
| [`reference detail shows reference information`](../frontend/tests/viewer.spec.ts#L992) | test | 992 | `test('reference detail shows reference information', callback)` | reference detail shows reference information |
| [`enters fullscreen from the toolbar with the drawer expanded by default`](../frontend/tests/viewer.spec.ts#L1011) | test | 1011 | `test('enters fullscreen from the toolbar with the drawer expanded by default', callback)` | enters fullscreen from the toolbar with the drawer expanded by default |
| [`collapses and expands the review drawer through the edge control`](../frontend/tests/viewer.spec.ts#L1026) | test | 1026 | `test('collapses and expands the review drawer through the edge control', callback)` | collapses and expands the review drawer through the edge control |
| [`selecting PDF text expands a collapsed drawer`](../frontend/tests/viewer.spec.ts#L1041) | test | 1041 | `test('selecting PDF text expands a collapsed drawer', callback)` | selecting PDF text expands a collapsed drawer |
| [`exits fullscreen and restores the embedded article layout`](../frontend/tests/viewer.spec.ts#L1055) | test | 1055 | `test('exits fullscreen and restores the embedded article layout', callback)` | exits fullscreen and restores the embedded article layout |
| [`inverts the rendered page through the Dark toggle and restores it`](../frontend/tests/viewer.spec.ts#L1081) | test | 1081 | `test('inverts the rendered page through the Dark toggle and restores it', callback)` | inverts the rendered page through the Dark toggle and restores it |
| [`keeps the inverted theme while entering and leaving fullscreen`](../frontend/tests/viewer.spec.ts#L1101) | test | 1101 | `test('keeps the inverted theme while entering and leaving fullscreen', callback)` | keeps the inverted theme while entering and leaving fullscreen |
| [`shows error for invalid API route`](../frontend/tests/viewer.spec.ts#L1122) | test | 1122 | `test('shows error for invalid API route', callback)` | shows error for invalid API route |
| [`shows error for invalid per_page value`](../frontend/tests/viewer.spec.ts#L1130) | test | 1130 | `test('shows error for invalid per_page value', callback)` | shows error for invalid per_page value |
| [`rejects SQL injection attempts in sort parameter`](../frontend/tests/viewer.spec.ts#L1137) | test | 1137 | `test('rejects SQL injection attempts in sort parameter', callback)` | rejects SQL injection attempts in sort parameter |
| [`rejects invalid order parameter`](../frontend/tests/viewer.spec.ts#L1144) | test | 1144 | `test('rejects invalid order parameter', callback)` | rejects invalid order parameter |
| [`rejects unknown query parameters`](../frontend/tests/viewer.spec.ts#L1151) | test | 1151 | `test('rejects unknown query parameters', callback)` | rejects unknown query parameters |
| [`returns 404 for nonexistent table`](../frontend/tests/viewer.spec.ts#L1158) | test | 1158 | `test('returns 404 for nonexistent table', callback)` | returns 404 for nonexistent table |
| [`handles article detail with nonexistent ID gracefully`](../frontend/tests/viewer.spec.ts#L1165) | test | 1165 | `test('handles article detail with nonexistent ID gracefully', callback)` | handles article detail with nonexistent ID gracefully |
| [`legacy view query parameters are rejected and the root renders Home`](../frontend/tests/viewer.spec.ts#L1170) | test | 1170 | `test('legacy view query parameters are rejected and the root renders Home', callback)` | legacy view query parameters are rejected and the root renders Home |
| [`renders on mobile viewport (375px)`](../frontend/tests/viewer.spec.ts#L1184) | test | 1184 | `test('renders on mobile viewport (375px)', callback)` | renders on mobile viewport (375px) |
| [`renders on tablet viewport (768px)`](../frontend/tests/viewer.spec.ts#L1193) | test | 1193 | `test('renders on tablet viewport (768px)', callback)` | renders on tablet viewport (768px) |
| [`renders on desktop viewport (1280px)`](../frontend/tests/viewer.spec.ts#L1199) | test | 1199 | `test('renders on desktop viewport (1280px)', callback)` | renders on desktop viewport (1280px) |
| [`respects prefers-color-scheme: dark`](../frontend/tests/viewer.spec.ts#L1209) | test | 1209 | `test('respects prefers-color-scheme: dark', callback)` | respects prefers-color-scheme: dark |
| [`respects prefers-color-scheme: light`](../frontend/tests/viewer.spec.ts#L1220) | test | 1220 | `test('respects prefers-color-scheme: light', callback)` | respects prefers-color-scheme: light |
| [`page has a skip-to-content link`](../frontend/tests/viewer.spec.ts#L1230) | test | 1230 | `test('page has a skip-to-content link', callback)` | page has a skip-to-content link |
| [`main content area has a landmark role or id`](../frontend/tests/viewer.spec.ts#L1236) | test | 1236 | `test('main content area has a landmark role or id', callback)` | main content area has a landmark role or id |
| [`navigation is a landmark`](../frontend/tests/viewer.spec.ts#L1242) | test | 1242 | `test('navigation is a landmark', callback)` | navigation is a landmark |
| [`images have alt text`](../frontend/tests/viewer.spec.ts#L1248) | test | 1248 | `test('images have alt text', callback)` | images have alt text |
| [`selecting a search revision shows its plans`](../frontend/tests/viewer.spec.ts#L1262) | test | 1262 | `test('selecting a search revision shows its plans', callback)` | selecting a search revision shows its plans |
| [`viewing a failed run shows failure indicators`](../frontend/tests/viewer.spec.ts#L1269) | test | 1269 | `test('viewing a failed run shows failure indicators', callback)` | viewing a failed run shows failure indicators |
| [`uses an unlabeled, plain disclosure column`](../frontend/tests/viewer.spec.ts#L1280) | test | 1280 | `test('uses an unlabeled, plain disclosure column', callback)` | uses an unlabeled, plain disclosure column |
| [`toggle arrow expands a row showing property grid`](../frontend/tests/viewer.spec.ts#L1288) | test | 1288 | `test('toggle arrow expands a row showing property grid', callback)` | toggle arrow expands a row showing property grid |
| [`clicking anywhere on the row expands it`](../frontend/tests/viewer.spec.ts#L1299) | test | 1299 | `test('clicking anywhere on the row expands it', callback)` | clicking anywhere on the row expands it |
| [`clicking toggle again collapses the row`](../frontend/tests/viewer.spec.ts#L1308) | test | 1308 | `test('clicking toggle again collapses the row', callback)` | clicking toggle again collapses the row |
| [`toggle arrow and aria-expanded update on click`](../frontend/tests/viewer.spec.ts#L1318) | test | 1318 | `test('toggle arrow and aria-expanded update on click', callback)` | toggle arrow and aria-expanded update on click |
| [`multiple rows can be expanded simultaneously`](../frontend/tests/viewer.spec.ts#L1332) | test | 1332 | `test('multiple rows can be expanded simultaneously', callback)` | multiple rows can be expanded simultaneously |
| [`clicking close button hides the message`](../frontend/tests/viewer.spec.ts#L1346) | test | 1346 | `test('clicking close button hides the message', callback)` | clicking close button hides the message |
| [`mobile nav toggle shows and hides navigation links`](../frontend/tests/viewer.spec.ts#L1367) | test | 1367 | `test('mobile nav toggle shows and hides navigation links', callback)` | mobile nav toggle shows and hides navigation links |
