...
| [`(*Client).FetchAll`](../src/enrich/client.go#L110) | method | 110-224 | `func (*Client).FetchAll(ctx context.Context, urls []string) map[string]*FetchResult` | FetchAll fetches all URLs concurrently using a goroutine pool. It respects rate limiting across all workers and applies exponential backoff on 429. Returns a map of original URL -> FetchResult in the same order as input. |
| [`(*Client).Fetch`](../src/enrich/client.go#L229) | method | 229-231 | `func (*Client).Fetch(ctx context.Context, url string) *FetchResult` | Fetch retrieves one URL through the same rate-limited path as FetchAll. Workspace cache policy uses it so individual cache misses do not bypass the provider's configured request limit. |
| [`(*Client).fetchOne`](../src/enrich/client.go#L234) | method | 234-338 | `func (*Client).fetchOne(ctx context.Context, url string) *FetchResult` | fetchOne performs a single HTTP GET with retries and exponential backoff. |
| [`readProviderBody`](../src/enrich/client.go#L341) | function | 341-350 | `func readProviderBody(body io.Reader) ([]byte, error)` | readProviderBody reads one retained provider payload and rejects oversized responses. |
| [`discardProviderBody`](../src/enrich/client.go#L353) | function | 353-362 | `func discardProviderBody(body io.Reader) error` | discardProviderBody consumes a bounded non-retained response body before the connection is reused. |
| [`requestLogTarget`](../src/enrich/client.go#L365) | function | 365-371 | `func requestLogTarget(value string) (string, string)` | requestLogTarget returns query-free request fields suitable for operational logs. |
| [`(*Client).sourceName`](../src/enrich/client.go#L374) | method | 374-379 | `func (*Client).sourceName() string` | sourceName returns the configured provider name used in logging and evidence. |
| [`logFetchProgress`](../src/enrich/client.go#L382) | function | 382-394 | `func logFetchProgress(message, source string, started time.Time, completed, total, succeeded, notFound, failed int)` | logFetchProgress emits structured provider progress and elapsed-time fields. |
...
| [`TestConfigHelpers`](../src/workspace/config_unit_test.go#L356) | test | 356-539 | `func TestConfigHelpers(t *testing.T)` | TestConfigHelpers verifies config helpers. |
...
| [`StartWorkspaceAttempt`](../src/workspace/support.go#L84) | function | 84-244 | `func StartWorkspaceAttempt(db *database.Database, originalConfig []byte, run *Run, fresh bool) (int64, error)` | StartWorkspaceAttempt snapshots configuration and inputs, reuses or creates a plan, and starts an eligible pipeline attempt. |
| [`recordPreflightStep`](../src/workspace/support.go#L247) | function | 247-294 | `func recordPreflightStep(db *database.Database, runID, configArtifactID, inputManifestArtifactID int64, inputFingerprint, outputFingerprint string, reusedFromRunID *int64, failureSummary string) error` | recordPreflightStep records preflight step. |
| [`buildInputManifest`](../src/workspace/support.go#L297) | function | 297-317 | `func buildInputManifest(resolved *manifest.ResolvedManifest) (*manifest.InputManifest, error)` | buildInputManifest hashes configured source files and returns both captured evidence and aggregate read failure. |
| [`persistArtifact`](../src/workspace/support.go#L320) | function | 320-323 | `func persistArtifact(db *database.Database, runID int64, data []byte, contentType string) (int64, error)` | persistArtifact stores content-addressed artifact metadata and bytes for a run. |
| [`contentHash`](../src/workspace/support.go#L326) | function | 326-329 | `func contentHash(data []byte) string` | contentHash returns the lowercase hexadecimal SHA-256 digest of data. |
| [`freshReason`](../src/workspace/support.go#L332) | function | 332-340 | `func freshReason(explicitFresh bool, reusePolicy string) string` | freshReason returns the persisted reason for starting a non-reused attempt. |
| [`loadCSVEntries`](../src/workspace/support.go#L343) | function | 343-378 | `func loadCSVEntries(path, source string) ([]map[string]string, error)` | loadCSVEntries loads csv entries from the supplied source. |
| [`loadBibEntries`](../src/workspace/support.go#L381) | function | 381-403 | `func loadBibEntries(path, source string) ([]map[string]string, error)` | loadBibEntries loads bib entries from the supplied source. |
