...
| Symbol | Kind | Lines | Declaration, inputs, and outputs | Source description |
|---|---|---:|---|---|
| [`Client`](../src/enrich/client.go#L28) | struct | 28-32 | `type Client struct { cfg SourceConfig ticker *time.Ticker client *http.Client }` | Client fetches URLs concurrently with configurable rate limiting and exponential backoff on 429 responses. |
| [`NewClient`](../src/enrich/client.go#L40) | function | 40-53 | `func NewClient(cfg SourceConfig) (*Client, error)` | NewClient creates a Client for the given validated source config. |
| [`ValidateSourceConfig`](../src/enrich/client.go#L56) | function | 56-84 | `func ValidateSourceConfig(cfg SourceConfig) error` | ValidateSourceConfig verifies provider endpoints and request bounds before enrichment begins. |
| [`validProviderURL`](../src/enrich/client.go#L87) | function | 87-93 | `func validProviderURL(label, value string) error` | validProviderURL verifies an HTTP(S) provider endpoint has a host. |
| [`(*Client).Close`](../src/enrich/client.go#L96) | method | 96-98 | `func (*Client).Close()` | Close stops the rate-limit ticker. |
| [`FetchResult`](../src/enrich/client.go#L101) | struct | 101-105 | `type FetchResult struct { Body []byte StatusCode int Err error }` | FetchResult carries the response body (or error) for one URL fetch. |
| [`(*Client).FetchAll`](../src/enrich/client.go#L110) | method | 110-215 | `func (*Client).FetchAll(ctx context.Context, urls []string) map[string]*FetchResult` | FetchAll fetches all URLs concurrently using a goroutine pool. It respects rate limiting across all workers and applies exponential backoff on 429. Returns a map of original URL -> FetchResult in the same order as input. |
| [`(*Client).Fetch`](../src/enrich/client.go#L220) | method | 220-222 | `func (*Client).Fetch(ctx context.Context, url string) *FetchResult` | Fetch retrieves one URL through the same rate-limited path as FetchAll. Workspace cache policy uses it so individual cache misses do not bypass the provider's configured request limit. |
| [`(*Client).fetchOne`](../src/enrich/client.go#L225) | method | 225-320 | `func (*Client).fetchOne(ctx context.Context, url string) *FetchResult` | fetchOne performs a single HTTP GET with retries and exponential backoff. |
| [`readProviderBody`](../src/enrich/client.go#L323) | function | 323-332 | `func readProviderBody(body io.Reader) ([]byte, error)` | readProviderBody reads one retained provider payload and rejects oversized responses. |
| [`discardProviderBody`](../src/enrich/client.go#L335) | function | 335-344 | `func discardProviderBody(body io.Reader) error` | discardProviderBody consumes a bounded non-retained response body before the connection is reused. |
| [`requestLogTarget`](../src/enrich/client.go#L347) | function | 347-353 | `func requestLogTarget(value string) (string, string)` | requestLogTarget returns query-free request fields suitable for operational logs. |
| [`(*Client).sourceName`](../src/enrich/client.go#L356) | method | 356-361 | `func (*Client).sourceName() string` | sourceName returns the configured provider name used in logging and evidence. |
| [`logFetchProgress`](../src/enrich/client.go#L364) | function | 364-376 | `func logFetchProgress(message, source string, started time.Time, completed, total, succeeded, notFound, failed int)` | logFetchProgress emits structured provider progress and elapsed-time fields. |
...
| [`openalexEntryToArticle`](../src/enrich/openalex.go#L53) | function | 53-75 | `func openalexEntryToArticle(entry map[string]any, doi string) *ArticleEnrichment` | openalexEntryToArticle converts an OpenAlex work object to article enrichment fields. |
| [`reconstructAbstract`](../src/enrich/openalex.go#L78) | function | 78-104 | `func reconstructAbstract(index map[string]any) string` | reconstructAbstract rebuilds abstract text from OpenAlex's word-position index. |
| [`extractOpenAlexPublisher`](../src/enrich/openalex.go#L107) | function | 107-122 | `func extractOpenAlexPublisher(entry map[string]any) string` | extractOpenAlexPublisher returns the direct or primary-source publisher name from a work object. |
| [`extractOpenAlexAuthors`](../src/enrich/openalex.go#L125) | function | 125-160 | `func extractOpenAlexAuthors(authorships []any) []EnrichedAuthor` | extractOpenAlexAuthors converts usable OpenAlex authorships to enriched authors in source order. |
| [`openAlexReferenceIDs`](../src/enrich/openalex.go#L163) | function | 163-179 | `func openAlexReferenceIDs(entry map[string]any) []string` | openAlexReferenceIDs returns unique OpenAlex work identifiers from referenced-work URLs. |
| [`extractOpenAlexReferences`](../src/enrich/openalex.go#L182) | function | 182-194 | `func extractOpenAlexReferences(references []any) []EnrichedReference` | extractOpenAlexReferences converts referenced-work DOI URLs to enriched references. |
| [`normalizeOpenAlexDOI`](../src/enrich/openalex.go#L197) | function | 197-203 | `func normalizeOpenAlexDOI(value string) string` | normalizeOpenAlexDOI returns a lowercase DOI extracted from a URL or prefixed value. |
| [`extractOpenAlexID`](../src/enrich/openalex.go#L211) | function | 211-220 | `func extractOpenAlexID(url string) string` | extractOpenAlexID returns a work identifier from an OpenAlex URL or bare identifier. |
| [`extractDOIFromURL`](../src/enrich/openalex.go#L223) | function | 223-229 | `func extractDOIFromURL(url string) string` | extractDOIFromURL returns a lowercase DOI embedded in a DOI URL. |
...
| Symbol | Kind | Lines | Declaration, inputs, and outputs | Source description |
|---|---|---:|---|---|
| [`decodeJSONObject`](../src/enrich/payload_validation.go#L12) | function | 12-18 | `func decodeJSONObject(body []byte) map[string]any` | decodeJSONObject decodes one JSON object and returns nil for malformed input. |
| [`ValidateProviderPayload`](../src/enrich/payload_validation.go#L24) | function | 24-60 | `func ValidateProviderPayload(provider, namespace string, body []byte) error` | ValidateProviderPayload verifies that a successful provider response has the envelope expected by the workspace decoder. It deliberately validates only transport envelopes, not optional bibliographic fields: a valid provider record may legitimately omit a requested field. |
| [`requireObject`](../src/enrich/payload_validation.go#L63) | function | 63-73 | `func requireObject(object map[string]json.RawMessage, key string) error` | requireObject requires a valid object value. |
| [`requireArray`](../src/enrich/payload_validation.go#L76) | function | 76-86 | `func requireArray(object map[string]json.RawMessage, key string) error` | requireArray requires a valid array value. |
| [`requireArrayOrNull`](../src/enrich/payload_validation.go#L89) | function | 89-102 | `func requireArrayOrNull(object map[string]json.RawMessage, key string) error` | requireArrayOrNull requires a valid array or null value. |
| [`requireString`](../src/enrich/payload_validation.go#L105) | function | 105-115 | `func requireString(object map[string]json.RawMessage, key string) error` | requireString requires a valid string value. |
...
| Symbol | Kind | Lines | Declaration, inputs, and outputs | Source description |
|---|---|---:|---|---|
| [`version`](../src/main.go#L30) | function | 30-32 | `func version() string` | version returns the semantic version string. |
| [`usage`](../src/main.go#L35) | function | 35-93 | `func usage()` | usage writes the supported command syntax to standard error. |
| [`main`](../src/main.go#L96) | function | 96-118 | `func main()` | main dispatches the analysis command selected by process arguments and exits on command failure. |
| [`migrateMain`](../src/main.go#L121) | function | 121-134 | `func migrateMain()` | migrateMain applies pending metadata migrations to one existing database. |
| [`serveMain`](../src/main.go#L137) | function | 137-178 | `func serveMain()` | serveMain serves main. |
| [`validateLoopbackAddress`](../src/main.go#L181) | function | 181-191 | `func validateLoopbackAddress(address string) error` | validateLoopbackAddress accepts exact IP loopback authorities and rejects names and wildcard hosts. |
| [`frontendAssets`](../src/main.go#L194) | function | 194-203 | `func frontendAssets(dir string) (fs.FS, error)` | frontendAssets validates one frontend asset directory and exposes it as a file system. |
| [`runPipelineMain`](../src/main.go#L206) | function | 206-236 | `func runPipelineMain()` | runPipelineMain parses run flags, resolves workspace selections, and executes each pipeline workspace. |
| [`changeToRepositoryRoot`](../src/main.go#L239) | function | 239-245 | `func changeToRepositoryRoot()` | changeToRepositoryRoot moves one directory upward only when execution starts inside the module directory. |
...
| Symbol | Kind | Lines | Declaration, inputs, and outputs | Source description |
|---|---|---:|---|---|
| [`options`](../src/tools/prepare-osf/main.go#L29) | struct | 29 | `type options struct{ DB, Config, Out string }` | options contains source and new-output paths accepted by the export command. |
| [`hashMapping`](../src/tools/prepare-osf/main.go#L32) | struct | 32-35 | ``type hashMapping struct { Original string `json:"original"` Sanitized string `json:"sanitized"` }`` | hashMapping records a reviewer-bearing raw configuration hash and its sanitized replacement. |
| [`exportManifest`](../src/tools/prepare-osf/main.go#L38) | struct | 38-45 | ``type exportManifest struct { CreatedAt string `json:"created_at"` SourceSchemaVersions map[string]string `json:"source_schema_versions"` SanitizedSchemaVersions map[string]string `json:"sanitized_schema_versions"` Files map[string]string `json:"files"` ConfigurationMappings []hashMapping `json:"configuration_hash_mappings"` BrowserDraftDisclaimer string `json:"browser_draft_disclaimer"` }`` | exportManifest records schema identity, copied file hashes, sanitization mappings, and limitations. |
| [`main`](../src/tools/prepare-osf/main.go#L48) | function | 48-58 | `func main()` | main parses the copy-only export command and reports failures without publishing partial output. |
| [`prepare`](../src/tools/prepare-osf/main.go#L61) | function | 61-212 | `func prepare(ctx context.Context, input options) error` | prepare validates source ownership, sanitizes temporary snapshots, and atomically publishes a new bundle. |
| [`sanitizeMetadata`](../src/tools/prepare-osf/main.go#L215) | function | 215-324 | `func sanitizeMetadata(ctx context.Context, db *sql.DB) ([]hashMapping, string, error)` | sanitizeMetadata redacts reviewer identity and rewires copied content-addressed configuration artifacts. |
| [`validateArtifactBlobs`](../src/tools/prepare-osf/main.go#L327) | function | 327-347 | `func validateArtifactBlobs(ctx context.Context, tx *sql.Tx) error` | validateArtifactBlobs recomputes every copied artifact size and SHA-256 identity. |
| [`sanitizeReviewerAssignments`](../src/tools/prepare-osf/main.go#L350) | function | 350-408 | `func sanitizeReviewerAssignments(source []byte) ([]byte, bool, error)` | sanitizeReviewerAssignments replaces only provable inline reviewer values and fails closed otherwise. |
| [`tokenKind`](../src/tools/prepare-osf/main.go#L411) | type | 411 | `type tokenKind int` | tokenKind distinguishes source identifiers from skipped strings, comments, multiline data, and punctuation. |
| [`nextCodeToken`](../src/tools/prepare-osf/main.go#L420) | function | 420-436 | `func nextCodeToken(text string, start int) (int, tokenKind)` | nextCodeToken returns the next executable SOMETHING source position without inspecting data tokens. |
| [`somethingMultilineEnd`](../src/tools/prepare-osf/main.go#L439) | function | 439-473 | `func somethingMultilineEnd(text string, start int) (int, bool)` | somethingMultilineEnd skips one SOMETHING multiline literal including its named closing delimiter. |
| [`skipTrivia`](../src/tools/prepare-osf/main.go#L476) | function | 476-492 | `func skipTrivia(text string, start int) int` | skipTrivia advances over whitespace and line comments. |
| [`matchingBrace`](../src/tools/prepare-osf/main.go#L495) | function | 495-525 | `func matchingBrace(text string, open int) (int, error)` | matchingBrace finds an inline reviewer value boundary while ignoring quoted data and comments. |
| [`quotedEnd`](../src/tools/prepare-osf/main.go#L528) | function | 528-540 | `func quotedEnd(text string, start int) (int, error)` | quotedEnd returns the position after an escaped single or double quoted token. |
| [`hasWordAt`](../src/tools/prepare-osf/main.go#L543) | function | 543-549 | `func hasWordAt(text string, start int, word string) bool` | hasWordAt matches one identifier without accepting a longer identifier prefix. |
| [`isIdentifierByte`](../src/tools/prepare-osf/main.go#L552) | function | 552-554 | `func isIdentifierByte(value byte) bool` | isIdentifierByte reports whether a byte may continue the identifiers used by the sanitizer. |
| [`copyAndSanitizeConfiguration`](../src/tools/prepare-osf/main.go#L557) | function | 557-616 | `func copyAndSanitizeConfiguration(source, destinationRoot string) (string, error)` | copyAndSanitizeConfiguration copies one include tree without permitting lexical or symbolic-link escape. |
| [`localIncludes`](../src/tools/prepare-osf/main.go#L619) | function | 619-646 | `func localIncludes(source string) ([]string, error)` | localIncludes extracts literal local include paths outside comments, strings, and multiline data. |
| [`snapshotSQLite`](../src/tools/prepare-osf/main.go#L649) | function | 649-655 | `func snapshotSQLite(ctx context.Context, source *sql.DB, destination string) error` | snapshotSQLite uses VACUUM INTO to capture a WAL-consistent source without overwriting a destination. |
| [`schemaVersion`](../src/tools/prepare-osf/main.go#L658) | function | 658-665 | `func schemaVersion(ctx context.Context, path string) (string, error)` | schemaVersion opens one SQLite file read-only and returns its latest recorded migration filename. |
| [`schemaVersionDB`](../src/tools/prepare-osf/main.go#L668) | function | 668-674 | `func schemaVersionDB(ctx context.Context, db *sql.DB) (string, error)` | schemaVersionDB returns the latest recorded migration filename from an open SQLite connection. |
| [`exportedFileHashes`](../src/tools/prepare-osf/main.go#L677) | function | 677-697 | `func exportedFileHashes(root string) (map[string]string, error)` | exportedFileHashes hashes every regular copied file and rejects unsupported file types. |
| [`safeCompanionPath`](../src/tools/prepare-osf/main.go#L701) | function | 701-713 | `func safeCompanionPath(metadataPath, relative string) (string, error)` | safeCompanionPath resolves a clean bundle-relative PDF binding and rejects a resolved target that escapes the metadata bundle through a symbolic link. |
| [`existingFile`](../src/tools/prepare-osf/main.go#L716) | function | 716-733 | `func existingFile(path string) (string, error)` | existingFile returns a resolved absolute path only for an existing regular file. |
| [`samePath`](../src/tools/prepare-osf/main.go#L736) | function | 736-743 | `func samePath(left, right string) bool` | samePath detects aliases using filesystem identity when both targets exist. |
| [`sqliteReadURI`](../src/tools/prepare-osf/main.go#L746) | function | 746-751 | `func sqliteReadURI(path string) string` | sqliteReadURI returns an existing-only read URI with the project busy timeout. |
| [`sqliteWritableURI`](../src/tools/prepare-osf/main.go#L754) | function | 754-758 | `func sqliteWritableURI(path string) string` | sqliteWritableURI returns a writable SQLite URI with the project pragmas. |
| [`fileHash`](../src/tools/prepare-osf/main.go#L761) | function | 761-772 | `func fileHash(path string) (string, error)` | fileHash streams one file into a lowercase SHA-256 digest. |
| [`mustRelative`](../src/tools/prepare-osf/main.go#L775) | function | 775-781 | `func mustRelative(root, path string) string` | mustRelative returns the previously validated path relative to its export root. |

### [`src/tools/prepare-osf/main_integration_test.go`](../src/tools/prepare-osf/main_integration_test.go)

| Symbol | Kind | Lines | Declaration, inputs, and outputs | Source description |
|---|---|---:|---|---|
| [`TestPrepareCopiesAndSanitizesWithoutMutatingSources`](../src/tools/prepare-osf/main_integration_test.go#L21) | test | 21-116 | `func TestPrepareCopiesAndSanitizesWithoutMutatingSources(t *testing.T)` | TestPrepareCopiesAndSanitizesWithoutMutatingSources verifies the atomic copy-only export boundary. |
| [`mustExec`](../src/tools/prepare-osf/main_integration_test.go#L119) | function | 119-126 | `func mustExec(t *testing.T, db *sql.DB, query string, args ...any) sql.Result` | mustExec runs one fixture statement and fails the current test on error. |
| [`TestSanitizeReviewerAssignments`](../src/tools/prepare-osf/main_integration_test.go#L129) | test | 129-141 | `func TestSanitizeReviewerAssignments(t *testing.T)` | TestSanitizeReviewerAssignments verifies inline redaction and fail-closed references. |
| [`TestSanitizeReviewerAfterCommentedMultiline`](../src/tools/prepare-osf/main_integration_test.go#L144) | test | 144-153 | `func TestSanitizeReviewerAfterCommentedMultiline(t *testing.T)` | TestSanitizeReviewerAfterCommentedMultiline verifies redaction continues after a multiline block whose closing tag has a trailing comment. |
| [`TestConfigurationCopyRejectsSymlinkEscape`](../src/tools/prepare-osf/main_integration_test.go#L156) | test | 156-173 | `func TestConfigurationCopyRejectsSymlinkEscape(t *testing.T)` | TestConfigurationCopyRejectsSymlinkEscape verifies that configuration includes cannot follow links outside the copied root. |
| [`TestSafeCompanionPathRejectsSymlinkEscape`](../src/tools/prepare-osf/main_integration_test.go#L177) | test | 177-193 | `func TestSafeCompanionPathRejectsSymlinkEscape(t *testing.T)` | TestSafeCompanionPathRejectsSymlinkEscape verifies a bundle-local link cannot select a companion database outside the selected metadata directory. |
...
| [`syncNormalizedPDFInventory`](../src/workspace/store.go#L310) | function | 310-352 | `func syncNormalizedPDFInventory(ctx context.Context, db *database.Database, dbPath, registryPath string) (int, int, error)` | syncNormalizedPDFInventory reconciles the companion inventory from the authoritative normalized revisions. It is safe to call after a new run or when a completed execution plan is reused. |
| [`normalizedInventoryBatch`](../src/workspace/store.go#L356) | function | 356-382 | `func normalizedInventoryBatch(ctx context.Context, db *database.Database, afterWorkID int64) ([]normalizedInventoryWork, error)` | normalizedInventoryBatch reads one bounded, deterministic inventory page and closes the metadata cursor before callers touch the companion database. |
| [`articlesWithFailedIdentityEvidence`](../src/workspace/store.go#L385) | function | 385-399 | `func articlesWithFailedIdentityEvidence(articles []*article.Article, evidence []uncertainORCIDSearchEvidence) []*article.Article` | articlesWithFailedIdentityEvidence selects articles whose identity provider search failed. |
| [`resultCountComparison`](../src/workspace/store.go#L402) | function | 402-411 | `func resultCountComparison(expected, observed int) string` | resultCountComparison classifies an observed source count as below, above, or matching its expectation. |
| [`loadWorkspaceEntries`](../src/workspace/store.go#L414) | function | 414-423 | `func loadWorkspaceEntries(fileType, path, source string) ([]map[string]string, error)` | loadWorkspaceEntries loads workspace entries from the supplied source. |
| [`cloneStringMap`](../src/workspace/store.go#L426) | function | 426-432 | `func cloneStringMap(in map[string]string) map[string]string` | cloneStringMap clones string map into an independent value. |
| [`persistWorkspaceStage`](../src/workspace/store.go#L435) | function | 435-459 | `func persistWorkspaceStage(db *database.Database, runID int64, articles []*article.Article, producerStage, stage, outcome string, reasons map[string][]string) (map[string]int64, map[string]int64, error)` | persistWorkspaceStage persists workspace stage through the owning repository. |
| [`setWorkspaceStageOutcome`](../src/workspace/store.go#L462) | function | 462-476 | `func setWorkspaceStageOutcome(db *database.Database, runID int64, articles []*article.Article, stage, outcome, reason string) error` | setWorkspaceStageOutcome sets workspace stage outcome using the supplied values. |
| [`setRunMetrics`](../src/workspace/store.go#L479) | function | 479-486 | `func setRunMetrics(db *database.Database, runID int64, metrics ...any) error` | setRunMetrics sets run metrics using the supplied values. |
| [`recordWorkspaceStage`](../src/workspace/store.go#L489) | function | 489-520 | `func recordWorkspaceStage(db *database.Database, run *Run, runID int64, name string, input any, output any) error` | recordWorkspaceStage records workspace stage. |
| [`persistWorkSnapshot`](../src/workspace/store.go#L523) | function | 523-564 | `func persistWorkSnapshot(db *database.Database, runID int64, a *article.Article, producerStage, extensionData string) (int64, int64, error)` | persistWorkSnapshot persists work snapshot through the owning repository. |
| [`workspaceRevisionExtension`](../src/workspace/store.go#L567) | struct | 567-571 | ``type workspaceRevisionExtension struct { NormalizedJournal string `json:"normalized_journal,omitempty"` ValidationReasons []string `json:"validation_reasons,omitempty"` NormalizedAuthors map[string]string `json:"normalized_authors,omitempty"` }`` | workspaceRevisionExtension stores normalized values and validation evidence outside canonical revision columns. |
| [`workspaceExtension`](../src/workspace/store.go#L574) | function | 574-583 | `func workspaceExtension(a *article.Article, reasons []string) string` | workspaceExtension serializes normalized journal, author names, and validation reasons for a revision. |
| [`validateWorkspaceArticles`](../src/workspace/store.go#L586) | function | 586-598 | `func validateWorkspaceArticles(articles []*article.Article) (valid, discarded []*article.Article, reasons map[string][]string)` | validateWorkspaceArticles partitions articles by validation outcome and records discard reasons by DOI. |
| [`normalizationFieldResult`](../src/workspace/store.go#L601) | struct | 601-605 | `type normalizationFieldResult struct { DOI string Field string Outcome string }` | normalizationFieldResult records one field's normalization outcome for an article DOI. |
| [`normalizeWorkspaceArticles`](../src/workspace/store.go#L624) | function | 624-646 | `func normalizeWorkspaceArticles(articles []*article.Article) []normalizationFieldResult` | normalizeWorkspaceArticles applies each normalizer and records one outcome for every checked field. Journal normalization is retained in revision extension data, so its result is measured here but not assigned to Journal. |
| [`normalizationResult`](../src/workspace/store.go#L649) | function | 649-657 | `func normalizationResult(doi, field, input, output string) normalizationFieldResult` | normalizationResult classifies a field as changed, already canonical, or unavailable. |
| [`recordNormalizationMetrics`](../src/workspace/store.go#L663) | function | 663-692 | `func recordNormalizationMetrics(db *database.Database, runID int64, processedArticles int, results []normalizationFieldResult) error` | recordNormalizationMetrics stores mutually exclusive outcomes for each checked field. No per-field audit events are emitted: the immutable normalized revision already records the output, and the prior revision is retained as the corresponding input evidence. |
| [`sumNormalizationOutcomes`](../src/workspace/store.go#L695) | function | 695-701 | `func sumNormalizationOutcomes(outcomes map[string]int) int` | sumNormalizationOutcomes totals the recognized mutually exclusive normalization outcomes. |
| [`enrichWorkspaceMetadata`](../src/workspace/store.go#L706) | function | 706-739 | `func enrichWorkspaceMetadata(db *database.Database, runID int64, run *Run, articles []*article.Article) (int, []fieldChange, error)` | enrichWorkspaceMetadata applies article-level providers. Its output is persisted before identity enrichment so an ORCID provider failure cannot erase the metadata and authorship evidence already gathered for the run. |
| [`enrichWorkspaceIdentity`](../src/workspace/store.go#L744) | function | 744-755 | `func enrichWorkspaceIdentity(db *database.Database, runID int64, run *Run, articles []*article.Article) (int, []fieldChange, []uncertainORCIDSearchEvidence, error)` | enrichWorkspaceIdentity applies exact observed-ORCID profiles and records name-search evidence. The caller persists this evidence against the prior enrich_metadata snapshot, including when this function returns an error. |
| [`gatherCachedCrossref`](../src/workspace/store.go#L758) | function | 758-784 | `func gatherCachedCrossref(ctx context.Context, cache *workspaceCache, source enrich.SourceConfig, articles []*article.Article) (*enrich.GatherResult, error)` | gatherCachedCrossref gathers cached crossref from the supplied inputs. |
| [`gatherCachedOpenAlex`](../src/workspace/store.go#L787) | function | 787-831 | `func gatherCachedOpenAlex(ctx context.Context, cache *workspaceCache, source enrich.SourceConfig, articles []*article.Article) (*enrich.GatherResult, error)` | gatherCachedOpenAlex gathers cached open alex from the supplied inputs. |
| [`resolveCachedOpenAlexReferences`](../src/workspace/store.go#L834) | function | 834-873 | `func resolveCachedOpenAlexReferences(ctx context.Context, cache *workspaceCache, client *enrich.Client, source enrich.SourceConfig, refIDsByDOI map[string][]string) (map[string]enrich.EnrichedReference, error)` | resolveCachedOpenAlexReferences resolves cached open alex references from the supplied context. |
| [`uncertainORCIDCandidate`](../src/workspace/store.go#L876) | struct | 876-882 | `type uncertainORCIDCandidate struct { ORCID string ProviderDisplay string QueryURL string PayloadArtifactID int64 ProviderRank int }` | uncertainORCIDCandidate stores one unconfirmed ORCID name-search result and its provenance. |
| [`uncertainORCIDSearchEvidence`](../src/workspace/store.go#L885) | struct | 885-892 | `type uncertainORCIDSearchEvidence struct { DOI string AuthorIndex int CitationName string Candidates []uncertainORCIDCandidate Status string ErrorMessage string }` | uncertainORCIDSearchEvidence records candidate-search results or provider failure for one author occurrence. |
| [`enrichCachedORCID`](../src/workspace/store.go#L895) | function | 895-961 | `func enrichCachedORCID(ctx context.Context, cache *workspaceCache, orcidSource, openAlexSource enrich.SourceConfig, hasOpenAlex bool, articles []*article.Article) (int, []fieldChange, []uncertainORCIDSearchEvidence, error)` | enrichCachedORCID gathers confirmed ORCID records and uncertain name-search evidence through the workspace cache. |
| [`resolveCachedORCIDProfile`](../src/workspace/store.go#L964) | function | 964-992 | `func resolveCachedORCIDProfile(ctx context.Context, cache *workspaceCache, orcidSource enrich.SourceConfig, orcidClient *enrich.Client, openAlexSource enrich.SourceConfig, openAlexClient *enrich.Client, hasOpenAlex bool, orcid string) (*enrich.EnrichedAuthor, error)` | resolveCachedORCIDProfile resolves cached orcid profile from the supplied context. |
| [`resolveCachedORCIDNameCandidates`](../src/workspace/store.go#L995) | function | 995-1025 | `func resolveCachedORCIDNameCandidates(ctx context.Context, cache *workspaceCache, source enrich.SourceConfig, client *enrich.Client, name string) ([]uncertainORCIDCandidate, error)` | resolveCachedORCIDNameCandidates resolves cached orcid name candidates from the supplied context. |
| [`persistUncertainORCIDEvidence`](../src/workspace/store.go#L1028) | function | 1028-1068 | `func persistUncertainORCIDEvidence(db *database.Database, runID int64, revisionIDs map[string]int64, evidence []uncertainORCIDSearchEvidence) error` | persistUncertainORCIDEvidence persists uncertain orcid evidence through the owning repository. |
| [`normalizeCacheName`](../src/workspace/store.go#L1071) | function | 1071-1073 | `func normalizeCacheName(name string) string` | normalizeCacheName normalizes cache name. |
| [`authorProfileProvenance`](../src/workspace/store.go#L1076) | struct | 1076-1079 | `type authorProfileProvenance struct { DOI string Index int }` | authorProfileProvenance identifies the persisted article location of one author. |
| [`applyAuthorProfile`](../src/workspace/store.go#L1083) | function | 1083-1120 | `func applyAuthorProfile(authors []*article.Author, profile *enrich.EnrichedAuthor, matchedORCID string, provenance map[*article.Author]authorProfileProvenance) (bool, []fieldChange)` | applyAuthorProfile fills missing author values and returns exactly the fields it changed, using the supplied provenance for audit and metric evidence. |
| [`fieldChange`](../src/workspace/store.go#L1123) | struct | 1123-1127 | `type fieldChange struct { DOI string Field string Provider string }` | fieldChange records one enriched field change for audit purposes. |
| [`applyArticleEnrichment`](../src/workspace/store.go#L1130) | function | 1130-1180 | `func applyArticleEnrichment(articles map[string]*article.Article, result *enrich.GatherResult) (int, []fieldChange)` | applyArticleEnrichment applies article enrichment to the supplied state. |
| [`usableEnrichedAuthors`](../src/workspace/store.go#L1183) | function | 1183-1196 | `func usableEnrichedAuthors(enriched []enrich.EnrichedAuthor) []article.Author` | usableEnrichedAuthors converts enriched authors that contain a non-empty citation name. |
| [`applyConfiguredArticleEnrichment`](../src/workspace/store.go#L1199) | function | 1199-1204 | `func applyConfiguredArticleEnrichment(articles map[string]*article.Article, source enrich.SourceConfig, result *enrich.GatherResult) (int, []fieldChange)` | applyConfiguredArticleEnrichment applies configured article enrichment to the supplied state. |
| [`emitFieldEnrichedAuditEvents`](../src/workspace/store.go#L1208) | function | 1208-1236 | `func emitFieldEnrichedAuditEvents(db *database.Database, runID int64, revisionIDs map[string]int64, changes []fieldChange) error` | emitFieldEnrichedAuditEvents records one field_enriched audit event per field change. revisionIDs maps DOI -> work_revision ID. |
| [`recordFieldEnrichmentMetrics`](../src/workspace/store.go#L1242) | function | 1242-1266 | `func recordFieldEnrichmentMetrics(db *database.Database, runID int64, changes []fieldChange) error` | recordFieldEnrichmentMetrics records per-field and per-provider enrichment metrics to pipeline_run_metrics. Per-author-index fields (e.g. author_orcid_0, author_first_name_1) are aggregated into a single count per field type (author_orcid, author_first_name, etc.). |
| [`completePipelineRun`](../src/workspace/store.go#L1269) | function | 1269-1275 | `func completePipelineRun(db *database.Database, runID int64) error` | completePipelineRun completes pipeline run and records its terminal state. |
| [`recordValidationAudit`](../src/workspace/store.go#L1278) | function | 1278-1302 | `func recordValidationAudit(db *database.Database, runID int64, articles []*article.Article, reasons map[string][]string) error` | recordValidationAudit records validation audit. |
...
| [`loadCSVEntries`](../src/workspace/support.go#L355) | function | 355-390 | `func loadCSVEntries(path, source string) ([]map[string]string, error)` | loadCSVEntries loads csv entries from the supplied source. |
| [`loadBibEntries`](../src/workspace/support.go#L393) | function | 393-415 | `func loadBibEntries(path, source string) ([]map[string]string, error)` | loadBibEntries loads bib entries from the supplied source. |
...
| [`resolutionMatches`](../frontend/src/components/note-parser.tsx#L506) | function | 506 | `function resolutionMatches(source: NoteLink, resolved?: ResolvedNoteLink)` | Confirms that a persisted ordinal resolution still describes the exact parsed draft link identity. |
