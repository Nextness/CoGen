...
- Every internal link uses `link()`, targets the generated page file for its destination view, and preserves `search_id`, `search_revision_id`, `plan_id`, `run_id`, focused `note_id`, focused `anchor_id`, and `pdf_page` unless a parent or target change invalidates descendant context. Cross-view transitions use native document navigation; same-view filters, pages, cursors, modes, and sections use history state plus an in-document render.
