...
Frontend production source is TypeScript with native ES modules and no application framework, compiled per file by `make frontend-build` into the assembled `frontend/dist` served root. The build reads the single `frontend/index.html` shell template and generates one identified HTML document per Home, Deepdive, and detail view. Relative imports use explicit `.ts` or `.tsx` specifiers, type-only imports use `import type` (`verbatimModuleSyntax` and `erasableSyntaxOnly` are enforced by `tsc --noEmit`), and new exports carry required type annotations plus the maintained adjacent JSDoc prose description. Rendering uses the project-owned classic-mode JSX runtime; [JSX-RUNTIME.md](JSX-RUNTIME.md) is the authoritative contract for its semantics and must be updated whenever the runtime's attribute/children handling, escaping, event binding, or wiring change. `state.tsx` owns URL, view-to-page mapping, and shared rendering state, `api.tsx` owns JSON transport, `router.tsx` owns dispatch, cross-view navigation, same-view history, and abort lifecycle, views fetch page data, assign the JSX tree to a descriptive binding, and pass that binding to `render`, while components own reusable JSX elements or behavior. The generated `frontend/src/jsx/classes.ts` registry is derived from the six authoritative stylesheets and is checked, together with non-JSX class uses, before `make check-frontend` runs `tsc --noEmit`.
...
- Add a CSS selector in its owning stylesheet before regenerating `frontend/src/jsx/classes.ts` with `make frontend-classes`; use `make frontend-classes-check` as the non-mutating freshness and static-use proof, and never edit the registry directly.
- Every internal link uses `link()`, targets the clean per-view path for its destination, and preserves `search_id`, `search_revision_id`, `plan_id`, `run_id`, focused `note_id`, focused `anchor_id`, and `pdf_page` unless a parent or target change invalidates descendant context. Cross-view transitions use native document navigation with the destination state persisted first; same-view filters, pages, cursors, modes, and sections use history state plus an in-document render. Anchors carrying state beyond the view path must render `data-state` JSON.
...
- The custom PDF viewer renders exactly one bounded current page and selectable text, advances one page per Previous or Next activation, cancels stale render work, destroys render, document, loading, and worker lifecycles on route navigation, and treats highlights as supplemental to keyboard-operable textual anchor evidence.
