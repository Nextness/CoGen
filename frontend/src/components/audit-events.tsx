...
export function AuditEventMarkup(props: { event: AuditEventRecord }): JSX.Element {
  ...
  return (
    <article className={"rw-audit-event rw-audit-event--" + category} data-audit-event-id={eventID}>
      ...
      <div className="rw-audit-event__main">
        <div className="rw-audit-event__heading">
          <h5>{humanLabel(event.action || "event")}</h5>
...
export function auditEventMarkup(event: AuditEventRecord): string {
  return renderToString(<AuditEventMarkup event={event} />);
}
...
export function AuditStream(props: { events: AuditEventRecord[]; emptyMessage?: string }): JSX.Element {
  ...
  return (
    <Fragment>
      {Array.from(groups.entries()).map(function(entry, index) {
        const headingID = "audit-day-" + index;
        return (
          <section className="rw-audit-day" aria-labelledby={headingID}>
            <h4 id={headingID}>{entry[0]}</h4>
            <ol className="rw-audit-events">
              {entry[1].map(function(event) {
...
export function auditStream(events: AuditEventRecord[], emptyMessage?: string): string {
  return renderToString(<AuditStream events={events} emptyMessage={emptyMessage} />);
}
...
export function RecordAuditInvestigation(props: { events: AuditEventRecord[] }): JSX.Element {
  const actions = Array.from(new Set(props.events.map(function(event) { return String(event.action || "event"); }))).sort();
  ...
  return (
    <div className="rw-record-audit" data-record-audit>
      <div className="rw-record-audit__controls">
        <label>Search events<input type="search" data-record-audit-search placeholder="Action, source, identifier, or metadata" /></label>
        ...
        <label>Event type<select data-record-audit-action>{actionOptions}</select></label>
      </div>
...
export function recordAuditInvestigation(events: AuditEventRecord[]): string {
  return renderToString(<RecordAuditInvestigation events={events} />);
}
