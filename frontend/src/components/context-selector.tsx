...
function renderDropdownOptions(key: string, query: string): void {
  ...
  const options = nativeOptions.filter((option) => {
    return option.value && (!normalized || optionLabel(option).toLocaleLowerCase().includes(normalized));
  });
  if (!options.length) {
    const emptyMarkup: JSX.Element = <p className="rw-search-dropdown__empty">No matching values.</p>;
    renderTree(emptyMarkup, dropdown.options);
    return;
  }
  const optionButtons: JSX.Element[] = options.map((option) => {
    const selected = option.value === select.value;
    var optionClass = "rw-search-dropdown__option";
    if (selected) optionClass += " selected";
    return (
      <button type="button" className={optionClass} role="option" aria-selected={String(selected)} data-context-value={option.value}>
        {optionLabel(option)}
      </button>
    );
  });
  const optionsMarkup: JSX.Element = <Fragment>{optionButtons}</Fragment>;
...
function syncDropdown(key: string): void {
  ...
  if (hasValue) {
    triggerMarkup = <span className="rw-search-dropdown__value">{label}</span>;
  }
...
function initializeDropdown(key: string): void {
  ...
  dropdown.trigger.addEventListener("click", () => {
    ...
    dropdownNames.forEach(closeDropdown);
    if (opening) {
  ...
  select.addEventListener("change", () => { syncDropdown(key); });
  syncDropdown(key);
}
...
function selectOptions(select: HTMLSelectElement, items: any[], selected: string, label: string, labelFn?: (item: any) => string): void {
  const key = Object.keys(selects).find((name) => {
    return selects[name as keyof ContextSelects] === select;
  });
  const optionElements: JSX.Element[] = items.map((item) => {
    var itemLabel = text(item, ["label", "search_id", "execution_fingerprint", "name", "title", "fingerprint", "id"]);
    if (labelFn) itemLabel = labelFn(item);
    return <option value={pickID(item)}>{itemLabel}</option>;
  });
  const optionsMarkup: JSX.Element = (
    <Fragment>
...
export async function hydrateSelectors(): Promise<void> {
  const loadingSelects = ["revision", "plan", "run"];
  loadingSelects.forEach(showLoading);
  const selectNames = Object.keys(selects);
  selectNames.forEach(hideDropdownError);
  if (!state.searches.length) {
    showLoading("search");
    try {
      state.searches = list(await api("/api/searches", {}, {
        method: "GET",
  ...
  const search = state.searches.find((item) => {
  ...
  try {
    ...
    selectOptions(selects.plan, state.plans, value("plan_id"), "Select an execution plan", (plan) => {
      return `Plan ${pickID(plan)} · ${String(plan.execution_fingerprint || "No fingerprint").slice(0, 12)}`;
    });
    selectOptions(selects.run, state.runs, value("run_id"), "Select a run attempt", (run) => {
      var trashed = "";
      if (run.visibility_state === "trashed") trashed = " · trashed";
      var date = "";
      if (run.started_at) date = ` · ${String(run.started_at).slice(0, 10)}`;
...
const selectNames = Object.keys(selects);
selectNames.forEach(initializeDropdown);
