...
function extractLinks(text, baseOffset, links, errors) {
...
function validateLink(scheme, target, raw) {
...
function unescapedIndex(text, character) {
  for (let index = 0; index < text.length; index += 1) {
...
export function renderNote(document, resolvedLinks) {
...
function renderLink(label, resolved) {
  ...
  const updates = { view: 'article', article_id: resolved.work_revision_id, note_id: '', anchor_id: '', pdf_page: '' };
