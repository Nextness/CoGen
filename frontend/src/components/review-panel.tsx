...
export async function mountArticleReview(host: HTMLElement, pdfHost: HTMLElement | null, record: any, detailData: any, onAuditChange?: () => Promise<void>): Promise<{ destroy: () => any }> {
  ...
  if (detailData.pdf_status?.status === "available" && pdfHost) {
    pdfController = await mountPDFViewer(pdfHost, {
      ...
      renderTree(<p className="ui negative message">The embedded PDF could not be rendered. The original PDF remains available through the download endpoint: {error.message}</p>, pdfHost!);
      return null;
    });
  }
  ...
  function renderStartReview(proposed: ProposedParent | null): void {
    const proposedSummary = proposed
      ? "Run " + proposed.pipeline_run_id + " from " + proposed.search_id + " / " + proposed.search_revision + " contains " + proposed.inherited_work_count + " matching work" + (proposed.inherited_work_count === 1 ? "" : "s") + "."
    ...
    renderTree(
      <section className="ui segment rw-review-panel rw-review-panel--empty">
        <div className="ui top attached header"><div><h3>Article review</h3><p>Record a run-scoped decision, notes, and PDF anchors without changing pipeline evidence.</p></div><span className="ui label">Not started</span></div>
        <div className="content">
          <div className="rw-review-onboarding"><div><h4>Start a review context for this run</h4><p>Starting review freezes any inherited article decisions, notes, and anchors so later changes remain independent.</p></div><button type="button" className="ui primary button" data-start-review>Start review</button></div>
        ...
        <dialog className="rw-review-dialog" data-review-dialog aria-labelledby="review-dialog-title" aria-describedby="review-dialog-description">
          <form className="ui form rw-review-dialog__form" data-review-context-form>
            <div className="rw-review-dialog__header"><div><p className="rw-review-dialog__eyebrow">Review lineage</p><h3 id="review-dialog-title">Start article review</h3><p id="review-dialog-description">Choose which earlier review context to inherit, or start empty. This choice cannot be changed after initialization.</p></div><button type="button" className="ui icon basic button rw-review-dialog__close" data-review-close aria-label="Close review setup">{"\u00D7"}</button></div>
            <div className="rw-review-dialog__body">
              <div className="ui info message"><span className="header">Recommended starting point</span>{proposedSummary + " Inheritance is frozen when review starts."}</div>
              <div className="ui field"><label htmlFor="review-parent-context">Parent review context</label><div className="ui selection dropdown"><select id="review-parent-context" data-review-parent><option value="">Start empty with no inherited review evidence</option>{proposed ? <option selected value={proposed.context_id}>{"Recommended: run " + proposed.pipeline_run_id + " · " + proposed.search_id + " / " + proposed.search_revision + " · " + proposed.inherited_work_count + " matching"}</option> : null}</select></div><p className="rw-field-help">Only earlier review contexts are eligible. Matching work heads are copied by immutable version reference.</p></div>
              <section className="rw-review-candidate-panel" aria-labelledby="review-candidate-heading"><div><h4 id="review-candidate-heading">Available context scope</h4><p>Same-search contexts load automatically. Expand only when you intentionally need lineage from another search.</p></div><button type="button" className="ui basic button" data-all-review-candidates>Include all earlier searches</button></section>
            ...
            <div className="rw-review-dialog__actions"><button type="button" className="ui basic button" data-review-cancel>Cancel</button><button type="submit" className="ui primary button" data-confirm-review>Initialize review</button></div>
          </form>
        </dialog>
      </section>,
      host
    ...
    async function appendCandidates(scope: string): Promise<void> {
      ...
      renderTree(<><span className="header">Searching review history</span>Loading eligible {(scope === "all" ? "cross-search" : "same-search")} contexts.</>, status);
      if (scope === "all") {
      ...
      try {
        ...
        if (scope === "all") {
          ...
          renderTree(<><span className="header">All earlier searches checked</span>{added ? added + " additional eligible context" + (added === 1 ? " was" : "s were") + " added. " : "No additional cross-search contexts were found. "}{total} total parent option{total === 1 ? " is" : "s are"} available.</>, status);
        } else {
          sameSearchLoaded = true;
          renderTree(<><span className="header">Same-search contexts ready</span>{total ? total + " eligible parent option" + (total === 1 ? " is" : "s are") + " available, including the recommendation when present." : "No same-search parent is available. Start empty or deliberately include all earlier searches."}</>, status);
        }
      } catch (error: any) {
        status.className = "ui error message rw-review-candidate-status";
        renderTree(<><span className="header">Context search failed</span>{error.message}</>, status);
    ...
    host.querySelector("[data-review-context-form]")!.addEventListener("submit", async function(event) {
      ...
      try {
        ...
        status.className = "ui error message rw-review-candidate-status";
        renderTree(<><span className="header">Review could not be initialized</span>{error.message}</>, status);
  ...
  async function renderReview(): Promise<void> {
    ...
    renderTree(
      <section className="ui segment rw-review-panel">
        <div className="ui top attached header"><div><h3>Article review</h3><p>Run-scoped decisions, notes, and anchors append immutable versions.</p></div>{state.inherited_from_context_id ? <span className="ui violet label">Inherited from context {state.inherited_from_context_id}</span> : <span className="ui neutral label">This context</span>}</div>
        <div className="content">
          <nav className="ui tabular menu rw-review-nav" aria-label="Article review sections" role="tablist"><button id="review-tab-decision" type="button" className="item active" role="tab" data-review-section="decision" aria-selected="true" aria-controls="review-panel-decision" tabindex={0}>Decision</button><button id="review-tab-notes" type="button" className="item" role="tab" data-review-section="notes" aria-selected="false" aria-controls="review-panel-notes" tabindex={-1}>Notes</button><button id="review-tab-anchors" type="button" className="item" role="tab" data-review-section="anchors" aria-selected="false" aria-controls="review-panel-anchors" tabindex={-1}>PDF anchors</button></nav>
          <section id="review-panel-decision" className="rw-review-section" role="tabpanel" data-review-section-panel="decision" aria-labelledby="review-tab-decision"><div className="rw-review-section__heading"><div><h4 id="review-decision-heading">Review decision</h4><p>Save the complete state for this article in the selected run context.</p></div></div>
            <form className="ui form rw-review-form" data-review-form><div className="rw-review-form__primary"><div className="ui field"><label htmlFor="article-review-status">Decision status</label><div className="ui selection dropdown"><select id="article-review-status" data-review-status disabled={!data.editable}>{statuses.map(function(status) { return <option value={status} selected={selectedStatus === status}>{humanLabel(status)}</option>; })}</select></div></div>
              <div className="ui field"><label htmlFor="article-review-reason">Reason or review summary <span className="rw-optional">Optional</span></label><textarea id="article-review-reason" rows={4} data-review-reason maxlength={32768} disabled={!data.editable}>{version?.reason || ""}</textarea><p className="rw-field-help">The saved reason is included in the append-only audit change for this decision.</p></div></div>
              <fieldset className="rw-review-substatuses" data-review-substatuses><legend>Decision qualifiers</legend><p>Select qualifiers only when the status is Not Approved or Removed.</p><div className="rw-review-option-grid">{substatuses.map(function(status) { return <label className="rw-review-check"><input type="checkbox" value={status} checked={selectedSubstatuses.has(status)} disabled={!data.editable} /><span>{humanLabel(status)}</span></label>; })}</div></fieldset>
              <div className="ui info message rw-review-feedback" data-review-message aria-live="polite">{version ? <><span className="header">Current saved version</span>Version {version.id} by {version.reviewer_display} at {formatTime(version.created_at)}.</> : <><span className="header">No saved decision yet</span>The current context defaults to Not Evaluated until you save a complete review state.</>}</div>
              <div className="rw-review-actions"><button type="submit" className="ui primary button" data-review-save disabled={!data.editable}>Save review decision</button><button type="button" className="ui basic button" data-review-history aria-expanded="false">Show version history</button></div></form>
            <div className="rw-review-history-panel" data-review-history-list hidden></div>
          </section>
          <section id="review-panel-notes" className="rw-review-section" role="tabpanel" data-review-section-panel="notes" aria-labelledby="review-tab-notes" hidden><div data-note-host></div></section>
          <section id="review-panel-anchors" className="rw-review-section rw-anchor-panel" role="tabpanel" data-review-section-panel="anchors" aria-labelledby="review-tab-anchors" hidden><div className="rw-review-section__heading"><div><h4 id="review-anchors-heading">PDF anchors</h4><p>Select text in the document reader, then save a named anchor for this review context.</p></div></div><div data-anchor-candidate></div><div data-anchor-list></div></section>
        </div>
      </section>,
      host
    );
      ...
      try {
        ...
        if (saved.changed && onAuditChange) {
          try {
            ...
            renderTree(<><span className="header">Decision saved</span>The audit display could not be refreshed. Reload the article to see the persisted event.</>, currentMessage);
          }
        }
      } catch (error: any) {
        message.className = "ui error message rw-review-feedback";
        renderTree(<><span className="header">Review was not saved</span>{error instanceof APIError && error.status === 409 ? "A newer version exists. Your input is preserved; inspect version history before retrying." : error.message}</>, message);
    ...
    host.querySelector("[data-review-history]")!.addEventListener("click", async function() {
      ...
      try {
        ...
        renderTree(
          <Fragment>
            <div className="rw-review-section__heading"><div><h4>Version history</h4><p>The newest immutable decision appears first.</p></div></div>
            <ol className="rw-review-history">{(historyData.versions || []).map(function(item: any) {
              return <li><div><strong>Version {item.id} · {humanLabel(item.status)}</strong><p>{item.reviewer_display} · {formatTime(item.created_at)}</p></div>{item.reason ? <blockquote>{item.reason}</blockquote> : null}{item.sub_statuses?.length ? <p className="rw-review-qualifiers">{item.sub_statuses.map(humanLabel).join(" · ")}</p> : null}</li>;
            })}</ol>
          </Fragment>,
          target
        );
        ...
        renderTree(<p className="ui error message"><span className="header">History could not be loaded</span>{error.message}</p>, target);
        target.hidden = false;
      } finally {
        button.disabled = false;
        button.classList.remove("loading");
    ...
    if (data.editable) {
      ...
      renderTree(<p className="ui faded text">An available PDF is required before review notes can be changed.</p>, host.querySelector("[data-note-host]") as HTMLElement);
    }
    await loadAnchors();
    renderAnchorCandidate();
  ...
  function renderAnchorCandidate(): void {
    ...
    renderTree(
      <form className="ui form rw-anchor-candidate" data-anchor-form>
        <div><span className="ui blue label">Selection from page {selection.page}</span><blockquote>{"\u201C"}{selection.selectedText}{"\u201D"}</blockquote></div>
        <div className="ui field"><label htmlFor="review-anchor-id">Anchor ID</label><input id="review-anchor-id" required pattern="[A-Za-z][A-Za-z0-9._-]{0,63}" placeholder="methods-sample" data-anchor-id /><p className="rw-field-help">Begin with a letter, then use letters, numbers, periods, underscores, or hyphens.</p></div>
        <div className="rw-review-actions"><button type="submit" className="ui primary button" data-anchor-save>Save anchor</button><button type="button" className="ui basic button" data-anchor-discard>Discard selection</button></div>
    ...
    target.querySelector("form")!.addEventListener("submit", async function(event) {
      ...
      try {
        ...
        renderTree(<><span className="header">Anchor was not saved</span>{error.message}</>, message);
        button.disabled = false;
        button.classList.remove("loading");
      }
    });
  ...
  async function loadAnchors(): Promise<void> {
    ...
    renderTree(
      activeAnchors.length ? <ul className="rw-anchor-list">{activeAnchors.map(function(anchor) {
        const mismatch = anchor.version.pdf_content_hash !== detailData.pdf_status?.content_hash;
        return <li data-anchor-id={anchor.id}><div className="rw-anchor-card__meta"><div><span className="ui label">{anchor.id}</span><span className="ui label">Page {anchor.version.page}</span>{anchor.inherited_from_context_id ? <span className="ui violet label">Inherited</span> : <span className="ui neutral label">This context</span>}</div><span className={"ui " + (mismatch ? "red" : "green") + " label"}>{mismatch ? "PDF changed" : "Available"}</span></div>
          <blockquote>{anchor.version.selected_text || ""}</blockquote><div className="rw-anchor-card__actions"><button type="button" className="ui primary button" data-anchor-page={anchor.version.page} aria-label={"Open anchor " + anchor.id + " on PDF page " + anchor.version.page}>Open page {anchor.version.page}</button><button type="button" className="ui basic button" data-anchor-history>History</button><button type="button" className="ui danger button" data-anchor-delete disabled={!reviewEditable}>Remove</button></div></li>;
      })}</ul> : <p className="ui faded text">No active anchors. Select PDF text to add one, or use this keyboard-operable list to revisit existing anchors.</p>,
      target
    );
  ...
  async function showAnchorHistory(anchorID: string): Promise<void> {
    ...
    const historyNode = (
      <section className="rw-anchor-history">
        <div className="rw-review-section__heading"><div><h4>Anchor {anchorID} history</h4><p>The newest immutable anchor version appears first.</p></div></div>
        <ol>{(data.versions || []).map(function(version: any) {
          return <li><div><strong>Version {version.id} · {version.state}</strong><p>{version.reviewer_display} · {formatTime(version.created_at)}{version.state === "active" ? " · page " + version.page : " · tombstone"}</p></div>{version.state === "active" ? <blockquote>{version.selected_text || ""}</blockquote> : null}</li>;
        })}</ol>
      </section>
    );
