...
import { app, esc, value, pageSizes, pageHeader, emptyState } from '../state.js';
...
export async function advancedView() {
  ...
  const columns = (selectedTable?.columns || []).map(function(column) {
  ...
  const expandableFields = columns.map(function(column) {
    return { f: column, w: 'full' };
  });
  ...
  document.querySelector('#table-select').addEventListener('change', function(event) {
    setURL({ table: event.target.value, page: 1, sort: '', order: '' });
  });
