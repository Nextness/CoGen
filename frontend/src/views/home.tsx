...
import {
  app, state, esc, list, pickID, formatNumber, formatTime, formatDuration,
  statusChip, link, params, PageHeader, Panel
} from '../state.tsx';
import { h, Fragment, raw, renderToString, render as renderTree } from '../jsx/jsx-runtime.ts';
...
export async function homeView(): Promise<void> {
  ...
  const searchHistory = searches.length
    ? <div className="rw-home-searches">{searches.map(function(search) { return raw(searchCard(search, plans, runs)); })}</div>
  ...
  renderTree(
    <Fragment>
      ...
      {metrics}
      {latestMessage}
      <Panel title="Research history" description="Search terms organize immutable revisions, execution plans, and recorded run attempts." body={renderToString(searchHistory)} classes="rw-home-history" />
      <Panel title="Run attempts" description="Explore a complete context or move a terminal run into the reversible trash lifecycle." body={runTable(searches, plans, runs)} classes="rw-home-run-panel" />
      {raw(runDialog())}
    </Fragment>,
    app
  );
