...
import { setURL, bindFocusContext } from '../router.js';
import { dataTable, bindTableControls } from '../components/data-table.js';
import { auditStream as sharedAuditStream } from '../components/audit-events.js';
...
function auditMultiSelect(name, label, options, selectedRaw) {
...
function pageSizeOptions(current) {
...
function stageStatus(summary, step) {
...
function stageFlow(summaries, steps) {
  ...
  const available = new Set([...summaryByName.keys(), ...stepByName.keys()]);
  ...
  if (form) {
    ...
    form.querySelectorAll('[data-multi-select]').forEach(function(control) {
      ...
      const refresh = function() {
        ...
        control.querySelector('summary strong').textContent = count ? count + ' selected' : 'Any';
      };
      control.querySelector('[data-multi-select-all]').addEventListener('click', function() {
        checkboxes.forEach(function(input) { input.checked = true; });
        refresh();
      });
      control.querySelector('[data-multi-select-clear]').addEventListener('click', function() {
      ...
      control.addEventListener('toggle', function() {
        if (control.open) {
          form.querySelectorAll('[data-multi-select][open]').forEach(function(other) {
            if (other !== control) {
              other.open = false;
            }
          });
...
function bindArtifactInspection() {
  document.querySelectorAll('[data-inspect-artifact]').forEach(function(button) {
    button.addEventListener('click', async function() {
      const id = button.dataset.inspectArtifact;
      activeArtifactRow = button.closest('tr');
      document.querySelectorAll('[data-artifact-row]').forEach(function(row) { row.classList.toggle('selected', row === activeArtifactRow); });
      ...
      try {
        ...
        document.querySelector('#artifact-inspector .content').innerHTML = '<div class="ui error message"><div class="header">Preview unavailable</div>'
...
function renderArtifactInspector() {
  ...
  document.querySelector('#artifact-inspector .content').innerHTML = truncation
    + formatNotice
  ...
  document.querySelectorAll('[data-artifact-preview-mode]').forEach(function(button) {
  ...
  document.querySelector('[data-copy-artifact-preview]').addEventListener('click', async function() {
    const status = document.querySelector('[data-artifact-copy-status]');
    try {
