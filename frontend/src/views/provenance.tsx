...
function AuditMultiSelect(props: { name: string; label: string; options: any[]; selectedRaw: any }): JSX.Element {
  ...
  const choices = (props.options || []).map(function(item) {
  ...
  const empty = choices.length ? <Fragment>{choices}</Fragment> : <p className="ui faded text">No recorded values are available for this run.</p>;
  return (
    <details className="rw-multi-select" data-multi-select>
      <summary><span>{props.label}</span><strong>{summary}</strong></summary>
      <div className="rw-multi-select__menu">
        <div className="rw-multi-select__actions"><button type="button" className="ui basic button" data-multi-select-all>Select all</button><button type="button" className="ui basic button" data-multi-select-clear>Clear</button></div>
        {empty}
      </div>
    </details>
  );
...
function AuditFilters(props: { facets: any }): JSX.Element {
  ...
  return (
    <aside className="ui segment rw-audit-filters">
      <div className="ui top attached header"><div><h3>Filter audit evidence</h3><p>Filters are applied by the server and remain visible in the URL.</p></div></div>
      <div className="content">
        <form id="audit-filter-form" className="ui form">
          ...
          <details className="rw-filter-disclosure"><summary>Stage and outcome filters</summary>
          ...
          <AuditFilterSummary />
          <div className="rw-filter-actions"><button type="submit" className="ui primary button">Apply filters</button><a className="ui basic button" href={link(Object.fromEntries(Object.keys(auditFilterKeys).map(function(key) { return [key, '']; })))}>Reset</a></div>
        </form>
      </div>
    </aside>
  );
...
function AuditSummary(props: { data: any }): JSX.Element {
  ...
  const scope = value('run_id')
    ? 'Run ' + value('run_id') + (selectedValues(value('audit_category')).includes('pdf') ? ' plus global PDF evidence' : '')
    : 'All recorded runs';
  return (
  ...
  return (
    <div className="rw-audit-layout">
      ...
      <section className="ui segment rw-audit-results">
        <div className="ui top attached header"><div><h3>Audit timeline</h3><p>Newest evidence appears first. Open Recorded data only when identifiers or payload changes are needed.</p></div></div>
        <div className="content">
...
function ArtifactContext(props: { context: any }): JSX.Element {
  ...
  return <dl className="rw-context-strip">{entries.map(function(entry) {
...
function ArtifactActions(props: { row: any }): JSX.Element {
  ...
  const inspect = row.preview_available
    ? <button type="button" className="ui button" data-inspect-artifact={row.id}>Inspect preview</button>
    : <span className="ui label">Download only</span>;
  return <div className="artifact-actions">{inspect}<a className="ui basic button" href={'/api/artifacts/' + encodeURIComponent(row.id) + '/content'} download>Download</a></div>;
}
...
function ArtifactsView(props: { data: any }): JSX.Element {
  ...
  if (artifacts.length) {
    rows = artifacts.map(function(row) {
      ...
      return <tr data-artifact-row={row.id}>
        <td><strong>{humanLabel(role)}</strong><small className="rw-cell-note">Artifact {row.id}</small></td>
        <td>{stages ? stages : <span className="ui faded text">Not linked to a recorded step</span>}</td>
    ...
    rows = [<tr><td colspan={6} className="empty">No artifacts were recorded for this run.</td></tr>];
  }
  return (
    <Fragment>
      <ArtifactContext context={data.context || {}} />
      ...
      <section className="ui segment rw-artifact-list">
        <div className="ui top attached header"><div><h3>Recorded artifacts</h3><p>Content-addressed files linked to this run through configuration roles or execution steps.</p></div><span className="ui label">{artifacts.length.toLocaleString()} artifacts</span></div>
        <div className="content"><div className="table-wrap"><table className="ui table rw-artifact-table">
      ...
      <section className="ui segment rw-artifact-inspector" id="artifact-inspector">
        <div className="ui top attached header"><div><h3>Artifact preview</h3><p>Inspect a safe prefix of a text-based artifact without loading the full file into the document.</p></div></div>
...
function PageSizeOptions(props: { current: any }): JSX.Element {
  return <Fragment>{pageSizes.map(function(size) {
...
function CacheView(props: { data: any }): JSX.Element {
  ...
  const filterSummary = value('cache_q') ? <FilterChips filters={{ cache_q: value('cache_q') }} labels={{ cache_q: 'Search' }} options={{ removeUpdates: { cache_page: 1 }, clearUpdates: { cache_q: '', cache_page: 1 } }} /> : null;
  const controls = (
  ...
  const tableMarkup = <DataTable tableName="Cache uses" result={result} context={{
    page: page,
    perPage: perPage,
    ...
    columnConfig: {
      ...
      outcome: { label: 'Outcome', render: function(row, raw) { return <StatusChip raw={raw} />; } },
      cache_layer: { label: 'Cache layer' }, response_status: { label: 'Response' },
      used_at: { label: 'Used', render: function(row, raw) { return <>{formatTime(raw)}</>; } },
      expires_at: { label: 'Expires', render: function(row, raw) { return <>{formatTime(raw)}</>; } },
      payload_artifact_id: { label: 'Payload artifact', render: function(row, raw) {
        return raw ? <a href={link({ section: 'artifacts' })}>Artifact {raw}</a> : <span className="ui faded text">None</span>;
      } },
      request_fingerprint: { label: 'Request fingerprint', className: 'rw-column-fingerprint' }
    }
  }} />;
  return <Panel title="Cache use records" description="Recorded provider-response reuse and negative or stale outcomes for this run." body={<Fragment>{controls}{tableMarkup}</Fragment>} classes="rw-cache-view" />;
}
...
function StageFlow(props: { summaries: any[]; steps: any[] }): JSX.Element {
  const summaryByName = new Map(props.summaries.map(function(item) { return [item.stage_name, item]; }));
  const stepByName = new Map(props.steps.map(function(item) { return [item.step_name, item]; }));
  ...
  return (
    <ol className="rw-stage-flow">{names.map(function(name, index) {
      ...
      const records = summary ? <>{formatNumber(summary.total_records)}</> : <span className="ui faded text">Not applicable</span>;
      return <li className={"rw-stage-step rw-stage-step--" + status}>
        ...
        <div className="rw-stage-step__body">
          ...
          <p>{outcomeText || (step ? 'Execution step recorded.' : 'Stage summary recorded.')}</p>
          <dl><div><dt>Work outcome records</dt><dd>{records}</dd></div><div><dt>Duration</dt><dd>{duration}</dd></div></dl>
          {artifacts.length ? <div className="rw-stage-step__artifacts">{artifacts}</div> : null}
        </div>
      </li>;
    })}</ol>
  );
...
function StagesView(props: { data: any }): JSX.Element {
  ...
  const controls = (
    <form className="rw-table-controls" id="stage-controls">
      ...
      {value('stage_q') ? <FilterChips filters={{ stage_q: value('stage_q') }} labels={{ stage_q: 'Search' }} options={{ removeUpdates: { stage_page: 1 }, clearUpdates: { stage_q: '', stage_page: 1 } }} /> : null}
    </form>
  );
  const details = <DataTable tableName="Detailed stage outcomes" result={result} context={{
    ...
    columnConfig: {
      ...
      outcome: { label: 'Outcome', render: function(row, raw) { return <StatusChip raw={raw} />; } },
      reason: { label: 'Reason' },
      created_at: { label: 'Recorded', render: function(row, raw) { return <>{formatTime(raw)}</>; } },
      updated_at: { label: 'Updated', render: function(row, raw) { return <>{formatTime(raw)}</>; } }
    }
  }} />;
  return (
    <Fragment>
      <section className="ui segment rw-stage-overview">
        <div className="ui top attached header"><div><h3>Stage outcomes and progression</h3><p>This view explains what happened during the run. It does not provide pipeline controls.</p></div></div>
      ...
      <Panel title="Detailed stage outcomes" description="Per-work evidence remains available below the run-level progression." body={<Fragment>{controls}{details}</Fragment>} classes="rw-stage-details" />
    </Fragment>
  );
}
...
function RunView(props: { artifactData: any }): JSX.Element {
  const artifactData = props.artifactData;
  ...
  const summary = (
    <dl className="rw-summary-strip">
      <div><dt>Run attempt</dt><dd>{run?.attempt_number || value('run_id')}</dd></div>
      <div><dt>Status</dt><dd><StatusChip raw={run?.status} /></dd></div>
  ...
  const properties = (
    <dl className="property-grid property-grid--compact">{Object.entries(fields).map(function(entry) {
      return <div><dt>{humanLabel(entry[0])}</dt><dd>{raw(cell(entry[1], entry[0]))}</dd></div>;
    })}</dl>
  );
  var snapshotRows: JSX.Element;
  if (snapshots.length) {
    snapshotRows = <ul className="rw-file-list">{snapshots.map(function(artifact) {
      const action = artifact.has_blob
        ? <a className="ui basic button" href={'/api/artifacts/' + encodeURIComponent(artifact.id) + '/content'} download>Download</a>
        : <span className="ui label">Payload unavailable</span>;
      return <li><div><strong>{humanLabel(artifact.artifact_roles)}</strong><span>{formatBytes(artifact.byte_size)} / {artifact.content_type}</span></div>{action}</li>;
    })}</ul>;
  } else {
    snapshotRows = <p className="ui faded text">No configuration snapshots were recorded for this legacy run.</p>;
  }
  return (
    <Fragment>
      {summary}
...
export async function provenanceView(): Promise<void> {
  ...
  const navItems: Array<[string, string]> = Object.entries(provenanceSections).map(function(entry) { return [entry[0], entry[1][0]]; });
  ...
  if (button) {
    button.addEventListener('click', async function() {
      ...
      try {
        ...
        renderTree(<AuditStream events={auditEvents} />, stream);
    ...
    button.addEventListener('click', async function() {
      ...
      try {
        ...
        renderTree(
          ...
          document.querySelector('#artifact-inspector .content') as HTMLElement
        );
      } finally {
        button.disabled = false;
        button.classList.remove('loading');
...
function renderArtifactInspector(): void {
  ...
  const truncation = preview.truncated
    ? <div className="ui warning message"><div className="header">Preview truncated</div><p>This page shows the first {formatBytes(preview.preview_byte_size)} of {formatBytes(preview.stored_byte_size || preview.byte_size)}. Download the original file to inspect its complete contents.</p></div>
    : <p className="ui success message">The complete stored text fits within the safe preview limit.</p>;
  const modeButtons = canFormat
    ? <Fragment><button type="button" className="ui basic button" data-artifact-preview-mode="formatted" disabled={preview.mode === 'formatted'}>Formatted JSON</button><button type="button" className="ui basic button" data-artifact-preview-mode="raw" disabled={preview.mode === 'raw'}>Raw text</button></Fragment>
    : null;
  ...
  renderTree(
    <Fragment>
      ...
      {formatNotice}
      <dl className="rw-summary-strip">
        <div><dt>Original size</dt><dd>{formatBytes(preview.byte_size)}</dd></div>
        <div><dt>Stored size</dt><dd>{formatBytes(preview.stored_byte_size)}</dd></div>
        <div><dt>Bytes shown</dt><dd>{formatBytes(preview.preview_byte_size)}</dd></div>
        <div><dt>Content type</dt><dd>{preview.content_type}</dd></div>
      </dl>
      <div className="artifact-inspector-toolbar">
        {modeButtons}
        <button type="button" className="ui basic button" data-toggle-artifact-wrap>{preview.wrap ? 'Disable line wrapping' : 'Wrap long lines'}</button>
      ...
      <pre className={'rw-artifact-preview' + (preview.wrap ? ' rw-artifact-preview--wrap' : '')}>{shown}</pre>
