...
import {
  app, esc, value, link, state, provenanceSections, section,
  emptyPanel, panel, subnav, cell, list, selectedRun, pickID, formatTime, formatBytes,
  formatNumber, humanLabel, statusChip, filterChips, pageSizes, PageHeader, Subnav, EmptyPanel
} from '../state.tsx';
...
import { auditStream as sharedAuditStream } from '../components/audit-events.tsx';
  ...
  if (!value('run_id') && current !== 'audit') {
    renderTree(
      <Fragment>
        ...
        <EmptyPanel title={title} detail="Select a run attempt to inspect this provenance view." action={'<button type="button" class="ui button" data-focus-context>Focus context selector</button>'} />
      </Fragment>,
      app
    );
    bindFocusContext();
