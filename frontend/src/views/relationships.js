...
import { app, esc, value, graphFilters, pageHeader, emptyState, list, filterChips, formatNumber } from '../state.js';
import { api } from '../api.js';
import { graphClusters, graphField, graphQuery, graphResult, mountGraph } from '../components/graph.js';
...
function appliedFilters() {
  ...
  const clear = { view: 'relationships', mode: value('mode') || 'research_network', node: '' };
...
export async function relationshipsView() {
  if (!value('run_id')) {
  ...
  document.querySelector('#graph-form').addEventListener('submit', function(event) {
    ...
    const updates = { view: 'relationships', node: '' };
  ...
  document.querySelectorAll('input[name="graph_mode"]').forEach(function(input) {
  ...
  document.querySelector('#graph-reset').addEventListener('click', function() {
    const updates = { view: 'relationships', mode: 'research_network', node: '' };
