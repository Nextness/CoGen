...
import '../setup.ts';
import { selects, hydrateSelectors } from '../../../src/components/context-selector.tsx';
...
describe('context-selector.js — hydrateSelectors', function() {
  ...
  it('fetches searches and populates the search selector', async function() {
    ...
    globalThis.fetch = function(url) {
      if (String(url).includes('/api/searches')) {
  ...
  it('supports searchable keyboard-ready selection without implying multiple active contexts', async function() {
    ...
    await hydrateSelectors();
    const dropdown = document.querySelector('[data-context-dropdown="search"]') as HTMLElement;
    const trigger = dropdown.querySelector('.rw-search-dropdown__trigger') as HTMLButtonElement;
    trigger.click();
    assert.equal(trigger.getAttribute('aria-expanded'), 'true');
    const query = dropdown.querySelector('.rw-search-dropdown__query') as HTMLInputElement;
    query.value = 'mapping';
