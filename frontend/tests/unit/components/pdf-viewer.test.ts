...
describe('pdf-viewer.js', function() {
  ...
  it('renders one page and advances it with one Next activation', async function() {
    ...
    const documentMock = {
      numPages: 3,
      getPage: async function(pageNumber: number) {
        return {
    ...
    const pdfjsMock = {
      GlobalWorkerOptions: {},
      Util: { transform: function(_viewport: unknown, item: unknown) { return item; } },
      getDocument: function() { return loadingTask; },
    };
    const host = document.createElement('div');
    document.body.append(host);
    const changes: number[] = [];
    const controller = await mountPDFViewer(host, { url: '/fixture.pdf', page: 1, onPageChange: function(page: number) { changes.push(page); } }, async function() { return pdfjsMock; });
    ...
    assert.equal((host.querySelector('[data-pdf-previous]') as HTMLButtonElement).disabled, true);
    (host.querySelector('[data-pdf-next]') as HTMLButtonElement).click();
    for (let attempt = 0; attempt < 10 && (host.querySelector('[data-pdf-status]') as HTMLElement).textContent !== 'PDF page 2 of 3.'; attempt += 1) await new Promise(function(resolve) { setTimeout(resolve, 0); });

    assert.equal(host.querySelectorAll('.rw-pdf-page').length, 1);
    assert.equal((host.querySelector('.rw-pdf-page') as HTMLElement).dataset.pdfPageNumber, '2');
    assert.equal((host.querySelector('[data-pdf-page]') as HTMLInputElement).value, '2');
