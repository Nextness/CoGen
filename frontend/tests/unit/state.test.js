...
describe('state.js — display helpers', function() {
  ...
  it('uses namespaced pagination updates when a filter chip is removed', function() {
    ...
    host.innerHTML = html;
    const url = new URL(host.querySelector('.rw-filter-chip').href);
...
describe('state.js — table', function() {
  ...
  it('handles column objects with render functions', function() {
    const columns = [{ label: 'Name', render: function(row) { return row.name; } }];
  ...
  it('handles column objects with label property', function() {
    const columns = [{ label: 'Full Name', render: function(row) { return row.name; } }];
    const rows = [{ name: 'Alice' }];
    const result = table('Test', '', columns, rows);
...
describe('state.js — bindCopyButtons', function() {
  ...
  it('falls back to prompt when clipboard fails', async function() {
    ...
    var prompted = '';
    globalThis.prompt = function(msg, val) { prompted = val; return ''; };
