...
describe('evaluation.js - evaluationView', function() {
  ...
  it('renders normalized articles with warning and success inventory tags', async function() {
    ...
    assert.ok((app.querySelector('.ui.green.label') as HTMLElement).textContent.includes('Available'));
    assert.ok((app.querySelector('.ui.orange.label') as HTMLElement).textContent.includes('Not Available'));
    globalThis.fetch = originalFetch;
  });
});
