...
describe('home.js — homeView', function() {
  ...
  it('renders research hierarchy metrics, elapsed run data, Deepdive links, and reversible lifecycle controls', async function() {
    ...
    const explore = Array.from(document.querySelectorAll('a')).find(function(anchor) { return anchor.textContent === 'Explore'; }) as HTMLAnchorElement;
    ...
    const lifecycle = document.querySelector('[data-run-visibility="trashed"]') as HTMLButtonElement;
    lifecycle.click();
    const dialog = document.querySelector('[data-run-dialog]') as HTMLElement;
    assert.equal(dialog.hidden, false);
    assert.match(dialog.textContent, /without deleting immutable evidence/);
    (dialog.querySelector('[data-run-dialog-close]') as HTMLButtonElement).click();
