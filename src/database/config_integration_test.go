...
func TestBaselineAppliesCorrectly(t *testing.T) {
	...
	if count != 28 {
		t.Fatalf("expected 28 applied metadata migrations, got %d", count)
	}
	...
	if count2 != 28 {
		t.Fatalf("expected 28 migrations after reopen, got %d", count2)
	}
}
...
func TestV00003MigrationApplies(t *testing.T) {
	...
	if count != 28 {
		t.Fatalf("expected 28 applied migrations, got %d", count)
	}
...
func TestProductionRegistryCreatesMetadataAndPDFStores(t *testing.T) {
	...
	if metadataMigrations != 28 {
		t.Fatalf("metadata migrations = %d, want 28", metadataMigrations)
	}
...
func TestProductionMetadataUpgradePreservesAppliedBasenames(t *testing.T) {
	...
	if total != 28 || historical != 1 || additions != 3 {
