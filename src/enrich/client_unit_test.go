...
func TestNewClientRejectsInvalidSourceConfig(t *testing.T) {
	...
	tests := []struct {
		...
		{"unbounded timeout", func(cfg *SourceConfig) { cfg.TimeoutSecs = maxProviderTimeoutSeconds + 1 }},
		{"unbounded retries", func(cfg *SourceConfig) { cfg.MaxRetries = maxProviderRetries + 1 }},
		{"unbounded batch size", func(cfg *SourceConfig) { cfg.BatchSize = maxProviderBatchSize + 1 }},
