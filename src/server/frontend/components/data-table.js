...
export function rowFilter(rows, query) {
  if (!query) {
    return rows;
  }
...
export function dataTable(tableName, result, context) {
  if (!context) {
    context = {};
  }
...
export function bindTableControls(tableName, page) {
  var context = arguments.length > 2 && arguments[2] ? arguments[2] : {};
  const keys = {
    page: context.pageKey || 'page',
    perPage: context.perPageKey || 'per_page',
    sort: context.sortKey || 'sort',
  ...
  const perPage = document.querySelector(context.perPageSelector || '#per-page');
  if (perPage) {
    perPage.addEventListener('change', function(event) {
      scrollTableIntoView();
      setURL(updates({ perPage: event.target.value, page: 1, expanded: '' }));
    });
  }

  const queryInput = document.querySelector(context.querySelector || '#corpus-query');
  const queryForm = queryInput?.closest('form');
  if (queryForm) {
  ...
  const searchButton = document.querySelector(context.searchButtonSelector || '[data-search-query]');
  if (searchButton && searchButton.type !== 'submit') {
    searchButton.addEventListener('click', function() {
      const input = document.querySelector(context.querySelector || '#corpus-query');
      var query = '';
      if (input) {
        query = input.value;
      }
  ...
  const clearButton = document.querySelector(context.clearButtonSelector || '[data-clear-query]');
  if (clearButton) {
  ...
  const expandTable = document.querySelector('[data-expandable-table]');
  if (expandTable) {
...
function handleExpandToggle(event) {
  ...
  const rowIdx = toggle.dataset.expandRow;
  const expandRow = document.querySelector('tr.expansion-row[data-expand-row="' + rowIdx + '"]');
  if (!expandRow) {
    return;
  }
  ...
  if (rowKey) {
    const table = toggle.closest('[data-expandable-table]');
