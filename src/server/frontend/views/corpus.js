...
function identityEvidenceTable(data, context) {
...
function clippedRecordLink(kind, idKey, id, title) {
...
function clippedRecordText(title) {
...
function corpusColumnConfig(current) {
  const config = {};
  ...
  if (current === 'articles') {
    config.title = {
      label: 'Title',
      className: 'col-title',
      render: function(row) { return clippedRecordLink('article', 'article_id', row.id, row.title); }
    };
  }
  if (current === 'references') {
    config.title = {
      ...
      className: 'col-title',
      render: function(row) { return clippedRecordLink('reference', 'reference_id', row.id, row.title); }
    };
    config.citing_title = {
  ...
  if (current === 'authors') {
    config.citation_name = {
      ...
      render: function(row) { return clippedRecordLink('author', 'author_id', row.id, row.citation_name); }
    };
  }
  if (current === 'sources') {
...
export async function corpusView() {
  var current;
