...
function deepdiveLink(searchID, revisionID, planID, runID) {
  const updates = {};
...
function searchCard(search, plans, runs) {
  ...
  const latest = searchRuns.reduce(function(current, run) {
...
function runTable(searches, plans, runs) {
  const revisionContext = new Map();
...
function bindRunLifecycle(runs) {
  const dialog = document.querySelector('[data-run-dialog]');
  if (!dialog) return;
  const form = dialog.querySelector('[data-run-dialog-form]');
  const title = dialog.querySelector('#run-dialog-title');
  const description = dialog.querySelector('#run-dialog-description');
  const guidance = dialog.querySelector('[data-run-dialog-guidance]');
  const reasonField = dialog.querySelector('[data-run-reason-field]');
  const reason = form.elements.reason;
  const submit = dialog.querySelector('[data-run-dialog-submit]');
  const error = dialog.querySelector('[data-run-dialog-error]');
  ...
  document.querySelectorAll('[data-run-visibility]').forEach(function(button) {
    button.addEventListener('click', function() { open(button); });
  });
  dialog.querySelectorAll('[data-run-dialog-close]').forEach(function(button) { button.addEventListener('click', close); });
...
export async function homeView() {
  ...
  const latest = runs.reduce(function(current, run) {
    if (!current || String(run.started_at || '') > String(current.started_at || '')) return run;
    return current;
  }, null);
