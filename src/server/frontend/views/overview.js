...
const executionMetricStages = [
  { id: 'ingest', label: 'Input capture', description: 'Raw records accepted from configured source exports.', matches: function(name) { return name === 'input_records'; } },
  { id: 'parse', label: 'Parsing', description: 'Source records converted into article metadata.', matches: function(name) { return name.startsWith('parsed_'); } },
  { id: 'deduplicate', label: 'Deduplication', description: 'Unique articles retained and duplicate records identified.', matches: function(name) { return name.startsWith('deduplicated_') || name.startsWith('duplicate_'); } },
  { id: 'enrich', label: 'Enrichment', description: 'Candidate articles and fields updated from provider evidence.', matches: function(name) { return name.startsWith('enrichment_') || name.startsWith('enriched_'); } },
  { id: 'validate', label: 'Validation', description: 'Articles accepted into or discarded from the analysis-ready corpus.', matches: function(name) { return name === 'valid_articles' || name === 'discarded_articles'; } },
  { id: 'normalize', label: 'Normalization', description: 'Valid article fields assessed and converted to canonical forms.', matches: function(name) { return name.startsWith('normalized_') || name.startsWith('normalization_'); } },
  { id: 'cache', label: 'Cache and network', description: 'Recorded provider-cache decisions and network fetches.', matches: function(name) { return name.startsWith('cache_'); } },
];

/** Returns the numeric value of a captured metric, or null when unavailable. */
function capturedMetricValue(item) {
...
function capturedMetricsByStage(metrics) {
  const groups = executionMetricStages.map(function(stage) {
    return { ...stage, metrics: [] };
  });
  const other = { id: 'other', label: 'Other run evidence', description: 'Recorded metrics without a recognized execution-stage prefix.', metrics: [] };
...
function capturedMetricsMarkup(metrics) {
...
function fixedPercentageMetric(metric) {
...
export async function overviewView() {
  ...
  const normalizationFieldLabels = {
    publisher: 'Publisher',
  ...
  app.innerHTML = pageHeader('', 'Overview', 'Recorded execution evidence and current coverage are shown separately to preserve their meaning.')
    ...
    + panel('Normalization activity', 'Every valid article is processed. Field checks are mutually exclusive: changed, already canonical, or unavailable. Journal canonical forms are stored in revision metadata.',
        ...
        + table('Normalization field outcomes', 'Changed, already-canonical, and unavailable counts use each field\u2019s assessed count as their denominator.', [
          { label: 'Field', render: function(row) { return esc(row.field); } },
          { label: 'Assessed', render: function(row) { return normalizationValue(row.processed); } },
          { label: 'Changed', render: function(row) { return normalizationValue(row.changed); } },
          { label: 'Already canonical', render: function(row) { return normalizationValue(row.already_canonical); } },
          { label: 'Unavailable', render: function(row) { return normalizationValue(row.unavailable); } },
